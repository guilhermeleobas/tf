...
static double testcase[4][4] = {{27.05, 1.5137, 63.6, 0.52},
                                {-16.68, 1, 0, 0.138},
...
static double pi = pic, twopi = pic * 2.0, piover4 = pic / 4.0,
              fouroverpi = 4.0 / pic, piover2 = pic / 2.0;

/*  Coefficients for ATAN evaluation  */

static double atanc[] = {0.0,
...
atl:
  z = x * x;
  b = ((((893025.0 * z + 49116375.0) * z + 425675250.0) * z + 1277025750.0) *
           z +
       1550674125.0) *
          z +
      654729075.0;
  a = (((13852575.0 * z + 216602100.0) * z + 891080190.0) * z + 1332431100.0) *
          z +
      654729075.0;
  a = (a / b) * x + atanc[y];
  if (l) a = piover2 - a;
...
static double atan2(y, x) double y, x;
{
  double temp;
...
static trace_line(line, ray_h) int line;
double ray_h;
{
  int i;
...
char *argv[];
{
  ...
  sprintf(outarr[0], "%15s   %21.11f  %14.11f", "Marginal ray", od_sa[0][0],
          od_sa[0][1]);
  sprintf(outarr[1], "%15s   %21.11f  %14.11f", "Paraxial ray", od_sa[1][0],
          od_sa[1][1]);
  sprintf(outarr[2], "Longitudinal spherical aberration:      %16.11f",
          aberr_lspher);
  sprintf(outarr[3], "    (Maximum permissible):              %16.11f",
          max_lspher);
  sprintf(outarr[4], "Offense against sine condition (coma):  %16.11f",
          aberr_osc);
  sprintf(outarr[5], "    (Maximum permissible):              %16.11f",
          max_osc);
  sprintf(outarr[6], "Axial chromatic aberration:             %16.11f",
          aberr_lchrom);
