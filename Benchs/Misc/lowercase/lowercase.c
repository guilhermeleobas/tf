...
static size_t lower_StringImp(const UChar* data, size_t m_length, UChar* output)
    __attribute__((__noinline__));
static size_t lower_StringImpl(const UChar* __restrict data, size_t length,
                               UChar* __restrict output)
{
...
static UChar staticData[] = {'H', 'E', 'L', 'L', 'O', ' ', 'W', 'O',
                             'R', 'L', 'D', '!', 'H', 'E', 'L', 'L'};
static const size_t staticDataLength =
    sizeof(staticData) / sizeof(staticData[0]);

static void doTest(size_t numberOfIterations)
{
  const size_t numberOfCharacters = numberOfIterations;
  const size_t testDataLength =
      ((numberOfCharacters + staticDataLength - 1) / staticDataLength) *
      staticDataLength;
