...
int main(int argc, char *argv[])
{
  ...
  for (d = 0; d < dim; d++)
  {
    for (ib = 0; ib < nblocks; ib++)
    {
      if (ilower[ib][d] == istart[d])
      {
        ...
        stencil_indices[0] = d;
        HYPRE_StructMatrixSetBoxValues(A, ilower[ib], iupper[ib], 1,
                                       stencil_indices, values);
        iupper[ib][d] = i;
      }
    }
  ...
  if (solver_id > 0)
  {
    ...
    if (solver_id == 1)
    {
      ...
      HYPRE_PCGSetPrecond(
          (HYPRE_Solver)solver, (HYPRE_PtrToSolverFcn)HYPRE_StructSMGSolve,
          (HYPRE_PtrToSolverFcn)HYPRE_StructSMGSetup, (HYPRE_Solver)precond);
    }
    ...
    HYPRE_PCGSetup((HYPRE_Solver)solver, (HYPRE_Matrix)A, (HYPRE_Vector)b,
                   (HYPRE_Vector)x);
    ...
    HYPRE_PCGSolve((HYPRE_Solver)solver, (HYPRE_Matrix)A, (HYPRE_Vector)b,
                   (HYPRE_Vector)x);
 ...
 *-----------------------------------------------------------*/
  ...
  for (i = 0; i < (dim + 1); i++) hypre_TFree(offsets[i]);
  hypre_TFree(offsets);
