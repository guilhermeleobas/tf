...
hypre_StructVector *hypre_StructVectorCreate(MPI_Comm comm,
                                             hypre_StructGrid *grid)
{
  ...
  /* set defaults */
  for (i = 0; i < 6; i++) hypre_StructVectorNumGhost(vector)[i] = 1;
...
int hypre_StructVectorSetValues(hypre_StructVector *vector,
                                hypre_Index grid_index, double values,
                                int add_to)
{
  int ierr = 0;
...
int hypre_StructVectorSetBoxValues(hypre_StructVector *vector,
                                   hypre_Box *value_box, double *values,
                                   int add_to)
{
  int ierr = 0;
...
int hypre_StructVectorGetValues(hypre_StructVector *vector,
                                hypre_Index grid_index, double *values_ptr)
{
  int ierr = 0;
...
int hypre_StructVectorSetNumGhost(hypre_StructVector *vector, int *num_ghost)
{
  ...
  for (i = 0; i < 6; i++) hypre_StructVectorNumGhost(vector)[i] = num_ghost[i];
...
hypre_CommPkg *hypre_StructVectorGetMigrateCommPkg(
    hypre_StructVector *from_vector, hypre_StructVector *to_vector)
{
  ...
  hypre_CreateCommInfoFromGrids(hypre_StructVectorGrid(from_vector),
                                hypre_StructVectorGrid(to_vector), &send_boxes,
  ...
  comm_pkg = hypre_CommPkgCreate(
      ...
      hypre_StructVectorDataSpace(to_vector), send_processes, recv_processes,
      num_values, hypre_StructVectorComm(from_vector),
      hypre_StructGridPeriodic(hypre_StructVectorGrid(from_vector)));
                              ...
                              hypre_StructVector *to_vector)
{
  ...
  hypre_InitializeCommunication(comm_pkg, hypre_StructVectorData(from_vector),
...
hypre_StructVector *hypre_StructVectorRead(MPI_Comm comm, char *filename,
                                           int *num_ghost)
{
  FILE *file;
  char new_filename[255];
