...
hypre_StructMatrix *hypre_SMGCreateInterpOp(hypre_StructMatrix *A,
                                            hypre_StructGrid *cgrid, int cdir)
{
...
int hypre_SMGSetupInterpOp(void *relax_data, hypre_StructMatrix *A,
                           hypre_StructVector *b, hypre_StructVector *x,
                           hypre_StructMatrix *PT, int cdir, hypre_Index cindex,
                           hypre_Index findex, hypre_Index stride)
{
  ...
  for (si = 0; si < PT_stencil_size; si++)
  {
    ...
    hypre_CreateComputeInfo(fgrid, compute_pkg_stencil, &send_boxes,
    ...
    hypre_ComputePkgCreate(send_boxes, recv_boxes, stride, stride,
                           send_processes, recv_processes, indt_boxes,
                           dept_boxes, stride, fgrid,
                           hypre_StructVectorDataSpace(x), 1, &compute_pkg);
    ...
    for (compute_i = 0; compute_i < 2; compute_i++)
    {
      ...
      hypre_ForBoxArrayI(i, compute_box_aa)
      {
        ...
        hypre_ForBoxI(j, compute_box_a)
        {
          ...
          for (d = 0; d < 3; d++)
          {
            hypre_IndexD(start, d) += hypre_IndexD(PT_stencil_shape[si], d);
          }

          hypre_BoxGetStrideSize(compute_box, stride, loop_size);
          hypre_BoxLoop2Begin(loop_size, x_data_box, start, stride, xi,
                              PT_data_box, startc, stridec, PTi);
#define HYPRE_BOX_SMP_PRIVATE loopk, loopi, loopj, xi, PTi
#include "hypre_box_smp_forloop.h"
          hypre_BoxLoop2For(loopi, loopj, loopk, xi, PTi) { PTp[PTi] = xp[xi]; }
