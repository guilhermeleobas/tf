...
int hypre_SMGResidualSetup(void *residual_vdata, hypre_StructMatrix *A,
                           hypre_StructVector *x, hypre_StructVector *b,
                           hypre_StructVector *r)
{
  int ierr = 0;
  ...
  hypre_ComputePkgCreate(send_boxes, recv_boxes, unit_stride, unit_stride,
                         ...
                         base_stride, grid, hypre_StructVectorDataSpace(x), 1,
                         &compute_pkg);
  ...
  (residual_data->flops) =
      ...
      (hypre_IndexX(base_stride) * hypre_IndexY(base_stride) *
...
int hypre_SMGResidual(void *residual_vdata, hypre_StructMatrix *A,
                      hypre_StructVector *x, hypre_StructVector *b,
                      hypre_StructVector *r)
{
  int ierr = 0;
...
int hypre_SMGResidualSetBase(void *residual_vdata, hypre_Index base_index,
                             hypre_Index base_stride)
{
  hypre_SMGResidualData *residual_data = residual_vdata;
  int d;
