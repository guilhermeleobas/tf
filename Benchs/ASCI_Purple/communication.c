...
hypre_CommPkg *hypre_CommPkgCreate(
    hypre_BoxArrayArray *send_boxes, hypre_BoxArrayArray *recv_boxes,
    hypre_Index send_stride, hypre_Index recv_stride,
    hypre_BoxArray *send_data_space, hypre_BoxArray *recv_data_space,
    int **send_processes, int **recv_processes, int num_values, MPI_Comm comm,
    hypre_Index periodic)
{
  ...
  hypre_CommPkgCreateInfo(
      send_boxes, send_stride, send_data_space, send_processes, num_values,
      comm, periodic, &num_sends, &send_procs, &send_types, &copy_from_type);
  ...
  hypre_CommPkgCreateInfo(recv_boxes, recv_stride, recv_data_space,
                          recv_processes, num_values, comm, periodic,
                          &num_recvs, &recv_procs, &recv_types, &copy_to_type);
  ...
  hypre_ForBoxArrayI(i, send_boxes) hypre_TFree(send_processes[i]);
  hypre_BoxArrayArrayDestroy(send_boxes);
  hypre_TFree(send_processes);

  hypre_ForBoxArrayI(i, recv_boxes) hypre_TFree(recv_processes[i]);
...
int hypre_InitializeCommunication(hypre_CommPkg *comm_pkg, double *send_data,
                                  double *recv_data,
...
int hypre_ExchangeLocalData(hypre_CommPkg *comm_pkg, double *send_data,
                            double *recv_data)
{
  hypre_CommType *copy_from_type;
  hypre_CommType *copy_to_type;
...
hypre_CommType *hypre_CommTypeCreate(hypre_CommTypeEntry **comm_entries,
                                     int num_entries)
{
...
hypre_CommTypeEntry *hypre_CommTypeEntryCreate(hypre_Box *box,
                                               hypre_Index stride,
                                               hypre_Box *data_box,
                                               int num_values,
                                               int data_box_offset)
{
  ...
  hypre_CopyIndex(hypre_BoxIMin(box), hypre_CommTypeEntryIMin(comm_entry));
  hypre_CopyIndex(hypre_BoxIMax(box), hypre_CommTypeEntryIMax(comm_entry));
  ...
  for (i = 0; i < 3; i++) length_array[i] = hypre_IndexD(size, i);
...
int hypre_CommPkgCreateInfo(hypre_BoxArrayArray *boxes, hypre_Index stride,
                            hypre_BoxArray *data_space, int **processes,
                            int num_values, MPI_Comm comm, hypre_Index periodic,
                            int *num_comms_ptr, int **comm_processes_ptr,
...
int hypre_CommPkgCommit(hypre_CommPkg *comm_pkg)
{
  ...
  hypre_CommTypeBuildMPI(
      hypre_CommPkgNumSends(comm_pkg), hypre_CommPkgSendProcs(comm_pkg),
      hypre_CommPkgSendTypes(comm_pkg), hypre_CommPkgSendMPITypes(comm_pkg));

  /* create recv MPI_Datatypes */
  hypre_CommPkgRecvMPITypes(comm_pkg) =
      hypre_TAlloc(MPI_Datatype, hypre_CommPkgNumRecvs(comm_pkg));
  hypre_CommTypeBuildMPI(
      hypre_CommPkgNumRecvs(comm_pkg), hypre_CommPkgRecvProcs(comm_pkg),
      hypre_CommPkgRecvTypes(comm_pkg), hypre_CommPkgRecvMPITypes(comm_pkg));
...
int hypre_CommTypeBuildMPI(int num_comms, int *comm_procs,
                           hypre_CommType **comm_types,
...
int hypre_CommTypeEntryBuildMPI(hypre_CommTypeEntry *comm_entry,
