...
#else
#define hypre_global_timing_ref(index, field) hypre_global_timing[index].field
#endif
...
int hypre_FinalizeTiming(int time_index)
{
  ...
  if (hypre_global_timing == NULL) return ierr;
      ...
      if (hypre_TimingNumRegs(i) > 0)
        {
          ...
          MPI_Allreduce(&local_wall_time, &wall_time, 1, MPI_DOUBLE, MPI_MAX, comm);
          MPI_Allreduce(&local_cpu_time, &cpu_time, 1, MPI_DOUBLE, MPI_MAX, comm);
                      ...
                      if (index >= 0)
                        {
                          for (threadid = 0; threadid < hypre_NumThreads; threadid++)
                            {
                              wall_mflops += hypre_TimingFLOPS(index) / wall_time / 1.0E6;
                            }
                        }
                      if (i < hypre_global_timing[my_thread].size)
                        {
                      ...
                      if (index >= 0)
                        {
                          for (threadid = 0; threadid < hypre_NumThreads; threadid++)
                            {
                              cpu_mflops += hypre_TimingFLOPS(index) / cpu_time / 1.0E6;
                            }
                        }
                      if (i < hypre_global_timing[my_thread].size)
                        {
