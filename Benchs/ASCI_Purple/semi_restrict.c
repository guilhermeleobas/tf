...
int hypre_SemiRestrictSetup(void *restrict_vdata, hypre_StructMatrix *R,
                            int R_stored_as_transpose, hypre_StructVector *r,
                            hypre_StructVector *rc, hypre_Index cindex,
                            hypre_Index findex, hypre_Index stride)
{
  ...
  hypre_ComputePkgCreate(send_boxes, recv_boxes, stride, stride, send_processes,
                         ...
                         hypre_StructVectorDataSpace(r), 1, &compute_pkg);
...
int hypre_SemiRestrict(void *restrict_vdata, hypre_StructMatrix *R,
  ...
  for (compute_i = 0; compute_i < 2; compute_i++)
  {
    ...
    hypre_ForBoxArrayI(ci, cgrid_boxes)
    {
      ...
      hypre_ForBoxI(j, compute_box_a)
      {
        ...
        hypre_BoxLoop3Begin(loop_size, R_dbox, startc, stridec, Ri, r_dbox,
                            start, stride, ri, rc_dbox, startc, stridec, rci);
#define HYPRE_BOX_SMP_PRIVATE loopk, loopi, loopj, Ri, ri, rci
#include "hypre_box_smp_forloop.h"
        hypre_BoxLoop3For(loopi, loopj, loopk, Ri, ri, rci)
        {
          rcp[rci] = rp[ri] + (Rp0[Ri] * rp0[ri] + Rp1[Ri] * rp1[ri]);
        }
