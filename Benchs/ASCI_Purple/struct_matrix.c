...
double *hypre_StructMatrixExtractPointerByIndex(hypre_StructMatrix *matrix,
                                                int b, hypre_Index index)
{
...
hypre_StructMatrix *hypre_StructMatrixCreate(MPI_Comm comm,
                                             ...
                                             hypre_StructStencil *user_stencil)
{
  ...
  for (i = 0; i < 6; i++) hypre_StructMatrixNumGhost(matrix)[i] = 0;
...
int hypre_StructMatrixInitializeShell(hypre_StructMatrix *matrix)
{
  ...
  if (hypre_StructMatrixStencil(matrix) == NULL)
  {
    ...
    if (!hypre_StructMatrixSymmetric(matrix))
    {
      ...
      for (i = 0; i < stencil_size; i++) symm_elements[i] = -1;
  ...
  if (hypre_StructMatrixDataIndices(matrix) == NULL)
  {
    ...
    hypre_ForBoxI(i, data_space)
    {
      ...
      for (j = 0; j < stencil_size; j++)
      {
        ...
        {
          data_indices[i][j] =
              data_indices[i][symm_elements[j]] +
...
#include "hypre_box_smp_forloop.h"
      hypre_BoxLoop1For(loopi, loopj, loopk, datai) { datap[datai] = 1.0; }
...
int hypre_StructMatrixSetValues(hypre_StructMatrix *matrix,
                                hypre_Index grid_index, int num_stencil_indices,
                                int *stencil_indices, double *values,
                                int add_to)
{
  int ierr = 0;
        ...
        for (s = 0; s < num_stencil_indices; s++)
        {
          ...
          if (add_to)
          {
            hypre_BoxLoop2Begin(loop_size, data_box, data_start, data_stride,
                                datai, dval_box, dval_start, dval_stride,
                                dvali);
...
int hypre_StructMatrixAssemble(hypre_StructMatrix *matrix)
{
  ...
  if (!comm_pkg)
  {
    ...
    hypre_CreateCommInfoFromNumGhost(hypre_StructMatrixGrid(matrix), num_ghost,
                                     &send_boxes, &recv_boxes, &send_processes,
                                     &recv_processes);

    comm_pkg = hypre_CommPkgCreate(
        send_boxes, recv_boxes, unit_stride, unit_stride,
        hypre_StructMatrixDataSpace(matrix),
        hypre_StructMatrixDataSpace(matrix), send_processes, recv_processes,
        hypre_StructMatrixNumValues(matrix), hypre_StructMatrixComm(matrix),
        hypre_StructGridPeriodic(hypre_StructMatrixGrid(matrix)));
  ...
  hypre_InitializeCommunication(comm_pkg, hypre_StructMatrixData(matrix),
                                hypre_StructMatrixData(matrix), &comm_handle);
...
int hypre_StructMatrixSetNumGhost(hypre_StructMatrix *matrix, int *num_ghost)
{
  ...
  for (i = 0; i < 6; i++) hypre_StructMatrixNumGhost(matrix)[i] = num_ghost[i];
...
#undef MPI_Comm_rank
  ...
  for (i = 0; i < hypre_StructStencilSize(stencil); i++)
  {
    if (symm_elements[i] < 0)
    {
      fprintf(file, "%d: %d %d %d\n", j++, hypre_IndexX(stencil_shape[i]),
              hypre_IndexY(stencil_shape[i]), hypre_IndexZ(stencil_shape[i]));
    }
  }
...
int hypre_StructMatrixMigrate(hypre_StructMatrix *from_matrix,
                              hypre_StructMatrix *to_matrix)
{
  ...
  hypre_CreateCommInfoFromGrids(hypre_StructMatrixGrid(from_matrix),
                                hypre_StructMatrixGrid(to_matrix), &send_boxes,
                                &recv_boxes, &send_processes, &recv_processes);

  comm_pkg = hypre_CommPkgCreate(
      send_boxes, recv_boxes, unit_stride, unit_stride,
      hypre_StructMatrixDataSpace(from_matrix),
      hypre_StructMatrixDataSpace(to_matrix), send_processes, recv_processes,
      hypre_StructMatrixNumValues(from_matrix),
      hypre_StructMatrixComm(from_matrix),
      hypre_StructGridPeriodic(hypre_StructMatrixGrid(from_matrix)));
  ...
  hypre_InitializeCommunication(comm_pkg, hypre_StructMatrixData(from_matrix),
...
hypre_StructMatrix *hypre_StructMatrixRead(MPI_Comm comm, char *filename,
                                           int *num_ghost)
{
  FILE *file;
  char new_filename[255];
...
#undef MPI_Comm_rank
  ...
  for (i = 0; i < stencil_size; i++)
  {
    fscanf(file, "%d: %d %d %d\n", &idummy, &hypre_IndexX(stencil_shape[i]),
           &hypre_IndexY(stencil_shape[i]), &hypre_IndexZ(stencil_shape[i]));
  }
