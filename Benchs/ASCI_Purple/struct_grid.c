...
int hypre_StructGridSetExtents(hypre_StructGrid *grid, hypre_Index ilower,
                               hypre_Index iupper)
{
  int ierr = 0;
  hypre_Box *box;
...
int hypre_StructGridSetHood(hypre_StructGrid *grid, hypre_BoxArray *hood_boxes,
                            int *hood_procs, int *hood_ids, int first_local,
                            int num_local, int num_periodic,
                            hypre_Box *bounding_box)
{
  ...
  hypre_BoxNeighborsCreate(hood_boxes, hood_procs, hood_ids, first_local,
                           num_local, num_periodic, &neighbors);
  ...
  if (hypre_StructGridNeighbors(grid) == NULL)
    {
      ...
      hypre_StructGridPeriodicAllBoxes(grid, &all_boxes, &all_procs, &first_local,
                                       &num_periodic);

      /* set all_ids */
      all_ids = hypre_TAlloc(int, hypre_BoxArraySize(all_boxes));
      hypre_ForBoxI(i, all_boxes) { all_ids[i] = i; }
      /* set neighbors */
      hypre_BoxNeighborsCreate(all_boxes, all_procs, all_ids, first_local,
                               num_local, num_periodic, &neighbors);
      hypre_StructGridNeighbors(grid) = neighbors;

      /* set ids */
      ids = hypre_TAlloc(int, hypre_BoxArraySize(boxes));
      hypre_ForBoxI(i, boxes) { ids[i] = all_ids[first_local + i]; }
      hypre_StructGridIDs(grid) = ids;
...
int hypre_GatherAllBoxes(MPI_Comm comm, hypre_BoxArray *boxes,
                         hypre_BoxArray **all_boxes_ptr, int **all_procs_ptr,
                         int *first_local_ptr)
{
  hypre_BoxArray *all_boxes;
  int *all_procs;
  ...
  MPI_Allgatherv(sendbuf, sendcount, MPI_INT, recvbuf, recvcounts, displs,
                 MPI_INT, comm);
...
int hypre_StructGridPeriodicAllBoxes(hypre_StructGrid *grid,
                                     hypre_BoxArray **all_boxes_ptr,
                                     int **all_procs_ptr, int *first_local_ptr,
                                     int *num_periodic_ptr)
{
  ...
  if (px != 0) i_periodic = 1;
  if (py != 0) j_periodic = 1;
  if (pz != 0) k_periodic = 1;
  ...
  if (!(i_periodic == 0 && j_periodic == 0 && k_periodic == 0))
    {
      ...
      new_num_all = num_all * ((1 + 2 * i_periodic) * (1 + 2 * j_periodic) *
                               (1 + 2 * k_periodic));
                          ...
                          for (i = first_i; i < (first_i + num_local); i++)
                            {
                              ...
                              /* shift box */
                              hypre_BoxIMinD(box, 0) = hypre_BoxIMinD(box, 0) + (ip * px);
                              hypre_BoxIMinD(box, 1) = hypre_BoxIMinD(box, 1) + (jp * py);
                              hypre_BoxIMinD(box, 2) = hypre_BoxIMinD(box, 2) + (kp * pz);
                              hypre_BoxIMaxD(box, 0) = hypre_BoxIMaxD(box, 0) + (ip * px);
                              hypre_BoxIMaxD(box, 1) = hypre_BoxIMaxD(box, 1) + (jp * py);
                              hypre_BoxIMaxD(box, 2) = hypre_BoxIMaxD(box, 2) + (kp * pz);
