...
int hypre_BoxNeighborsCreate(hypre_BoxArray *boxes, int *procs, int *ids,
    ...
    for (j = 0; j < num_local + num_periodic; j++)
    {
      ...
      if (i != ilocal)
      {
        ...
        for (d = 0; d < 3; d++)
        {
          ...
          diff = hypre_BoxIMinD(neighbor_box, d) - hypre_BoxIMaxD(local_box, d);
          if (diff > 0)
          {
            distance_index[d] = 1;
          ...
          diff = hypre_BoxIMinD(local_box, d) - hypre_BoxIMaxD(neighbor_box, d);
          if (diff > 0)
          {
            distance_index[d] = -1;
            distance = hypre_max(distance, diff);
        ...
        if (distance <= max_distance)
        {
          ...
          if (j < num_local)
          {
            ...
            hypre_RankLinkNext(rank_link) = hypre_BoxNeighborsRankLink(
                neighbors, j, distance_index[0], distance_index[1],
                distance_index[2]);
            hypre_BoxNeighborsRankLink(neighbors, j, distance_index[0],
                                       distance_index[1], distance_index[2]) =
                rank_link;
          }
        }
      }
