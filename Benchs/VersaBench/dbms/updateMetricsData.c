...
{                                        /*  begin updateMetricsData()   */
  ...
  assert(metrics);
  assert(!(metrics->lastCommand != INIT && metrics->lastCommand != INSERT &&
           metrics->lastCommand != QUERY && metrics->lastCommand != DELETE &&
           metrics->lastCommand != NONE && metrics->lastCommand != INVALID));
