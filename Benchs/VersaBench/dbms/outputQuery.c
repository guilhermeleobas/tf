...
#define NUM_OF_NON_KEY_ATTRIBUTES \
  (NUM_OF_LARGE_ATTRIBUTES - NUM_OF_KEY_ATTRIBUTES)
...
{                                        /*  beginning of outputQuery()  */
  ...
  assert(outputFile);
  assert(dataObject);
  assert(!(dataObject->type != SMALL && dataObject->type != MEDIUM &&
