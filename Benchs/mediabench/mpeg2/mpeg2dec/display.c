...
/* private prototypes */
static void Display_Image _ANSI_ARGS_((XImage * Ximage_Ptr,
                                       unsigned char *Dithered_Image));
static void Dither_Frame _ANSI_ARGS_((unsigned char *src[]));
static void Dither_Top_Field _ANSI_ARGS_((unsigned char *src[],
                                          unsigned char *dst));
static void Dither_Bottom_Field _ANSI_ARGS_((unsigned char *src[],
                                             unsigned char *dst));
...
static int HandleXError(Dpy, Event) Display *Dpy;
XErrorEvent *Event;
{
  gXErrorFlag = 1;
 ...
 */
void Initialize_Display_Process(name) char *name;
{
  int crv, cbu, cgu, cgv;
  int Y, Cb, Cr, R, G, B;
  int i;
  ...
  if (Display_Ptr == NULL) Error("Can not open display\n");
  ...
  Window_Instance =
      XCreateSimpleWindow(Display_Ptr, DefaultRootWindow(Display_Ptr), hint.x,
                          hint.y, hint.width, hint.height, 4, fg, bg);

  XSelectInput(Display_Ptr, Window_Instance, StructureNotifyMask);

  /* Tell other applications about this window */

  XSetStandardProperties(Display_Ptr, Window_Instance, hello, hello, None, NULL,
                         0, &hint);
 ...
 private
  ...
  for (i = 16; i < 240; i++)
    {
      ...
      if (XAllocColor(Display_Ptr, cmap, &xcolor) != 0)
      ...
      else
        {
          ...
          if (private) Error("Couldn't allocate private colormap");
         ...
         private
          ...
          cmap =
              XCreateColormap(Display_Ptr, Window_Instance, xwa.visual, AllocNone);
  ...
  if (Shmem_Flag)
    {
      Ximage_Ptr = XShmCreateImage(Display_Ptr, None, 8, ZPixmap, NULL, &Shminfo1,
                                   Coded_Picture_Width, Coded_Picture_Height);

      if (!progressive_sequence)
        Ximage_Ptr2 =
            XShmCreateImage(Display_Ptr, None, 8, ZPixmap, NULL, &Shminfo2,
                            Coded_Picture_Width, Coded_Picture_Height);
      ...
      if (Ximage_Ptr == NULL || (!progressive_sequence && Ximage_Ptr2 == NULL))
        {
          if (Ximage_Ptr != NULL) XDestroyImage(Ximage_Ptr);
      ...
      Shminfo1.shmid =
          shmget(IPC_PRIVATE, Ximage_Ptr->bytes_per_line * Ximage_Ptr->height,
                 IPC_CREAT | 0777);
      if (!progressive_sequence)
        Shminfo2.shmid =
            shmget(IPC_PRIVATE, Ximage_Ptr2->bytes_per_line * Ximage_Ptr2->height,
                   IPC_CREAT | 0777);

      if (Shminfo1.shmid < 0 || (!progressive_sequence && Shminfo2.shmid < 0))
        {
          XDestroyImage(Ximage_Ptr);
          if (!progressive_sequence) XDestroyImage(Ximage_Ptr2);
          if (!Quiet_Flag)
      ...
      if (Shminfo1.shmaddr == ((char *)-1) ||
          (!progressive_sequence && Shminfo2.shmaddr == ((char *)-1)))
        {
          ...
          if (!progressive_sequence)
            {
              XDestroyImage(Ximage_Ptr2);
              if (Shminfo2.shmaddr != ((char *)-1)) shmdt(Shminfo2.shmaddr);
            }
          if (!Quiet_Flag)
            {
      ...
      if (gXErrorFlag)
      ...
      else
        {
          ...
          if (!progressive_sequence) shmctl(Shminfo2.shmid, IPC_RMID, 0);
        }
    ...
    shmemerror:
      ...
      if (!progressive_sequence)
        {
          Ximage_Ptr2 =
              XCreateImage(Display_Ptr, None, 8, ZPixmap, 0, &dummy,
                           Coded_Picture_Width, Coded_Picture_Height, 8, 0);
...
static void Display_Image(Ximage_Ptr, Dithered_Image) XImage *Ximage_Ptr;
unsigned char *Dithered_Image;
{
/* display dithered image */
  ...
  if (Shmem_Flag)
    {
      XShmPutImage(Display_Ptr, Window_Instance, GC_Instance, Ximage_Ptr, 0, 0, 0,
                   0, Ximage_Ptr->width, Ximage_Ptr->height, True);
      XFlush(Display_Ptr);
      ...
      while (1)
        {
          ...
          XNextEvent(Display_Ptr, &xev);
          if (xev.type == CompletionType) break;
        }
    }
  else
#endif
    {
      ...
      XPutImage(Display_Ptr, Window_Instance, GC_Instance, Ximage_Ptr, 0, 0, 0, 0,
                Ximage_Ptr->width, Ximage_Ptr->height);
    }
}

void Display_Second_Field() { Display_Image(Ximage_Ptr2, Dithered_Image2); }
/* 4x4 ordered dither
 *
 * threshold pattern:
 *   0  8  2 10
...
void dither(src) unsigned char *src[];
{
  ...
  if (progressive_frame || Display_Progressive_Flag)
    ...
    {
      if ((picture_structure == FRAME_PICTURE && top_field_first) ||
...
static void Dither_Frame(src) unsigned char *src[];
{
  int i, j;
  int y, u, v;
...
static void Dither_Top_Field(src, dst) unsigned char *src[];
unsigned char *dst;
{
  int i, j;
  int y, Y2, u, v;
      ...
      for (i = 0; i < Coded_Picture_Width; i += 4)
        {
          ...
          v = *pv++ >> 1;
          *dst++ = Pixel[Y_Table[y] | Cb_Table[u] | Cr_Table[v]];
          *dst2++ = Pixel[Y_Table[((y + Y2) >> 1) + 12] | Cb_Table[u + 12] |
                          Cr_Table[v + 12]];
          ...
          *dst++ = Pixel[Y_Table[y + 8] | Cb_Table[u + 8] | Cr_Table[v + 8]];
          *dst2++ = Pixel[Y_Table[((y + Y2) >> 1) + 4] | Cb_Table[u + 4] |
                          Cr_Table[v + 4]];
          ...
          v = *pv++ >> 1;
          *dst++ = Pixel[Y_Table[y + 2] | Cb_Table[u + 2] | Cr_Table[v + 2]];
          *dst2++ = Pixel[Y_Table[((y + Y2) >> 1) + 14] | Cb_Table[u + 14] |
                          Cr_Table[v + 14]];
          ...
          *dst++ = Pixel[Y_Table[y + 10] | Cb_Table[u + 10] | Cr_Table[v + 10]];
          *dst2++ = Pixel[Y_Table[((y + Y2) >> 1) + 6] | Cb_Table[u + 6] |
                          Cr_Table[v + 6]];
        }
      ...
      for (i = 0; i < Coded_Picture_Width; i += 4)
        {
          ...
          v = *pv++ >> 1;
          *dst++ = Pixel[Y_Table[y + 3] | Cb_Table[u + 3] | Cr_Table[v + 3]];
          *dst2++ = Pixel[Y_Table[((y + Y2) >> 1) + 15] | Cb_Table[u + 15] |
                          Cr_Table[v + 15]];
          ...
          *dst++ = Pixel[Y_Table[y + 11] | Cb_Table[u + 11] | Cr_Table[v + 11]];
          *dst2++ = Pixel[Y_Table[((y + Y2) >> 1) + 7] | Cb_Table[u + 7] |
                          Cr_Table[v + 7]];
          ...
          v = *pv++ >> 1;
          *dst++ = Pixel[Y_Table[y + 1] | Cb_Table[u + 1] | Cr_Table[v + 1]];
          *dst2++ = Pixel[Y_Table[((y + Y2) >> 1) + 13] | Cb_Table[u + 13] |
                          Cr_Table[v + 13]];
          ...
          *dst++ = Pixel[Y_Table[y + 9] | Cb_Table[u + 9] | Cr_Table[v + 9]];
          *dst2++ = Pixel[Y_Table[((y + Y2) >> 1) + 5] | Cb_Table[u + 5] |
                          Cr_Table[v + 5]];
        }
...
static void Dither_Bottom_Field(src, dst) unsigned char *src[];
unsigned char *dst;
{
  int i, j;
  int y, Y2, u, v;
      ...
      for (i = 0; i < Coded_Picture_Width; i += 4)
        {
          ...
          *dst++ = Pixel[Y_Table[((y + Y2) >> 1) + 8] | Cb_Table[u + 8] |
                         Cr_Table[v + 8]];
          *dst2++ = Pixel[Y_Table[Y2 + 4] | Cb_Table[u + 4] | Cr_Table[v + 4]];
          ...
          u = *pu++ >> 1;
          v = *pv++ >> 1;
          *dst++ = Pixel[Y_Table[((y + Y2) >> 1) + 2] | Cb_Table[u + 2] |
                         Cr_Table[v + 2]];
          ...
          *dst++ = Pixel[Y_Table[((y + Y2) >> 1) + 10] | Cb_Table[u + 10] |
                         Cr_Table[v + 10]];
      ...
      for (i = 0; i < Coded_Picture_Width; i += 4)
        {
          ...
          u = *pu++ >> 1;
          v = *pv++ >> 1;
          *dst++ = Pixel[Y_Table[((y + Y2) >> 1) + 3] | Cb_Table[u + 3] |
                         Cr_Table[v + 3]];
          ...
          *dst++ = Pixel[Y_Table[((y + Y2) >> 1) + 11] | Cb_Table[u + 11] |
                         Cr_Table[v + 11]];
          *dst2++ = Pixel[Y_Table[Y2 + 7] | Cb_Table[u + 7] | Cr_Table[v + 7]];
          ...
          u = *pu++ >> 1;
          v = *pv++ >> 1;
          *dst++ = Pixel[Y_Table[((y + Y2) >> 1) + 1] | Cb_Table[u + 1] |
                         Cr_Table[v + 1]];
          ...
          *dst++ = Pixel[Y_Table[((y + Y2) >> 1) + 9] | Cb_Table[u + 9] |
                         Cr_Table[v + 9]];
...
static void Dither_Top_Field420(src, dst) unsigned char *src[];
unsigned char *dst;
{
  int i, j;
      ...
      for (i = 0; i < Coded_Picture_Width; i += 4)
        {
          ...
          Cr2 = *Cr2_ptr++ >> 1;
          *dst++ =
              Pixel[Y_Table[((3 * Y1 + Y2) >> 2)] | Cb_Table[Cb1] | Cr_Table[Cr1]];
          *dst2++ = Pixel[Y_Table[((Y1 + 3 * Y2) >> 2) + 12] |
                          Cb_Table[((3 * Cb1 + Cb2) >> 2) + 12] |
                          Cr_Table[((3 * Cr1 + Cr2) >> 2) + 12]];

          Y1 = *Y1_ptr++;
          Y2 = *Y2_ptr++;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 8] | Cb_Table[Cb1 + 8] |
                         Cr_Table[Cr1 + 8]];
          *dst2++ = Pixel[Y_Table[((Y1 + 3 * Y2) >> 2) + 4] |
                          Cb_Table[((3 * Cb1 + Cb2) >> 2) + 4] |
          ...
          Cb2 = *Cb2_ptr++ >> 1;
          Cr2 = *Cr2_ptr++ >> 1;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 2] | Cb_Table[Cb1 + 2] |
                         Cr_Table[Cr1 + 2]];
          *dst2++ = Pixel[Y_Table[((Y1 + 3 * Y2) >> 2) + 14] |
                          Cb_Table[((3 * Cb1 + Cb2) >> 2) + 14] |
                          Cr_Table[((3 * Cr1 + Cr2) >> 2) + 14]];

          Y1 = *Y1_ptr++;
          Y2 = *Y2_ptr++;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 10] | Cb_Table[Cb1 + 10] |
                         Cr_Table[Cr1 + 10]];
          *dst2++ = Pixel[Y_Table[((Y1 + 3 * Y2) >> 2) + 6] |
                          Cb_Table[((3 * Cb1 + Cb2) >> 2) + 6] |
      ...
      for (i = 0; i < Coded_Picture_Width; i += 4)
        {
          ...
          Cb2 = *Cb2_ptr++ >> 1;
          Cr2 = *Cr2_ptr++ >> 1;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 3] |
                         Cb_Table[((Cb1 + Cb2) >> 1) + 3] |
                         Cr_Table[((Cr1 + Cr2) >> 1) + 3]];
          *dst2++ = Pixel[Y_Table[((Y1 + 3 * Y2) >> 2) + 15] |
                          Cb_Table[((Cb1 + 3 * Cb2) >> 2) + 15] |
                          Cr_Table[((Cr1 + 3 * Cr2) >> 2) + 15]];

          Y1 = *Y1_ptr++;
          Y2 = *Y2_ptr++;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 11] |
                         Cb_Table[((Cb1 + Cb2) >> 1) + 11] |
                         Cr_Table[((Cr1 + Cr2) >> 1) + 11]];
          *dst2++ = Pixel[Y_Table[((Y1 + 3 * Y2) >> 2) + 7] |
                          Cb_Table[((Cb1 + 3 * Cb2) >> 2) + 7] |
          ...
          Cb2 = *Cb2_ptr++ >> 1;
          Cr2 = *Cr2_ptr++ >> 1;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 1] |
                         Cb_Table[((Cb1 + Cb2) >> 1) + 1] |
                         Cr_Table[((Cr1 + Cr2) >> 1) + 1]];
          *dst2++ = Pixel[Y_Table[((Y1 + 3 * Y2) >> 2) + 13] |
                          Cb_Table[((Cb1 + 3 * Cb2) >> 2) + 13] |
                          Cr_Table[((Cr1 + 3 * Cr2) >> 2) + 13]];

          Y1 = *Y1_ptr++;
          Y2 = *Y2_ptr++;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 9] |
                         Cb_Table[((Cb1 + Cb2) >> 1) + 9] |
                         Cr_Table[((Cr1 + Cr2) >> 1) + 9]];
          *dst2++ = Pixel[Y_Table[((Y1 + 3 * Y2) >> 2) + 5] |
                          Cb_Table[((Cb1 + 3 * Cb2) >> 2) + 5] |
...
static void Dither_Bottom_Field420(src, dst) unsigned char *src[];
unsigned char *dst;
{
  int i, j;
  int Y1, Cb1, Cr1, Y2, Cb2, Cr2;
  ...
  for (j = 0; j < Coded_Picture_Height; j += 4)
    {
      ...
      for (i = 0; i < Coded_Picture_Width; i += 4)
        {
          ...
          Cb2 = *Cb2_ptr++ >> 1;
          Cr2 = *Cr2_ptr++ >> 1;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 15] |
                         Cb_Table[((3 * Cb1 + Cb2) >> 2) + 15] |
                         Cr_Table[((3 * Cr1 + Cr2) >> 2) + 15]];
          *dst2++ =
              Pixel[Y_Table[((Y1 + 3 * Y2) >> 2)] | Cb_Table[((Cb1 + Cb2) >> 1)] |
                    Cr_Table[((Cr1 + Cr2) >> 1)]];

          Y1 = *Y1_ptr++;
          Y2 = *Y2_ptr++;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 7] |
                         Cb_Table[((3 * Cb1 + Cb2) >> 2) + 7] |
                         Cr_Table[((3 * Cr1 + Cr2) >> 2) + 7]];
          *dst2++ = Pixel[Y_Table[((Y1 + 3 * Y2) >> 2) + 8] |
                          Cb_Table[((Cb1 + Cb2) >> 1) + 8] |
          ...
          Cb2 = *Cb2_ptr++ >> 1;
          Cr2 = *Cr2_ptr++ >> 1;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 13] |
                         Cb_Table[((3 * Cb1 + Cb2) >> 2) + 13] |
                         Cr_Table[((3 * Cr1 + Cr2) >> 2) + 13]];
          *dst2++ = Pixel[Y_Table[((Y1 + 3 * Y2) >> 2) + 2] |
                          Cb_Table[((Cb1 + Cb2) >> 1) + 2] |
                          Cr_Table[((Cr1 + Cr2) >> 1) + 2]];

          Y1 = *Y1_ptr++;
          Y2 = *Y2_ptr++;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 5] |
                         Cb_Table[((3 * Cb1 + Cb2) >> 2) + 5] |
                         Cr_Table[((3 * Cr1 + Cr2) >> 2) + 5]];
          *dst2++ = Pixel[Y_Table[((Y1 + 3 * Y2) >> 2) + 10] |
                          Cb_Table[((Cb1 + Cb2) >> 1) + 10] |
      ...
      if (j != 0) dst += Coded_Picture_Width;
      ...
      for (i = 0; i < Coded_Picture_Width; i += 4)
        {
          ...
          Cb2 = *Cb2_ptr++ >> 1;
          Cr2 = *Cr2_ptr++ >> 1;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 12] |
                         Cb_Table[((Cb1 + 3 * Cb2) >> 2) + 12] |
          ...
          *dst2++ = Pixel[Y_Table[((Y1 + 3 * Y2) >> 2) + 3] | Cb_Table[Cb2 + 3] |
          ...
          Y1 = *Y1_ptr++;
          Y2 = *Y2_ptr++;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 4] |
                         Cb_Table[((Cb1 + 3 * Cb2) >> 2) + 4] |
          ...
          *dst2++ = Pixel[Y_Table[((Y1 + 3 * Y2) >> 2) + 11] | Cb_Table[Cb2 + 11] |
          ...
          Cb2 = *Cb2_ptr++ >> 1;
          Cr2 = *Cr2_ptr++ >> 1;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 14] |
                         Cb_Table[((Cb1 + 3 * Cb2) >> 2) + 14] |
          ...
          *dst2++ = Pixel[Y_Table[((Y1 + 3 * Y2) >> 2) + 1] | Cb_Table[Cb2 + 1] |
          ...
          Y1 = *Y1_ptr++;
          Y2 = *Y2_ptr++;
          *dst++ = Pixel[Y_Table[((3 * Y1 + Y2) >> 2) + 6] |
                         Cb_Table[((Cb1 + 3 * Cb2) >> 2) + 6] |
