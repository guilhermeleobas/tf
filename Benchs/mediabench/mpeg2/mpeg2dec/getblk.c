...
void Decode_MPEG1_Intra_Block(comp, dc_dct_pred) int comp;
int dc_dct_pred[];
{
  int val, i, j, sign;
  unsigned int code;
  ...
  for (i = 1;; i++)
    {
      ...
      if (val != 0)
        { /* should always be true, but it's not guaranteed */
...
void Decode_MPEG1_Non_Intra_Block(comp) int comp;
{
  int val, i, j, sign;
  unsigned int code;
  DCTtab *tab;
  ...
  for (i = 0;; i++)
    {
      ...
      j = scan[ZIG_ZAG][i];
      val = (((val << 1) + 1) * ld->quantizer_scale *
             ld->non_intra_quantizer_matrix[j]) >>
            4;
...
void Decode_MPEG2_Intra_Block(comp, dc_dct_pred) int comp;
int dc_dct_pred[];
{
...
#ifdef TRACE
  if (Trace_Flag) printf("DCT(%d)i:", comp);
#endif /* TRACE */
...
#ifdef TRACE
      if (Trace_Flag) printf("): %d/%d", run, sign ? -val : val);
#endif /* TRACE */
...
void Decode_MPEG2_Non_Intra_Block(comp) int comp;
{
...
#ifdef TRACE
  if (Trace_Flag) printf("DCT(%d)n:", comp);
#endif /* TRACE */
