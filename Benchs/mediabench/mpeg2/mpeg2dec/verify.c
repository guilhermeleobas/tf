...
static void Check_VBV_Delay _ANSI_ARGS_((int Bitstream_Framenum,
                                         int Sequence_Framenum));

void Check_Headers(Bitstream_Framenum,
...
static void Check_VBV_Delay(Bitstream_Framenum,
                            Sequence_Framenum) int Bitstream_Framenum;
int Sequence_Framenum;
{
  ...
  if ((Sequence_Framenum == 0) && (!Second_Field))
...
#ifdef DEBUG
      printf(
          "vbv_buffer_size (B) = %.0f, Bn=%f, E=%f, \nbitrate=%f, vbv_delay=%d "
          "frame_rate=%f\n",
    ...
    {
      /* derive the interval (I).  The interval tells us how many constant rate
     * bits
     ...
     */
      ...
      if (progressive_sequence) /* Annex C.9 (progressive_sequence==1,
                                 low_delay==0) */
        {
      ...
      else /* Annex C.11 (progressive_sequence==0, low_delay==0) */
        {
          T = 1 /
              (2 *
               frame_rate); /* inverse of two times the frame rate (field period) */
...
/* variables to keep track of the occurance of redundant headers between
 * pictures */
