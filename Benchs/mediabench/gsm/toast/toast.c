...
int(*output) P((gsm_signal *)), (*input)P((gsm_signal *));
int(*init_input) P((void)), (*init_output)P((void));
...
struct fmtdesc
{
  ...
  int(*input) P((gsm_signal *)), (*output)P((gsm_signal *));

} f_audio = {"audio",
             "8 kHz, 8 bit u-law encoding with Sun audio header",
             ".au",
             audio_init_input,
             audio_init_output,
             ulaw_input,
...
struct fmtdesc *alldescs[] = {&f_audio, &f_alaw, &f_ulaw, &f_linear,
                              (struct fmtdesc *)NULL};
...
static void version P0()
{
  printf("%s 1.0, version %s\n", progname,
         "$Id: toast.c 42693 2007-10-06 10:37:39Z asl $");
}
