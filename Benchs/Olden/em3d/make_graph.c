...
void make_all_neighbors(table_t *table, int groupname)
{
  ...
  make_neighbors(first_node, local_table_array, n_nodes / PROCS, d_nodes,
                 local_p, groupname);
...
void clear_nummiss(table_t *table, int groupname) { NumMisses = 0; }
void do_all(table_t *table, int groupname, int nproc, void func(table_t *, int),
            int groupsize)
{
