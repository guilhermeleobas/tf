...
int to_lincoln, to_off, to_3d_out, to_color, voronoi, delaunay, interactive,
    ahost;
char *see;
int NumNodes, NDim;
...
QUAD_EDGE connect_left(a, b) QUAD_EDGE a, b;
{
  VERTEX_PTR t1, t2;
...
QUAD_EDGE connect_right(a, b) QUAD_EDGE a, b;
{
  VERTEX_PTR t1, t2;
...
VERTEX_PTR get_low(tree) register VERTEX_PTR tree;
{
    ...
    b, c, d;
{
  ...
  adx = X(loc_a) - dx;
  ady = Y(loc_a) - dy;
  anorm = NORM(loc_a); /* <--- 4% load penalty */
  loc_b = b;
  bdx = X(loc_b) - dx;
  bdy = Y(loc_b) - dy;
  bnorm = NORM(loc_b); /* <--- 5% load penalty */
  loc_c = c;
  cdx = X(loc_c) - dx;
  cdy = Y(loc_c) - dy;
  cnorm = NORM(loc_c);
...
/****************************************************************/
QUAD_EDGE makeedge(origin, destination) VERTEX_PTR origin, destination;
{
...
int valid(l, basel) QUAD_EDGE l, basel;
{
...
void dump_quad(ptr) QUAD_EDGE ptr;
{
  int i;
  QUAD_EDGE j;
  VERTEX_PTR v;
...
void in_order(tree) VERTEX_PTR tree;
{
...
void free_all(cont, my_stack) int cont;
struct EDGE_STACK *my_stack;
{
  free(my_stack->elts);
  free(my_stack);
                            ...
                            int processor, int numnodes)
{
  ...
  /*printf("node = 0x%x\n",node);*/
  point =
      get_points(n / 2, X(node), i - 1, point.seed, processor, numnodes / 2);
  node->left = point.v;
  point.v = node;
  return point;
}
