...
int CountTree(QuadTree tree)
{
  ...
  if (nw == NULL && ne == NULL && sw == NULL && se == NULL)
    ...
    return CountTree(nw) + CountTree(ne) + CountTree(sw) + CountTree(se);
}
...
#ifdef DEBUG
  printf(
      "After MakeTree(2048...), tree=%p *tree is: color=%d  childtype=%d nw=%p "
      "ne=%p sw=%p se=%p parent=%p\n",
      tree, tree->color, tree->childtype, tree->nw, tree->ne, tree->sw,
      tree->se, tree->parent);
#endif
