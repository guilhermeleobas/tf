...
#define debug(level, str)            \
  {                                  \
    if (level < dbglvl) printf(str); \
  }
#define debug1(level, str, a)           \
  {                                     \
    if (level < dbglvl) printf(str, a); \
  }
#define debug2(level, str, a, b)           \
  {                                        \
    if (level < dbglvl) printf(str, a, b); \
  }
#define debug3(level, str, a, b, c)           \
  {                                           \
    if (level < dbglvl) printf(str, a, b, c); \
  }
#define debug4(level, str, a, b, c, d)           \
  {                                              \
    if (level < dbglvl) printf(str, a, b, c, d); \
  }
#else
#define debug(level, str)
#define debug1(level, str, a)
...
int spec_ungetc(unsigned char ch, int fd)
{
  ...
  {
    fprintf(stderr,
            "spec_ungetc: can't unget something that wasn't what was in the "
            "buffer!\n");
    exit(0);
  }
  debug1(4, "%d\n", rc);
