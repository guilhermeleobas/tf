...
#else
long primal_net_simplex(net) network_t *net;
#endif
{
  flow_t delta;
  flow_t new_flow;
      ...
      if ((bea = primal_bea_mpp(m, arcs, stop_arcs, &red_cost_of_bea)))
        {
          ...
          printf("it %ld: bea = (%ld,%ld), red_cost = %ld\n", *iterations,
                 bea->tail->number, bea->head->number, red_cost_of_bea);
#endif
          ...
          if (!iminus)
          ...
          else
            {
              ...
              update_tree(!xchange, new_orientation, delta, new_flow, iplus, jplus,
