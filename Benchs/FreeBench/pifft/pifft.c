...
---- calculation of PI(= 3.14159...) using FFT ----
    by T.Ooura, ver. LG1.1.2-MP1.5 Mar. 1999.

This is a test program to estimate the performance of
the FFT routines: fft4g.c, fft8g.c.
...
double mp_mul_radix_test(int n, int radix, int nfft, double tmpfft[], int ip[],
                         double w[]);
void mp_mul(int n, int radix, int in1[], int in2[], int out[], int tmp[],
            int nfft, double tmp1fft[], double tmp2fft[], double tmp3fft[],
            int ip[], double w[]);
void mp_squ(int n, int radix, int in[], int out[], int tmp[], int nfft,
            double tmp1fft[], double tmp2fft[], int ip[], double w[]);
void mp_mulh(int n, int radix, int in1[], int in2[], int out[], int nfft,
             double in1fft[], double outfft[], int ip[], double w[]);
void mp_squh(int n, int radix, int in[], int out[], int nfft, double inoutfft[],
             int ip[], double w[]);
int mp_inv(int n, int radix, int in[], int out[], int tmp1[], int tmp2[],
           int nfft, double tmp1fft[], double tmp2fft[], int ip[], double w[]);
int mp_sqrt(int n, int radix, int in[], int out[], int tmp1[], int tmp2[],
            int nfft, double tmp1fft[], double tmp2fft[], int ip[], double w[]);
...
int main(int argc, char *argv[])
{
  ...
  free(d2);
  free(d1);
  dgt = (char *)malloc((log10_radix * n + 32) * sizeof(char));
...
{
  int mp_unsgn_cmp(int n, int in1[], int in2[]);
  int mp_unexp_add(int n, int radix, int expdif, int in1[], int in2[],
                   int out[]);
  int mp_unexp_sub(int n, int radix, int expdif, int in1[], int in2[],
                   int out[]);
  ...
  if (outsgn >= 0)
    {
      ...
      if (expdif >= 0)
        {
          outexp += mp_unexp_add(n, radix, expdif, &in1[2], &in2[2], &out[2]);
        }
      else
        {
          outexp += mp_unexp_add(n, radix, -expdif, &in2[2], &in1[2], &out[2]);
        }
    }
  else
    {
      ...
      if (outsgn >= 0)
        {
          expdif = mp_unexp_sub(n, radix, expdif, &in1[2], &in2[2], &out[2]);
        }
      else
        {
          expdif = mp_unexp_sub(n, radix, -expdif, &in2[2], &in1[2], &out[2]);
        }
      outexp -= expdif;
      outsgn *= in1[0];
      if (expdif == n)
...
int mp_unexp_add(int n, int radix, int expdif, int in1[], int in2[], int out[])
{
  int j, x, carry;
...
int mp_unexp_sub(int n, int radix, int expdif, int in1[], int in2[], int out[])
{
            ...
            int ip[], double w[])
{
  void mp_add(int n, int radix, int in1[], int in2[], int out[]);
  void rdft(int n, int isgn, double *a, int *ip, double *w);
  void mp_mul_i2d(int n, int radix, int nfft, int shift, int in[],
                  double dout[]);
  void mp_mul_cmul(int nfft, double din[], double dinout[]);
  void mp_mul_cmuladd(int nfft, double din1[], double din2[], double dinout[]);
...
void mp_mulh_use_in1fft(int n, int radix, double in1fft[], int shift, int in2[],
                        int out[], int nfft, double outfft[], int ip[],
                        double w[])
{
  void rdft(int n, int isgn, double *a, int *ip, double *w);
  void mp_mul_i2d(int n, int radix, int nfft, int shift, int in[],
           ...
           int nfft, double tmp1fft[], double tmp2fft[], int ip[], double w[])
{
  ...
  int mp_inv_newton(int n, int radix, int in[], int inout[], int tmp1[],
                    int tmp2[], int nfft, double tmp1fft[], double tmp2fft[],
                    int ip[], double w[]);
  ...
  do
    {
      ...
      prc = mp_inv_newton(n_nwt, radix, in, out, tmp1, tmp2, nfft_nwt, tmp1fft,
                          tmp2fft, ip, w);
#ifdef DEBUG
            ...
            int nfft, double tmp1fft[], double tmp2fft[], int ip[], double w[])
{
  ...
  int mp_sqrt_newton(int n, int radix, int in[], int inout[], int inout_rev[],
                     int tmp[], int nfft, double tmp1fft[], double tmp2fft[],
                     int ip[], double w[], int *n_tmp1fft);
  ...
  do
    {
      ...
      prc = mp_sqrt_newton(n_nwt, radix, in, out, tmp1, tmp2, nfft_nwt, tmp1fft,
                           tmp2fft, ip, w, &n_tmp1fft);
#ifdef DEBUG
      printf("n=%d, nfft=%d, prc=%d\n", n_nwt, nfft_nwt, prc);
#endif
      if (thr * nfft_nwt >= nfft)
                  ...
                  int ip[], double w[])
{
  ...
  void mp_mulh(int n, int radix, int in1[], int in2[], int out[], int nfft,
               double in1fft[], double outfft[], int ip[], double w[]);
  void mp_mulh_use_in1fft(int n, int radix, double in1fft[], int shift,
                          int in2[], int out[], int nfft, double outfft[],
                          int ip[], double w[]);
  int n_h, shift, prc;
  ...
  mp_mulh(n, radix, inout, in, tmp1, nfft, tmp1fft, tmp2fft, ip, w);
  /* ---- tmp2 = 1 - tmp1 ---- */
  mp_load_1(n, radix, tmp2);
  mp_sub(n, radix, tmp2, tmp1, tmp2);
  /* ---- tmp2 -= inout * (lower) in (half precision) ---- */
  mp_mulh_use_in1fft(n, radix, tmp1fft, shift, in, tmp1, nfft, tmp2fft, ip, w);
  ...
  mp_mulh_use_in1fft(n_h, radix, tmp1fft, 0, tmp2, tmp2, nfft, tmp2fft, ip, w);
  /* ---- inout += tmp2 ---- */
                   ...
                   int ip[], double w[], int *n_tmp1fft)
{
  ...
  void mp_mulh(int n, int radix, int in1[], int in2[], int out[], int nfft,
               double in1fft[], double outfft[], int ip[], double w[]);
  void mp_squh(int n, int radix, int in[], int out[], int nfft,
               double inoutfft[], int ip[], double w[]);
  void mp_squh_use_in1fft(int n, int radix, double inoutfft[], int out[],
                          int nfft, int ip[], double w[]);
  ...
  if (*n_tmp1fft != nfft_h)
    {
      mp_squh(n_h, radix, inout_rev, tmp, nfft_h, tmp1fft, ip, w);
    }
  else
    {
      mp_squh_use_in1fft(n_h, radix, tmp1fft, tmp, nfft_h, ip, w);
    }
  /* ---- tmp = inout_rev - inout * tmp (half precision) ---- */
  mp_round(n, radix, shift, inout);
  mp_mulh(n_h, radix, inout, tmp, tmp, nfft, tmp1fft, tmp2fft, ip, w);
  mp_sub(n_h, radix, inout_rev, tmp, tmp);
  /* ---- inout_rev += tmp ---- */
  mp_add(n_h, radix, inout_rev, tmp, inout_rev);
  /* ---- tmp = in - inout^2 (half to normal precision) ---- */
  mp_squh_use_in1fft(n, radix, tmp1fft, tmp, nfft, ip, w);
  ...
  mp_mulh(n_h, radix, inout_rev, tmp, tmp, nfft, tmp1fft, tmp2fft, ip, w);
  *n_tmp1fft = nfft;
