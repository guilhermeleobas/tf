...
OBJ objs[] = {
    /* Object 1 */
    {{0, 4, 1.0}, 1, {{1.0, 0.4, 0.0}, 0.4, 0.8, 0.02}},
    /* Object 2 */
    {{-1, 3, 0.4}, 0.4, {{0.5, 0.3, 1.0}, 0.5, 0.9, 0.01}},
    /* Object 3 */
    {{-0.3, 1, 0.4}, 0.4, {{0.1, 0.95, 0.2}, 0.6, 0.8, 0.01}},
    /* Object 4 */
    {{1.0, 2, 0.4}, 0.4, {{0.86, 0.83, 0}, 0.7, 0.6, 0.01}}};
...
TEXTURE Groundtxt[2] = {
    {{0.0, 0.1, 0.5}, 0.8, 0.44, 0.02}, {{0.6, 1.0, 0.5}, 0.8, 0.44, 0.01},
};
...
static double IntersectObjs(VECTOR *LinP, VECTOR *LinD, VECTOR *Pnt,
                            VECTOR *Norm, TEXTURE **txt)
{
  int objn, tilenum;
  double t, ttmp, A, B, C;
  VECTOR Pos;
  ...
  for (objn = 0; objn < NUMOBJS; objn++)
  {
    ...
    C = (objs[objn].r * objs[objn].r - Pos.x * Pos.x - Pos.y * Pos.y -
         Pos.z * Pos.z) *
        A;
    if ((A = C + B * B) > 0.0)
    { /* ...else no hit */
...
static void TraceLine(VECTOR *LinP, VECTOR *LinD, VECTOR *Color, int reccount)
{
  ...
  if (reccount > 0)
  {
    ...
    if (t > EPSILON)
    {
      ...
      if (cosfi > 0.0)
      { /* If angle between lightline and normal < PI/2 */
        shadowcount = 0;
        if (usedist)
        {
        ...
        if (shadowcount > 0)
        {
          ...
          cosfi =
              (cosfi / sqrt(A)) * txt->diffuse * (double)shadowcount / DISTRIB;
        }
        else
        {
          cosfi = 0.0;
    ...
    else
    {
      ...
      if (A > 0.0)
        A = atan(fabs(LinD->z) / A) * 0.63661977;
      else
        A = 1.0;
...
static void TraceScene(void)
{
  ...
  for (sy = 0; sy < HEIGHT; sy++)
  {
    ...
    for (sx = 0; sx < WIDTH; sx++)
    {
      ...
      LinD.x = Cameraright.x * Scale.x + Cameradir.x * Scale.y +
               Cameraup.x * Scale.z;
      LinD.y = Cameraright.y * Scale.x + Cameradir.y * Scale.y +
               Cameraup.y * Scale.z;
      LinD.z = Cameraright.z * Scale.x + Cameradir.z * Scale.y +
               Cameraup.z * Scale.z;
...
int main(int c, char *v[])
{
  ...
  fprintf(stdout, "P6");
  fputc(10, stdout);
  fprintf(stdout, "%d %d", WIDTH, HEIGHT);
  fputc(10, stdout);
  fprintf(stdout, "255");
  fputc(10, stdout);
