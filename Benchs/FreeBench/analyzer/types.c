...
void hard_raw_mod(uint32 address, int w_epoch, int w_place, uint32 w_issue,
                  int r_epoch, int r_place, uint32 r_issue)
{
...
uint32 def_hash(uint32 address) { return address % def_table_size; }
...
void def_list_mod(uint32 address, int epoch, int place_in_epoch)
{
  ...
  {
    for (counter = def_table[place]; counter != NULL;
         counter = (def_list_t *)counter->next)
    {
      temp = counter;
      if (address == counter->address)
