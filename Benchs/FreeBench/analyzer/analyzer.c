...
void init_def_table(int def_size)
{
  ...
  for (i = 0; i < def_size; i++) def_table[i] = NULL;
}

void error(char *name) { exit(1); }
int main(int c, char *v[])
{
  ...
  int i;
  /* Settings */
  int data = 1, name = 0, verbose = 0, speed = 1, forward = 1, fastspeed = 0,
      imix = 1;
  int load_pen = 0, store_pen = 0, nofile = 1, kernel = 3, cpulimit = 0,
      configs = 0;
  int show_speedup = 3, thread_pen = 0, commit_pen = 0,
      use_epoch_length_as_num_of_epochs = 0;
  /* End -- settings */
  ...
  if (loops < epoch_length)
  {
    fprintf(
        stderr,
        "ERROR: Number of loop iterations is less than epoch length: %d<%d\n",
  ...
  if (fastspeed) speedup_test(fp);

  if (imix) mem_ops = imix_test(fp);
  ...
  if (data || name || speed)
  {
    ...
    while (1)
    {
      if (!strcmp(string, "START:"))
      {
        ...
        while (1)
        {
          sscanf(indata_lines[counter++], "%s %lx %lu", string, &address,
                 &issue_no);
                      ...
                      trans->address == address)
                  {
                    ...
                    if (def_placeholder != NULL)
                    {
                          ...
                          (def_placeholder->place_in_epoch == place_in_epoch))
                      {
                        ...
                        hard_raw_mod(
                            address, i, place_in_epoch, trans->issue_no,
                            varv / epoch_length, place_in_varv,
                            issue_no - epoch[varv / epoch_length].start_time);
                      }
                    }
                  }
          ...
          {
            fprintf(stderr,
                    "ALERT: \tName dependecy testing not implemented\n");
            exit(1);
          }
          if (verbose && !(varv % 10))
            fprintf(stderr, "\rProcessing %3.2f %%    ",
                    100 * (float)varv / (float)loops);
          place_in_varv++;
        }
      }
      else if (!strcmp(string, "END:"))
      {
        if (verbose) fprintf(stderr, "\rProcessing 100.00 %%    \n");
        break;
      }
      else
      {
  ...
  if (data)
  {
    ...
    {
      printf("%d RAW:s for 0x%lx\n", conf_iterator->accesser,
             conf_iterator->address);
  ...
  if (speed)
  {
    epoch[varv / epoch_length].end_time =
        epoch[varv / epoch_length].start_time +
        epoch[varv / epoch_length].last->issue_no;

    if (forward) find_hard_raws();

    speedup_test(fp);

    if (kernel == 1)
    {
      specul_time_o(epoch, num_epochs, graphfile, show_speedup, thread_pen,
                    commit_pen);
    }
    else if (kernel == 2)
    {
      specul_time_r(epoch, num_epochs, cpulimit, graphfile, show_speedup,
                    thread_pen, commit_pen);
      if (configs)
      {
        for (i = 1; i <= configs;)
        {
          specul_time_r(epoch, num_epochs, i, graphfile, show_speedup,
                        thread_pen, commit_pen);
          i = i * 2;
        }
      }
