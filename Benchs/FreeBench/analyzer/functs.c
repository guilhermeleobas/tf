...
uint32 imix_test(FILE *fp)
{
  ...
  /* printf("Instuction mix: Data accesses are %0.3g %% of
...
void specul_time_o(epoch_t *epoch, int num_epochs, FILE *graphfile,
                   int show_speedup, int thread_pen, int commit_pen)
{
  int i;
  uint32 max;
  uint32 restarts = 0;
...

  {
    ...
    while (iter != NULL)
      {
        if ((iter->w_issue + epoch[iter->w_epoch].stall_time) >
            epoch[iter->r_epoch].stall_time)
          {
            epoch[iter->r_epoch].stall_time =
                iter->w_issue + epoch[iter->w_epoch].stall_time;
            restarts++;
          }
        iter = iter->next;
      }
...
void specul_time_r(epoch_t *epoch, int num_epochs, int cpulimit,
                   FILE *graphfile, int show_speedup, int thread_pen,
                   int commit_pen)
{
  int i;
  uint32 max = 0;
  ...
  if (cpulimit == 0)
    { /* "Unlimited" amount of CPUs simulated */
        ...
        while (iter != NULL)
          {
            if ((iter->w_issue + epoch[iter->w_epoch].stall_time) >
              ...
              {
                epoch[iter->r_epoch].stall_time =
                    epoch[iter->w_epoch].run_time + epoch[iter->w_epoch].stall_time;
                restarts++;
              }
            iter = iter->next;
          }
    ...
    { /* Limited amount of CPUs simulated */
      int j;
