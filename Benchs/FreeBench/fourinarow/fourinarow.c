...
/* Prototypes */
int minimax_comp_ab(int depth, uint64 b1, uint64 b2, int *col, int alpha,
                    int beta);
int minimax_player_ab(int depth, uint64 b1, uint64 b2, int *col, int alpha,
                      int beta);
...
void init_board(char b[COLS][ROWS + 1])
{
  ...
  for (i = 0; i < COLS; i++)
    for (j = 0; j < ROWS; j++) b[i][j] = '.';

  for (i = 0; i < COLS; i++) b[i][ROWS] = 0;
}
...
int main(int c, char *v[])
{
  ...
  if (ab == 1) printf("Using pruning method 1\n");
  if (ab == 2) printf("Using pruning method 2\n");
