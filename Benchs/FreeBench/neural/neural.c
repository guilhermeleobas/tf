 ...
 * network using Hebbian/Delta learning. The program can be trained with all
 * letters
 * at once or step-by-step so that the progress can be studied closely.
 * Functions
 * for unlearning using random vectors and for creating generators for the
 * stored patterns using the clamping method are also available. For the
 * function
...
/*
char vnames[NUMPATS];
int vectors[NUMPATS][NNTOT], newvectors[NUMPATS][NNTOT],
generators[NUMPATS][NNTOT];
...
/* The main function types the menu and calls the appropriate function requested
 * by the user
...
int main(int c, char *v[])
{
  ...
  /* original menu system removed
   ...
   print original vectors");
      ...
      printf("\ng - compute generators for stored vectors and use the generators
   to print original vectors");
      printf("\nm - print T-matrix\nz - set continuous neuron mode\nx - set
   two-valued neuron mode (default)");
      printf("\nq - quit program\nOption > ");
      option=getchar();
      getchar();
      switch(option) {
...
static int hamming(signed int *ny, signed int *orig, int mode)
{
  ...
  for (neuron = 0; neuron < NNTOT; neuron++)
    if (ny[neuron] != (orig[neuron] * mode)) hd++;
  return hd;
} /* hamming */
 ...
 * is less than 2 the output is highlighted with square brackets because a
 * hamming
 * distance of <2 is not allowed and the original vectors will have to be
 * changed. */
static void checkham()
{
  int vec, comp, hd;
  ...
  if (hamwarn)
    printf(
        "WARNING: %d vectors have a hamming distance <2, please modify input "
        "vectors!\n",
        hamwarn);

} /* checkham */

/* Generates a T-matrix with Hebbian learning. Mode can be MODE_ALL or
 * MODE_TRACE.
...
static void generateT(int mode)
{
  ...
  for (row = 0; row < NNTOT; row++)
    for (col = 0; col < NNTOT; col++) Tmatrix[row][col] = 0.0;
      ...
      for (vec = 0; vec < NUMPATS; ++vec)
        {
          ...
          for (neuron = 0; neuron < NNTOT; ++neuron)
            if ((tempvecC[neuron] =
                     (real)(vectors[vec][neuron] - newvectors[vec][neuron]) * n) !=
                0.0)
              status = WFALSE;
...
/* Updates the vector in source with asynchronous updating and places the result
 * in dest.
 * This function uses the sgn function for the updating. The number of
 * iterations needed
...
static int run(signed int *source, signed int *dest)
{
  ...
  for (neuron = 0; neuron < NNTOT; neuron++) tempvecA[neuron] = source[neuron];
  ...
  while ((!stable) && (max < 500))
    {
      ...
      for (neuron = 0; neuron < NNTOT; neuron++)
        if (tempvecA[neuron] == 0) tempvecA[neuron] = tempvecB[neuron];
  ...
  for (neuron = 0; neuron < NNTOT; neuron++) dest[neuron] = tempvecA[neuron];

  return max;

} /* run */

/* Reads NUMPATS bit patterns from the file 'w.txt' and places them in
 * 'vectors'. The actual
...
/* Checks all original vectors to see if they are stored. All vectors are
 * written on the
 * screen. If a vector is stored the word 'Stored' appears beneath the typed
 * pattern,
 * otherwise the hamming distance between the original and computed vector is
 * displayed.
 * The number of iterations needed to reach a stable state is displayed below
 * the "hamming
...
#ifndef BENCHMARK
/* Prints the vectors in the NUMPATS vector storage variable 'vecs' on the
 * screen. -1's are
...
static void printT()
{
  ...
  for (row = 0; row < NNTOT; row++)
    {
      for (col = 0; col < NNTOT; col++) printf("%.1f ", Tmatrix[row][col]);
      putchar('\n');
    } /* for */
...
/* Prints the vector vect on screen as 1's and -1's. The argument 'vector' is
 * used to
 * indicate the number of the vector so that the actual letter can be printed
 * from vnames. */
static void printV(int vector, signed int vect[])
{
  int neuron;
  ...
  for (neuron = 0; neuron < NNTOT; ++neuron)
    if (vect[neuron] != 0) printf("%d ", vect[neuron]);
  putchar('\n');
} /* printV */

/* Applies random unlearning to the T-matrix. The user is asked to type in a
 * random
...
/* Creates generators for all (or n first) stored vectors in 'newvectors' and
 * places the
 * generators in the global variable 'generators'. The generators are found
 * using
 * the clamping method. */
#ifndef BENCHMARK
static void makegenerators(int n)
{
  ...
  for (vec = 0; vec < NUMPATS; vec++)
    for (neuron = 0; neuron < NNTOT; neuron++) generators[vec][neuron] = 0;
  ...
  for (vec = 0; vec < NUMPATS; vec++)
    {
      if (hamming(vectors[vec], newvectors[vec], MODE_NORMAL) == 0)
        {
          generator = 0;
          for (neuron = 0; neuron < NNTOT; neuron++) probe[neuron] = 0;
          do
            {
          ...
          while ((hamming(dest, newvectors[vec], MODE_NORMAL) != 0) &&
                 (generator < NNTOT));
          for (neuron = 0; neuron < NNTOT; neuron++)
...
static void usegenerators(int n)
{
  ...
  for (vec = 0; vec < NUMPATS; vec++)
    for (neuron = 0; neuron < NNTOT; neuron++) genvec[vec][neuron] = 0;
 ...
 * computed vector to 'dest'. But runcont uses continuous neurons. Number of
 * iterations
  ...
  while ((!stable) && (max < 500))
    {
      ...
      for (row = 0; row < NNTOT; row++)
        {
          ...
          tempvecA[row] =
              (1.0 - myexp(-1.0 * thesum)) / (1.0 + myexp(-1.0 * thesum));
        } /* for */
              ...
              if (fabs(tempvecA[row]) < 0.7)
                {
                  ...
                  for (neuron = 0; neuron < NNTOT; neuron++)
                    thesum += Tmatrix[row][neuron] * tempvecA[neuron];
                  if ((tempvecA[row] = (1.0 - myexp(-1.0 * thesum)) /
                                       (1.0 + myexp(-1.0 * thesum))) < 0.7)
                    threshold = WFALSE;
                } /* if */
            }
          maxcont++;
      ...
      if (hamming(dest, tempvecC, MODE_NORMAL) == 0)
        ...
        {
          neuron = 0;
          while ((neuron < NNTOT) && (dest[neuron] == tempvecC[neuron])) neuron++;
 ...
 */
static void mysrand(unsigned long seed) { randnum = seed; }
static real myrand()
{
