...
int32 TreeInsert(RBTree *tree, uint32 *attrs)
{
  ...
  if (xNd == NULL)
    {
      tree->count++;
      NEW_TREE_NODE(tree->mp, tree->memPool, tree->memaddr, tree->treeNodeSize,
      ...
      if (cmpres < 0)
        {
          ...
          if (yNd == NULL)
            {
              NEW_TREE_NODE(tree->mp, tree->memPool, tree->memaddr,
                            tree->treeNodeSize, tree->freeNodeCounter,
                            tree->memoryIsFull)
...
int32 WriteChunkToDisk(uint32 recordSize, FILE *fileOfChunks, treeNode *t,
                       FILE *logFile)
{
  if (!t)
    {
                        ...
                        unsigned char *memPool)
{
  ...
  tree->memaddr = 0;
  tree->treeNodeSize =
      sizeof(struct treeNode) + DIM_FSZ * (nd - 1) + MSR_FSZ * nm;
...
int32 DestroyTree(RBTree *tree)
{
  if (tree == NULL)
    {
      return ADC_TREE_DESTROY_FAILURE;
    }
