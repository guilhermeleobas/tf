...
void ShowFactorization(Factorization *nmbfct)
{
  ...
  for (i = 0; i < nmbfct->dim; i++)
  {
    ...
    {
      fprintf(stdout, "%ld^%ld", nmbfct->mlt[i], nmbfct->exp[i]);
    }
  }
  fprintf(stdout, "\n");
}

long int adcprime[] = {421,   601,   631,   701,  883,   419,   443,
                       647,   21737, 31769, 1427, 18353, 22817, 34337,
                       98717, 3527,  8693,  9677, 11093, 18233};
...
long long int GetLCM(long long int mask, Factorization **fctlist,
                     long int *adcexpons)
{
  int i = 0, j = 0, k = 0;
...
void ExtendFactors(long int nmb, long int firstdiv, Factorization *nmbfct,
...
long int adcexp[] = {11, 13, 17, 19, 23, 23, 29, 31, 37, 41,
                     41, 43, 47, 53, 59, 3,  5,  7,  11, 13};
long int adcexpS[] = {11, 13, 17, 19, 23};
long int adcexpW[] = {
    2 * 2, 2 * 2 * 2 * 5, 2 * 3,     2 * 2 * 5, 2 * 3 * 7, 23, 29,
    31,    2 * 2,         2 * 2 * 19};
long int adcexpA[] = {2 * 2,
                      2 * 2 * 2 * 5,
                      2 * 3,
                      2 * 2 * 5,
                      2 * 3 * 7,
...
{
  long long int *size0 = (long long int *)sz0, *size1 = (long long int *)sz1;
  int res = 0;
  if (*size0 - *size1 > 0)
  {
    res = 1;
  }
  else if (*size0 - *size1 < 0)
  {
    res = -1;
  }
  return res;
...
int CompareViewsBySize(const void *vw0, const void *vw1)
{
  ...
  int res = 0;
  if (lvw0->vsize > lvw1->vsize)
  {
    res = 1;
  }
  else if (lvw0->vsize < lvw1->vsize)
  {
    res = -1;
  }
  else if (lvw0->vidx > lvw1->vidx)
  {
    res = 1;
  }
  else if (lvw0->vidx < lvw1->vidx)
...
int ParseParFile(char *parfname, ADC_PAR *par)
{
  ...
  fclose(parfile);
  switch (par->clss)
  { /* overwriting parameters according the class */
    case 'S':
      par->dim = 5;
      par->mnum = 1;
      par->tuplenum = 1000;
...
long int adcgen[] = {2, 7, 3, 2, 2, 2, 2, 5, 31, 7,
                     2, 3, 3, 3, 2, 5, 2, 2, 2,  3};

int GetNextTuple(int dcdim, int measnum, long long int *attr,
                 long long int *meas, char clss)
{
  ...
  if (dcdim > maxdim)
  {
    fprintf(stderr, "GetNextTuple: number of dcdim is too large:%d", dcdim);
    return 0;
  }
  if (measnum > measbound)
  {
    fprintf(stderr, "GetNextTuple: number of mes is too large:%d", measnum);
    return 0;
  }
  locexp = adcexp;
  switch (clss)
...
int GenerateADC(ADC_PAR *par)
{
  int dcdim = par->dim, mesnum = par->mnum, tplnum = par->tuplenum;
  ...
  */
  ...
  fprintf(stdout,
          "\nGenerateADC: writing %d tuples of %d attributes and %d measures "
          "to %s\n",
          tplnum, dcdim, mesnum, adcfname);
  for (i = 0; i < tplnum; i++)
  {
    if (!GetNextTuple(dcdim, mesnum, attr, mes, par->clss))
    {
      return 0;
    }
    /*
