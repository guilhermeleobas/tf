...
int32 ComputeMemoryFittedView(ADC_VIEW_CNTL *avp)
{
  ...
  for (iRec = 1; iRec <= avp->nRowsToRead; iRec++)
    {
      SelectToView(&avp->inpDataBuffer[ib], avp->selection, viewBuf, avp->nd,
                   avp->nm, avp->nv);
      ib += inpBufferPace;
...
void SeekAndReadNextSubChunk(uint32 multiChunkBuffer[], uint32 k, FILE *inFile,
                             uint32 chunkRecSize, uint64 inFileOffs,
                             uint32 subChunkNum)
{
  int64 ret;
...
void ReadSubChunk(uint32 chunkRecSize, uint32 *multiChunkBuffer,
                  uint32 mwBufRecSizeInInt, uint32 iChunk,
  ...
  if (chunks[iChunk].curChunkNum > 0)
    {
      if (chunks[iChunk].curChunkNum < regSubChunkSize)
        {
          SeekAndReadNextSubChunk(
              multiChunkBuffer, (iChunk * regSubChunkSize + (regSubChunkSize - chunks[iChunk].curChunkNum)) * mwBufRecSizeInInt,
              fileOfChunks, chunkRecSize, chunks[iChunk].chunkOffset,
      ...
      else
        {
          SeekAndReadNextSubChunk(multiChunkBuffer,
                                  ...
                                  fileOfChunks, chunkRecSize,
                                  chunks[iChunk].chunkOffset, regSubChunkSize);
...
int32 MultiWayMerge(ADC_VIEW_CNTL *avp)
{
  ...
  for (i = 0; i < avp->numberOfChunks; i++)
    {
      ReadSubChunk(chunkRecSize, multiChunkBuffer, mwBufRecSizeInInt, i,
  ...
  while (1)
    {
          ...
          if (avp->chunksParams[iChunk].curSubChunk > 0)
            {
              ...
              compRes =
                  KeyComp(&r_buf[2 * avp->nm], &min_r_buf[2 * avp->nm], avp->nv);
              if (compRes < 0)
                {
      ...
      if (first_one)
        ...
        {
          compRes =
              KeyComp(&outputBuffer[2 * avp->nm], &min_r_buf[2 * avp->nm], avp->nv);
          if (!compRes)
            {
              for (i = 0; i < avp->nm; i++)
                {
      ...
      for (numEmptyBufs = 0, numEmptyRuns = 0, i = 0; i < avp->numberOfChunks;
           i++)
        {
      ...
      if (numEmptyBufs == avp->numberOfChunks &&
          numEmptyRuns == avp->numberOfChunks)
        {
          break;
        }
      ...
      if (avp->chunksParams[min_r_chunk].curSubChunk == 0)
        {
          ReadSubChunk(chunkRecSize, multiChunkBuffer, mwBufRecSizeInInt,
                       min_r_chunk, regSubChunkSize, avp->chunksParams,
                       avp->fileOfChunks);
        }
    } /* while(1) */
...
void SelectToView(uint32 *ib, uint32 *ix, uint32 *viewBuf, uint32 nd, uint32 nm,
                  uint32 nv)
{
  uint32 i, j;
  for (j = 0, i = 0; i < nv; i++)
...
void AdcFileName(char *adcFileName, const char *adcName, const char *fileName,
                 uint32 taskNumber)
{
...
ADC_VIEW_CNTL *NewAdcViewCntl(ADC_VIEW_PARS *adcpp, uint32 pnum)
{
  ...
  AdcFileName(adccntl->adcLogFileName, adccntl->adcName, "logf",
              adccntl->taskNumber);
  ...
  AdcFileName(adccntl->viewSizesFileName, adccntl->adcName, "viewsz.dat",
              adccntl->taskNumber);
  ...
  adccntl->jpp =
      (JOB_POOL *)malloc((adccntl->nViewLimit + 1) * sizeof(JOB_POOL));
  if (adccntl->jpp == NULL)
    {
      fprintf(adccntl->logf,
...
void InitAdcViewCntl(ADC_VIEW_CNTL *adccntl, uint32 nSelectedDims,
                     uint32 *selection, uint32 fromParent)
{
  uint32 i;
  ...
  while (fread(&binRepTuple, 8, 1, adccntlp->groupbyFile))
    {
      ...
      GetRegTupleFromBin64(binRepTuple, adccntlp->selection, adccntlp->nTopDims,
                           &nSelectedDims);
      ...
      if (smp != noneParent)
        {
          GetRegTupleFromParent(binRepTuple, adccntlp->parBinRepTuple,
                                adccntlp->selection, adccntlp->nTopDims);
        }
      InitAdcViewCntl(adccntlp, nSelectedDims, adccntlp->selection,
                      (smp == noneParent) ? 0 : 1);
#ifdef IN_CORE
      ...
      if (smp == prefixedParent)
        {
          if ((retCode == PrefixedAggregate(adccntlp, adccntlp->viewFile)))
            {
              fprintf(stderr, "ComputeGivenGroupbys.PrefixedAggregate failed.\n");
              return retCode;
            }
      ...
      else if (smp == sharedSortParent)
        {
          if ((retCode == SharedSortAggregate(adccntlp)))
            {
              fprintf(stderr, "ComputeGivenGroupbys.SharedSortAggregate failed.\n");
              return retCode;
            }
          ...
          if (!adccntlp->numberOfChunks)
            {
              ...
              retCode =
                  WriteViewToDiskCS(adccntlp, adccntlp->tree->root.left, &ordern);
              if (retCode != ADC_OK)
                {
                  fprintf(stderr,
      ...
      WriteOne32Tuple(ttout, adccntlp->groupby, adccntlp->nTopDims,
                      adccntlp->logf);
