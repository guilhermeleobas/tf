...
int32 GetParent(ADC_VIEW_CNTL *avp, uint32 binRepTuple)
{
  ...
  for (spFound = pfFound = ssFound = 0, level = l; level <= avp->nTopDims;
       level++)
    {
      ...
      {
        for (spFt = pfFt = ssFt = 1, ig = avp->lpp[level].layerIndex, i = 0;
             i < levelPop; i++)
          {
      ...
      if (found == prefixedParent || found == sharedSortParent ||
          found == smallestParent)
        {
          break;
        }
...
void GetRegTupleFromBin64(uint64 binRepTuple, uint32 *selTuple, uint32 numDims,
                          uint32 *numOfUnits)
{
  uint64 oc = MLB;
  uint32 i;
...
void getRegTupleFromBin32(uint32 binRepTuple, uint32 *selTuple, uint32 numDims,
                          uint32 *numOfUnits)
{
  uint32 oc = MLB32;
  uint32 i;
...
void GetRegTupleFromParent(uint64 bin64RepTuple, uint32 bin32RepTuple,
                           uint32 *selTuple, uint32 nd)
{
  uint32 oc = MLB32;
  uint32 i, j, k;
  uint32 ut32;
...
uint32 MultiFileProcJobs(TUPLE_VIEWSIZE *tuplesAndSizes, uint32 nViews,
                         ADC_VIEW_CNTL *avp)
{
  uint32 i;
