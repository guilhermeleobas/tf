  ...
  while (counter != ncol) /* for every column */
    {
      if (data[counter] > low)
        {
          break; /* if you get something higher */
        }
...
int in_wanted(int *data) /* checks if the current _data is in the wanted list */
{
  ...
  if (current == NULL)
    {
      return 0; /* if at the end, not found */
    }
  return 1;
}
...
/* contents for the specified row & col */
{
  ...
  for (count = 0; count != col; count++)
    { /* creates col-1 cells with nrow */
      new[count] = nrow;
    }
  for (; count != ncol; count++)
    { /* and the rest with row as value */
      new[count] = row;
    }
...
void make_wanted(
    int *data) /* makes up the list of positions from the full board */
{
...
int *get_good_move(
    struct _list *list) /* gets the first good move from a _list */
{
  if (list == NULL)
    {
      return NULL; /* if list is NULL, say so */
    }
...
int *get_winning_move(
    struct _play *play) /* just scans for the first good move */
/* in the last _list of a _play. This */
{ /* is the full board */
  int *temp;
  while (play->next != NULL)
    {
      play = play->next; /* go to end of _play */
    }
...
struct _list *where(int *data, struct _play *play)
{
  while (!equal_data(play->state, data))
    { /* search for given _data */
      play = play->next;
    }
  return play->first; /* return the pointer */
}

void get_real_move(int *data1, int *data2, int *row,
                   int *col) /* returns row & col of the move */
	...
	scanf("%d",&nrow);
      ...
      case 3:
        ...
        while (look != NULL)
          {
            if (look->value == 0)
              { /* show all positions bad for player 2 */
