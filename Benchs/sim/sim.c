...
static char *ckalloc(long amount)
{
  ...
  if ((p = malloc((unsigned)amount)) == NULL) fatal("Ran out of memory.");
  return (p);
}
#endif
      ...
      switch (argv[argc][0])
        {
          ...
          case 'E':
            ...
            break;
          default:
            fatal("options are M, I, V, O and E.");
        }
    }

  printf("\t\tSIM output with parameters:\n");
  printf("\t\tM = %g, I = %g, V = %g\n\t\tO = %g, E = %g\n\n", parm_M, parm_I,
         parm_V, parm_O, parm_E);

  /* set up scoring matrix */
  V['A']['A'] = V['C']['C'] = V['G']['G'] = V['T']['T'] = round(10 * parm_M);

  V['A']['G'] = V['G']['A'] = V['C']['T'] = V['T']['C'] = round(10 * parm_I);

  V['A']['C'] = V['A']['T'] = V['C']['A'] = V['C']['G'] = V['G']['C'] =
      V['G']['T'] = V['T']['A'] = V['T']['G'] = round(10 * parm_V);
...
big_pass(A, B, M, N, K, nseq) char A[], B[];
long M, N, K, nseq;
{
  register long i, j; /* row and column indices */
...
small_pass(A, B, count, nseq) char A[], B[];
long count, nseq;
{
  register long i, j; /* row and column indices */
  register long c; /* best score at current point */
...
long diff(A, B, M, N, tb, te) char *A, *B;
long M, N;
long tb, te;
...
long display(A, B, M, N, S, AP, BP) char A[], B[];
long M, N;
long S[], AP, BP;
{
  register char *a, *b, *c;
...
FILE *ckopen(name, mode) char *name, *mode;
{
  FILE *fopen(), *fp;
