...
void unsupp(s, hint) char *s, *hint;
{
  ...
  if (hint) (void)fprintf(stderr, "\t(maybe you mean \"%s\")\n", hint);
}
...
void notsupported(compiler, type1, type2) char *compiler, *type1, *type2;
{
  if (type2)
    ...
    (void)fprintf(stdout, "Warning: Unsupported in%s C%s -- '%s'\n", compiler,
                  CplusplusFlag ? "++" : "", type1);
}
...
char *cat VA_DCL(char *, s1)
{
  ...
  for (; str; str = va_arg(args, char *)) len += strlen(str);
  va_end(args);
...
void rmtmpfile()
{
  if (file4tmpfile) (void)unlink(file4tmpfile);
}
#else
...
int getopt(argc, argv, optstring) char **argv;
char *optstring;
{
  int ret;
  char *p;
  ...
  char *cpptext; /* C++ specific text */
    ...
    {/* up-to 23 lines of help text so it fits on (24x80) screens */
     ...
     /* 13 */ {"  [{ const | volatile | noalias }] pointer to <english>",
               "  [{const|volatile}] {pointer|reference} to [member of class "
               "<name>] <english>"},
     /* 14 */ {"  <type>", 0},
     /* 15 */ {"type:", 0},
     ...
     /* 20 */ {"gibberish: a C declaration, like 'int *x', or cast, like '(int "
               "*)x'",
               0},
     /* 21 */ {"storage-class: extern, static, auto, register", 0},
     /* 22 */ {"C-type: int, char, float, double, or void", 0},
     /* 23 */ {"modifier: short, long, signed, unsigned, const, volatile, or "
               "noalias",
...
void setprogname(argv0) const char *argv0;
{
#ifdef DOS
  char *dot;
#endif /* DOS */
...
#ifdef DOS
  if (!progname) progname = strrchr(argv0, '\\');
#endif /* DOS */
  ...
  dot = strchr(progname, '.');
  if (dot) *dot = '\0';
  for (dot = progname; *dot; dot++) *dot = tolower(*dot);
#endif /* DOS */
}
...
int namedkeyword(argn) char *argn;
{
  ...
  for (cmdptr = cmdlist; *cmdptr; cmdptr++)
    if (strcmp(*cmdptr, argn) == 0) return 1;
...
int dotmpfile(argc, argv) int argc;
char **argv;
{
  int ret = 0;
  FILE *tmpfp = tmpfile();
...
int dofileargs(argc, argv) int argc;
char **argv;
{
  FILE *ifp;
  int ret = 0;
...
void dodexplain(storage, constvol, type, decl) char *storage, *constvol, *type,
    *decl;
{
  ...
  if (*storage) (void)printf("%s ", storage);
  (void)printf("%s", decl);
  if (*constvol) (void)printf("%s ", constvol);
  (void)printf("%s\n", type ? type : "int");
}
...
void docexplain(constvol, type, cast, name) char *constvol, *type, *cast, *name;
{
  ...
  if (strlen(constvol) > 0) (void)printf("%s ", constvol);
  (void)printf("%s\n", type);
}
