...
void PASS1(FILE *INPUT_FILE, SYMBOL_TABLE *SYM_TABLE, FILE *TEMP_OUTPUT_STREAM)
{
  ...
  while (!feof(INPUT_FILE))
  {
    ...
    SCAN_LINE(LOCATION_COUNTER, &INPUT_LINE, LABEL_NAME, &EXTENDED_CODE, OPCODE,
              &ARGUMENTS, pass1, TEMP_OUTPUT_STREAM, INPUT_FILE);
    ...
    if (strcmp(OPCODE, ""))
    {
      /* --------- Have an OPERATOR. Change location counter depending on
       * format.  */
      ...
      switch ((*(OPCODE_INFO = LOOK_UP_OP(OPCODE))).FORMAT)
      {
        case NOT_FOUND:
          ...
          break;
        case ONE:
          CHANGE_LOCATION(&LOCATION_COUNTER, 1, TEMP_OUTPUT_STREAM);
          if (EXTENDED_CODE)
          {
            (void)fprintf(TEMP_OUTPUT_STREAM,
          ...
          break;
        case TWO:
          CHANGE_LOCATION(&LOCATION_COUNTER, 2, TEMP_OUTPUT_STREAM);
          if (EXTENDED_CODE)
          {
            (void)fprintf(TEMP_OUTPUT_STREAM,
                          "eERROR[8]: + is an illegal prefix to %s.", OPCODE);
        ...
        case THREE_FOUR:
          ...
          {
            CHANGE_LOCATION(&LOCATION_COUNTER, 3, TEMP_OUTPUT_STREAM);
          }
          break;
        case PSEUDO:
          if (EXTENDED_CODE)
          ...
          /* *************************************************************************
           */
          /* The following instruction calls the correct procedure to do what
           * needs to */
          /* be done for a particular pseudo operator. The operator table
           * contains     */
