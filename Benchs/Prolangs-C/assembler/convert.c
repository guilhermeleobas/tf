...
void RESET_MISSING_LABEL_NAME() { (void)strcpy(MISSING_LABEL_NAME, "_!"); }
  ...
  for (COUNT = MAX_NUM; COUNT >= 1; COUNT /= NUM_BASE)
    {
      ...
      if ((DIGIT >= 0) && (DIGIT <= 9))
        ...
        {
          (void)fprintf(OUTPUT, "%c", (char)(DIGIT - 10 + 'A'));
        }
    }
}
...
int GET_NUM(char **CURRENT_CHAR, int BITS, int NUM_BASE)
{
  ...
  if (((CONVERT > MAX_UNSIGNED_INT) && (SIGN == -1)) ||
      ((CONVERT > (MAX_UNSIGNED_INT - 1)) && (SIGN == 1)))
    {
      ...
      **CURRENT_CHAR = '\0';
      ERROR_MSG =
          (char *)malloc((unsigned int)(80 + ((int)log10((double)NUM_BASE)) +
                                        ((int)log10((double)MAX_UNSIGNED_INT)) +
                                        strlen(INPUT_START)));
          ...
          if (eoln(**CURRENT_CHAR))
          ...
          else
            {
              ...
              **CURRENT_CHAR = '\0';
              ...
              ERROR_MSG = (char *)malloc((unsigned int)(80 + strlen(START)));

              (void)sprintf(ERROR_MSG, "eERROR[17]: Illegal Hexidecimal. %s", START);
              ADD_TO_END_OF_BUFFER(&ERROR_REC_BUF, ERROR_MSG);
              free(ERROR_MSG);
              **CURRENT_CHAR = TEMP;
            }
    ...
    if (!strcmp(LABEL_NAME, ""))
      {
        ...
        ERROR_MSG = (char *)malloc((unsigned int)(80 + strlen(*CURRENT_CHAR)));
        (void)sprintf(ERROR_MSG,
    ...
    else
      {
        ...
        if (LABEL_INFO == NULL)
          {
            ...
            ERROR_MSG = (char *)malloc((unsigned int)(80 + strlen(LABEL_NAME)));
            if ((*WHAT_KIND) != ABSOLUTE_OR_RELATIVE_1)
              {
                (void)sprintf(ERROR_MSG,
                              "eERROR[24]: %s is not a defined symbol or is "
                              "forwardly declared.",
                              LABEL_NAME);
              }
            else
        ...
        else if ((*WHAT_KIND != RELATIVE_VALUE_1) &&
                 ((*LABEL_INFO).TYPE == ABSOLUTE))
          {
            (*WHAT_KIND) = ABSOLUTE_VALUE_1;
            return (*LABEL_INFO).LOCATION;
          }
        else if ((*WHAT_KIND != ABSOLUTE_VALUE_1) &&
                 ((*LABEL_INFO).TYPE != ABSOLUTE))
          {
