  ...
  while (!feof(INPUT_STREAM))
  {
    ...
    switch (TAG_FIELD)
    {
      ...
      case 't':
      {
         ...
         */
        /* Handle approperiately. (i.e. Let the text record handler deal with
         * it).   */
        char TEMP_CH;
        int LOCATION;
      ...
      case 'E':
        /* What follows is an end record... pass it and the 'E' to the object
         * file   */
        ...
        if (!strcmp(MAIN_ROUTINE, MODULE_NAME))
        {
          ...
          {
            START_ADDRESS =
                (*LOOK_UP_SYMBOL(MODULE_NAME, MODULE_NAME, SYM_TAB)).LOCATION;
          }

          NUM_TO_STR(START_ADDRESS, 16,
                     MEM_ADDR_SIZE_1 / BITS_PER_HALFBYTE_1 + 1, ADDR);
          (void)fprintf(OBJECT_STREAM, "E%s\n", ADDR);
        }
        else
        {
          (void)fprintf(OBJECT_STREAM, "E\n");
        }
        GET_LINE(&REST_OF_LINE, INPUT_STREAM);
        break;

      case 'R':
        /* What follows is a refer record. Pass it and the 'R' to the object
         * file.   */
        GET_LINE(&REST_OF_LINE, INPUT_STREAM);
      ...
      case 'M':
        /* What follows is a modification record. Put it on the modification
         * record  */
        /* buffer. */
        GET_LINE(&REST_OF_LINE, INPUT_STREAM);
      ...
      case 'd':
      {
        /* What follows is a define record. Pass1 didn't know the locations,
         * just    */
        /* the names. It made sure that the the defs will fit in one record
         * though,  */
             ...
             REST_OF_LINE += LABEL_SIZE_1)
        {
          ...
          {
            (void)fprintf(LISTING_STREAM,
                          "ERROR[42]: %s is wrong type. Expected LABEL, found "
                          "EXTREF or CONSTANT.\n",
                          TEMP);
            (void)fprintf(OBJECT_STREAM, "000000");
            (*ERROR) = 1;
