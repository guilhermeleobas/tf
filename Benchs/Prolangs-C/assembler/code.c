                 ...
                 FILE *LISTING_STREAM)
{
  ...
  if ((strcmp(OPERATOR, "WORD")) && (strcmp(OPERATOR, "BYTE")))
     ...
     */
      ...
      if (CH == 't')
        {
          /* -------------- Code in text record. Put into text record and listing
       * file */
        ...
        {
          /* --------------- No code was generated for the BYTE/WORD because of
       * errors */
      ...
      for (BYTE_COUNTER = 0; BYTE_COUNTER < BYTES; BYTE_COUNTER++)
        {
          ...
          for (BIT_COUNTER = 0; BIT_COUNTER < 4; BIT_COUNTER++)
            {
              CH = CH * 2 +
                   ((int)CHAR_TO_DIGIT(CODE[BYTE_COUNTER * 4 + BIT_COUNTER], 2));
            }
          if ((CH >= 0) && (CH <= 9))
            {
            ...
            {
              NUM_STR[BYTE_COUNTER] = (char)(CH - 10) + 'A';
            }
        }
      NUM_STR[BYTES] = '\0';
               ...
               FILE *LISTING_STREAM, int *ERROR)
{
  ...
  for (I = 0; I < LARGEST_INSTRUCT_SIZE_1; I++)
    {
      INSTRUCT_CODE[I] = '0';
    }
  SET_BITS_TO(0, 7, (CHAR_TO_DIGIT((*OP_CODE).OPCODE[0], 16) * 16 + CHAR_TO_DIGIT((*OP_CODE).OPCODE[1], 16)),
              INSTRUCT_CODE);
      ...
      if ((*OP_CODE).OPERAND == NUM)
      ...
      else
        {
          ...
          if ((*OP_CODE).OPERAND != REG)
            {
              /* -------------------- Two argument command, Get second after skipping
         * comma*/
              if ((*ARGUMENTS) != ',')
                {
  ...
  if ((*OP_CODE).FORMAT == THREE_FOUR)
     ...
     */
      ...
      if ((*OP_CODE).OPERAND == NONE)
        ...
        {
          /* ------ check for immediate or indirect mode, and set the bits
       * accordingly */
          switch (*ARGUMENTS)
            {
              case '@':
          ...
          /* ------- get the value of the argument and the type (kind) of that
       * argument*/
          VALUE = GET_EXPRESSION(&ARGUMENTS, MEM_ADDR_SIZE_1 + 1, LOCATION + 1,
                                 SYM_TAB, &WHAT_KIND);

          if (ERROR_REC_BUF.HEAD_OF_BUFFER != NULL)
            {
              (*ERROR) = TRUE_1;
            }
          ...
          if (EXTENDED)
          ...
          else
            {
              ...
              if (WHAT_KIND == EXTERN_VALUE_1)
                {
                  ADD_TO_END_OF_BUFFER(&ERROR_REC_BUF,
                                       "ERROR[53]: EXTREF label can not be used in a "
              ...
              if (((WHAT_KIND == RELATIVE_VALUE_1) &&
                  ...
                  ((WHAT_KIND != RELATIVE_VALUE_1) && (VALUE >= (MAX_PC_RELATIVE_1))))
                {
                  if (WHAT_KIND == RELATIVE_VALUE_1)
                    {
                      ADD_TO_END_OF_BUFFER(
                    ...
                    {
                      ADD_TO_END_OF_BUFFER(&ERROR_REC_BUF,
                                           "ERROR[52]: Constant too large for a format "
                                           "THREE instruction.");
                    }
                  (*ERROR) = TRUE_1;
                  VALUE = 0;
              ...
              if (VALUE < 0)
                {
                  ...
                  if ((VALUE >= MAX_PC_RELATIVE_1) || (VALUE < MAX_PC_RELATIVE_1 / 2))
                    {
                      if (WHAT_KIND == RELATIVE_VALUE_1)
                        {
                          ADD_TO_END_OF_BUFFER(
          ...
          if (!IS_BLANK_OR_TAB(*ARGUMENTS) && !eoln(*ARGUMENTS))
            {
              ADD_TO_END_OF_BUFFER(
                  &ERROR_REC_BUF, "ERROR[44]: Expected a <space> after the operand.");
              (*ERROR) = 1;
            }
        }
          ...
          FILE *LISTING_STREAM, int *ERROR)
{
  ...
  SCAN_LINE(LOCATION_COUNTER, &INPUT_LINE, LABEL, &EXTENDED, OPERATOR,
            &ARGUMENTS, pass2, (FILE *)NULL, INPUT_STREAM);
     ...
     */
          ...
          if (!strcmp(OPERATOR, "EQU"))
            {
              ...
              if (TABLE_ENTRY == NULL)
                ...
                {
                  LOCATION_COUNTER = (*TABLE_ENTRY).LOCATION;
                }
            }
          else
            {
              ...
              if (TABLE_ENTRY == NULL)
                {
                  (void)printf(
                      "CODE: Something is wrong with code. This shouldn't be "
                      "executed.\n");
                  /* ------ In all other case output the location counter and the
           * address      */
                }
              else
                {
       ...
       */
        ...
        if ((*OP_CODE).FORMAT == NOT_FOUND)
          ...
          {
            REAL_CODE(OP_CODE, EXTENDED, ARGUMENTS, LOCATION_COUNTER, SYM_TAB,
                      OBJECT_STREAM, LISTING_STREAM, ERROR);
          }
