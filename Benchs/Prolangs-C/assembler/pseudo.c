      ...
      ((*ARGUMENTS == 'C') && (*(ARGUMENTS + 1) = '\'')))
    {
      ...
      if (*ARGUMENTS == '\'')
      ...
      else
        {
          ...
          /*  ------- Step through argument till find end or string or end of
       * constant */
          while ((!eoln(*ARGUMENTS)) && (*ARGUMENTS != '\''))
            {
              if (IS_HEX_Q)
                {
                  ERROR = (ERROR || (CHAR_TO_DIGIT(*ARGUMENTS, 16) == -1));
                }
              ARGUMENTS++;
              if (!ERROR)
                {
          ...
          if (ERROR)
            {
              (void)fprintf(OUTPUT_STREAM, "eERROR[20]: Illegal hexidecimal.\n");
            }
          ...
          if (*ARGUMENTS == '\'')
            ...
            {
              (void)fprintf(OUTPUT_STREAM,
                            "eERROR[21]: Illegal hexidecimal/character constant. "
                            "Missing close quote.\n");
              ERROR = TRUE_1;
            }
          ...
          if (IS_HEX_Q && ((((int)SIZE / 2) * 2) != SIZE))
            {
              (void)fprintf(OUTPUT_STREAM,
                            "eERROR[22]: Hexidecimal number is not divisible into "
                            "whole bytes.\n");
              ERROR = TRUE_1;
            }
          ...
          if (!ERROR)
            {
              if (IS_HEX_Q)
                ...
                {
                  CHANGE_LOCATION(LOCATION, SIZE, OUTPUT_STREAM);
                }
  ...
  if (strcmp(ARGUMENTS, ""))
    {
      ...
      if (strcmp(OPERAND_LAB, ""))
         ...
         */
          ...
          if (!IS_BLANK_OR_TAB(*START) && !eoln(*START) && (START != ARGUMENTS))
            {
              (void)fprintf(
                  OUTPUT_STREAM,
                  "eERROR[13]: Expected a <space> after the operand, found %c.\n",
        ...
        {
          (void)fprintf(OUTPUT_STREAM, "eERROR[31]: Expected a Symbol, found %s.\n",
                        ARGUMENTS);
        }
    }
  else
                ...
                SYMBOL_TABLE *SYM_TAB, FILE *OUTPUT_STREAM)
{
  if (!strcmp(LABEL_NAME, ""))
  ...
  else
    {
      ...
      (*TABLE_ENTRY).LOCATION = GET_EXPRESSION(&START, MEM_ADDR_SIZE_1 + 1,
                                               *LOCATION, SYM_TAB, &ABS_VAL);
      OUTPUT_BUFFER(&ERROR_REC_BUF, OUTPUT_STREAM, 1);
                 ...
                 SYMBOL_TABLE *SYM_TAB, FILE *OUTPUT_STREAM)
{
  ...
  CHANGE_LOCATION(LOCATION, GET_EXPRESSION(&START, MEM_ADDR_SIZE_1 + 1, *LOCATION, SYM_TAB, &ABS_VAL),
                  OUTPUT_STREAM);
  OUTPUT_BUFFER(&ERROR_REC_BUF, OUTPUT_STREAM, 1);
  if (!IS_BLANK_OR_TAB(*START) && !eoln(*START) && (START != ARGUMENTS))
    {
                  ...
                  SYMBOL_TABLE *SYM_TAB, FILE *OUTPUT_STREAM)
{
  ...
  if (SEEN_START_OP == SEEN_START_1)
    {
      (void)fprintf(OUTPUT_STREAM, "eERROR[32]: Multiple STARTs in this file.\n");
    }
                 ...
                 SYMBOL_TABLE *SYM_TAB, FILE *OUTPUT_STREAM)
{
  ...
  VALUE = GET_EXPRESSION(&START, BITS_PER_WORD_1, *LOCATION, SYM_TAB, &ABS_VAL);
  OUTPUT_BUFFER(&ERROR_REC_BUF, OUTPUT_STREAM, 1);
...
void DO_PSEUDO(int WHICH_PSEUDO, char *LABEL_NAME, char *ARGUMENTS,
               int *LOCATION, SYMBOL_TABLE *SYM_TAB, FILE *OUTPUT_STREAM)
{
  switch (WHICH_PSEUDO)
    {
      case PSEUDO_BYTE0:
