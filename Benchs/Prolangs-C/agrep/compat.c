      ...
      if (COUNT || FILENAMEONLY || APPROX || PAT_FILE)
        {
          BESTMATCH = 0;
          fprintf(stderr,
                  "%s: WARNING!!! -B option ignored when -c, -l, -f, or -# is on\n",
                  Progname);
        }
    }
  if (PAT_FILE)
      ...
      if (APPROX)
        {
          fprintf(
              stderr,
              "WARNING!!!  approximate matching is not supported with -f option\n");
        }
      /*
                    if(INVERSE) {
                            fprintf(stderr, "%s: -f and -v are not
       compatible\n", Progname);
                            exit(2);
                    }
    */
  ...
  if (JUMP)
    {
      if (REGEX)
        {
          fprintf(stderr,
                  "WARNING!!! -D#, -I#, or -S# option is ignored for regular "
                  "expression pattern\n");
          JUMP = 0;
        }
      if (I == 0 || S == 0 || DD == 0)
