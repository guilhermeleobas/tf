...
void bitap(char old_D_pat[], char *Pattern, int fd, int M, int D)
{
  ...
  if (REGEX)
  {
    ...
    {
      fprintf(stderr,
              "%s: the maximum number of erorrs allowed for full regular "
              "expression is 4\n",
              Progname);
      exit(2);
    }
    if (M <= SHORTREG)
    {
      re(fd, M, D); /* SUN: need to find a even point */
      return;
    }
    else
    {
  ...
  if (D > 0 && JUMP == ON)
  {
    asearch1(old_D_pat, fd, D);
    return;
  }
  if (D > 0)
  {
    asearch(old_D_pat, fd, D);
    return;
  }
  if (I == 0)
  {
  ...
  while ((num_read = fill_buf(fd, buffer + Max_record, Max_record)) > 0)
  {
    ...
    if (FIRSTROUND)
    {
      ...
      if (DELIMITER)
      {
        ...
        {
          if (old_D_pat[k] != buffer[Max_record + k])
          {
            break;
          }
        }
    ...
    while (i < end)
    {
      ...
      {
        j++;
        if (((AND == 1) && ((r2 & endposition) == endposition)) ||
            ((AND == 0) && (r2 & endposition)) ^ INVERSE)
        {
          if (FILENAMEONLY)
          {
            num_of_matched++;
      ...
      {
        j++;
        if (((AND == 1) && ((r3 & endposition) == endposition)) ||
            ((AND == 0) && (r3 & endposition)) ^ INVERSE)
        {
          if (FILENAMEONLY)
          {
            num_of_matched++;
