...
void char_tr(unsigned char *pat, int *m)
{
  ...
  if (WORDBOUND)
  { /* SUN: To be added to be more complete */
    for (i = 0; i < 128; i++)
    {
      if (!isalnum(i))
      {
...
int verify(register int m, register int n, register int D, CHARTYPE *pat,
           CHARTYPE *text)
{
  ...
  */
  ...
  while (text <= textend)
  {
    ...
    if (j == m)
    {
      ...
      if (WORDBOUND)
      {
        if (TR[*(text + 1)] != W_DELIM)
        {
          goto CONT;
        }
        if (TR[*(text - m)] != W_DELIM)
        {
          goto CONT;
        }
      }
...
void initmask(CHARTYPE *pattern, unsigned *Mask, register int m, register int D,
              unsigned *endposition)
{
  ...
  *endposition = 0;
  ...
  {
    *endposition = *endposition | (Bit1 >> i);
  }
...
void prep(CHARTYPE *Pattern, register int M,
          register int D) /* preprocessing for partitioning_bm */
{
...
void agrep(register CHARTYPE *pat, int M, register CHARTYPE *text,
           register CHARTYPE *textend, int D)
{
  register int i;
  ...
  for (round = 0; round <= cdx; round++)
  {
    ...
    while (i < Candidate[round][1])
    {
      ...
      {
        R1[k] =
            ((R2[k] >> 1) | r1) & R2[k - 1] & ((R1[k - 1] & R2[k - 1]) >> 1);
      }
      if ((R1[D] & endpos) == 0)
      {
        num_of_matched++;
      ...
      {
        R2[k] =
            ((R1[k] >> 1) | r1) & R1[k - 1] & ((R1[k - 1] & R2[k - 1]) >> 1);
      }
      if ((R2[D] & endpos) == 0)
      {
        currentpos = i;
...
void a_monkey(register CHARTYPE *pat, register int m, register CHARTYPE *text,
              register CHARTYPE *textend, register int D)
{
...
void monkey(register CHARTYPE *pat, register int m, register CHARTYPE *text,
            register CHARTYPE *textend)
{
...
void am_preprocess(CHARTYPE *Pattern)
{
  int i, m;
  m = strlen(Pattern);
  for (i = 1, Hashmask = 1; i < 16; i++)
  {
    Hashmask = (Hashmask << 1) + 1;
  }
  for (i = 0; i < MAXMEMBER_1; i++)
  {
    MEMBER_1[i] = 0;
  }
  for (i = m - 1; i >= 0; i--)
...
void monkey4(register unsigned char *pat, register int m,
             register unsigned char *text, register unsigned char *textend,
             register int D)
{
...
void prep4(char *Pattern, int m)
{
  ...
  BSize = blog(4, m);
  for (i = 1, Hashmask = 1; i < BSize * LOG_DNA; i++)
  {
    Hashmask = (Hashmask << 1) + 1;
  }
...
void sgrep(CHARTYPE *pat, int m, int fd, int D)
{
  ...
  for (i = 1; i <= m; i++)
  {
    text[BLOCKSIZE + offset + i] = pat[m - 1];
  }
  ...
  while ((num_read = read(fd, text + offset, BLOCKSIZE)) > 0)
  {
    ...
    if (D == 0)
    {
      if (m > LONG_EXAC)
      {
        monkey(pat, m, text + start, text + end);
      }
      else
      {
        bm(pat, m, text + start, text + end);
      }
    }
    else
    {
      if (DNA)
      {
        monkey4(pat, m, text + start, text + end, D);
      }
      else
      {
        if (m >= LONG_APPX)
        {
          a_monkey(pat, m, text + start, text + end, D);
        }
        else
        {
          agrep(pat, m, text + start, text + end, D);
        }
      }
    }
