      ...
      while (i < end)
        {
          ...
          for (k = r3; k <= r4; k++) /* r3 = D+1, r4 = 2*D */
            {
              r5 = B[k];
              r1 = Init1 & r5;
              A[k] = ((r5 >> 1) & CMask) | B[k - I] |
                     (((A[k - DD] | B[k - S]) >> 1) & r_NO_ERR) | r1;
            }
          if (A[D] & endpos)
            {
              j++;
              if (((AND == 1) && ((A[D * 2] & endposition) == endposition)) ||
                  ((AND == 0) && (A[D * 2] & endposition)) ^ INVERSE)
                {
                  if (FILENAMEONLY)
                    {
                      num_of_matched++;
          ...
          for (k = r3; k <= r4; k++)
            {
              r1 = A[k] & Init1;
              B[k] = ((A[k] >> 1) & CMask) | A[k - I] |
                     (((B[k - DD] | A[k - S]) >> 1) & r_NO_ERR) | r1;
            }
          if (B[D] & endpos)
            {
              j++;
              if (((AND == 1) && ((B[r4] & endposition) == endposition)) ||
                  ((AND == 0) && (B[r4] & endposition)) ^ INVERSE)
                {
                  if (FILENAMEONLY)
                    {
                      num_of_matched++;
