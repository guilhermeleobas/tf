...
void ADDR_P(PRINT_EFFECT) BOOLEAN
    PRINT_EFFECT; /* Output effect of this command for debugging */
{
  ...
  else
  {
     ...
     */
    if (REGISTER[REG1] >= MAX_INT_1 / 2)
    {
      REG1_SIGN = -1;
    }
    if (REGISTER[REG2] >= MAX_INT_1 / 2)
    {
      REG2_SIGN = -1;
    }
    ...
    if ((REG2_SIGN == -1) && (REG1_SIGN == 1))
    {
      REGISTER[REG2] = REGISTER[REG2] + REGISTER[REG1];
      REGISTER[REG2] =
          REGISTER[REG2] - ((int)REGISTER[REG2] / MAX_INT_1) * MAX_INT_1;
    }
    if ((REG2_SIGN == 1) && (REG1_SIGN == -1))
    {
      REGISTER[REG2] = REGISTER[REG2] + REGISTER[REG1];
...
/* ----------------------------------- CLEAR_P ---------------------------- */
void CLEAR_P(PRINT_EFFECT) BOOLEAN
    PRINT_EFFECT; /* Output effect of this command for debugging */
{
  int REG1; /* First Register in instruction               */
  int REG2; /* Second Register in instruction              */
...
/* ---------------------------------- COMPR_P ------------------------------ */
void COMPR_P(PRINT_EFFECT) BOOLEAN
    PRINT_EFFECT; /* Output effect of this command for debugging */

{
  ...
  if (REGISTER[REG1] >= MAX_INT_1 / 2)
  {
    REG1_VAL = REGISTER[REG1] - MAX_INT_1;
  }
  else
  {
  ...
  if (REGISTER[REG1] >= MAX_INT_1 / 2)
  {
    REG2_VAL = REGISTER[REG2] - MAX_INT_1;
  }
  else
  {
...
/* This proceedure is messy because work had to be done with the signs of    */
/* the arguments.                                                            */
void DIVR_P(PRINT_EFFECT) BOOLEAN
    PRINT_EFFECT; /* Output effect of this command for debugging */
{
  int REG1;          /* First Register in instruction               */
  int REG2;          /* Second Register in instruction              */
  ...
  else
  {
    ...
    {
      REGISTER[REG2] =
          MAX_INT_1 - ((MAX_INT_1 - REGISTER[REG2]) / REGISTER[REG1]);
    }
    if ((REG2_SIGN == 1) && (REG1_SIGN == -1))
    {
      REGISTER[REG2] =
          MAX_INT_1 - (REGISTER[REG2] / (MAX_INT_1 - REGISTER[REG1]));
    }
    if ((REG2_SIGN == -1) && (REG1_SIGN == -1))
    {
      REGISTER[REG2] =
...
/* This proceedure is messy because work had to be done with the signs of    */
/* the arguments so that overflow could be detected.                         */
void MULR_P(PRINT_EFFECT) BOOLEAN
    PRINT_EFFECT; /* Output effect of this command for debugging */
{
  BOOLEAN OVERFLOW1 = FALSE_1; /* Has an overflow been detected.             */
  int REG1;                    /* First Register in instruction               */
  ...
  else
  {
    ...
    if ((REG2_SIGN == 1) && (REG1_SIGN == 1))
    {
      ...
      if (STORE_VALUE >= MAX_INT_1 / 2)
      {
        OVERFLOW1 = TRUE_1;
      }
      else
      {
        REGISTER[REG2] = (int)STORE_VALUE;
      }
    }
    if ((REG2_SIGN == -1) && (REG1_SIGN == 1))
    {
      ...
      {
        REGISTER[REG2] = MAX_INT_1 - (int)STORE_VALUE;
      }
    }
    if ((REG2_SIGN == 1) && (REG1_SIGN == -1))
    {
    ...
    {
      STORE_VALUE =
          1.0 * (MAX_INT_1 - REGISTER[REG2]) * (MAX_INT_1 - REGISTER[REG1]);
      if (STORE_VALUE >= MAX_INT_1 / 2)
      {
        OVERFLOW1 = TRUE_1;
      }
...
/* ----------------------------------- RMO_P ------------------------------- */
void RMO_P(PRINT_EFFECT) BOOLEAN
    PRINT_EFFECT; /* Output effect of this command for debugging */
{
  int REG1; /* First Register in instruction               */
  int REG2; /* Second Register in instruction              */
...
/* --------------------------------- SHIFTL_P ------------------------------ */
void SHIFTL_P(PRINT_EFFECT) BOOLEAN
    PRINT_EFFECT; /* Output effect of this command for debugging */
{
  int REG1; /* First Register in instruction               */
  int REG2; /* Second Register in instruction              */
...
/* --------------------------------- SHIFTR_P ------------------------------ */
void SHIFTR_P(PRINT_EFFECT) BOOLEAN
    PRINT_EFFECT; /* Output effect of this command for debugging */
{
  int REG1; /* First Register in instruction               */
  int REG2; /* Second Register in instruction              */
  ...
  else
  {
    ...
    for (LOOP = 0; LOOP < BITS_PER_WORD_1; LOOP++)
    {
      if (LOOP <= REG2)
      {
        SHIFTED_BITS[LOOP] = REG_BITS[0];
      }
      else
      {
        SHIFTED_BITS[LOOP] = REG_BITS[LOOP - REG2 - 1];
      }
    }
...
/* This proceedure is messy because work had to be done with the signs of    */
/* the arguments so that overflow could be detected.                         */
void SUBR_P(PRINT_EFFECT) BOOLEAN
    PRINT_EFFECT; /* Output effect of this command for debugging */
{
  BOOLEAN OVERFLOW1 = FALSE_1; /* Has an overflow been detected.             */
  int REG1;                    /* First Register in instruction               */
...
/* ----------------------------------- SVC_P ------------------------------- */
void SVC_P(PRINT_EFFECT) BOOLEAN
    PRINT_EFFECT; /* Output effect of this command for debugging */
{
  int REG1; /* First Register in instruction               */
  int REG2; /* Second Register in instruction              */
  ...
  else
  {
    ...
    if (PRINT_EFFECT)
    {
      (void)printf("Generated SVC interupt %d.\n", REG1);
    }
  }
}

/* ---------------------------------- TIXR_P ------------------------------- */
void TIXR_P(PRINT_EFFECT) BOOLEAN
    PRINT_EFFECT; /* Output effect of this command for debugging */
{
  int REG1;    /* First Register in instruction               */
  int REG2;    /* Second Register in instruction              */
  ...
  else
  {
    ...
    if (REG_VAL >= MAX_INT_1 / 2)
    {
      REG_VAL = REG_VAL - MAX_INT_1;
    }
