...
void LOAD(BOOLEAN DEBUG_MODE, int *ERROR, FILE *INPUT)
{
  ...
  else
  {
    ...
    if ((RECORD[0] != 'S') || (strlen(RECORD) != 14))
    {
      (*ERROR) = TRUE_1;
    }
    else
    {
      ...
      STR_TO_NUM(&(RECORD[LABEL_SIZE_1 + 1]), 5, 16, &START_ADDRESS, ERROR);
    }
  }
    ...
    switch (RECORD[0])
    {
      case 'T':
        ...
        else
        {
          ...
          {
            STORE_AT(&(RECORD[9]), LENGTH, LOCATION, MEMORY, ERROR);
          }
        }
        break;
      ...
      case 'G':
        /* ----------------------- GLOBAL RECORD
         * ----------------------------------- */
        if (strlen(RECORD) != (7 + LABEL_SIZE_1))
        {
          (*ERROR) = TRUE_1;
        }
        else if (DEBUG_MODE)
