  ...
  switch (FUNC)
    {
      ...
      case DIVR_P0:
        ...
        break;
      case COMPR_P0:
        COMPR_P(PRINT_EFFECT);
        break;
      case SHIFTL_P0:
        SHIFTL_P(PRINT_EFFECT);
        break;
      case SHIFTR_P0:
        SHIFTR_P(PRINT_EFFECT);
        break;
      case RMO_P0:
        RMO_P(PRINT_EFFECT);
        break;
...
int INSTR_CODE(int LOC) { return (INT(MEMORY[LOC]) / 4); }
/* ----------------------------- BYTES_TO_BITS ----------------------------- */
...
void SET_CC(int CODE)
{
  ...
  if ((CODE >= 0) && (CODE <= 3))
    ...
    {
      (void)printf("SET_CC called improperly.\n");
    }
}
      ...
      if (DEV == 0)
      ...
      else
        {
          ...
          if (DEV < 128)
            ...
            {
              DEVICE[DEV].STREAM = fopen(NAME, "w");
            }
        }
    }
}
      ...
      if ((*NEXT).LOCATION == ADDRESS)
        {
          ...
          for (COUNT = LABEL_SIZE_1 - 1;
               (COUNT > 0) && ((*NEXT).LABEL[COUNT] == ' '); COUNT--)
            {
              ;
            }
  ...
  if (TYPE_OUT_MODE[0] == 'D')
  ...
  else
    {
      ...
      if (!PRINT_INSTR)
        ...
        {
          (void)printf("%s%s%s", PREFIX, ADDR, SUFFIX);
        }
    }
}
  ...
  if (TYPE_OUT_MODE[0] == 'D')
    {
      if (CONST == 0)
        ...
        {
          DIGITS = ((int)(log(1.0 * CONST) / log(10.0))) + 1;
        }
  ...
  else
    {
      if (CONST == 0)
        ...
        {
          DIGITS = ((int)(log(1.0 * CONST) / log(16.0))) + 1;
        }
      ...
      if (!PRINT_INSTR)
        ...
        {
          (void)printf("%s%s%s", PREFIX, STR_CONST, SUFFIX);
        }
    }
}
...
void PRINT_INSTRUCTION(int LOCATION, BOOLEAN PRINT, int *BYTES)
{
  ...
  if ((0 > LOCATION) || (MEM_SIZE_1 <= LOCATION))
     ...
     * -----------*/
      if ((BIT_INSTR[11] == 1) && !((BIT_INSTR[6] == 0) && (BIT_INSTR[7] == 0)))
        {
          (*BYTES) = 4;
  ...
  if (PRINT)
    {
      ...
      /* ~~~~~~~~~~~~~~~~~~~~~~~~~ print FORMAT3/4 arguments */
      {
        ...
        if ((BIT_INSTR[6] == 1) && (BIT_INSTR[7] == 0))
          {
            (void)printf("@");
          }
        else if ((BIT_INSTR[6] == 0) && (BIT_INSTR[7] == 1))
          {
            (void)printf("#");
          }
        else
      ...
      if (INSTR[INSTR_CODE(PC_0)].FUNCTION == NULL0)
        {
          ...
          if (INSTR[INSTR_CODE(PC_0)].FORMAT == 3)
            ...
            {
              PC_0 += INSTR[INSTR_CODE(PC_0)].FORMAT;
            }
          (void)printf("\n");
        }
      else
