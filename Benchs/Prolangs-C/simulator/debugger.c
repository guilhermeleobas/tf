...
BOOLEAN IS_PREFIX_TO(char *PREFIX, char *WORD)
{
  ...
  if ((LEN = strlen(PREFIX)) > strlen(WORD))
    {
      return FALSE_1;
    }
          ...
          switch (RESULT_TYPE)
            {
              ...
              case NUMBER_0:
                ...
                break;
              case ADDR_REG_0:
                (void)printf("Expected an type-in mode specifier ($D, $S or $R).\n");
                break;
              case VALUE_0:
                (void)printf("Expected an type-in mode specifier ($C, $D, or $S).\n");
                break;
              default:;
            }
...
void EXAMINE(char *COMMAND, BOOLEAN *LEGAL)
{
  ...
  if (strlen(COMMAND) < 2)
  ...
  else
    {
      ...
      if (ADDRESS == -1)
        {
          (void)printf("Expected a valid register name (A,X,L,B,S,T,SW, or PC).\n");
                ...
                if (STORE)
                  {
                    ...
                    MEMORY[ADDRESS + 1] =
                        (VALUE - INT(MEMORY[ADDRESS]) * 256 * 256) / 256;
                    MEMORY[ADDRESS + 2] = (VALUE - INT(MEMORY[ADDRESS + 1]) * 256);
                    (void)printf("Set ");
                  }
                VALUE =
                    ((INT(MEMORY[ADDRESS]) * 256 + INT(MEMORY[ADDRESS + 1])) * 256 +
  ...
  if (NO_ERROR)
    {
      if ((0 > ADDR) || (MEM_SIZE_1 <= ADDR))
        {
          PRINT_CONSTANT("Address specified ", ADDR,
                         " out of range. No break point set.\n");
        }
      else if (NUM_BREAK_POINTS == MAX_NUM_BREAK_POINTS)
        {
          PRINT_CONSTANT("All ", MAX_NUM_BREAK_POINTS,
      ...
      else
        {
          ...
          for (COUNT = NUM_BREAK_POINTS;
               ((BREAK_POINTS[SORTED_BREAK_POINTS[COUNT]] > ADDR) && (COUNT > 0));
               COUNT--)
            {
  ...
  if (NO_ERROR)
    {
      if ((0 > NUM) || (NUM_BREAK_POINTS <= NUM))
        ...
        {
          for (BP = 1;
               ((SORTED_BREAK_POINTS[BP] != NUM) && (BP <= NUM_BREAK_POINTS)); BP++)
            {
              ;
            }
          BREAK_POINTS[NUM] = -1;
...
void HELP(char *COMMAND, BOOLEAN *LEGAL)
{
  ...
  if ((*COMMAND) != '\0')
  ...
  else
    {
      ...
      (void)printf(
          "   L<loc>     LOC<loc>        Examines contents of memory and "
          "registers.\n");
      (void)printf(
          "   L<loc>=<v> LOC<loc>=<v>    Examines contents of memory and "
          "registers.\n");
...
void TRACE_P(char *COMMAND, BOOLEAN *LEGAL)
{
  ...
  if ((*COMMAND) != '\0')
    ...
    {
      (void)printf("Trace is already on.\n");
    }
  else
    {
      TRACE = TRUE_1;
...
void UNTRACE(char *COMMAND, BOOLEAN *LEGAL)
{
  ...
  if ((*COMMAND) != '\0')
    ...
    {
      (void)printf("Trace is already off.\n");
    }
  else
    {
      TRACE = FALSE_1;
...
struct COMMAND_TYPE
{
  ...
  int FUNCTION;
    ...
    {"S", "STEP", STEP0}, {"B", "BREAK", SET_BP0}, {"L", "LOC", EXAMINE0}, {"RB", "RESET", RESET_BP0}, {"R", "RUN", RUN_IT0}, {"H", "HELP", HELP0}, {"M", "MODE", MODE0}, {"P", "PROCEED", PROCEED0}, {"T", "TRACE", TRACE_P0}, {"U", "UNTRACE", UNTRACE0}, {"Q", "QUIT", QUIT0}};
  ...
  switch (FUNC)
    {
      case STEP0:
        ...
        break;
      case SET_BP0:
        SET_BP(COMMAND, LEGAL);
        break;
      case EXAMINE0:
        EXAMINE(COMMAND, LEGAL);
        break;
      case RESET_BP0:
        RESET_BP(COMMAND, LEGAL);
        break;
      case RUN_IT0:
        RUN_IT(COMMAND, LEGAL);
        break;
      case HELP0:
        HELP(COMMAND, LEGAL);
        break;
      case MODE0:
        ...
        break;
      case PROCEED0:
        PROCEED(COMMAND, LEGAL);
        break;
      case TRACE_P0:
        TRACE_P(COMMAND, LEGAL);
        break;
      case UNTRACE0:
        UNTRACE(COMMAND, LEGAL);
        break;
      case QUIT0:
        QUIT(COMMAND, LEGAL);
        break;
              ...
              switch (CODE)
                {
                  case 0:
                    ...
                    break;
                  case 1:
                    (void)printf("Privileged instruction in user mode.\n");
                    break;
                  case 2:
                    (void)printf("Address out of range.\n");
                    break;
                  case 3:
                    (void)printf("Memory-protection violation.\n");
                    break;
                  case 4:
                    (void)printf("Aritmetic overflow.\n");
                    break;
                  case 10:
                    (void)printf("Page fault.\n");
                    break;
                  case 11:
                    (void)printf("Segment fault.\n");
                    break;
                  case 12:
                    (void)printf("Segment-protection violation.\n");
                    break;
                  case 13:
                    (void)printf("Segment length exceeded.\n");
                    break;
                  default:
                    (void)printf("\n");
                    break;
