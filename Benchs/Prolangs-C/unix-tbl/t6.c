  ...
  for (icol = 0; icol < ncol; icol++)
  {
    ...
    for (text = 0; text < 2; text++)
    {
      ...
      for (ilin = 0; ilin < nlin; ilin++)
      {
        ...
        vforml = ilin;
        for (il = prev(ilin); il >= 0 && vspen(table[il][icol].col);
             il = prev(il))
        {
          vforml = il;
        }
        ...
        switch (ctype(vforml, icol))
        {
          ...
          case 'n':
            if (table[ilin][icol].rcol != 0)
            {
              ...
              if (text == 0 && real(s = table[ilin][icol].rcol) && !vspen(s) &&
                  !barent(s))
              {
    ...
    if (doubled[icol])
    {
      ...
      fprintf(tabout, ".if \\n(%d>\\n(%d .nr %d \\n(%d\n", TMP, icol + CRIGHT,
              icol + CRIGHT, TMP);
      fprintf(tabout, ".if \\n(%d<\\n(%d .nr %d +(\\n(%d-\\n(%d)/2\n", TMP,
              icol + CRIGHT, icol + CMID, icol + CRIGHT, TMP);
    }
    if (cll[icol][0])
    {
...
int filler(char *s) { return (point((int)s) && s[0] == '\\' && s[1] == 'R'); }
