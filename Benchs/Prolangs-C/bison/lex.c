  ...
  switch (c)
  {
      ...
      c = getc(finput);
      {
        ...
        if (c == '\\')
        {
          ...
          else
          {
            ...
            {
              fatals("invalid literal token '\\%c'", c, 0, 0, 0, 0, 0, 0, 0);
            }
            c = getc(finput);
          }
        }
    ...
    case '<':
      ...
      while (c != '>')
      {
        ...
        {
          fatals("type name too long (%d max)", MAXTOKEN - 1, 0, 0, 0, 0, 0, 0,
                 0);
        }
...
/* parse a token which starts with %.  Assumes the % has already been read and
 * discarded.  */
...
int parse_percent_token(void)
{
  ...
  *p = 0;
  ...
  if (strcmp(token_buffer, "token") == 0 || strcmp(token_buffer, "term") == 0)
  {
    return (TOKEN);
  }
