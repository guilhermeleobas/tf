...
/* read in the grammar specification and record it in the format described in
gram.h.
...
void reader(void)
{
  ...
  /* Read the declaration section.  Copy %{ ... %} groups to ftable and fdefines
     file.
  ...
  output_headers();
  /* read in the grammar, build grammar in list form.  write out guards and
   * actions.  */
  readgram();
...
for the simple parser in which the stack is not popped until after the guard is
run.  */
