...
#define CHAMP(champ, conf, div)                                              \
  if ((champ == ERROR_0) && (divisions[rank[team_c]][CONFERENCE] == conf) && \
  ...
  for (i = 1; i <= NUM_TEAMS; i++)
    {
      if (!((i == NUM_TEAMS)
                ...
                : (tied(info, rank[i], rank[i + 1])
                       ? (within == DIVISION
                              ? (divisions[rank[i]][DIVISION] ==
                                     divisions[rank[i + 1]][DIVISION] &&
                                 divisions[rank[i]][CONFERENCE] ==
                                     divisions[rank[i + 1]][CONFERENCE])
                              : (within == CONFERENCE
                                     ? (divisions[rank[i]][CONFERENCE] ==
                                            divisions[rank[i + 1]][CONFERENCE] &&
                                        (a_champ(rank[i]) ==
              ...
              if (!(a_champ(conf_standings[team_c])))
                {
                  if (nfc == 0)
                    {
                      nfc++;
                      nfc_wild_card1 = conf_standings[team_c];
                    }
                  else if (nfc == 1)
                    {
                      nfc++;
                      nfc_wild_card2 = conf_standings[team_c];
                    }
                  else if (nfc == 2 && (iyear >= 90 || iyear <= 70))
                    {
                      nfc++;
                      nfc_wild_card3 = conf_standings[team_c];
                    }
                }
            }
          else
              ...
              if (!(a_champ(conf_standings[team_c])))
                {
                  if (afc == 0)
                    {
                      afc++;
                      afc_wild_card1 = conf_standings[team_c];
                    }
                  else if (afc == 1)
                    {
                      afc++;
                      afc_wild_card2 = conf_standings[team_c];
                    }
                  else if (afc == 2 && (iyear >= 90 || iyear <= 70))
                    {
                      afc++;
                      afc_wild_card3 = conf_standings[team_c];
                    }
                }
            }
        }
