...
TEAM_TYPE team = {
    {"error", "   "},        {"min", "Minnesota"},    {"atl", "Atlanta"},
    {"chi", "Chicago"},      {"dal", "Dallas"},       {"det", "Detroit"},
    {"gb", "Green Bay"},     {"ram", "L.A. Rams"},    {"no", "New Orleans"},
    {"gia", "N.Y. Giants"},  {"phi", "Philadelphia"}, {"pho", "Phoenix"},
    {"sf", "San Francisco"}, {"tb", "Tampa Bay"},     {"was", "Washington"},
    {"buf", "Buffalo"},      {"cin", "Cincinnati"},   {"cle", "Cleveland"},
    {"den", "Denver"},       {"hou", "Houston"},      {"ind", "Indianapolis"},
    {"kc", "Kansas City"},   {"rai", "L.A. Raiders"}, {"mia", "Miami"},
    {"ne", "New England"},   {"jet", "N.Y. Jets"},    {"pit", "Pittsburgh"},
    {"sd", "San Diego"},     {"sea", "Seattle"},      {"none", "None"}};

DIVISION_TYPE divisions = {{0, 0},
                           {NFC_0, CENTRAL_0},
                           {NFC_0, WEST_0},
                           {NFC_0, CENTRAL_0},
                           {NFC_0, EAST_0},
...
void compute_team_info(void)
{
  ...
  for (week = 1; week <= num_games; week++)
  {
    for (game = 1; game <= NUM_TEAMS / 2; game++)
    {
      ...
      {
        h_to_h_stats[sched[week][game][HOST]][sched[week][game][VISITOR]]
            .points_for += scores[week][game][HOST];
        h_to_h_stats[sched[week][game][HOST]][sched[week][game][VISITOR]]
            .points_against += scores[week][game][VISITOR];
        h_to_h_stats[sched[week][game][VISITOR]][sched[week][game][HOST]]
            .points_for += scores[week][game][VISITOR];
        h_to_h_stats[sched[week][game][VISITOR]][sched[week][game][HOST]]
            .points_against += scores[week][game][HOST];
        ...
        else if (scores[week][game][HOST] > scores[week][game][VISITOR])
        {
  ...
  for (team_code = 1; team_code <= NUM_TEAMS; team_code++)
  {
    for (week_num = 1; week_num <= num_games; week_num++)
    {
      ...
      if (opponent < team_code) /* already read in opponents information */
      {
        for (find_game = 1; find_game <= num_seen[week_num] - 1; find_game++)
        {
          if (sched[week_num][find_game][TEAM_FIELD] == team_code)
          {
            ...
            {
              (void)printf(
                  "Error in opponent for %s(%d). Expected %s(%d), Found "
                  "%s(%d). Week #%d\n",
        ...
        {
          (void)printf(
              "%s(%d) not found yet: team: %s(%d); week# %d; opp. str: %s\n",
              team[opponent][FULL_NAME_0], opponent,
              team[team_code][FULL_NAME_0], team_code, week_num, str);
          error = YES;
        }
      }
      else if (opponent != NUM_TEAMS + 1)
      {
        ...
        {
          (void)printf(
              "%s(%d) has opponent %s(%d), but wants %s(%d); \n          week# "
              "%d\n",
              team[team_code][FULL_NAME_0], team_code,
...
int main(int argc, char *argv[])
{
  ...
  else if (argc <= 3)
  {
    ...
    iyear = str_to_int(year);
    num_games =
        (((iyear >= 78) && (iyear <= 89)) ? NUM_GAMES_78_89 : NUM_GAMES);
    if (argc == 3)
    {
      in_fileP = 2;
    }
  ...
  else
  {
    ...
    for (week = 1; week <= num_games + 1; week++)
    {
      for (game = 1; game <= NUM_TEAMS / 2; game++)
      {
        ...
        {
          (void)fprintf(stderr,
                        "ERROR: %s is not a legally formated score file.\n",
                        input_score);
          exit(1);
        }
  ...
  while (!terminate)
  {
    ...
    {
      display_main_menu(stdout, stdin);
    }
  }
  if (in_fileP)
  {
