...
#define find_conf_rank(r, t)                                 \
  (divisions[t][CONFERENCE] == NFC_0 ? find_teams_rank(r, t) \
...
int str_to_int(char *s)
{
  ...
  for (temp = s; *temp != '\0'; temp++)
    {
      value =
          value * 10 + (((*temp >= '0') && (*temp <= '9')) ? (*temp) - '0' : 0);
    }
  return value;
}
...
BOOL end_of_line(char *str) { return *str == '\0'; }
...
BOOL matches(char *str1, char *comm, int min_len_for_match)
{
  ...
  else if (length > strlen(comm))
    {
      return (!strncmp(str1, comm, strlen(comm))) &&
             white_space(*(str1 + strlen(comm)));
    }
  else
    {
  ...
  if (!got_unused_responce)
    {
      ...
      for (resp = responce + strlen(responce) - 1;
           resp >= responce && white_space(*resp); resp--)
        {
          *resp = '\0';
        }
    }
  ...
  for (team_code = 1; team_code <= NUM_TEAMS; team_code++)
    {
      if ((divisions[standings[team_code]][CONFERENCE] == conf ||
           conf == WILD_CARD) &&
          (divisions[standings[team_code]][DIVISION] == div || div == WILD_CARD))
        {
          if (next_found_is == place)
            {
              return standings[team_code];
            }
          else
            {
              next_found_is++;
...
#define PRINT_RESULT(week)                                                    \
  if (week > num_games)                                                       \
    ...
    {                                                                         \
      int i, score, opp_score;                                                \
      for (i = 1; sched[week][i][HOST] != team_code &&                        \
                  sched[week][i][VISITOR] != team_code && i <= NUM_TEAMS / 2; \
           i++)                                                               \
        ;                                                                     \
      if (i == NUM_TEAMS / 2 + 1)                                             \
        (void)fprintf(output, "                            ");                \
        ...
        {                                                                     \
          if (sched[week][i][VISITOR] == team_code)                           \
            {                                                                 \
      ...
      switch (week)
        {
          ...
          case 7:
            if ((tmp_team ==
                 find_nth_place_team(standings, divisions[team_code][CONFERENCE],
                                     divisions[team_code][DIVISION], week - 2)))
              {
          ...
          case 9:
            (void)fprintf(
                output, "Record           %2d-%d-%d      %sRank %d/%d",
                team_info[team_code].wins, team_info[team_code].loses,
                team_info[team_code].ties,
                (team_info[team_code].wins < 10 && team_info[team_code].loses < 10
                     ? "  "
                     : " "),
          ...
          case 10:
            (void)fprintf(
                output, "Points scored   %3d [%6.1f]   Rank %d/%d",
                team_info[team_code].points_for,
                PER_GAME(team_info, team_info[team_code].points_for, team_code),
                find_conf_rank(offence_ranks, team_code),
                find_teams_rank(offence_ranks_nfl, team_code));
            break;
          case 11:
            (void)fprintf(
                output, "Points allowed  %3d [%6.1f]   Rank %d/%d",
                team_info[team_code].points_against,
                PER_GAME(team_info, team_info[team_code].points_against, team_code),
                find_conf_rank(defence_ranks, team_code),
                find_teams_rank(defence_ranks_nfl, team_code));
            break;
          case 12:
            (void)fprintf(
                output, "Net Points     %4d [%6.1f]   Rank %d/%d",
                NET_POINTS(team_info, team_code),
                PER_GAME(team_info, NET_POINTS(team_info, team_code), team_code),
                find_conf_rank(net_ranks, team_code),
                find_teams_rank(net_ranks_nfl, team_code));
            break;
          case 14:
            (void)fprintf(output, "Major Tie-breakers:");
            break;
          case 15:
            (void)fprintf(output, "-(1/1) Head to Head");
            break;
          case 16:
          ...
          case 17:
            ...
            break;
          case 18:
            (void)fprintf(output, "-(4/3) Common Games");
            break;
          case 19:
            (void)fprintf(
                output, "-(5/ ) Net points in %s %s  %d [%3.1f]",
                CONF_STR(team_code), DIVISION_STR(team_code),
                NET_POINTS(team_info_wi_div, team_code),
                PER_GAME(team_info_wi_div, NET_POINTS(team_info_wi_div, team_code),
                         team_code));
            break;
          case 20:
            (void)fprintf(
                output, "-( /4) Avg. net points in %s     %3.1f",
                CONF_STR(team_code),
                PER_GAME(team_info_wi_conf,
                         NET_POINTS(team_info_wi_conf, team_code), team_code));
            break;
          case 21:
            (void)fprintf(
                output, "-(6/5) Total net points           %d [%3.1f]",
                NET_POINTS(team_info, team_code),
                PER_GAME(team_info, NET_POINTS(team_info, team_code), team_code));
            break;
          default:
  ...
  for (i = 1; i < num; i++)
    {
      ...
      same_div = same_div &&
                 (divisions[teams[i]][DIVISION] == divisions[teams[0]][DIVISION]);
    }
  if (!same_conf)
    {
      (void)fprintf(output,
                    "ERROR: All teams must be from the same conference.\n");
      return;
    }
  clear_screen(output);
  ...
  for (tmp_team1 = 1; tmp_team1 <= NUM_TEAMS; tmp_team1++)
    {
      ...
      if (is_any(teams, num, tmp_team))
        {
          (void)fprintf(output, "%-13s %2d-%2d-%1d ", team[tmp_team][FULL_NAME_0],
                        team_info[tmp_team].wins, team_info[tmp_team].loses,
          ...
          (void)fprintf(output, " %1d-%1d-%1d", team_info_wi_div[tmp_team].wins,
                        team_info_wi_div[tmp_team].loses,
                        team_info_wi_div[tmp_team].ties);
          (void)fprintf(output, " %2d-%2d-%1d", team_info_wi_conf[tmp_team].wins,
          ...
          if ((common_games[tmp_team].wins != -1))
            {
              (void)fprintf(output, "%2d-%2d-%1d[%2d]", common_games[tmp_team].wins,
                            common_games[tmp_team].loses, common_games[tmp_team].ties,
                            common_games[tmp_team].points_for);
            }
          else
            { /* 123456789012 */
              (void)fprintf(output, "            ");
      ...
      if (divisions[tmp_team][CONFERENCE] == divisions[team_code][CONFERENCE])
        {
          ...
          if (tmp_team != team_code && (common_games[team_code].wins != -1 &&
            ...
            {
              (void)fprintf(
                  output, " %2d-%2d-%1d[%2d] %2d-%2d-%1d[%2d]",
          ...
          (void)fprintf(
              output, "%5.1f",
              PER_GAME(team_info_wi_conf, NET_POINTS(team_info_wi_conf, tmp_team),
                       tmp_team));
  ...
  for (tmp_team1 = 1; tmp_team1 <= NUM_TEAMS / 2; tmp_team1++)
    {
      ...
      (void)fprintf(output, " %2d-%2d-%2d %s.%03d", team_info[tmp_team].wins,
                    team_info[tmp_team].loses, team_info[tmp_team].ties,
...
void display_week(FILE *output, int week)
{
  ...
  for (game = 1; game < NUM_TEAMS / 2 + 2; game += 3)
    {
      (void)fprintf(output, "%-16s",
                    team[sched[week][game][VISITOR]][FULL_NAME_0]);
      if (scores[week][game][VISITOR] < 0)
        ...
        {
          (void)fprintf(output, "  %2d      ", scores[week][game][VISITOR]);
        }
      if (game + 1 <= NUM_TEAMS / 2)
        {
          (void)fprintf(output, "%-16s",
                        team[sched[week][game + 1][VISITOR]][FULL_NAME_0]);
          if (scores[week][game + 1][VISITOR] < 0)
            ...
            {
              (void)fprintf(output, "  %2d      ", scores[week][game + 1][VISITOR]);
            }
        }
      if (game + 2 <= NUM_TEAMS / 2)
        {
          (void)fprintf(output, "%-16s",
                        team[sched[week][game + 2][VISITOR]][FULL_NAME_0]);
          if (scores[week][game + 2][VISITOR] < 0)
            ...
            {
              (void)fprintf(output, "  %2d      ", scores[week][game + 2][VISITOR]);
            }
        }
      (void)fprintf(output, "\n");
      ...
      if (scores[week][game][HOST] < 0)
        ...
        {
          (void)fprintf(output, "  %2d      ", scores[week][game][HOST]);
        }
      if (game + 1 <= NUM_TEAMS / 2)
        {
          (void)fprintf(output, "%-16s",
                        team[sched[week][game + 1][HOST]][FULL_NAME_0]);
          if (scores[week][game + 1][HOST] < 0)
            ...
            {
              (void)fprintf(output, "  %2d      ", scores[week][game + 1][HOST]);
            }
        }
      if (game + 2 <= NUM_TEAMS / 2)
        {
          (void)fprintf(output, "%-16s",
                        team[sched[week][game + 2][HOST]][FULL_NAME_0]);
          if (scores[week][game + 2][HOST] < 0)
            ...
            {
              (void)fprintf(output, "  %2d       ", scores[week][game + 2][HOST]);
            }
        }
      (void)fprintf(output, "\n\n");
    }
...
void enter_prompt_week(FILE *output, FILE *input, int week)
{
  ...
  (void)fprintf(output,
                "QUIT to return to menu, or new value for the score.\n");
  (void)fprintf(output, "\nWeek %d\n\n", week);
      ...
      if (!ABORT && sched[week][game][VISITOR] != 0 &&
          sched[week][game][HOST] != 0)
        {
          ...
          if (!ABORT)
            {
              (void)fprintf(output, "%-16s",
                            team[sched[week][game][HOST]][FULL_NAME_0]);
              if (scores[week][game][HOST] < 0)
                {
                  (void)fprintf(output, "         ? ");
                }
      ...
      if (!ABORT)
        {
          if (!FOUND_GAME)
          ...
          else
            {
              game--;
              (void)fprintf(output, "%-16s",
                            team[sched[week][game][FIRST]][FULL_NAME_0]);
              if (scores[week][game][FIRST] < 0)
                ...
                {
                  (void)fprintf(output, "  %2d     ? ", scores[week][game][FIRST]);
                }
              resp = get_responce(input);
              if (matches(resp, "quit", 1))
                {
              ...
              if (!ABORT)
                {
                  (void)fprintf(output, "%-16s",
                                team[sched[week][game][SECOND]][FULL_NAME_0]);
                  if (scores[week][game][SECOND] < 0)
                    ...
                    {
                      (void)fprintf(output, "  %2d     ? ", scores[week][game][SECOND]);
                    }
                  resp = get_responce(input);
                  if (matches(resp, "quit", 1))
                    {
  ...
  for (t = 1; t <= NUM_TEAMS / 2; t++)
    {
      (void)fprintf(
          output,
          "%-14s%1s %2d/%-2d %2d/%-2d %2d/%-2d %2d/%-2d %-14s%1s %2d/%-2d "
          "%2d/%-2d %2d/%-2d %2d/%-2d\n",
          team[t][FULL_NAME_0], (a_champ(t) ? "*" : (a_wild_card(t) ? "+" : " ")),
          find_teams_rank(conf_standings, t), find_teams_rank(abs_standings, t),
          find_teams_rank(offence_ranks, t),
          find_teams_rank(offence_ranks_nfl, t),
          find_teams_rank(defence_ranks, t),
          find_teams_rank(defence_ranks_nfl, t), find_teams_rank(net_ranks, t),
          find_teams_rank(net_ranks_nfl, t), team[t + NUM_TEAMS / 2][FULL_NAME_0],
...
void display_defence(FILE *output)
{
  ...
  for (t = 1; t <= NUM_TEAMS / 2; t++)
    {
      (void)fprintf(
          output, "%2d/%-2d) %-15s %-5.1f %3d   %2d/%-2d) %-15s %-5.1f %3d\n", t,
          find_teams_rank(defence_ranks_nfl, defence_ranks[t]),
          team[defence_ranks[t]][FULL_NAME_0],
          PER_GAME(team_info, team_info[defence_ranks[t]].points_against,
                   defence_ranks[t]),
          team_info[defence_ranks[t]].points_against, t,
          find_teams_rank(defence_ranks_nfl, defence_ranks[t + NUM_TEAMS / 2]),
  ...
  for (t = 1; t <= NUM_TEAMS / 2; t++)
    {
      (void)fprintf(
          output, "%2d/%-2d) %-15s %-5.1f %3d   %2d/%-2d) %-15s %-5.1f %3d\n", t,
          find_teams_rank(offence_ranks_nfl, offence_ranks[t]),
          team[offence_ranks[t]][FULL_NAME_0],
          PER_GAME(team_info, team_info[offence_ranks[t]].points_for,
                   offence_ranks[t]),
          team_info[offence_ranks[t]].points_for, t,
          find_teams_rank(offence_ranks_nfl, offence_ranks[t + NUM_TEAMS / 2]),
...
void display_net(FILE *output)
{
  ...
  for (t = 1; t <= NUM_TEAMS / 2; t++)
    {
      (void)fprintf(
          output, "%2d/%-2d) %-15s %-5.1f %4d   %2d/%-2d) %-15s %-5.1f %4d\n", t,
          find_teams_rank(net_ranks_nfl, net_ranks[t]),
          team[net_ranks[t]][FULL_NAME_0],
          PER_GAME(team_info, NET_POINTS(team_info, net_ranks[t]), net_ranks[t]),
          NET_POINTS(team_info, net_ranks[t]), t,
          find_teams_rank(net_ranks_nfl, net_ranks[t + NUM_TEAMS / 2]),
          team[net_ranks[t + NUM_TEAMS / 2]][FULL_NAME_0],
          PER_GAME(team_info, NET_POINTS(team_info, net_ranks[t + NUM_TEAMS / 2]),
  ...
  if (!got_unused_responce)
    {
      ...
      (void)fprintf(output,
                    "Display tie-breaker-information-for <team-name/abbr>\n");
  ...
  while (!valid_input)
    {
      ...
      else if (matches(resp, "display", 1))
        {
          resp = skip_first(resp);
