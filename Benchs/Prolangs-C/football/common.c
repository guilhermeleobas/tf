          ...
          for (i = 1; i <= 2; i++)
            {
              if (i == 1)
                {
                  team1 = sched[week][game][HOST];
                  site1 = HOST;
                  team2 = sched[week][game][VISITOR];
                  site2 = VISITOR;
                }
              else
                {
                  team1 = sched[week][game][VISITOR];
                  site1 = VISITOR;
                  team2 = sched[week][game][HOST];
                  site2 = HOST;
                }
                      ...
                      if (scores[week][game][HOST] >= 0 &&
                          scores[week][game][VISITOR] >= 0)
                        {
                          if (scores[week][game][site1] < scores[week][game][site2])
                            ...
                            {
                              common_games[team2].ties++;
                            }
                        }
                      else
                        {
                          common_games[team2].points_for++;
                        }
                    }
                }
            }
  ...
  for (team1 = 1; team1 <= 28; team1++)
    {
      if (common_games[team1].wins + common_games[team1].loses +
              common_games[team1].ties + common_games[team1].points_for <
          4)
        {
...
void split_around(TEAM *teams, int num_best, int num, double *pct,
                  double best_pct)
{
  int t1, t2;
  int i;
  ...
  for (i = 0; i < num; i++)
    {
      ...
      pct[i] =
          (wins + loses + ties == 0 ? 0.0 : ((double)wins + 0.5 * ties) / ((double)(wins + loses + ties)));
      if (pct[i] > best_pct)
        {
          best_pct = pct[i];
          num_best = 0;
...
int break_common(TEAM *teams, int num)
{
  ...
  for (i = 0; i < num; i++)
    {
      if (common_games[teams[i]].wins == -1)
        {
          return num;
        }
    }
  ...
  for (i = 0; i < num; i++)
    {
      if (which == DIVISION)
        ...
        {
          pct[i] = WIN_PCT(team_info_wi_conf[teams[i]]);
        }
      if (pct[i] > best_pct)
        {
          best_pct = pct[i];
...
int break_net_points(TEAM *teams, int num, int which)
{
  ...
  if (num > NUM_TEAMS)
    {
      (void)printf(
          "ERROR in break_net_points (common.c). Assumption violated.\n");
      exit(1);
    }
  ...
  for (i = 0; i < num; i++)
    {
      if (which == DIVISION)
        ...
        {
          pct[i] = (double)NET_POINTS(team_info, teams[i]);
        }
      if (pct[i] > best_pct)
        {
          best_pct = pct[i];
