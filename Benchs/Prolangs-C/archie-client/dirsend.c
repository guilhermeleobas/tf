...
PTEXT dirsend(PTEXT pkt_p, char *hostname_p, struct sockaddr_in *hostaddr_p)
{
  ...
  while (!dirsendDone) processEvent();
...
static int initDirsend(void)
{
  ...
  if (!hostaddr || (hostaddr->sin_addr.s_addr == 0))
    {
      ...
      if ((hostname == NULL) || (*hostname == '\0'))
        {
#ifdef DEBUG
          if (pfs_debug) fprintf(stderr, "dirsrv: Null hostname specified\n");
#endif
    ...
    bcopy(hostaddr, &to, S_AD_SZ);
  /* lmjm: Save away the hostname */
  strncpy(to_hostname, inet_ntoa(/* SUN_GNU_FIX */ to.sin_addr),
          sizeof(to_hostname) - 1);

  if (req_udp_port)
    to.sin_port = htons(req_udp_port);
  else
  ...
  if (hostaddr)
    {
      if (hostaddr->sin_port)
        to.sin_port = hostaddr->sin_port;
      else
        hostaddr->sin_port = to.sin_port;
    }
  ...
  if ((lp = socket(AF_INET, SOCK_DGRAM, 0)) < 0)
    {
#ifdef DEBUG
      if (pfs_debug) fprintf(stderr, "dirsrv: Can't open socket\n");
#endif
...
/* system assign a non-privileged port later                    */
  ...
  if (!notprived)
    {
      for (tmp = PROS_FIRST_PRIVP; tmp < PROS_FIRST_PRIVP + PROS_NUM_PRIVP; tmp++)
        {
#endif
  ...
  if (pfs_debug > 2)
    {
      ...
      if (to.sin_family == AF_INET)
        {
          if (req_udp_port)
            fprintf(stderr, "Sending message to %s+%d(%d)...", to_hostname,
                    req_udp_port, ntohs(this_conn_id));
          else
            fprintf(stderr, "Sending message to %s(%d)...", to_hostname,
                    ntohs(this_conn_id));
        }
#else
      if (to.sin_family == AF_INET)
...
#ifndef CUTCP
  ns = sendto(lp, (char *)(pkt->start), pkt->length, 0, (struct sockaddr *)&to,
              S_AD_SZ);
#else
  while (--lretry)
    {
...
static void timeoutProc(XtPointer client_data, XtIntervalId *id)
{
  ...
  if (pfs_debug)
    {
      fprintf(stderr, "select failed(timeoutProc): readfds=%x ", readfds);
      perror("");
    }
#endif
#ifndef CUTCP
...
#ifndef CUTCP
  if ((nr = recvfrom(lp, next->start, sizeof(next->dat), 0,
                     (struct sockaddr *)&from, &from_sz)) < 0)
    {
#else
  ...
  if ((hdr_len = (unsigned char)*(next->start)) < 20)
    {
      ...
      if (hdr_len >= 5)
        ...
        { /* No packet number specified, so this is the only one */
          next->seq = 1;
          nd_pkts = 1;
        }
      if (hdr_len >= 7)
      ...
      if (hdr_len >= 12)
        { /* Flags (1st byte) */
          ...
          if (rdflag11 & 0x80)
            {
#ifdef DEBUG
              if (pfs_debug > 2) fprintf(stderr, "Ack requested\n");
#endif
              ackpend++;
            }
          if (rdflag11 & 0x40)
            {
#ifdef DEBUG
              if (pfs_debug > 2) fprintf(stderr, "Sequenced control packet\n");
#endif
              next->length = -1;
              scpflag++;
            }
...
#endif
#ifndef CUTCP
      ns = sendto(lp, (char *)(pkt->start), pkt->length, 0,
                  (struct sockaddr *)&to, S_AD_SZ);
#else
      while (--lretry)
        {
          ns = netusend(&to.sin_addr, ntohs(to.sin_port), ntohs(us.sin_port),
...
/* see if timeout or error or wrong descriptor */
  ...
  if (tmp == 0)
  ...
  else if ((tmp < 0) || !FD_ISSET(lp, &readfds))
    {
      ...
      if (pfs_debug)
        {
          fprintf(stderr, "select failed(processEvent): readfds=%x ", readfds);
          perror("");
        }
#endif
      close(lp);
