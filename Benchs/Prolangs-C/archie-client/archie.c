...
int main(int argc, char *argv[])
{
  ...
  if ((p = getenv("ARCHIE_HOST")) != (char *)NULL) host = p;
          ...
          switch (*cur_arg++)
            {
              ...
              case 'L':
                ...
                printf(
                    " * For the most up-to-date list, write to an Archie server and "
                    "give it\n   the command `servers'.\n");
                exitflag = 1;
                break;
#endif
              ...
              case 'm': /* Max hits */
                ...
                if (max_hits < 1)
                  {
                    fprintf(stderr,
                            "%s: -m option requires a value for max hits (>= 1)\n",
                            progname);
                    exit(A_EXIT);
                  }
                break;
              ...
              case 'v': /* Display version */
                fprintf(stderr, "Client version %s based upon Prospero version %s\n",
              ...
              default:
                fprintf(stderr,
                        "Usage: %s [-[cers][l][t][m#][h host][%s][%s#]%s string\n",
  ...
  if (argc != 1)
    {
      ...
      fprintf(
          stderr,
          "      -m# : specifies maximum number of hits to return (default %d)\n",
          max_hits);
  ...
  if (argc = Snetinit())
    {
      fprintf(stderr, "Error %d from SNetinit (bad or missing config.tel ?)\n",
              argc);
      if (argc == -2) netshut(); /* rarp lookup failure */
      exit(1);
    }
  if (!hostset)
    { /* if no host on command line, look in config.tel file
       for name=archie */
      struct machinfo *mp;
