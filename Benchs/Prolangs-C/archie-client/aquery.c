                   ...
                   char query_type, int (*cmp_proc)(), int flags)
{
  ...
  sprintf(qstring, "ARCHIE/MATCH(%d,%d,%c)/%s", max_hits, offset, query_type,
          string);
...
/* Retrieve the list of matches, return error if there was one */
  ...
  if (!(flags & AQ_NOTRANS))
    for (p = links; p != NULL; p = p->next) translateArchieResponse(p);
...
int invdatecmplink(VLINK p, VLINK q)
{
  ...
  for (pat = p->lattrib; pat; pat = pat->next)
    if (strcmp(pat->aname, "LAST-MODIFIED") == 0) pdate = pat->value.ascii;
  for (qat = q->lattrib; qat; qat = qat->next)
    if (strcmp(qat->aname, "LAST-MODIFIED") == 0) qdate = qat->value.ascii;
  if (!pdate && !qdate) return (defcmplink(p, q));
  if (!pdate) return (1);
  if (!qdate) return (-1);
  if ((result = strcmp(qdate, pdate)) == 0)
    return (defcmplink(p, q));
  else
    return (result);
}
