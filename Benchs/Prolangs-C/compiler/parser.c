  ...
  else
  {
    if (NextLookahead == '[')
    {
      tempGlobalIndex = GlobalIndex =
          GlobalInsert(lexbuf, ARRAY, type, 0, 0, 0);
      if (ErrorFlag)
      {
        return;
      }
    ...
    case ID:
      if (NextLookahead == '(')
      {
        ...
        else if (tempGlobalIndex)
        {
          ...
          {
            emit(PUSH_CODE_LABEL, GlobalTable[tempGlobalIndex].functionlabel,
                 0.0);
          }  /* end else !function */
        }    /* end GlobalIndex */
        ...
        {
          tempGlobalIndex =
              GlobalInsert(templexbuf, ID, INT, 0, 1, LabelCounter);
          if (ErrorFlag)
          {
            ErrorFlag = 0;
          }
        ...
        { /* default return type of undefined function is INT */
          return (0);
        }
      } /* end if NextLookahead == ')' */
