...
void finalcheck(void)
{
  ...
  for (net = 1; net <= numnets; net++)
  {
    ...
    fcost += ((int)(dimptr->Hweight * (double)(dimptr->xmax - dimptr->xmin))) +
  ...
  for (cell = 1; cell <= numcells + numpads + 4; cell++)
  {
    ...
    value +=
        (*overlapf)(cell, cellarray[cell]->xcenter, cellarray[cell]->ycenter,
...
void initcheck(void)
{
  ...
  for (net = 1; net <= numnets; net++)
  {
    ...
    icost += ((int)(dimptr->Hweight * (double)(dimptr->xmax - dimptr->xmin))) +
             ((int)(dimptr->Vweight * (double)(dimptr->ymax - dimptr->ymin)));
  }
  return;
}
