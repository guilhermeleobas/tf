...
int whoOverlaps(int cell, int xc, int yc, int orient, int b, int flag,
                int borient);
...
void fixpenal(void)
{
  ...
  while (problems)
  {
    ...
    {
      other =
          whoOverlaps(cell, cellarray[cell]->xcenter, cellarray[cell]->ycenter,
...
void padOverlaps(void)
{
  ...
  for (cell = 1; cell <= numcells; cell++)
  {
    ...
    if (startx1 >= blockl && endx1 <= blockr && starty1 >= blockb &&
        endy1 <= blockt)
    {
      continue;
    }
    if (startx1 < blockl)
    {
      for (pad = numcells + 1; pad <= numcells + numpads; pad++)
      {
        ...
        deltaX =
            padptr->xcenter + padptr->config[padptr->orient]->right - startx1;
        if (deltaX <= 0)
        {
          break;
        }
    ...
    else if (endx1 > blockr)
    {
      for (pad = numcells + 1; pad <= numcells + numpads; pad++)
      {
        ...
        deltaX =
            endx1 - (padptr->xcenter + padptr->config[padptr->orient]->left);
        if (deltaX <= 0)
        {
          break;
        }
    ...
    else if (starty1 < blockb)
    {
      for (pad = numcells + 1; pad <= numcells + numpads; pad++)
      {
        ...
        deltaY =
            padptr->ycenter + padptr->config[padptr->orient]->top - starty1;
        if (deltaY <= 0)
        {
          break;
        }
    ...
    else if (endy1 > blockt)
    {
      for (pad = numcells + 1; pad <= numcells + numpads; pad++)
      {
        ...
        deltaY =
            endy1 - (padptr->ycenter + padptr->config[padptr->orient]->bottom);
        if (deltaY <= 0)
        {
          break;
        }
