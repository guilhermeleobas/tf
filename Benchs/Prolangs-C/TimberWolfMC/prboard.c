...
void prboard(void)
{
  ...
  for (cell = 1; cell <= numcells + numpads; cell++)
  {
    ...
    {
      fprintf(fp, "LEFT:%6d  BOTTOM:%6d", tileptr->left + xcenter,
              tileptr->bottom + ycenter);
      fprintf(fp, "  RIGHT:%6d  TOP:%6d\n\n", tileptr->right + xcenter,
              tileptr->top + ycenter);
    }
    ...
    for (; termptr != TERMNULL; termptr = termptr->nextterm)
    {
      fprintf(fp, "TERMINAL:%s   XPOS:%6d   YPOS:%6d\n\n",
              pinnames[termptr->terminal], termptr->xpos + xcenter,
    ...
    if (cellptr->softflag == 1)
    {
      for (j = 1; j <= cellptr->numUnComTerms; j++)
      {
        ...
        xpos = cellptr->config[cellptr->orient]->siteLocArray[site].xpos;
        ypos = cellptr->config[cellptr->orient]->siteLocArray[site].ypos;
        fprintf(fp, "TERMINAL:%s   XPOS:%6d   YPOS:%6d\n", pinnames[terminal],
