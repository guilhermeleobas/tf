...
void findratio(double *res1, double *res2)
{
  ...
  for (i = 0; i < 1000000; i++)
  {
    ...
    for (; j < 100; j++)
    {
      ...
      x = (k / (6.0 * N)) * ((k * k * k) + (2.0 - 8.0 * rootN) * (k * k) +
  ...
  */
  ...
  if (k <= m)
  {
    x = 0.4 * (((2.0) * (k * k * k * k) + (5.0 - 15.0 * rootN) * (k * k * k) +
                (20.0 * N - 30.0 * rootN) * (k * k) +
                (30.0 * N - 15.0 * rootN - 5.0) * (k) + (10.0 * N - 2.0)) /
               (6.0 * CC * N / k));
  }
  else
  {
    x = (-2.0) * (k * k * k * k * k) + (15.0 * rootN - 5.0) * (k * k * k * k) +
        (30.0 * rootN - 40.0 * N) * (k * k * k) +
        (40.0 * N * rootN - 60.0 * N + 5.0 * rootN + 5.0) * (k * k) +
        (40.0 * N * rootN - 20.0 * N - 10.0 * rootN + 2.0) * (k);

    x += (4.0) * (m * m * m * m * m) - (30.0 * rootN - 10.0) * (m * m * m * m) -
  ...
  *res1 = y / x;
  ...
  if (k > m)
  {
    N1 = Nterm1(m) - Nterm1(1.0) + Nterm2(m) - Nterm2(1.0) + Nterm3(m) -
         Nterm3(1.0) + Nterm4(m) - Nterm4(1.0) + Nterm5(m) - Nterm5(1.0);
    D1 = Dterm1(m) - Dterm1(1.0) + Dterm2(m) - Dterm2(1.0) + Dterm3(m) -
         Dterm3(1.0) + Dterm4(m) - Dterm4(1.0);
  }
  else
  {
    N1 = Nterm1(k) - Nterm1(1.0) + Nterm2(k) - Nterm2(1.0) + Nterm3(k) -
         Nterm3(1.0) + Nterm4(k) - Nterm4(1.0) + Nterm5(k) - Nterm5(1.0);
    D1 = Dterm1(k) - Dterm1(1.0) + Dterm2(k) - Dterm2(1.0) + Dterm3(k) -
         Dterm3(1.0) + Dterm4(k) - Dterm4(1.0);
  }

  if (k > m)
  {
    N2 = NNterm1(k) - NNterm1(m) + NNterm2(k) - NNterm2(m) + NNterm3(k) -
         NNterm3(m) + NNterm4(k) - NNterm4(m) + NNterm5(k) - NNterm5(m);
    D2 = DDterm1(k) - DDterm1(m) + DDterm2(k) - DDterm2(m) + DDterm3(k) -
         DDterm3(m) + DDterm4(k) - DDterm4(m);
...
double Nterm2(double k)
{
  ...
  exa = -exp(-(a * (k - 1.0)));
  exc = -exp(-(c * (k - 1.0)));
  return ((k * k * k) * (((2.0 / 3.0) * ((exa / (a * a)) - (exc / (c * c)))) -
                         rootN * ((exa / (a)) - (exc / (c)))));
}
...
double Nterm3(double k)
{
  ...
  exa = -exp(-(a * (k - 1.0)));
  exc = -exp(-(c * (k - 1.0)));
  return ((k * k) * ((2.0 * ((exa / (a * a * a)) - (exc / (c * c * c)))) -
...
double Nterm4(double k)
{
  ...
  exa = -exp(-(a * (k - 1.0)));
  exc = -exp(-(c * (k - 1.0)));
  return ((k) * ((4.0 * ((exa / (a * a * a * a)) - (exc / (c * c * c * c)))) -
...
double Nterm5(double k)
{
  ...
  exc = -exp(-(c * (k - 1.0)));
  return (
      (4.0 * ((exa / (a * a * a * a * a)) - (exc / (c * c * c * c * c)))) -
...
double Dterm2(double k)
{
  ...
  exa = -exp(-(a * (k - 1.0)));
  exc = -exp(-(c * (k - 1.0)));
  return ((k * k) * (((1.0 / 2.0) * ((exa / (a * a)) - (exc / (c * c)))) -
                     rootN * ((exa / (a)) - (exc / (c)))));
}
...
double getptree(int h, int L, int N)
{
  ...
  for (i = 1; i <= start - 1; i++)
  {
    ...
    for (j = 0; j <= h; j++, target++)
    {
      ...
      array[target].prob = array[i].prob * probability(array[i].value, h, j, N);
    }
  }
