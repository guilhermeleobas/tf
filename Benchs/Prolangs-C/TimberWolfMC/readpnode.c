...
void readpnode(FILE *fp)
{
  ...
  gnodeArray = (GNODEPTR *)realloc(
      gnodeArray, (1 + numnodes + maxpnode) * sizeof(GNODEPTR));
  ...
  for (i = 1; i <= 4; i++)
  {
    gtrace[i] = (short int *)malloc((1 + maxpnode) * sizeof(short int));
  }
  ...
  for (i = 1; i <= largestNet; i++)
  {
    ...
    netRoutes[i].alternate =
        (ALTBOXPTR *)malloc((1 + MAXPATHS) * sizeof(ALTBOXPTR));
  ...
  savePaths = (short int **)malloc((1 + 2 * MAXPATHS) * sizeof(short int *));
  ...
  segList =
      (short int *)malloc((1 + 3 * (numnodes + maxpnode)) * sizeof(short int));
  sourceList = (short int *)malloc((1 + maxpnode) * sizeof(short int));
  targetList =
      (short int *)malloc((1 + maxpnode + numnodes) * sizeof(short int));
  delSourceList = (short int *)malloc((1 + maxpnode) * sizeof(short int));
  addTargetList =
      (short int *)malloc((1 + maxpnode + numnodes) * sizeof(short int));

  pathList = (int **)malloc((1 + EXTRASOURCES * MAXPATHS) * sizeof(int *));
  for (i = 1; i <= EXTRASOURCES * MAXPATHS; i++)
  {
    pathList[i] = (int *)malloc((2 + numnodes + maxpnode) * sizeof(int));
  ...
  {
    pathArray[i].nodeset =
        (short int *)malloc((1 + numnodes + maxpnode) * sizeof(short int));
  }
  tempArray =
      (short int *)malloc((1 + numnodes + maxpnode) * sizeof(short int));
  ...
  while (fscanf(fp, " %s ", input) == 1)
  {
    if (strcmp(input, "net") == 0)
    {
      if (net > 0)
      {
            ...
            ((pnodeAlength > 30) && (2 * pnode < pnodeAlength)))
        {
          ...
          pnodeAlength = pnode;
          pnodeArray = (PNODEPTR)malloc((1 + pnode) * sizeof(PNODE));
          for (i = 1; i <= pnode; i++)
          {
            pnodeArray[i].nodeList =
                (NNODEPTR)malloc((1 + numnodes + pnode) * sizeof(NNODE));
            pnodeArray[i].eptr = 0;
            pnodeArray[i].equiv = (LIST2PTR)NULL;
          }
          netSegArray = (short int **)malloc(pnode * sizeof(short int *));
          for (i = 0; i < pnode; i++)
          {
            netSegArray[i] =
                (short int *)malloc((1 + numnodes + pnode) * sizeof(short int));
          }
        }
      ...
      pinOffset[net] = pin;
      fscanf(fp, " %s ", input);
      nnameArray[net] = (char *)malloc((strlen(input) + 1) * sizeof(char));
    ...
    else if (strcmp(input, "pin") == 0)
    {
      ...
      segments++;
      fscanf(fp, " %s ", input);
      pnameArray[pin] = (char *)malloc((strlen(input) + 1) * sizeof(char));
