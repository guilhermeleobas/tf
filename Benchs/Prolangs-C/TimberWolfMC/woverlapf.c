          ...
          for (k = 1; k <= occuptr[0]; k++)
            {
              ...
              if (othercell <= numcells)
                {
                  startx02 -= wireestx(startx02, starty02, endy02, tileptr02->lweight);
                  endx02 += wireestx(endx02, starty02, endy02, tileptr02->rweight);
                  starty02 -= wireesty(starty02, startx02, endx02, tileptr02->bweight);
                  endy02 += wireesty(endy02, startx02, endx02, tileptr02->tweight);
                }
              if (startx02 >= endx01 || startx01 >= endx02 || starty02 >= endy01 ||
                  starty01 >= endy02)
                {
                  continue;
                }
              if (cellptr1->numtiles == 1 && cellptr2->numtiles == 1)
                {
                  ...
                  value += (int)(lapFactor *
                                 (double)((endy - starty) * (endx - startx) + offset));
                  fprintf(fpo, "Overlap condition: cell1:%d cell2:%d", cell, othercell);
                ...
                {
                  tempval = 0;
                  for (tileptr1 = tileptr01->nexttile; tileptr1 != TILENULL;
                       tileptr1 = tileptr1->nexttile)
                    {
                      ...
                      if (cell <= numcells)
                        {
                          startx1 -= wireestx(startx1, starty1, endy1, tileptr1->lweight);
                          endx1 += wireestx(endx1, starty1, endy1, tileptr1->rweight);
                          starty1 -= wireesty(starty1, startx1, endx1, tileptr1->bweight);
                          endy1 += wireesty(endy1, startx1, endx1, tileptr1->tweight);
                        }
                      for (tileptr2 = tileptr02->nexttile; tileptr2 != TILENULL;
                           tileptr2 = tileptr2->nexttile)
                        {
                          ...
                          if (othercell <= numcells)
                            {
                              startx2 -= wireestx(startx2, starty2, endy2, tileptr2->lweight);
                              endx2 += wireestx(endx2, starty2, endy2, tileptr2->rweight);
                              starty2 -= wireesty(starty2, startx2, endx2, tileptr2->bweight);
                              endy2 += wireesty(endy2, startx2, endx2, tileptr2->tweight);
                            }

                          if (startx2 >= endx1 || startx1 >= endx2 || starty2 >= endy1 ||
                              starty1 >= endy2)
                            {
                              continue;
                            }
                  ...
                  if (tempval != 0)
                    {
                      value += (int)(lapFactor * (double)(tempval + offset));
                      fprintf(fpo, "Overlap condition: cell1:%d cell2:%d", cell,
                              othercell);
                      fprintf(fpo, "   value:%d   amount:%d\n",
                              (int)(lapFactor * (double)(tempval + offset)), tempval);
                    }
                }
            }
        }
