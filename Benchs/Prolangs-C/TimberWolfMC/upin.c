         ...
         int lastNewSite)
{
  ...
  */
  ...
  for (site = UCptr[UCpin].site; site <= UCptr[UCpin + seq - 1].site; site++)
    {
      SCptr[site].newContents = SCptr[site].contents;
    }
      ...
      if (excess > 0)
        {
          if (excess == 1)
            ...
            {
              newOverFill -=
                  (int)(Tscale * 10.0 * ((double)(excess * excess))) -
                  (int)(Tscale * 10.0 * ((double)((excess - 1) * (excess - 1))));
            }
        }
    }
  ...
  for (tm = UCpin; tm < UCpin + seq; tm++)
    {
      ...
      cost +=
          ((int)(dimptr->Hweight * (double)(dimptr->newxmax - dimptr->newxmin))) -
          ((int)(dimptr->Hweight * (double)(dimptr->xmax - dimptr->xmin))) +
          ((int)(dimptr->Vweight * (double)(dimptr->newymax - dimptr->newymin))) -
          ((int)(dimptr->Vweight * (double)(dimptr->ymax - dimptr->ymin)));
    }

  if ((cost + newOverFill <= funccost + overfill) ||
      (exp((double)(funccost + overfill - cost - newOverFill) / T) >
