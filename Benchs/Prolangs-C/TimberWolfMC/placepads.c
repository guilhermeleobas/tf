...
void placepads(void)
{
  ...
  for (pad = numcells + 1; pad <= numcells + numpads; pad++)
  {
    if (cellarray[pad]->padside == B)
    {
      count = 1;
      height =
          cellarray[pad]->config[0]->top - cellarray[pad]->config[0]->bottom;
      width =
          cellarray[pad]->config[0]->right - cellarray[pad]->config[0]->left;
      for (pad++; pad <= numcells + numpads; pad++)
      {
        ...
        count++;
        if (cellarray[pad]->config[0]->top - cellarray[pad]->config[0]->bottom >
            height)
        {
      ...
      for (pad = numcells + 1; pad <= numcells + numpads; pad++)
      {
        if (cellarray[pad]->padside == B)
        {
          ...
          {
            cellarray[pad]->xcenter = padspace[pad - numcells] * space;
          }
          cellarray[pad]->ycenter = maxHeight - (height - height / 2);
          ...
          for (pad++; pad <= numcells + numpads; pad++)
          {
            ...
            if (fixLRBT[2] == 0)
            {
              cellarray[pad]->xcenter = last + separation + width / 2;
            }
            else
            {
              cellarray[pad]->xcenter = padspace[pad - numcells] * space;
    ...
    if (cellarray[pad]->padside == T)
    {
      ...
      for (pad = numcells + 1; pad <= numcells + numpads; pad++)
      {
        if (cellarray[pad]->padside == T)
        {
          ...
          cellarray[pad]->ycenter =
              blockt + 0.1 * (blockt - blockb) + height / 2;
          last = separation + width;
  ...
  for (pad = numcells + 1; pad <= numcells + numpads; pad++)
  {
    if (cellarray[pad]->padside == L)
    {
      ...
      for (pad++; pad <= numcells + numpads; pad++)
      {
        ...
        count++;
        if (cellarray[pad]->config[0]->right - cellarray[pad]->config[0]->left >
            width)
        {
          width = cellarray[pad]->config[0]->right -
                  cellarray[pad]->config[0]->left;
      ...
      for (pad = numcells + 1; pad <= numcells + numpads; pad++)
      {
        if (cellarray[pad]->padside == L)
        {
          ...
          if (fixLRBT[0] == 0)
          {
            cellarray[pad]->ycenter = blockb + separation + height / 2;
          }
          else
          {
            cellarray[pad]->ycenter = blockb + padspace[pad - numcells] * space;
          }
          cellarray[pad]->xcenter = maxWidth - (width - width / 2);
          ...
          for (pad++; pad <= numcells + numpads; pad++)
          {
            ...
            if (fixLRBT[0] == 0)
            {
              cellarray[pad]->ycenter = last + separation + height / 2;
            }
            else
            {
              cellarray[pad]->ycenter =
                  blockb + padspace[pad - numcells] * space;
            }
            cellarray[pad]->xcenter = maxWidth - (width - width / 2);
      ...
      for (pad = numcells + 1; pad <= numcells + numpads; pad++)
      {
        if (cellarray[pad]->padside == R)
        {
          ...
          cellarray[pad]->xcenter =
              blockr + 0.1 * (blockr - blockl) + width / 2;
          last = blockb + separation + height;
