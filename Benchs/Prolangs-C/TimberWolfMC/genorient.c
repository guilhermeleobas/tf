...
void genorient(void)
{
  ...
  for (cell = 1; cell <= numcells + numpads; cell++)
  {
    ...
    if (aorient != 0)
    {
      ...
      for (tilenum = 1; tilenum <= ptr->numtiles; tilenum++)
      {
        ...
        tptr = tptr->nexttile = (TILEBOXPTR)malloc(sizeof(TILEBOX));
        tptr->nexttile = TILENULL;
        tptr->termptr = TERMNULL;
        tptr->left = tptr0->left;
        tptr->right = tptr0->right;
      ...
      if (ptr->numterms != 0)
      {
        ...
        tmptr = ptr->config[aorient]->termptr =
        ...
        for (termnum = 2; termnum <= ptr->numterms; termnum++)
        {
          ...
          tmptr = tmptr->nextterm = (TERMBOXPTR)malloc(sizeof(TERMBOX));
          tmptr->nextterm = TERMNULL;
          tmptr->terminal = tmptr0->terminal;
          tmptr->xpos = tmptr0->xpos;
          tmptr->ypos = tmptr0->ypos;
      ...
      if (ptr->numsites != 0)
      {
        ...
        siteptr = ptr->config[aorient]->siteLocArray =
            (LOCBOX *)malloc((ptr->numsites + 1) * sizeof(LOCBOX));
        for (site = 1; site <= ptr->numsites; site++)
        {
          ...
          point(&(siteptr0[site].xpos), &(siteptr0[site].ypos));
  ...
  for (cell = 1; cell <= numcells + numpads; cell++)
  {
    ...
    for (orient = 1; orient <= 7; orient++)
    {
      ...
      tptr0 = ptr->config[0];
      tptr = ptr->config[orient] = (TILEBOXPTR)malloc(sizeof(TILEBOX));
      tptr->nexttile = TILENULL;
      tptr->left = tptr0->left;
      tptr->right = tptr0->right;
      tptr->bottom = tptr0->bottom;
  ...
  for (cell = 1; cell <= numcells + numpads; cell++)
  {
    ...
    for (orient = 1; orient <= 7; orient++)
    {
      ...
      tmptr0 = ptr->config[0]->termptr;
      tmptr = ptr->config[orient]->termptr =
          (TERMBOXPTR)malloc(sizeof(TERMBOX));
      tmptr->nextterm = TERMNULL;
      tmptr->terminal = tmptr0->terminal;
      tmptr->xpos = tmptr0->xpos;
  ...
  for (cell = 1; cell <= numcells + numpads; cell++)
  {
    ...
    for (orient = 1; orient <= 7; orient++)
    {
      ...
      for (site = 1; site <= ptr->numsites; site++)
      {
        ...
        point(&(siteptr[site].xpos), &(siteptr[site].ypos));
  ...
  for (net = 1; net <= numnets; net++)
  {
    ...
    {
      termarray[netptr->terminal] = (TERMNETSPTR)malloc(sizeof(TERMNETS));
