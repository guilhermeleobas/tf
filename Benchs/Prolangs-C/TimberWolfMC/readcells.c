  ...
  while (fscanf(fp, " %s ", input) == 1)
    {
      if (strcmp(input, "cell") == 0)
        {
          ...
          tile = ptr->config[0] = (TILEBOXPTR)malloc(sizeof(TILEBOX));
          ...
          do
            {
              ptr->numtiles++;
              tile = tile->nexttile = (TILEBOXPTR)malloc(sizeof(TILEBOX));
              tile->nexttile = TILENULL;
              tile->termptr = TERMNULL;
              tile->siteLocArray = LNULL;
              buster();
      ...
      else if (strcmp(input, "softcell") == 0)
        {
          ...
          ptr->numsides = ncorners;
          ptr->sideArray = (SIDEBOX *)malloc((ncorners + 1) * sizeof(SIDEBOX));
          perim += perimeter();
          ...
          if (ptr->numUnComTerms != 0)
            {
              lastSite = 0;
              ptr->unComTerms =
                  (UNCOMBOX *)malloc((ptr->numUnComTerms + 1) * sizeof(UNCOMBOX));
              for (k = 1; k <= B[0].xc; k++)
                {
                  xx1 = B[k].xc;
                  yy1 = B[k].yc;
          ...
          ptr->siteContent =
              (CONTENTBOX *)malloc((numsites + 1) * sizeof(CONTENTBOX));
          ptr->config[0]->siteLocArray =
              (LOCBOX *)malloc((numsites + 1) * sizeof(LOCBOX));
          if (ptr->numUnComTerms != 0)
            {
              numsites = 0;
              for (k = 1; k <= B[0].xc; k++)
                {
                  ...
                  for (j = 1, l = prevsites + 1; l <= numsites; l++, j++)
                    {
                      ptr->siteContent[l].HorV = kArray[j].HV;
                      ptr->siteContent[l].span = kArray[j].sp;
                      ptr->config[0]->siteLocArray[l].xpos = kArray[j].x - xcenter;
                      ptr->config[0]->siteLocArray[l].ypos = kArray[j].y - ycenter;
                      ptr->config[0]->siteLocArray[l].oxpos = kArray[j].x - xcenter;
                      ptr->config[0]->siteLocArray[l].oypos = kArray[j].y - ycenter;
                      ptr->siteContent[l].contents = 0;
                      ptr->siteContent[l].newContents = 0;
                      ptr->siteContent[l].capacity = kArray[j].cap;
                    }
                }
            }
          unComTerms = 0;
      ...
      else if (strcmp(input, "pin") == 0)
        {
          ...
          pinnames[++pinctr] = (char *)malloc((strlen(input) + 1) * sizeof(char));
          ...
          netptr = netarray[netx]->netptr = (NETBOXPTR)malloc(sizeof(NETBOX));
          ...
          if (ptr->softflag == 1)
             ...
             */
                      hit = 1;
                      distant = ABS(ptr->config[0]->siteLocArray[m].ypos - ypos);
                      for (m++; m <= ptr->numsites; m++)
                        {
                          if (ptr->config[0]->siteLocArray[m].xpos == xpos)
                            {
                              if (ABS(ptr->config[0]->siteLocArray[m].ypos - ypos) < distant)
                                {
                                  distant = ABS(ptr->config[0]->siteLocArray[m].ypos - ypos);
                                }
                              else
                  ...
                  else if (ptr->config[0]->siteLocArray[m].ypos == ypos)
                    {
                      /*
             *  Find the site with the nearest xpos
             */
                      hit = 1;
                      distant = ABS(ptr->config[0]->siteLocArray[m].xpos - xpos);
                      for (m++; m <= ptr->numsites; m++)
                        {
                          if (ptr->config[0]->siteLocArray[m].ypos == ypos)
                            {
                              if (ABS(ptr->config[0]->siteLocArray[m].xpos - xpos) < distant)
                                {
                                  distant = ABS(ptr->config[0]->siteLocArray[m].xpos - xpos);
                                }
                              else
          ...
          if (hit == 0)
            {
              ...
              if (ptr->numterms == 1)
                {
                  term = ptr->config[0]->termptr = (TERMBOXPTR)malloc(sizeof(TERMBOX));
                }
              else
                {
      ...
      else if (strcmp(input, "sequence") == 0 || strcmp(input, "group") == 0)
        {
          ptr->numgroups++;
