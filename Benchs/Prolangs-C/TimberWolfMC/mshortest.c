...
extern void tmax(TNODEPTR *root, TNODEPTR *node, int *value, int *property);

void greduce(short int *ptr, int p, int ex0, int ex1, int ex2, int ex3,
             int ex4);
void gunreduce(short int *ptr, int p, int ex0, int ex1, int ex2, int ex3,
               int ex4);
    ...
    for (;;)
    {
      ...
      else
      {
        for (k = 1; k <= q - p + 1; k++)
        {
          ...
          greduce(ptr, p + k - 1, ex[0], ex[1], ex[2], ex[3], ex[4]);
          d = prestrict(ptr, p + k - 1, source, target);
          gunreduce(ptr, p + k - 1, ex[0], ex[1], ex[2], ex[3], ex[4]);
          if (d < 0)
          {
            continue;
          }
