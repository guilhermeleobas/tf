...
void readgeo(FILE *fp)
{
      ...
      if (strcmp(input, "cell") == 0)
        {
          ...
          if (++cell % 50 == 0)
            {
              cellList =
                  (CLBOXPTR *)realloc(cellList, (cell + 50) * sizeof(CLBOXPTR));
          ...
          for (v = 1; v <= vertices; v++)
            {
              ...
              if (v == 1)
              ...
              else
                {
                  ...
                  if (v % 2 == 0)
                    {
                      ...
                      edgeList[edgeCount].length = (uy <= vy) ? (vy - uy) : (uy - vy);
                  ...
                  if (v == 2)
                    {
                      edgeList[edgeCount].prevEdge = edgeCount + vertices - 1;
                    }
                  else
                    {
          ...
          if (++rippleCount > 400)
            {
              edgeList =
                  (EDGEBOXPTR)realloc(edgeList, (edgeCount + 400) * sizeof(EDGEBOX));
              rippleCount = 1;
            }
          edgeList[edgeCount].cell = cell;
          edgeList[edgeCount].start = (ux <= vx) ? ux : vx;
        ...
        {
          fprintf(fpdebug, "Found unparsable keyword: < %s > in ", input);
  ...
  edgeList =
      (EDGEBOXPTR)realloc(edgeList, (3 * edgeCount + 5) * sizeof(EDGEBOX));
