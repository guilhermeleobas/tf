...
void changraph(void)
{
  ...
  for (i = 1; i <= numRects; i++)
  {
    fprintf(fpdebug, "rect Node: %d  at: %d %d  width:%d  height:%d\n", i,
            rectArray[i].xc, rectArray[i].yc, rectArray[i].r - rectArray[i].l,
...
int constructVedge(int index1, int index2)
{
  ...
  if (edgeList[edge1].fixed == 0)
  {
    ...
    for (vptr = vptr->prev; vptr != (DLINK1PTR)NULL; vptr = vptr->prev)
    {
      if (vptr == v2ptr)
      {
        continue;
...
int constructHedge(int index1, int index2)
{
  ...
  if (edgeList[edge1].fixed == 0)
  {
    ...
    for (hptr = hptr->prev; hptr != (DLINK1PTR)NULL; hptr = hptr->prev)
    {
      if (hptr == h2ptr)
      {
        continue;
