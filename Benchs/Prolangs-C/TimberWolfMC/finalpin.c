  ...
  for (cell = 1; cell <= numcells; cell++)
    {
       ...
       */
          ...
          if (HorV)
            {
              ...
              if ((double)span - (double)trueSpan / aspFactor >
                  (double)(trueSpan + 1) / aspFactor - (double)span)
                {
                  trueSpan++;
                }
            }
          ...
          if (trueSpan - nPinLocs * pinSpacing >
              (nPinLocs + 1) * pinSpacing - trueSpan)
            {
              nPinLocs++;
            }
          /*
              ...
              for (i = 1; i <= nPinLocs; i++)
                {
                  ...
                  if (nextUp == 0)
                    {
                      lArray[i].finaly =
                          SLptr[site].ypos - trueSpan / 2 + (i - 1) * pinSpacing;
                    }
                  else
                    {
                      lArray[i].finaly =
                          SLptr[site].ypos - (trueSpan + 1) / 2 + i * pinSpacing;
                    }
                }
            }
          else
            { /* a horizontal trueSpan  */
              if (site == ptr->numsites)
                {
                  nextS = 1;
                }
              ...
              for (i = 1; i <= nPinLocs; i++)
                {
                  ...
                  if (nextUp == 0)
                    {
                      lArray[i].finalx =
                          SLptr[site].xpos - trueSpan / 2 + (i - 1) * pinSpacing;
                    }
                  else
                    {
                      lArray[i].finalx =
                          SLptr[site].xpos - (trueSpan + 1) / 2 + i * pinSpacing;
                    }
                }
            }
             ...
             */
                 ...
                 */
                          ...
                          for (++pin1; pin1 <= ptr->numUnComTerms; pin1++)
                            {
                              if (UCptr[pin1].sequence != 0 || UCptr[pin1].site != site)
                                {
                                  break;
                                }
                   ...
                   */
                                  ...
                                  if (begSeqs[0] == 0)
                                    {
                                      for (; i >= 0; i--)
                                        {
                                          begSeqs[i] = conSeqs[conSeqs[0][0]][i];
                                          conSeqs[conSeqs[0][0]][i] = 0;
                                        }
                                    }
                                  else
                                    {
                          ...
                          conSeqs[conSeqs[0][0]][++conSeqs[conSeqs[0][0]][0] + 1] = pin1;
                        }
                      pin1--;
                    }
                  else
                    { /* sequence # was numero uno  */
       ...
       */
         ...
         */
              ...
              if (ABS(lArray[1].finalx - xnext) + ABS(lArray[1].finaly - ynext) <
       ...
       */
          ...
          while (hit == 1)
             ...
             */
                      if (conSeqs[i][1] > 0)
                        {
                          for (j = 1; j <= conSeqs[i][0]; j++)
                            {
                              lArray[loFill + j].pin = conSeqs[i][j + 1];
                              lArray[loFill + j].placed = 1;
                            }
                        }
                      else
                        {
                          for (j = 1, k = conSeqs[i][0]; j <= conSeqs[i][0]; j++, k--)
                            {
                              lArray[loFill + j].pin = conSeqs[i][k + 1];
                              lArray[loFill + j].placed = 1;
                            }
                        }
                      loFill += conSeqs[i][0];
                  ...
                  else if (ABS(conSeqs[i][1]) + conSeqs[i][0] >= hiFill)
                    {
                      /*
             ...
             */
                      if (conSeqs[i][1] > 0)
                        {
                          for (j = hiFill - conSeqs[i][0], k = 1; j < hiFill; j++, k++)
                            {
                        ...
                        {
                          for (j = hiFill - conSeqs[i][0], k = conSeqs[i][0]; j < hiFill;
                               j++, k--)
                            {
                              lArray[j].pin = conSeqs[i][k + 1];
                              lArray[j].placed = 1;
       ...
       */
         ...
         */
              if (secLoc < firstLoc + conSeqs[first][0] - 1)
                {
                  overlapping = firstLoc + conSeqs[first][0] - 1 - secLoc;
                  while ((firstLoc + conSeqs[first][0] - 1 - secLoc) > overlapping / 2)
                    {
                  ...
                  if (conSeqs[first][1] > 0)
                    {
                      for (j = 1; j <= conSeqs[first][0]; j++)
                        {
                          lArray[firstLoc - 1 + j].pin = conSeqs[first][j + 1];
                          lArray[firstLoc - 1 + j].placed = 1;
                        }
                    }
                  else
                    {
                      for (j = 1, k = conSeqs[first][0]; j <= conSeqs[first][0]; j++, k--)
                        {
                          lArray[firstLoc - 1 + j].pin = conSeqs[first][k + 1];
                          lArray[firstLoc - 1 + j].placed = 1;
                        }
                    }
                  ...
                  if (conSeqs[second][1] > 0)
                    {
                      for (j = 1; j <= conSeqs[second][0]; j++)
                        {
                          lArray[loFill + j].pin = conSeqs[second][j + 1];
                          lArray[loFill + j].placed = 1;
                        }
                    }
                  else
                    {
                      for (j = 1, k = conSeqs[second][0]; j <= conSeqs[second][0];
                           j++, k--)
                        {
                          lArray[loFill + j].pin = conSeqs[second][k + 1];
                          lArray[loFill + j].placed = 1;
                        }
                    }
              ...
              if (lArray[isolated[2 * i - 1]].placed == 0)
                {
                  ...
                  lArray[isolated[2 * i - 1]].pin = isolated[2 * i];
                  isolated[2 * i - 1] = 0;
                }
            }
          /*
                      ...
                      if (lArray[prefer + k].placed == 0)
                        {
                          ...
                          lArray[prefer + k].pin = isolated[2 * i];
                          isolated[2 * i - 1] = 0;
                          break;
                        }
                    }
      ...
      for (i = 1; i <= ptr->numUnComTerms; i++)
        {
          if (UCptr[i].finalx == -100000000 && UCptr[i].finaly == -100000000)
            {
              /*
