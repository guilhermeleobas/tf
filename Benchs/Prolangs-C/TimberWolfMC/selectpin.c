...
extern int upin(CELLBOXPTR acellptr, int UCpin, int seq, int firstNewSite,
                int lastNewSite);
extern int upinswap(CELLBOXPTR acellptr, int pin1, int pin2, int site1,
                    int site2);
...
void selectpin(CELLBOXPTR acellptr)
{
  ...
  if (UCptr[i].groupflag == 1 && seq >= 2)
  {
    do
    {
      pin1 = (int)((double)seq * ((double)RAND / (double)0x7fffffff));
    } while (pin1 == seq);
    do
    {
      pin2 = (int)((double)seq * ((double)RAND / (double)0x7fffffff));
    } while (pin2 == seq || pin2 == pin1);

    if (UCptr[i + pin1].site != UCptr[i + pin2].site)
    {
      if (upinswap(acellptr, i + pin1, i + pin2, UCptr[i + pin1].site,
                   UCptr[i + pin2].site))
      {
        flips++;
        flipp++;
