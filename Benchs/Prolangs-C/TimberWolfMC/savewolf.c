...
void savewolf(void)
{
  ...
  for (cell = 1; cell <= numcells; cell++)
  {
    ...
    fprintf(fp, "%d %d %d %d\n", cell, cellptr->orient, cellptr->xcenter,
            cellptr->ycenter);
    for (terminal = 1; terminal <= cellptr->numUnComTerms; terminal++)
    {
      fprintf(fp, " %d ", cellptr->unComTerms[terminal].site);
...
void TW_oldinput(FILE *fp)
{
  ...
  while (fscanf(fp, " %d %d %d %d ", &cell, &orient, &xcenter, &ycenter) == 4)
  {
    ptr = cellarray[cell];
    ...
    if (ptr->softflag == 1 && ptr->aspUB > 0.01 + ptr->aspLB)
    {
      ...
      for (termptr = atermptr; termptr != TERMNULL; termptr = termptr->nextterm)
      {
    ...
    for (term = ptr->config[aorient]->termptr; term != TERMNULL;
         term = term->nextterm)
    {
      xpos = term->xpos;
      ypos = term->ypos;
