...
void printgph(void)
{
  ...
  for (i = 1; i <= eNum; i++)
  {
    if (eArray[i].notActive == 0)
    {
      ...
      fprintf(fpr,
              "edge  node: %5d  xloc: %d  yloc: %d    node: %5d  xloc: %d  "
              "yloc: %d   length: %d   capacity: %d\n",
              index1, rectArray[index1].xc, rectArray[index1].yc, index2,
              rectArray[index2].xc, rectArray[index2].yc, length, width);
    }
  }
  fclose(fp);
  fclose(fpr);
