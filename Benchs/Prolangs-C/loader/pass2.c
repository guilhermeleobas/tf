  ...
  while (!feof(INPUT) && ((*LOCATION) <= MEM_SIZE_1))
    {
         ...
         * -------------------------------- */
            if (strlen(RECORD) < 9)
            ...
            else
              {
                ...
                if (strlen(RECORD) != 9 + LENGTH * 2)
                  {
                    LOCAL_ERROR = TRUE_1;
                  }
                else
                  {
                    STORE_AT(&(RECORD[9]), LENGTH, UNREL_LOCATION + UNREL_TO_REL_SHIFT,
                             MEMORY, &LOCAL_ERROR);
                  }
              }
            if (LOCAL_ERROR)
       ...
       * ------------------------ */
           ...
           * ------------------ */
                STR_TO_NUM(&(RECORD[1]), 6, 16, &UNREL_LOCATION, &LOCAL_ERROR);
                STR_TO_NUM(&(RECORD[7]), 2, 16, &LENGTH, &LOCAL_ERROR);
                ADD_INT_TO_LOC(UNREL_TO_REL_SHIFT,
                               UNREL_LOCATION + UNREL_TO_REL_SHIFT, LENGTH, MEMORY,
                               &LOCAL_ERROR);
              }
            else
            ...
            else
              {
                ...
                if (SYMBOL == NULL)
                  ...
                  {
                    ADD_INT_TO_LOC((*SYMBOL).LOCATION * SIGN,
                                   UNREL_LOCATION + UNREL_TO_REL_SHIFT, LENGTH, MEMORY,
                                   &LOCAL_ERROR);
                  }
              }
                    ...
                    if (LOOK_UP_SYMBOL(GLOBAL_1, TEMP_NAME, SYM_TAB) == NULL)
                      {
                        ...
                        (void)INSERT_IN_SYM_TAB(GLOBAL_1, TEMP_NAME, 0, UNDEFINED,
                                                SYM_TAB);
                        for (I = LABEL_SIZE_1 - 1; ((I < 1) || (TEMP_NAME[I] == ' '));
                             I--)
                          {
                            ;
                          }
                        TEMP_NAME[I + 1] = '\0';
                        (void)printf("ERROR: Undefined global %s. Program not loaded.\n",
                                     TEMP_NAME);
