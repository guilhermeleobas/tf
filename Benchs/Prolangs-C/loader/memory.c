...
void CREATE_MEMORY(MEM_SPACE *MEMORY)
{
  if ((*MEMORY) != NULL)
    ...
    {
      (*MEMORY) = malloc((unsigned int)MEM_SIZE_1);
    }
}
  ...
  if ((ADDRESS + BYTES <= MEM_SIZE_1) && (ADDRESS + BYTES >= 0))
    {
      ...
      for (TEMP = 0; ((TEMP < BYTES) && !eoln(VALUE[TEMP]) && !LOCAL_ERROR);
           TEMP++)
        {
          STR_TO_NUM(&(VALUE[TEMP * HEX_CHAR_PER_BYTE_1]), HEX_CHAR_PER_BYTE_1, 16,
                     &INT_VAL, &LOCAL_ERROR);
                    ...
                    BOOLEAN *ERROR)
{
  ...
  if ((LOCATION + HALF_BYTES / HEX_CHAR_PER_BYTE_1 <= MEM_SIZE_1) &&
      (LOCATION + HALF_BYTES / HEX_CHAR_PER_BYTE_1 >= 0))
    {
      ...
      DO_STORE(CHAR_MEM_VAL, HALF_BYTES / HEX_CHAR_PER_BYTE_1, LOCATION, MEMORY,
               ERROR, FALSE_1);
    }
  else
    {
...
int LINES_OF_GAP(struct BUFFER_ELEMENT *REC1, struct BUFFER_ELEMENT *REC2)
{
  ...
  if (REC1 == NULL)
    ...
    {
      REC1_END = START_OF_LINE_ADDR((*REC1).START + (*REC1).LENGTH - 1);
    }

  if (REC2 == NULL)
    {
      REC2_START = START_OF_LINE_ADDR(MEM_SIZE_1 - 1);
    }
  else
    {
      REC2_START = START_OF_LINE_ADDR((*REC2).START);
    }
  ...
  if (LINES_OF_GAP(PREV, CURR) >= LINES_OF_GAP_TO_ELIPSE)
    {
      ...
      (void)fprintf(OUTPUT, " %s    xxxxxxxx  xxxxxxxx  xxxxxxxx  xxxxxxxx\n",
                    STR_ADDR);
      ...
      if (CURR != NULL)
        {
          NUM_TO_STR(
              START_OF_LINE_ADDR((*CURR).START) - BYTES_PER_GROUP * GROUPS_PER_LINE,
              16, MEM_ADDR_SIZE_1 / BITS_PER_HALFBYTE_1, STR_ADDR);
        }
      else
        {
          NUM_TO_STR(
              START_OF_LINE_ADDR(MEM_SIZE_1) - BYTES_PER_GROUP * GROUPS_PER_LINE,
              16, MEM_ADDR_SIZE_1 / BITS_PER_HALFBYTE_1, STR_ADDR);
        }
...
void PRINT_MEM(MEM_SPACE MEMORY, FILE *OUTPUT)
{
  ...
  char STR_ADDR[MEM_ADDR_SIZE_1 / BITS_PER_HALFBYTE_1 +
                1]; /* String version of*/
  /*                     address            */
  ...
  if (LOCATIONS_USED.HEAD == NULL)
    {
      (void)fprintf(OUTPUT, "=========> NOTHING LOADED INTO MEMORY <=========\n");
    }
  else
    {
      ...
      for (STEP = LOCATIONS_USED.HEAD; STEP != NULL; STEP = (*STEP).NEXT)
        {
          ...
          do
            {
              ...
              NUM_TO_STR(ADDRESS, 16, MEM_ADDR_SIZE_1 / BITS_PER_HALFBYTE_1,
                         STR_ADDR);
          ...
          while (ADDRESS <= END_OF_LINE_ADDR((*STEP).START + (*STEP).LENGTH - 1));
...
void OUTPUT_MEM(MEM_SPACE MEMORY, FILE *OUTPUT)
{
  ...
  char ADDRESS[MEM_ADDR_SIZE_1 / BITS_PER_HALFBYTE_1 +
               2]; /* String version of */
  /*                        address             */
  ...
  while (CURRENT != NULL)
    {
      ...
      for (COUNT = (*CURRENT).START;
           COUNT < ((*CURRENT).START + (*CURRENT).LENGTH); COUNT++)
        {
