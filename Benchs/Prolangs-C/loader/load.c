...
int main(int argc, char **argv)
{
  ...
  if (argc == 1)
    {
      (void)printf("usage:  load [-l location] file ...\n");
    }
  else
    {
              ...
              if (ARGUMENT < argc)
                {
                  ...
                  NEXT = argv[ARGUMENT];
                  LOCATION = GET_NUM(&NEXT, MEM_ADDR_SIZE_1, 10, &NOT_LEGAL_NUM);
                  if (NOT_LEGAL_NUM)
                    {
                      (void)printf(
                          "load: %s is not a legal starting address. Starting at %d\n",
                ...
                {
                  (void)printf(
                      "load: -l must be followed by a location. Starting at %d\n",
                      LOCATION);
                }
            }
          else
      ...
      if (ARGUMENT < argc)
        {
          ...
          if (LOOP_COUNTER <= 0)
            {
              LOOP_COUNTER = strlen(argv[ARGUMENT]);
            }
