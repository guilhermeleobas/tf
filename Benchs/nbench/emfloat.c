...
*/
void SetupCPUEmFloatArrays(InternalFPF *abase, InternalFPF *bbase,
                           InternalFPF *cbase, ulong arraysize)
{
  ulong i;
...
*/
ulong DoEmFloatIteration(InternalFPF *abase, InternalFPF *bbase,
                         InternalFPF *cbase, ulong arraysize, ulong loops)
{
...
*/
static void AddSubInternalFPF(uchar operation, InternalFPF *x, InternalFPF *y,
                              InternalFPF *z)
{
  ...
  switch ((x->type * IFPF_TYPE_COUNT) + y->type)
    {
      ...
      */
        ...
        */
            if (locx.type == IFPF_IS_SUBNORMAL || locy.type == IFPF_IS_SUBNORMAL)
              {
        ...
        */
          ...
          */
                ...
                for (j = (INTERNAL_FPF_PRECISION - 1); j >= 0; j--)
                  {
                    Add16Bits(&carry, &z->mantissa[j], z->mantissa[j], x->mantissa[j]);
                  }
              }
            else
              {
  ...
  switch ((x->type * IFPF_TYPE_COUNT) + y->type)
    {
      ...
      */
        ...
        */
            ...
            for (j = (INTERNAL_FPF_PRECISION - 1); j >= 0; j--)
              {
                Sub16Bits(&carry, &extra_bits[j], extra_bits[j], y->mantissa[j]);
              }
...
static void Int32ToInternalFPF(int32 mylong, InternalFPF *dest)
{
  int i; /* Index */
        ...
        for (j = 0; j < 9; j++)
          {
            ...
            for (k = (INTERNAL_FPF_PRECISION - 1); k >= 0; k--)
              Add16Bits(&mycarry, &(IFPFComp.mantissa[k]), locFPFNum.mantissa[k],
                        IFPF10.mantissa[k]);
