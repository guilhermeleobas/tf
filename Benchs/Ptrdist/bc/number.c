...
void init_num(bc_num *num) { *num = copy_num(_zero_); }
...
_PROTOTYPE(static int _do_compare,
           (bc_num n1, bc_num n2, int use_sign, int ignore_last));
...
_PROTOTYPE(static void _one_mult,
           (char *num, int size, int digit, char *result));
...
char is_neg(bc_num num) { return num->n_sign == MINUS; }
...
static void _one_mult(char *num, int size, int digit, char *result)
{
  int carry, value;
  char *nptr;
  char *rptr;
...
void out_long(long val, int size, int space, void (*out_char)(int))
{
  char digits[40];
  int len, ix;
...
void out_num(bc_num num, int o_base, void (*out_char)(int))
{
  char *nptr;
...
void p_v(char *name, unsigned char *num, int len)
{
  int i;
  printf("%s=", name);
...
void str2num(bc_num *num, char *str, int scale)
{
  ...
  *num = new_num(digits, strscale);
  ...
  for (; digits > 0; digits--) *nptr++ = CH_VAL(*ptr++);

  /* Build the fractional part. */
  if (strscale > 0)
    {
      ptr++; /* skip the decimal point! */
      for (; strscale > 0; strscale--) *nptr++ = CH_VAL(*ptr++);
    }
}
...
char *num2str(bc_num num)
{
  ...
  nptr = num->n_value;
  for (index = num->n_len; index > 0; index--) *sptr++ = BCD_CHAR(*nptr++);

  /* Now the fraction. */
  if (num->n_scale > 0)
    {
      *sptr++ = '.';
      for (index = 0; index < num->n_scale; index++) *sptr++ = BCD_CHAR(*nptr++);
    }
