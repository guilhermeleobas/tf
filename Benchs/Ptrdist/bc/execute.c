      ...
      switch (inst)
        {
          ...
          case 'C': /* Call a function. */
            ...
            /* Push auto variables. */
            for (auto_list = functions[new_func].f_autos; auto_list != NULL;
          ...
          case 'P': /* Write the value on the top of the stack.  No newline. */
            out_num(ex_stack->s_num, o_base, (void (*)(int))out_char);
            if (inst == 'W')
              {
                out_char('\n');
              }
            ...
            switch (new_func)
              {
                case 'L': /* Length function. */
                  ...
                  if (ex_stack->s_num->n_len == 1 && ex_stack->s_num->n_scale != 0 &&
          ...
          case '&': /* compare greater than */
            if (check_stack(2))
              {
                c_code =
                    !is_zero(ex_stack->s_next->s_num) && !is_zero(ex_stack->s_num);
                pop();
                assign(c_code);
              }
            break;
          ...
          case '*': /* multiply */
            if (check_stack(2))
              {
                bc_multiply(ex_stack->s_next->s_num, ex_stack->s_num, &temp_num,
                            scale);
                pop();
                pop();
                push_num(temp_num);
          ...
          case '/': /* divide */
            if (check_stack(2))
              {
                if (bc_divide(ex_stack->s_next->s_num, ex_stack->s_num, &temp_num,
                              scale) == 0)
                  {
                    pop();
                    pop();
            ...
            if (check_stack(2))
              {
                if (is_zero(ex_stack->s_num))
                  ...
                  {
                    bc_modulo(ex_stack->s_next->s_num, ex_stack->s_num, &temp_num,
                              scale);
                    pop();
                    pop();
                    push_num(temp_num);
...
char prog_char(void) { return byte(&(pc)); }
