...
void more_functions(void)
{
  ...
  f_count += STORE_INCR;
  functions = (bc_function *)malloc(f_count * sizeof(bc_function));
  f_names = (char **)malloc(f_count * sizeof(char *));
...
void more_variables(void)
{
  ...
  v_count += STORE_INCR;
  variables = (bc_var **)malloc(v_count * sizeof(bc_var *));
...
void more_arrays(void)
{
  ...
  /* Increment by a fixed amount and allocate. */
  a_count += STORE_INCR;
  arrays = (bc_var_array **)malloc(a_count * sizeof(bc_var_array *));
...
void push_copy(bc_num num)
{
  ...
  temp = (estack_rec *)malloc(sizeof(estack_rec));
...
bc_var *get_var(int var_name)
{
  ...
  if (var_ptr == NULL)
  {
    var_ptr = variables[var_name] = (bc_var *)malloc(sizeof(bc_var));
...
bc_num *get_array_num(int var_index, long index)
{
  ...
  if (ary_ptr == NULL)
  {
    ary_ptr = arrays[var_index] = (bc_var_array *)malloc(sizeof(bc_var_array));
  ...
  if (a_var == NULL)
  {
    a_var = ary_ptr->a_value = (bc_array *)malloc(sizeof(bc_array));
    a_var->a_tree = NULL;
    a_var->a_depth = 0;
  }
  ...
  {
    temp = (bc_array_node *)malloc(sizeof(bc_array_node));
    if (a_var->a_depth != 0)
    {
  ...
  while (log-- > 1)
  {
    ...
    {
      temp->n_down[ix1] = (bc_array_node *)malloc(sizeof(bc_array_node));
  ...
  else
  {
    ...
    switch (var_name)
    {
      ...
      case 1:
        ...
        else if (temp > BC_BASE_MAX || toobig)
        {
          o_base = BC_BASE_MAX;
...
void auto_var(int name)
{
  ...
  if (name > 0)
  {
    /* A simple variable. */
    ix = name;
    v_temp = (bc_var *)malloc(sizeof(bc_var));
  ...
  else
  {
    /* An array variable. */
    ix = -name;
    a_temp = (bc_var_array *)malloc(sizeof(bc_var_array));
...
void free_a_tree(bc_array_node *root, int depth)
{
  ...
  if (root != NULL)
  {
    if (depth > 1)
    {
      for (ix = 0; ix < NODE_SIZE; ix++)
      {
        free_a_tree(root->n_down[ix], depth - 1);
      }
    }
    else
    {
  ...
  while (list != NULL)
  {
    ...
    else
    {
      ...
      if (a_temp != NULL)
      {
        ...
        {
          free_a_tree(a_temp->a_value->a_tree, a_temp->a_value->a_depth);
          free(a_temp->a_value);
        }
        free(a_temp);
      }
...
void process_params(program_counter *pc, int func)
{
  char ch;
  arg_list *params;
  char warned = FALSE;
