...
void ReadNetList(char *fname)
{
  ...
  TRY(inFile = fopen(fname, "r"), inFile != NULL, "ReadData",
      "unable to open input file [%s]", inFile, 0, 0, exit(1));

  TRY(fgets(line, BUF_LEN, inFile),
      sscanf(line, "%lu %lu", &numNets, &numModules) == 2, "ReadData",
      "unable to parse header in file [%s]", inFile, 0, 0, exit(1));
  ...
  for (net = 0; net < numNets; net++)
    {
      ...
      TRY(head = prev = (Module *)malloc(sizeof(Module)), prev != NULL,
          "ReadData", "unable to allocate a module list node", 0, 0, 0, exit(1));
      (*prev).module = atol(strtok(NULL, " \t\n")) - 1;
      (*prev).next = NULL;
      while ((tok = strtok(NULL, " \t\n")) != NULL)
        {
          TRY(node = (Module *)malloc(sizeof(Module)), node != NULL, "ReadData",
              "unable to allocate a module list node", 0, 0, 0, exit(1));
  ...
  for (net = 0; net < numNets; net++)
    {
      for (modNode = nets[net]; modNode != NULL; modNode = (*modNode).next)
        {
          TRY(netNode = (Net *)malloc(sizeof(Net)), netNode != NULL,
              "NetsToModules", "unable to allocate net list node", 0, 0, 0,
              exit(1));
          (*netNode).net = net;
...
#else
      count = 0;
      for (nn = nets[i]; nn != NULL; nn = (*nn).next) count++;
  ...
  for (p = 0; p < numModules / 2; p++)
    {
      ...
      TRY(mr = (ModuleRec *)malloc(sizeof(ModuleRec)), mr != NULL, "main",
          "unable to allocate ModuleRec", 0, 0, 0, exit(1));
      (*mr).module = p;
      if (groupA.head == NULL)
        {
          /* first item */
