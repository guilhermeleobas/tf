...
void InitAllocMaps(void)
{
  ...
  if (horzPlane == NULL || vertPlane == NULL || viaPlane == NULL ||
      mazeRoute == NULL)
  {
 ...
 */
void DrawSegment(char *plane, unsigned long x1, unsigned long y1,
 ...
 *	draw a via at (x, y)
 */
void DrawVia(unsigned long x, unsigned long y)
{
 ...
 */
int HasVia(unsigned long x, unsigned long y)
{
 ...
 *	they are sorted as needed by the line drawer
 */
int SegmentFree(char *plane, unsigned long x1, unsigned long y1,
                unsigned long x2, unsigned long y2)
{
  unsigned long x, y;
  unsigned long index;
...
void PrintChannel(void)
{
  ...
  for (x = 1; x <= channelColumns; x++)
  {
    printf(" %lu ", (TOP[x] - (TOP[x] / 100) * 100) / 10);
  }
  printf("\n");
  ...
  for (x = 1; x <= channelColumns; x++)
  {
    printf(" %lu ", (BOT[x] - (BOT[x] / 100) * 100) / 10);
  }
  printf("\n");
...
int DrawNets(void)
{
  ...
  bzero(horzPlane, (int)((channelColumns + 1) * (channelTracks + 2)));
  bzero(vertPlane, (int)((channelColumns + 1) * (channelTracks + 2)));
  bzero(viaPlane, (int)((channelColumns + 1) * (channelTracks + 2)));
  bzero(mazeRoute, (int)(channelColumns + 1));
  ...
  for (i = 1; i <= channelNets; i++)
  {
    if (FIRST[i] != LAST[i])
    {
      DrawSegment(horzPlane, FIRST[i], netsAssign[i], LAST[i], netsAssign[i]);
    }
#ifdef VERBOSE
  ...
  for (i = 1; i <= channelColumns; i++)
  {
    ...
    else if ((BOT[i] == 0) && (TOP[i] != 0))
    {
      ...
      DrawSegment(vertPlane, i, 0, i, netsAssign[TOP[i]]);
      DrawVia(i, netsAssign[TOP[i]]);
    }
    else if ((TOP[i] == 0) && (BOT[i] != 0))
    {
      /* only one segment, therefore no vertical constraint violation */
      DrawSegment(vertPlane, i, netsAssign[BOT[i]], i, channelTracks + 1);
    ...
    else if ((TOP[i] == BOT[i]) && (FIRST[TOP[i]] == LAST[TOP[i]]))
    {
      ...
      DrawSegment(vertPlane, i, 0, i, channelTracks + 1);
    }
    else if (TOP[i] == BOT[i])
    {
 ...
 * is mazed
 */
static void CleanNet(unsigned long net)
{
  unsigned long i;
 ...
 */
static int HasVCV(unsigned long i)
{
  return ((TOP[i] != 0) && (BOT[i] != 0) && (TOP[i] != BOT[i]) &&
 ...
 *	routed
 */
static int Maze1Mech(unsigned long i,  /* column */
                     ...
                     int bXdelta, int bYdelta) /* bend X, Y delta from s */
{
  if (SegmentFree(vertPlane, /* straight vert seg in col i */
      ...
      SegmentFree(vertPlane, /* bent jog in vert plane */
                  i, s2 + bYdelta, i + bXdelta, s2 + bYdelta) &&
      SegmentFree(vertPlane, /* run along segment in vert plane */
                  i + bXdelta, s2 + bYdelta, i + bXdelta, b2) &&
      !HasVCV(i + bXdelta))
  {
...
#ifdef TODD
      if ((i > 1) && (p > 1) &&
          Maze1Mech(i, channelTracks + 1, p, /* straight */
                    0, s,                    /* bent */
                    -1, -1))
      {
        mazeRoute[i] = 0;
 ...
 */
int ExtendOK(unsigned long net, char *plane, unsigned long _x1,
             unsigned long _y1,                    /* start seg */
             unsigned long _x2, unsigned long _y2) /* end seg */
{
  ...
  {
    /* subsumes */
    return (SegmentFree(plane, x1, y1, FIRST[net] - 1, y1) &&
            SegmentFree(plane, LAST[net] + 1, y1, x2, y1));
  }
  ...
  else if (x2 > LAST[net])
  {
    ...
    return SegmentFree(plane, LAST[net] + 1, y1, x2, y1);
  }
  /* should not get here */
  abort();
  return 0;
...
static int Maze2Mech(unsigned long bentNet, /* net to bend */
                     ...
                     int bYdelta) /* direction of bend vert seg */
{
  ...
  { /* search for row */
    colFree = 1;
    for (col = xStart; colFree && (col != xEnd); col += bXdelta)
    {                                       /* search for col */
      if ((colFree = SegmentFree(horzPlane, /* bent horz seg */
          ...
          SegmentFree(vertPlane, /* bent to net */
                      col, row, col, b2 - 1) &&
          !HasVCV(col) && ExtendOK(bentNet, horzPlane, col, b2, i, b2))
      {
        /* draw it! */
    ...
    if (mazeRoute[i])
    {
...
#ifdef TODD
      if ((i > 1) && (p > 1) &&
          Maze2Mech(TOP[i], i, channelTracks + 1, p, /* straight */
                    0, s,                            /* bent */
                    i - 1, 1, -1,                    /* left srch */
                    1, p - 1, 1))
      {
                         ...
                         1, p - 1, 1))
      {
        ...
        CleanNet(BOT[i]);
               ...
               Maze2Mech(BOT[i], i, 0, s,      /* straight */
                         channelTracks + 1, p, /* bent */
                         i - 1, 1, -1,         /* left srch */
                         s + 1, channelTracks, 1))
      {
...
void FindFreeHorzSeg(unsigned long startCol, unsigned long row,
...
static int Maze3Mech(unsigned long topNet, /* top net to bend */
  ...
  for (topRow = b2 + 1; topRow < s2 - 1; topRow++)
  {
    ...
    for (botRow = topRow + 1; botRow < s2; botRow++)
    {
      ...
      for (topCol = topStart; topCol <= topEnd; topCol++)
      {
        ...
        {
          if ((topCol != i) && (botCol != i) && (topRow != botRow) &&
              (topCol != botCol) && SegmentFree(vertPlane, /* top down */
                                                i, s1, i, topRow) &&
              SegmentFree(horzPlane, /* over to drop point */
                          i, topRow, topCol, topRow) &&
              SegmentFree(vertPlane, /* down to net */
                          topCol, topRow, topCol, s2 + 1) &&
              !HasVCV(topCol) &&
              ExtendOK(topNet, horzPlane, topCol, s2, i, s2) &&
              SegmentFree(vertPlane, /* bot up */
                          i, b1, i, botRow) &&
              SegmentFree(horzPlane, /* over to up point */
                          i, botRow, botCol, botRow) &&
              SegmentFree(vertPlane, /* up to net */
                          botCol, botRow, botCol, b2 - 1) &&
              !HasVCV(botCol) && ExtendOK(botNet, horzPlane, botCol, b2, i, b2))
          {
            /* draw it! */
...
int Maze3(void)
{
  ...
  for (i = 1; i <= channelColumns; i++)
  {
    if (mazeRoute[i])
    {
      ...
      if (Maze3Mech(TOP[i], BOT[i], i, 0, s, channelTracks + 1, p))
      {
        mazeRoute[i] = 0;
        CleanNet(TOP[i]);
        CleanNet(BOT[i]);
