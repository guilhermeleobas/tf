...
void AllocVCG(void)
{
  ...
  storageRootVCG = (constraintVCGType *)malloc(
      (channelNets + 1) * (channelNets + 1) * sizeof(constraintVCGType));
  ...
  removeVCG = (constraintVCGType **)malloc(
      (channelNets + 1) * (channelNets + 1) * sizeof(constraintVCGType *));
}
  ...
  for (net = 1; net <= channelNets; net++)
  {
    ...
    for (which = 0; which < VCG[net].netsAbove; which++)
    {
      if (!VCG[net].netsAboveHook[which].removed)
      {
        ...
        printf("%lu ", VCG[net].netsAboveHook[which].bot);
      }
    }
    ...
    for (which = 0; which < VCG[net].netsBelow; which++)
    {
      if (!VCG[net].netsBelowHook[which].removed)
      {
        ...
        printf("%lu ", VCG[net].netsBelowHook[which].top);
      }
    }
    printf("\n\n");
  }
...
void AcyclicVCG(void)
{
  ...
  {
    printf("\n*** Input is cyclic! ***\n");
    printf("*** VC's removed (%lu) ***\n", total);
  }
}

void RemoveConstraintVCG(nodeVCGType *VCG, ulong *SCC, ulong *perSCC,
  ...
  for (net = 1; net <= channelNets; net++)
  {
    if (assign[net])
    {
      ...
      else
      {
         ...
         */
        if (ExistPathAboveVCG(VCG, net, check) ||
            ExistPathAboveVCG(VCG, check, net))
        {
          vcv++;
        }
      }
