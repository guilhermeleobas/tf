...
void MaxNetsAssign(void)
{
  ...
  for (net = 1; net <= channelNets; net++)
    {
      if ((FIRST[net] <= channelDensityColumn) &&
          (LAST[net] >= channelDensityColumn))
        {
          CROSSING[net] = TRUE;
          netsCrossing++;
        }
...
void Assign(nodeVCGType *VCG, ulong *assign, ulong select)
{
  long dist;
  ulong ideal;
  ulong track;
  ...
  if (tracks == 0)
    {
      for (track = 1; track <= channelTracks; track++)
        {
          if (tracksNoHCV[track] && tracksTopNotPref[track] &&
              tracksBotNotPref[track])
            {
              tracksAssign[track] = TRUE;
              tracks++;
            }
...
void Select(nodeVCGType *VCG, nodeHCGType *HCG, ulong *netsAssign,
            ulong *netSelect, ulong *CROSSING)
{
  ulong net;
  ulong track;
  ulong select;
...
void BuildCostMatrix(nodeVCGType *VCG, nodeHCGType *HCG, ulong *netsAssign,
                     ulong *CROSSING)
{
  ulong net;
  ulong track;
              ...
              if (tracksNoHCV[track])
                {
                  if (cardNotPref != channelTracks)
                    ...
                    {
                      if ((track > (channelTracks - cardBotNotPref)) &&
                          (track <= cardTopNotPref))
                        {
                          mult = cardNotPref -
                                 (cardTopNotPref + cardBotNotPref - channelTracks);
                          assert(mult >= 0);
...
void IdealTrack(ulong tracks, ulong top, ulong bot, ulong *ideal)
{
  ulong num;
  ulong den;
