...
char **Calignm1(float *wm, char **aseq, char *seq, double *effarr, int icyc,
                int ex)
{
  ...
  for (i = 0; i < icyc + 1; i++) fprintf(stdout, " %f", effarr[i]);
  printf("\n");
#endif
  ...
  */
  ...
  /*
     ...
     g[i][2][0], g[i][0][1], g[i][1][1], g[i][0][2], g[i][2][2] );
          }
          printf( "\n\n\n\n\n" );
  */
  /*
  ...
  */
  ...
  for (i = 1; i < lgth + 1; i++)
  {
    ...
    for (j = 1; j < lgth1 + 1; j++)
    {
      ...
      {
        mi = w[i - 1][0] +
             g[i - 0][0][2] /* + n_dis[24][0] * totaleff */; /* 0.0? */
        /*
        fprintf( stderr, " i == %d j == 1, w[i][0] == %f, mi == %f, g[i][0][2]
        == %f\n", i, w[i][0], mi, g[i][0][2] );
        */
        /*
      ...
      if (i > 1)
      {
        ...
        m[j] +=
            g[i - 1][1][1] + gl[i - 2][i - mp[j] - 2] + gvsa[i - 1]; /* ??? */
        if (m[j] < x)
        {
          m[j] = x;
          mp[j] = i - 2;
  ...
  for (k = 0; k < len; k++)
  {
    ...
    for (i = 0; i < locnjob; i++)
    {
      ...
      cob = !gb1 * gc1 * !gb2 * !gc2
            ...
            + gb1 * !gc1 * gb2 * gc2 * tmp1

            + gb1 * gc1 * gb2 * !gc2 * tmp2;

      pen += (double)cob * penalty * efficient;
      tmpscore +=
          (double)amino_dis[(int)seq[i][k]][(int)seq[ex][k]] * efficient;
      /*
      nglen += ( !gc1 * !gc2 );
      */
      /*
      if( k == 0 )
      {
              printf( "%c<->%c * %f = %f\n", seq[ex][k], seq[i][k], efficient,
      amino_dis[seq[i][k]][seq[ex][k]] * efficient );
      }
      */
    }
    score += tmpscore;
