...
static void match_ribosum(float *match, float **cpmx1, float **cpmx2, int i1,
...
static float MSalign2m2m_rec(int icyc, int jcyc, double *eff1, double *eff2,
                             char **seq1, char **seq2, float **cpmx1,
                             float **cpmx2, int ist, int ien, int jst, int jen,
                             int alloclen, char **mseq1, char **mseq2,
                             int depth, float **gapinfo, float **map)
...
//	short **shortmtx;
  ...
  //  if( lgth1 < DPTANNI && lgth2 < DPTANNI ) // & dato lgth ==1 no kanousei ga
  //  arunode yokunai
  ...
  match_ribosum(initverticalw, cpmx2 + jst, cpmx1 + ist, 0, lgth1, floatwork,
                intwork, 1);

  match_ribosum(currentw, cpmx1 + ist, cpmx2 + jst, 0, lgth2, floatwork,
                intwork, 1);
...
//			g = *mjpt + fpenalty;
  ...
  match_ribosum(initverticalw, cpmx2 + jst, cpmx1 + ist, lgth2 - 1, lgth1,
                floatwork, intwork, 1);
  match_ribosum(currentw, cpmx1 + ist, cpmx2 + jst, lgth1 - 1, lgth2, floatwork,
                intwork, 1);
...
static float MSalignmm_rec(int icyc, int jcyc, double *eff1, double *eff2,
                           char **seq1, char **seq2, float **cpmx1,
                           float **cpmx2, int ist, int ien, int jst, int jen,
                           int alloclen, char **mseq1, char **mseq2, int depth,
                           float **gapinfo, float **map)
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
  int alnlen;
  float value = 0.0;
...
//	short **shortmtx;
  ...
  match_calc(initverticalw, cpmx2 + jst, cpmx1 + ist, 0, lgth1, floatwork,
             intwork, 1);

  match_calc(currentw, cpmx1 + ist, cpmx2 + jst, 0, lgth2, floatwork, intwork,
             1);
...
//			g = *mjpt + fpenalty;
  ...
  match_calc(initverticalw, cpmx2 + jst, cpmx1 + ist, lgth2 - 1, lgth1,
             floatwork, intwork, 1);
  match_calc(currentw, cpmx1 + ist, cpmx2 + jst, lgth1 - 1, lgth2, floatwork,
             intwork, 1);
...
#endif
  value = MSalignmm_rec(icyc, jcyc, eff1, eff2, seq1, seq2, cpmx1, cpmx2, ist,
                        ist + jumpi, jst, jst + jumpj, alloclen, aseq1, aseq2,
                        depth, gapinfo, map);
#if 0
  ...
  for (i = 0; i < jcyc; i++)
    {
      aseq2[i] += alnlen;
		...
		fprintf( stderr, "seq2[0] = %s\n", seq2[0] );
#endif
  value += MSalignmm_rec(icyc, jcyc, eff1, eff2, seq1, seq2, cpmx1, cpmx2,
                         ist + imid, ien, jst + jmid, jen, alloclen, aseq1,
                         aseq2, depth, gapinfo, map);
#if 0
		fprintf( stderr, "aseq1[0] = %s\n", aseq1[0] );
		fprintf( stderr, "aseq2[0] = %s\n", aseq2[0] );
#endif
...
float Lalignmm_hmout(char **seq1, char **seq2, double *eff1, double *eff2,
                     int icyc, int jcyc, int alloclen, char *sgap1, char *sgap2,
                     char *egap1, char *egap2, float **map)
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
  //	int k;
  int i, j;
  ...
  wm = MSalignmm_rec(icyc, jcyc, eff1, eff2, seq1, seq2, cpmx1, cpmx2, 0,
                     lgth1 - 1, 0, lgth2 - 1, alloclen, mseq1, mseq2, 0,
                     gapinfo, map);
#if DEBUG
  fprintf(stderr, " seq1[0] = %s\n", seq1[0]);
  fprintf(stderr, " seq2[0] = %s\n", seq2[0]);
...
float Lalign2m2m_hmout(char **seq1, char **seq2, char **seq1r, char **seq2r,
                       char *dir1, char *dir2, double *eff1, double *eff2,
                       int icyc, int jcyc, int alloclen, char *sgap1,
                       char *sgap2, char *egap1, char *egap2, float **map)
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
  //	int k;
  int i, j;
  ...
  wm = MSalign2m2m_rec(icyc, jcyc, eff1, eff2, seq1, seq2, cpmx1, cpmx2, 0,
                       lgth1 - 1, 0, lgth2 - 1, alloclen, mseq1, mseq2, 0,
                       gapinfo, map);
#if DEBUG
  fprintf(stderr, " seq1[0] = %s\n", seq1[0]);
