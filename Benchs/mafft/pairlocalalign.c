...
static float recallpairfoldalign(char **mseq1, char **mseq2, int m1, int m2,
                                 int *of1pt, int *of2pt, int alloclen)
{
  static FILE *fp = NULL;
  float value;
  char *aln1;
  ...
  readpairfoldalign(fp, *mseq1, *mseq2, aln1, aln2, m1, m2, &of1tmp, &of2tmp,
                    alloclen);
  ...
  if (aln1[0] == 0)
    {
      fprintf(stderr,
              "FOLDALIGN returned no alignment between %d and %d.  Sequence "
  ...
  //	value = naivepairscore11( *mseq1, *mseq2, penalty ); // v6.511 ha kore
  //wo tsukau, global nomi dakara.
...
static void callfoldalign(int nseq, char **mseq)
{
  ...
  sprintf(com, "env PATH=%s  foldalign210 %s _foldalignin > _foldalignout ",
          whereispairalign, foldalignopt);
  res = system(com);
  if (res)
    {
...
static void calllara(int nseq, char **mseq, char *laraarg)
{
  ...
  sprintf(com,
          "env PATH=%s:/bin:/usr/bin mafft_lara -i _larain -w _laraout -o "
          "_lara.params %s",
          whereispairalign, laraarg);
  res = system(com);
  if (res)
    {
...
static float recallmxscarna(char **mseq1, char **mseq2, int m1, int m2,
                            int alloclen)
{
  FILE *fp = NULL;
...
static float callmxscarna_giving_bpp(char **mseq1, char **mseq2, char **bpp1,
                                     char **bpp2, int alloclen)
{
  FILE *fp;
  int res;
  static char com[10000];
  ...
  sprintf(com,
          "env PATH=%s mxscarnamod -readbpp _mxscarnain > _mxscarnaout 2>_dum",
          whereispairalign);
  res = system(com);
  if (res)
    {
...
static float callmxscarna_slow(char **mseq1, char **mseq2, int alloclen)
{
  ...
  sprintf(com, "env PATH=%s mxscarnamod _mxscarnain > _mxscarnaout 2>_dum",
          whereispairalign);
  res = system(com);
  if (res)
    {
...
static void pairalign(char name[M][B], int nlen[M], char **seq, char **aseq,
                      char **mseq1, char **mseq2, double *effarr, int alloclen)
{
  int i, j, ilim;
  int clus1, clus2;
  int off1, off2;
  ...
  if (alg == 'B')
    {
      fprintf(stderr,
              "Running LARA (Bauer et al. http://www.planet-lisa.net/)\n");
      calllara(njob, seq, "");
      fprintf(stderr, "done.\n");
    }
  if (alg == 'T')
    {
      fprintf(stderr,
              "Running SLARA (Bauer et al. http://www.planet-lisa.net/)\n");
      calllara(njob, seq, "-s");
      fprintf(stderr, "done.\n");
    }
  if (alg == 's')
    {
      ...
      fprintf(stderr, "done.\n");
      fprintf(stderr,
              "Running MXSCARNA (Tabei et al. "
              "http://www.ncrna.org/software/mxscarna)\n");
    }
      ...
      for (j = i + 1; j < njob; j++)
        {
          ...
          strcpy(aseq[i], seq[i]);
          strcpy(aseq[j], seq[j]);
          clus1 = conjuctionfortbfast(pair, i, aseq, mseq1, effarr1, effarr,
                                      indication1);
          clus2 = conjuctionfortbfast(pair, j, aseq, mseq2, effarr2, effarr,
                                      indication2);
          //		fprintf( stderr, "mseq1 = %s\n", mseq1[0] );
          //		fprintf( stderr, "mseq2 = %s\n", mseq2[0] );
			...
			fprintf( stderr, "\n" );
#endif
          //		for( l=0; l<clus1; l++ ) fprintf( stderr, "## STEP-eff for
          //mseq1-%d %f\n", l, effarr1[l] );
              ...
              switch (alg)
                {
                  case ('a'):
                    ...
                    break;
                  case ('t'):
                    pscore = G__align11_noalign(amino_dis, penalty, penalty_ex, mseq1,
                                                mseq2, alloclen);
                    off1 = off2 = 0;
                    break;
                  case ('A'):
                  ...
                  case ('L'):
                    ...
                    //);
                    //						pscore = (float)naivepairscore11( *mseq1, *mseq2,
                    //penalty ); // nennnotame
                  ...
                  case ('H'):
                    pscore =
                        recallpairfoldalign(mseq1, mseq2, i, j, &off1, &off2, alloclen);
                    break;
                  case ('B'):
                  case ('T'):
                  ...
                  case ('s'):
                    pscore =
                        callmxscarna_giving_bpp(mseq1, mseq2, bpp[i], bpp[j], alloclen);
                    //						pscore = G__align11_noalign( amino_disLN, -1200, -60,
                    //mseq1, mseq2, alloclen );
                    off1 = off2 = 0;
                    //						fprintf( stderr, "scarna, pscore = %f\n", pscore
                    //);
                    break;
                  case ('M'):
                    //						pscore = MSalign11( mseq1, mseq2, effarr1, effarr2,
                    //clus1, clus2, alloclen, NULL, NULL, NULL, NULL );
          ...
          if (alg == 't' ||
              (mseq1[0][0] != 0 && mseq2[0][0] != 0))  // 't' no jouken ha iranai to
            // omou. if( ( mseq1[0][0] !=
            // 0 && mseq2[0][0] != 0  ) )
            {
...
//				offset = makelocal( *mseq1, *mseq2, pslocal );
              ...
              if (alg == 't' || out_align_instead_of_hat3 == 1)
                ...
                {
                  putlocalhom_ext(mseq1[0], mseq2[0], localhomtable[i] + j, off1, off2,
                                  (int)pscore, strlen(mseq1[0]));
                }
              else if (alg != 'S' && alg != 'V')
                {
                  putlocalhom2(mseq1[0], mseq2[0], localhomtable[i] + j, off1, off2,
                               (int)pscore, strlen(mseq1[0]));
                }
            }
          else
...
#endif
          if (distancemtx[i][j] == NODIST ||
              (bunbo = MIN(distancemtx[i][i], distancemtx[j][j])) == 0.0 ||
              bunbo < distancemtx[i][j])
            {
              distancemtx[i][j] = 2.0;
            }
          else
...
// tmptmptmptmptmp
//					if( alg == 'B' || alg == 'T' )
//						fprintf( hat3p, "%d %d %d %7.5f %d %d %d
//%d %p\n", i, j, tmpptr->overlapaa, 1.0, tmpptr->start1, tmpptr->end1,
//tmpptr->start2, tmpptr->end2, (void *)tmpptr->next );
//					else
#ifdef LLVM
                  fprintf(stdout, "%d %d %d %7.5f %d %d %d %d\n", i, j,
                          tmpptr->overlapaa, tmpptr->opt, tmpptr->start1, tmpptr->end1,
                          tmpptr->start2, tmpptr->end2);
#else
...
static void WriteOptions(FILE *fp)
{
  ...
  fprintf(stderr, "Gap Penalty = %+5.2f, %+5.2f, %+5.2f\n",
          (double)ppenalty / 1000, (double)ppenalty_ex / 1000,
          (double)poffset / 1000);
  if (use_fft)
    {
      fprintf(fp, "FFT on\n");
  ...
  fprintf(fp, "Gap Penalty = %+5.2f, %+5.2f, %+5.2f\n", (double)ppenalty / 1000,
          (double)ppenalty_ex / 1000, (double)poffset / 1000);
  ...
  if (njob > M)
    {
      ...
      fprintf(stderr,
              "Please try the splittbfast program for such large data.\n");
      exit(1);
    }
