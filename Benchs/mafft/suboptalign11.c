...
static float gentracking(int **used, char **seq1, char **seq2, char **mseq1,
                         char **mseq2, float **cpmx1, float **cpmx2, int **ijpi,
                         int **ijpj, int *off1pt, int *off2pt, int endi,
                         int endj)
{
  ...
  *mspt2 = 0;
  ...
  for (k = 0; k <= limk; k++)
  {
    ...
    *--mspt1 = seq1[0][ifi];
    *--mspt2 = seq2[0][jfi];
    if (ijpi[ifi][jfi] == localstop)
    {
      break;
    }
    if (ijpj[ifi][jfi] == localstop)
    {
      break;
    }
    k++;
...
float suboptalign11(char **seq1, char **seq2, int alloclen, int *off1pt,
                    int *off2pt, LocalHom *lhmpt)
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
  ...
  //	fprintf( stderr, "@@@@@@@@@@@@@ penalty_OP = %f, penalty_EX = %f,
...
fprintf( stderr, "\n" );
  ...
  for (j = 0; j < lgth2 + 1; j++) fprintf(stderr, "%5.0f ", currentw[j]);
  fprintf(stderr, "\n");
#endif
...
//			fprintf( stderr, "wm=%f\n", wm );
      ...
      *mjpt += fpenalty_ex;
      ...
      if (g > wm)
      {
        ...
        *ijpipt = tbki;
        *ijpjpt = tbkj;
        //				fprintf( stderr, "hit! i%d, j%d, ijpi = %d,
        //ijpj = %d\n", i, j, *ijpipt, *ijpjpt );
      }
      g = Mi;
      if (g > tbk)
      ...
      //			fprintf( stderr, "wm=%f, tbk=%f(%c-%c), mi=%f,
...
//seq1[0][i], seq2[0][j], *ijppt, localstop );
  ...
  for (k = 0; k < numshuryo; k++)
  {
    ...
    fprintf(stderr, "k=%d, shuryo[k].i,j,wm=%d,%d,%f go\n", k, shuryo[k].i,
            shuryo[k].j, shuryo[k].wm);
    resf = gentracking(used, seq1, seq2, mseq1, mseq2, cpmx1, cpmx2, ijpi, ijpj,
                       off1pt, off2pt, shuryo[k].i, shuryo[k].j);
    if (resf == -1.0)
    {
      continue;
    }
    putlocalhom3(mseq1[0], mseq2[0], lhmpt, *off1pt, *off2pt, (int)shuryo[k].wm,
                 strlen(mseq1[0]));
#if 0
		fprintf( stderr, "\n" );
		fprintf( stderr, ">\n%s\n", mseq1[0] );
