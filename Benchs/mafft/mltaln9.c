...
void intergroup_score_consweight(char **seq1, char **seq2, double *eff1,
                                 double *eff2, int clus1, int clus2, int len,
                                 double *value)
{
  int i, j, k;
  int len2 = len - 2;
  ...
  for (i = 0; i < clus1; i++)
    {
      for (j = 0; j < clus2; j++)
        {
          efficient =
              eff1[i] * eff2[j]; /* なぜか配列を使わないとおかしくなる,
                                多分バグ */
          mseq1 = seq1[i];
          mseq2 = seq2[j];
          tmpscore = 0.0;
              ...
              if (ms1 == (int)'-')
                {
                  ...
                  //						tmpscore +=
                  //(double)amino_dis_consweight_multi[ms1][ms2];
                  k--;
                  if (k > len2)
                    {
                      break;
...
void intergroup_score_gapnomi(char **seq1, char **seq2, double *eff1,
                              double *eff2, int clus1, int clus2, int len,
                              double *value)
{
  int i, j, k;
...
void intergroup_score(char **seq1, char **seq2, double *eff1, double *eff2,
                      int clus1, int clus2, int len, double *value)
{
  int i, j, k;
  int len2 = len - 2;
  int ms1, ms2;
...
void intergroup_score_new(char **seq1, char **seq2, double *eff1, double *eff2,
                          int clus1, int clus2, int len, double *value)
{
  int i, j, k;
  int len2 = len - 2;
...
double score_calc5(char **seq, int s, double **eff,
                   int ex) /* method 3 deha nai */
{
  int i, j, k;
  double c;
  int len = strlen(seq[0]);
  ...
  for (i = 0; i < s; i++)
    {
      ...
      score += (double)tmpscore * efficient;
      /*
                            fprintf( stdout, "%d-%d tmpscore = %f, eff = %f,
...
double score_calc4(char **seq, int s, double **eff,
                   int ex) /* method 3 deha nai */
{
  int i, j, k;
  double c;
...
static void setnearest(int nseq, Bchain *acpt, float **eff, float *mindisfrompt,
                       int *nearestpt, int pos)
{
  int j;
  float tmpfloat;
  float **effptpt;
...
//			fprintf( stderr, "k=%d i=%d\n", k, i );
      ...
      intpt = topol[k][0] =
          (int *)realloc(topol[k][0], (nmemim + 1) * sizeof(int));
      if (prevnode == -1)
        {
          *intpt++ = im;
          *intpt = -1;
      ...
      intpt = topol[k][1] =
          (int *)realloc(topol[k][1], (nmemjm + 1) * sizeof(int));
      if (!intpt)
        {
      ...
      sprintf(treetmp, "(%s:%7.5f,%s:%7.5f)", tree[im], len[k][0], tree[jm],
              len[k][1]);
...
static float cluster_minimum_float(float d1, float d2) { return (MIN(d1, d2)); }
...
void loadtop(int nseq, float **eff, int ***topol,
             float **len)  // computes branch length BUG!!
{
...
#else
              eff[miniim][maxiim - miniim] = (clusterfuncpt[0])(eff0, eff1);
#endif
            }
        }
...
void fixed_musclesupg_float_realloc_nobk_halfmtx_treeout(int nseq, float **eff,
                                                         int ***topol,
                                                         float **len,
                                                         char **name, int *nlen)
{
...
void fixed_musclesupg_float_realloc_nobk_halfmtx(int nseq, float **eff,
                                                 int ***topol, float **len)
{
  int i, j, k, miniim, maxiim, minijm, maxijm;
  int *intpt, *intpt2;
...
void veryfastsupg_double_loadtop(int nseq, double **eff, int ***topol,
                                 double **len)  // BUG!!!
{
  int i, k, miniim, maxiim, minijm, maxijm;
  int *intpt, *intpt2;
          ...
          if (i != im && i != jm)
            {
              ...
              eff[miniim][maxiim] =
                  MIN(eff0, eff1) * (1.0 - SUEFF) + (eff0 + eff1) * 0.5 * SUEFF;
            }
        }
      acjmprev = ac[jm].prev;
      acjmnext = ac[jm].next;
...
void veryfastsupg_double_loadtree(int nseq, double **eff, int ***topol,
                                  double **len)
{
  int i, k, miniim, maxiim, minijm, maxijm;
  int *intpt, *intpt2;
...
void veryfastsupg_double_outtree(int nseq, double **eff, int ***topol,
                                 double **len, char name[M][B])
{
  int i, j, k, miniim, maxiim, minijm, maxijm;
  int *intpt, *intpt2;
  double tmpdouble;
          ...
          if (i != im && i != jm)
            {
              ...
              eff1 = eff[minijm][maxijm];
				...
				( eff0 + eff1 ) * 0.5 * SUEFF;
#else
              eff[miniim][maxiim] = (clusterfuncpt[0])(eff0, eff1);
#endif
            }
        }
      acjmprev = ac[jm].prev;
  ...
  for (k = 0; k < nseq - 1; k++)
    {
      ...
      intpt = topol[k][0];
      for (i = 0; i < nseq; i++)
        if (pair[im][i] > -2) *intpt++ = i;
      *intpt = -1;

      intpt = topol[k][1];
      for (i = 0; i < nseq; i++)
        if (pair[jm][i] > -2) *intpt++ = i;
      *intpt = -1;
#endif
          ...
          if (i != im && i != jm)
            {
              ...
              eff0 = eff[miniim][maxiim];
              eff1 = eff[minijm][maxijm];
              eff[miniim][maxiim] = (int)((float)MIN(eff0, eff1) * (1.0 - SUEFF) +
                                          (float)(eff0 + eff1) * 0.5 * SUEFF);
            }
        }
      acjmprev = ac[jm].prev;
      acjmnext = ac[jm].next;
...
void countnode(int nseq, int ***topol,
               double **node) /* node[j][i] != node[i][j] */
{
  int i, j, k, s1, s2;
...
void countnode_int(int nseq, int ***topol,
                   int **node) /* node[i][j] == node[j][i] */
{
  int i, j, k, s1, s2;
  int rootnode[M];
...
void treeconstruction(char **seq, int nseq, int ***topol, double **len,
                      double **eff)
{
  int i, j;
          ...
          for (i = 0; i < nseq - 1; i++)
            {
              for (j = i + 1; j < nseq; j++)
                {
                  /*
                                   eff[i][j] = (double)score_calc1( seq[i],
             seq[j] );
          */
...
void gappick(int nseq, int s, char **aseq, char **mseq2, double **eff,
             double *effarr)
{
            ...
            cob =
                 ...
                 *  gb2  * !gc2      *BEFF
                 ;
                        ...
                        score += (double)amino_dis[seq[i][k]][seq[ex][k]] *
efficient;
                        *
                        nglen += ( !gc1 * !gc2 );
                        *
          ...
          for (k = 0; k < len; k++)
            {
              ...
              score +=
                  (float)amino_dis[(int)seq[i][k]][(int)seq[j][k]] * (float)efficient;
              nglen += (!gc1 * !gc2);
            }
        }
    }
      ...
      for (k = 0; k < nseq - 1; k++)
        {
          for (j = k + 1; j < nseq; j++)
            {
              stra[i] +=
                  n_dis[(int)amino_n[(int)seq[k][i]]][(int)amino_n[(int)seq[j][i]]];
            }
        }
...
#else
                  importance[pos] +=
                      eff[j] * tmpptr->opt / MIN(nogaplen[i], nogaplen[j]);
              ...
              for (tmpptr = localhom[i] + j; tmpptr; tmpptr = tmpptr->next)
                {
                  if (tmpptr->opt == -1.0) continue;
                  tmpptr->importance = tmpdouble * tmpptr->opt;
                  //				tmpptr->importance = tmpptr->opt /
                  //tmpptr->overlapaa; //なかったことにする
                }
#endif

          //			fprintf( stderr, "importance of match between %d
          //- %d = %f\n", i, j, tmpdouble );
        }
    }
      ...
      for (j = i + 1; j < nseq; j++)
        {
          ...
          for (; tmpptr1 && tmpptr2;
               tmpptr1 = tmpptr1->next, tmpptr2 = tmpptr2->next)
            {
              if (tmpptr1->opt == -1.0 || tmpptr2->opt == -1.0)
                {
                  //				fprintf( stderr, "WARNING: i=%d, j=%d,
                  //tmpptr1->opt=%f, tmpptr2->opt=%f\n", i, j, tmpptr1->opt,
                  //tmpptr2->opt );
                  continue;
                }
...
static void addlocalhom2_e(LocalHom *pt, LocalHom *lh, int sti, int stj,
                           int eni, int enj, double opt, int overlp, int interm)
{
  // dokka machigatteru
  if (pt != lh)  // susumeru
    {
  ...
  lh->nokori++;
  //	fprintf( stderr, "in addlocalhom2_e, pt = %p, pt->next = %p, interm=%d,
  //sti-eni-stj-enj=%d %d %d %d\n", pt, pt->next, interm, sti, eni, stj, enj );
      ...
      for (j = i + 1; j < nseq; j++)
        {
          ...
          //dists = %f,%f,%f thrinter=%f\n", i, j, k, dist[i][j],
          //dist[MIN(i,k)][MAX(i,k)], dist[MIN(j,k)][MAX(j,k)], thrinter );
          if (k == i || k == j)
            {
              continue;  // mou yatta nomo habuita hoga ii
            }
          if (dist[MIN(i, k)][MAX(i, k)] > dist[i][j] * thrinter ||
              dist[MIN(j, k)][MAX(j, k)] > dist[i][j] * thrinter)
            {
              continue;
            }
          ipt = ini;
...

          {
            for (pt = localhom[i] + k; pt; pt = pt->next)
              {
                //						fprintf( stderr,
                //"i=%d,k=%d,st1:st2=%d:%d,pt=%p,extended=%p\n", i, k, pt->start1,
                //pt->start2, pt, pt->extended );
                if (pt->opt == -1)
                  {
          ...
          for (pk = 0; pk < plim; pk++)
            {
              ...
              if (status)
                {
                  if (ini[pk] == -1 || inj[pk] == -1 || ini[pk - 1] != ini[pk] - 1 ||
                      inj[pk - 1] != inj[pk] - 1)  // saigonoshori
                    {
                      status = 0;
                      ...
                      //							fprintf( stderr, "in ex (ba), pt = %p, nokori=%d,
                      //i,j,k=%d,%d,%d\n", pt, localhom[i][j].nokori, i, j, k );
                      addlocalhom2_e(
                          pt, localhom[i] + j, sti, stj, ini[pk - 1], inj[pk - 1],
                          MIN(localhom[i][k].opt, localhom[j][k].opt) * 1.0, overlp, k);
                      //							fprintf( stderr, "in ex, pt = %p, pt->next = %p,
                      //pt->next->next = %p\n", pt, pt->next, pt->next->next );

                      pt = localhom[j][i].last;
                      //							fprintf( stderr, "in ex (ba), pt = %p, pt->next =
                      //%p\n", pt, pt->next );
                      //							fprintf( stderr, "in ex (ba), pt = %p, pt->next =
                      //%p, k=%d\n", pt, pt->next, k );
                      addlocalhom2_e(
                          pt, localhom[j] + i, stj, sti, inj[pk - 1], ini[pk - 1],
                          MIN(localhom[i][k].opt, localhom[j][k].opt) * 1.0, overlp, k);
                      //							fprintf( stderr, "in ex, pt = %p, pt->next = %p,
                      //pt->next->next = %p\n", pt, pt->next, pt->next->next );
                    }
          ...
          //				if( status ) fprintf( stderr, "end
          //here\n" );

          //				exit( 1 );
          //					fprintf( hat3p, "%d %d %d %6.3f
          //%d %d %d %d %p\n", i, j, tmpptr->overlapaa, tmpptr->opt, tmpptr->start1,
          //tmpptr->end1, tmpptr->start2, tmpptr->end2, tmpptr->next );
        }
#if 0
      ...
      if (score > maxscore)
        {
          ...
          maxscore = score;
          maxstart = start;
          //			fprintf( stderr, "## max! maxstart = %d, start =
          //%d\n", maxstart, start );
        }
      if (score < 0.0)
        {
          //			fprintf( stderr, "## resetting, start = %d,
          //maxstart = %d\n", start, maxstart );
          if (start == maxstart)
            {
              maxend = pt1 - s1;
...
void new_OpeningGapCount(float *ogcp, int clus, char **seq, double *eff,
                         int len, char *sgappat)
#if 0
{
	int i, j, gc, gb; 
	float feff;
...
#endif
    void new_OpeningGapCount_zure(float *ogcp, int clus, char **seq,
                                  double *eff, int len, char *sgappat,
                                  char *egappat)
#if 0
{
	int i, j, gc, gb; 
    ...
    for (j = 0; j < clus; j++)
      {
        ...
        void new_FinalGapCount_zure(float *fgcp, int clus, char **seq,
                                    double *eff, int len, char *sgappat,
                                    char *egappat)
#if 0
{
...
#endif
            void new_FinalGapCount(float *fgcp, int clus, char **seq,
                                   double *eff, int len, char *egappat)
#if 0
{
	int i, j, gc, gb; 
	float feff;
            ...
            if (gb * !gc)
              {
                *fpt += feff;
                ...
                void st_OpeningGapCount(float *ogcp, int clus, char **seq,
                                        double *eff, int len)
{
  int i, j, gc, gb;
  float feff;
...
void st_FinalGapCount_zure(float *fgcp, int clus, char **seq, double *eff,
                           int len)
{
  int i, j, gc, gb;
  float feff;
...
void getGapPattern(float *fgcp, int clus, char **seq, double *eff, int len,
                   char *xxx)
{
  int i, j, gc, gb;
  float feff;
  ...
  for (i = 0; i < clus; i++)
    {
      ...
      if (1 && seq[i][len - 1] != '-')
        {  // xxx wo tsukawanaitoki [len-1] nomi
          freq[len] += feff;
        }
    }
  //	fprintf( stderr, "\ndiaaf = \n" );
  ...
  for (i = 0; i < clus; i++)
    {
      ...
      //		if( 1 && seq[i][len-1] != '-' ) // xxx wo tsukawanaitoki [len-1]
      //nomi
      freq[len] += feff;
    }
  //	fprintf( stderr, "\ndiaaf = \n" );
...
void getdigapfreq_part(float *freq, int clus, char **seq, double *eff, int len,
                       char *sgappat, char *egappat)
{
  int i, j;
  float feff;
  for (i = 0; i < len + 2; i++)
  ...
  for (i = 0; i < clus; i++)
    {
      ...
      //		if( seq[i][len] == '-' && seq[i][len-1] == '-' ) // xxx wo
      //tsukawanaitoki arienai
...
void getdiaminofreq_part(float *freq, int clus, char **seq, double *eff,
                         int len, char *sgappat, char *egappat)
{
  int i, j;
  float feff;
  for (i = 0; i < len + 2; i++)
...
void getgapfreq_zure_part(float *freq, int clus, char **seq, double *eff,
                          int len, char *sgap)
{
  int i, j;
  float feff;
              ...
              if (gb && gaplen)
                {
                  ...
                  if (known == 0)
                    {
                      *fpt = (Gappat *)realloc(
                          *fpt,
                          (k + 3) * sizeof(Gappat));  // mae1 (total), ato2 (len0), term
                      if (!*fpt)
                        {
                          fprintf(stderr, "Cannot allocate gappattern!'n");
                          fprintf(stderr,
                                  "Use an approximate method, with the --mafft5 option.\n");
                          exit(1);
                        }
                      (*fpt)[k].freq = 0.0;
                      (*fpt)[k].len = gaplen;
                  ...
                  //					fprintf( stderr, "adding pos %d, len=%d, k=%d,
                  //freq=%f->", i, gaplen, k, (*fpt)[k].freq );
                  (*fpt)[k].freq += feff;
                  //					fprintf( stderr, "%f\n", (*fpt)[k].freq
                  //);
                  gaplen = 0;
      ...
      if (pat[j])
        {
          ...
          //				fprintf( stderr, "pos=%d, i=%d, len=%d,
          ...
          for (i = 1; pat[j][i].len != -1; i++)
            {
              ...
              //				fprintf( stderr, "totaling, i=%d, result =
              //%f\n", i, pat[j][0].freq );
            }
...
float naiveRpairscore(int n1, int n2, char **seq1, char **seq2, double *eff1,
                      double *eff2, int penal)
{
  //	return( 0 );
  int i, j;
  float val;
      ...
      for (j = 0; j < n2; j++)
        {
          ...
          //		if( pv ) fprintf( stderr, "Penal!, %f, %d-%d,
...
float naiveQpairscore(int n1, int n2, char **seq1, char **seq2, double *eff1,
                      double *eff2, int penal)
{
  int i, j;
  float val;
...
float naiveHpairscore(int n1, int n2, char **seq1, char **seq2, double *eff1,
                      double *eff2, int penal)
{
  int i, j;
  float val;
      ...
      for (j = 0; j < n2; j++)
        {
          ...
          if (pv)
            {
              fprintf(stderr, "Penal!, %f, %d-%d, pos1,pos2=%d,%d\n", pv * deff * 0.5,
                      i, j, (int)(p1 - seq1[i]), (int)(p2 - seq2[j]));
            }
          p1p = p1;
          p2p = p2;
          valf += (float)amino_dis[(int)*p1++][(int)*p2++] + 0.5 * pv;
    ...
    while (*p1)
      {
        if (*p1 == '-')
          {
            //				fprintf( stderr, "Penal! %c-%c in %d-%d,
            //%f\n", *(p1-1), *(p2-1), i, j, feff );
            vali += penal;
            //				while( *p1 == '-' || *p2 == '-' )
            while (*p1 == '-')  // SP
...
float naivepairscore(int n1, int n2, char **seq1, char **seq2, double *eff1,
                     double *eff2, int penal)
{
  //	return( 0.0 );
  int i, j;
