...
static void match_calc(float *match, char **s1, char **s2, int i1, int lgth2)
{
  ...
  for (j = 0; j < lgth2; j++) match[j] = amino_dis[(*s1)[i1]][(*s2)[j]];
}
#endif
...
static float gentracking(float *lasthorizontalw, float *lastverticalw,
                         ...
                         float **cpmx1, float **cpmx2, int **ijpi, int **ijpj,
                         int *off1pt, int *off2pt, int endi, int endj)
{
...
float genL__align11(char **seq1, char **seq2, int alloclen, int *off1pt,
                    int *off2pt)
...
//seq1[0][i], seq2[0][j], *ijppt, localstop );
  ...
  if (ijpi[endali][endalj] ==
      localstop)  // && ijpj[endali][endalj] == localstop )
  {
    strcpy(seq1[0], "");
    strcpy(seq2[0], "");
    *off1pt = *off2pt = 0;
  ...
  gentracking(currentw, lastverticalw, seq1, seq2, mseq1, mseq2, cpmx1, cpmx2,
              ijpi, ijpj, off1pt, off2pt, endali, endalj);
