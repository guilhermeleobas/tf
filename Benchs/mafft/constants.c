...
void constants(int nseq, char **seq)
{
  ...
  if (dorp == 'd') /* DNA */
  {
    ...
    sprintf(modelname, "%s%d (%d), %6.3f (%6.3f), %6.3f (%6.3f)",
            rnakozo ? "RNA" : "DNA", pamN, kimuraR, -(double)ppenalty * 0.001,
            -(double)ppenalty * 0.003, -(double)poffset * 0.001,
            -(double)poffset * 0.003);
    ...
    else
    {
      ...
      fprintf(stderr, "generating %dPAM scoring matrix for nucleotides ... ",
              pamN);
...
// RIBOSUM
    ...
    for (i = 0; i < 4; i++)
    {
      for (j = 0; j < 4; j++)
      {
        for (k = 0; k < 4; k++)
        {
          for (m = 0; m < 4; m++)
          {
            //			if( i%4==0&&j%4==3 || i%4==3&&j%4==0 || i%4==1&&j%4==2 ||
            //i%4==2&&j%4==1 || i%4==1&&j%4==3 || i%4==3&&j%4==1 )
            //			if( k%4==0&&m%4==3 || k%4==3&&m%4==0 || k%4==1&&m%4==2 ||
            //k%4==2&&m%4==1 || k%4==1&&m%4==3 || k%4==3&&m%4==1 )
            average += ribosum16[i * 4 + j][k * 4 + m] * freq[i] * freq[j] *
                       freq[k] * freq[m];
          }
        }
      }
    ...
    //			for( k=0; k<9; k++ ) for( j=0; j<4; j++ ) ribosumdis[m*4+i][k*4+j] =
    //n_dis[i][j]; // loop-loop
    ...
    for (i = 0; i < 37; i++)
      for (j = 0; j < 37; j++) ribosumdis[i][j] = 0.0;  // iru
    for (m = 0; m < 9; m++)
      for (i = 0; i < 4; i++)  // loop
        for (k = 0; k < 9; k++)
          for (j = 0; j < 4; j++)
  ...
  else if (dorp == 'p' && scoremtx == 1) /* Blosum */
  {
    ...
    if (nblosum == -1)
    {
      sprintf(modelname, "User-defined, %6.3f, %+6.3f, %+6.3f",
              -(double)ppenalty / 1000, -(double)poffset / 1000,
              -(double)ppenalty_ex / 1000);
    }
    else
    {
      sprintf(modelname, "BLOSUM%d, %6.3f, %+6.3f, %+6.3f", nblosum,
              -(double)ppenalty / 1000, -(double)poffset / 1000,
              -(double)ppenalty_ex / 1000);
    }
#if 0
...
/* 注意 ！！！！！！！！！！ */
  ...
  else if (dorp == 'p' && scoremtx == 2) /* Miyata-Yasunaga */
  {
    ...
    for (i = 0; i < 26; i++)
    {
      if (i != 24)
      {
        n_dis[i][24] = n_dis[24][i] = exgpm;
      }
    }
    n_dis[24][24] = 0;
    if (ppenalty == NOTSPECIFIED)
    {
      ppenalty = locpenaltym;
    }
    if (poffset == NOTSPECIFIED)
    {
      poffset = -20;
    ...
    sprintf(modelname, "Miyata-Yasunaga, %6.3f, %6.3f",
            -(double)ppenalty / 1000, -(double)poffset / 1000);
    for (i = 0; i < 26; i++)
    {
      amino[i] = locaminom[i];
    ...
    for (i = 0; i < 26; i++)
    {
      amino_grp[(int)amino[i]] = locgrpm[i];
    }
#if DEBUG
  ...
  else /* JTT */
  {
    ...
    sprintf(modelname, "%s %dPAM, %6.3f, %6.3f",
            (TMorJTT == TM) ? "Transmembrane" : "JTT", pamN,
            -(double)ppenalty / 1000, -(double)poffset / 1000);
    ...
    fprintf(stderr, "generating %dPAM %s scoring matrix for amino acids ... ",
            pamN, (TMorJTT == TM) ? "Transmembrane" : "JTT");
...
/* 注意 ！！！！！！！！！！ */
    ...
    average = 0.0;
    for (i = 0; i < 20; i++)
      for (j = 0; j < 20; j++) average += pamx[i][j];
    average /= 400.0;
  ...
  for (i = 0; i < 26; i++)
  {
    for (j = 0; j < 26; j++)
    {
      ...
      amino_dis_consweight_multi[(int)amino[i]][(int)amino[j]] =
          (double)n_dis[i][j] * consweight_multi;
    }
  }
  ...
  if (dorp == 'd') /* DNA */
  {
    for (i = 0; i < 5; i++)
    {
      for (j = 0; j < 5; j++)
      {
        amino_disLN[(int)amino[i]][(int)amino[j]] =
            n_dis[i][j] + offset - offsetLN;
      }
    }
    for (i = 5; i < 10; i++)
    {
