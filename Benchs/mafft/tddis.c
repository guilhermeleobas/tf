...
void cpmx_ribosum(char **seq, char **seqr, char *dir, float **cpmx, double *eff,
                  int lgth, int clus)  // summ eff must be 1.0
{
  int i, j, k;
  float feff;
  float **cpmxptpt, *cpmxpt;
...
void mseqcat(char **seq1, char **seq2, double **eff, double *effarr1,
             double *effarr2, char name1[M][B], char name2[M][B], int clus1,
             int clus2)
{
  int i, j;
  for (i = 0; i < clus2; i++)
...
#else
int conjuctionforgaln(int s0, int s1, char **seq, char **aseq, double *peff,
                      double *eff, char **name, char **aname, char *d)
#endif
{
  int m, k;
  char b[B];
...
int fastconjuction_noname(int *memlist, char **seq, char **aseq, double *peff,
                          double *eff, char *d)
{
  int m, k, dln;
  char b[B];
...
int fastconjuction(int *memlist, char **seq, char **aseq, double *peff,
                   double *eff, char name[M][B], char aname[M][B], char *d)
{
  int m, k, dln;
  char b[B];
  double total;
...
int conjuctionfortbfast(char **pair, int s, char **seq, char **aseq,
                        double *peff, double *eff, char *d)
{
  int m, k;
  char b[B];
  double total;
...
int conjuction(char **pair, int s, char **seq, char **aseq, double *peff,
               double *eff, char name[M][B], char aname[M][B], char *d)
{
  int m, k;
  char b[B];
...
#else
void OneClusterAndTheOther(int locnjob, char **pair, int *s1, int *s2,
                           int ***topol, int step, int branch)
#endif
{
  int i;
  int r1;
...
int shrinklocalhom(char **pair, int s1, int s2, LocalHom **localhom,
...
int fastshrinklocalhom(int *mem1, int *mem2, LocalHom **localhom,
                       LocalHom ***localhomshrink)
{
  int k1, k2;
  int *intpt1, *intpt2;
...
int msfastshrinklocalhom(int *mem1, int *mem2, LocalHom **localhom,
                         LocalHom ***localhomshrink)
{
  int k1, k2;
  int *intpt1, *intpt2;
