...
static float singleribosumscore(int n1, int n2, char **s1, char **s2,
                                double *eff1, double *eff2, int p1, int p2)
{
  float val;
  int i, j;
  int code1, code2;
...
static float pairedribosumscore35(int n1, int n2, char **s1, char **s2,
                                  double *eff1, double *eff2, int p1, int p2,
                                  int c1, int c2)
{
  float val;
...
static void mccaskillextract(char **seq, char **nogap, int nseq,
                             RNApair **pairprob, RNApair ***single,
                             int **sgapmap, double *eff)
{
  int lgth;
  int nogaplgth;
  int i, j;
...
//	{
//		fprintf( stderr, "pair of %d = %d (%f) %c:%c\n", i,
//pairprob[i][0].bestpos, pairprob[i][0].bestscore, seq[0][i],
//seq[0][pairprob[i][0].bestpos] );
//	}
...
void foldrna(int nseq1, int nseq2, char **seq1, char **seq2, double *eff1,
             ...
             float **impmtx, int *gapmap1, int *gapmap2,
             RNApair *additionalpair)
{
  int i, j;
  //	int ui, uj;
  //	int uiup, ujup;
  ...
  /* similarity score */
  Lalignmm_hmout(oseq1, oseq2, eff1, eff2, nseq1, nseq2, 10000, NULL, NULL,
                 NULL, NULL, map);
