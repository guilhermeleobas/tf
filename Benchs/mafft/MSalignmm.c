...
static float Atracking(char **seq1, char **seq2, char **mseq1, char **mseq2,
                       int **ijp, int icyc, int jcyc, int ist, int ien, int jst,
                       int jen)
{
  ...
  *gaptable2 = 0;
  ...
  //	if( lgth2 == 1 ) fprintf( stderr, "in Atracking, mseq1 = %s, mseq2 =
  //%s\n", mseq1[0], mseq2[0] );
...
static float MSalignmm_tanni(int icyc, int jcyc, double *eff1, double *eff2,
...
//	short **shortmtx;
  ...
  fprintf(
      stderr,
      "==== MSalign (depth=%d, reccycle=%d), ist=%d, ien=%d, jst=%d, jen=%d\n",
  ...
  if (lgth1 < DPTANNI || lgth2 < DPTANNI)  // zettai ni anzen ka?
    {
      ...
      value =
          MSalignmm_tanni(icyc, jcyc, eff1, eff2, seq1, seq2, cpmx1, cpmx2, ist,
                          ien, jst, jen, alloclen, aseq1, aseq2, gapinfo);
...
float MSalignmm(char **seq1, char **seq2, double *eff1, double *eff2, int icyc,
                int jcyc, int alloclen, char *sgap1, char *sgap2, char *egap1,
                char *egap2)
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
  //	int k;
  ...
  if (seqlen(seq1[0]) != nglen1)
    {
      fprintf(stderr,
              "bug! hairetsu ga kowareta! (nglen1) seqlen(seq1[0])=%d but "
              "nglen1=%d\n",
  ...
  if (seqlen(seq2[0]) != nglen2)
    {
      fprintf(stderr,
              "bug! hairetsu ga kowareta! (nglen2) seqlen(seq2[0])=%d but "
              "nglen2=%d\n",
