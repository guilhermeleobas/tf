...
float L__align11(char **seq1, char **seq2, int alloclen, int *off1pt,
                 int *off2pt)
...
fprintf( stderr, "\n" );
  ...
  fprintf(stderr, "\n");
  fprintf(stderr, "       ");
  for (j = 0; j < lgth2; j++) fprintf(stderr, "%c     ", seq2[0][j]);
  fprintf(stderr, "\n");
#endif
...
//seq1[0][i], seq2[0][j], *ijppt, localstop );
  ...
  Ltracking(currentw, lastverticalw, seq1, seq2, mseq1, mseq2, ijp, off1pt,
            off2pt, endali, endalj);
