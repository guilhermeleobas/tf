...
static void imp_match_out_vead_gapmap(float *imp, int i1, int lgth2,
                                      int *gapmap2)
{
#if FASTMATCHCALC
  float *pt = impmtx[i1];
...
static void imp_match_out_vead_tate_gapmap(float *imp, int j1, int lgth1,
                                           int *gapmap1)
{
#if FASTMATCHCALC
...
void imp_rna(int nseq1, int nseq2, char **seq1, char **seq2, double *eff1,
             double *eff2, RNApair ***grouprna1, RNApair ***grouprna2,
             int *gapmap1, int *gapmap2, RNApair *pair)
{
  foldrna(nseq1, nseq2, seq1, seq2, eff1, eff2, grouprna1, grouprna2, impmtx,
          gapmap1, gapmap2, pair);
}

void imp_match_init_strict(float *imp, int clus1, int clus2, int lgth1,
...
float A__align(char **seq1, char **seq2, double *eff1, double *eff2, int icyc,
...
float A__align_gapmap(char **seq1, char **seq2, double *eff1, double *eff2,
                      int icyc, int jcyc, int alloclen, LocalHom ***localhom,
                      float *impmatch, int *gapmap1, int *gapmap2)
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
  //	int k;
  register int i, j;
...
//	float fpenalty = (float)penalty;
  ...
  if (localhom)
    {
      imp_match_out_vead_tate_gapmap(initverticalw, gapmap2[0], lgth1,
                                     gapmap1);  // 060306
    }
...
float translate_and_Calign(char **mseq1, char **mseq2, double *effarr1,
                           double *effarr2, int clus1, int clus2, int alloclen)
{
  int i;
  float wm;
  char **result;
  ...
  if (clus1 == 1 && clus2 != 1)
    ...
    {
      ErrorExit("ERROR in translate_and_Calign");
    }
