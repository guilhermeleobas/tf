...
static void part_imp_match_out_vead_gapmap(float *imp, int i1, int lgth2,
...
static void part_imp_match_out_vead_tate_gapmap(float *imp, int j1, int lgth1,
...
void part_imp_match_init_strict(float *imp, int clus1, int clus2, int lgth1,
...
void part_imp_rna(int nseq1, int nseq2, char **seq1, char **seq2, double *eff1,
...
void part_imp_match_init(float *imp, int clus1, int clus2, int lgth1, int lgth2,
                         char **seq1, char **seq2, double *eff1, double *eff2,
                         LocalHom ***localhom)
{
                               ...
                               int *gapmap1, int *gapmap2)
{
  ...
  *impwmpt = 0.0;
  for (k = 0; k <= lgth1 + lgth2; k++)
  {
    ...
    {
      *impwmpt +=
          part_imp_match_out_sc(gapmap1[iin] + start1, gapmap2[jin] + start2);
...
float partA__align(char **seq1, char **seq2, double *eff1, double *eff2,
...
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
  ...
  if (localhom)
  {
    part_imp_match_out_vead_tate_gapmap(initverticalw, gapmap2[0] + start2,
  ...
  if (localhom)
  {
    part_imp_match_out_vead_gapmap(currentw, gapmap1[0] + start1, lgth2, start2,
                                   gapmap2);
  }
#if 0  // -> tbfast.c
	if( localhom )
  ...
  for (i = 1; i < lasti; i++)
  {
    ...
    if (localhom)
    {
      ...
      part_imp_match_out_vead_gapmap(currentw, gapmap1[i] + start1, lgth2,
                                     start2, gapmap2);
    }
#if XXXXXXX
    fprintf(stderr, "\n");
