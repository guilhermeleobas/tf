...
static void imp_match_out_veadQ_gapmap(float *imp, int i1, int lgth2,
                                       int *gapmap2)
{
#if FASTMATCHCALC
  float *pt = impmtx[i1];
  ...
  int j;
  float *pt = impmtx[i1];
  for (j = 0; j < lgth2; j++) *imp++ += pt[gapmap2[j]];
#endif
}

static void imp_match_out_vead_tateQ_gapmap(float *imp, int j1, int lgth1,
                                            int *gapmap1)
{
#if FASTMATCHCALC
...
#else
  int i;
  for (i = 0; i < lgth1; i++) *imp++ += impmtx[gapmap1[i]][j1];
#endif
}
...
void imp_rnaQ(int nseq1, int nseq2, char **seq1, char **seq2, double *eff1,
              double *eff2, RNApair ***grouprna1, RNApair ***grouprna2,
              int *gapmap1, int *gapmap2, RNApair *additionalpair)
{
  foldrna(nseq1, nseq2, seq1, seq2, eff1, eff2, grouprna1, grouprna2, impmtx,
          gapmap1, gapmap2, additionalpair);
}

#if 1  // tbfast.c kara yobareru.
void imp_match_init_strictQ(float *imp, int clus1, int clus2, int lgth1,
...
static void Atracking_localhom_gapmap(float *impwmpt, float *lasthorizontalw,
                                      float *lastverticalw, char **seq1,
                                      char **seq2, char **mseq1, char **mseq2,
                                      float **cpmx1, float **cpmx2, int **ijp,
                                      int icyc, int jcyc, int *gapmap1,
                                      int *gapmap2)
{
      ...
      if (iin != lgth1 && jin != lgth2)  // ??
        {
          *impwmpt += imp_match_out_scQ(gapmap1[iin], gapmap2[jin]);
          //			fprintf( stderr, "impwm = %f (iin=%d, jin=%d)
...
float Q__align(char **seq1, char **seq2, double *eff1, double *eff2, int icyc,
  ...
  for (i = 0; i < lastj; i++)
    {
      ...
      og_t_fg_h_dg_n2_p[i] =
          (1.0 - ogcp2g[i] + fgcp2g[i] - digf2[i]) * 0.5 * fpenalty;
      fg_t_og_h_dg_n2_p[i] =
          (1.0 - fgcp2g[i] + ogcp2g[i] - digf2[i]) * 0.5 * fpenalty;
  ...
  for (i = 0; i < lastj; i++)
    {
      ...
      og_t_fg_h_dg_n1_p[i] =
          (1.0 - ogcp1g[i] + fgcp1g[i] - digf1[i]) * 0.5 * fpenalty;
      fg_t_og_h_dg_n1_p[i] =
          (1.0 - fgcp1g[i] + ogcp1g[i] - digf1[i]) * 0.5 * fpenalty;
      ...
      for (j = 1; j < lastj; j++)
        {
          ...
          //			g = ogcp1g[i] * ( 1.0-ogcp2g[j]-digf2[j] ) *
          //fpenalty * 0.5;
          ...
          //			g = ogcp2g[j] * ( 1.0-ogcp1g[i]-digf1[i] ) *
          //fpenalty * 0.5;
          //			if( g && i==j ) fprintf( stderr, "match
          //penal2=%f, %c-%c\n", g, seq1[0][i], seq2[0][j] );
          wm += g;
          ...
          //			g = fgcp1g[i] * ( 1.0-fgcp2g[j]-digf2[j] ) *
          //fpenalty * 0.5;
          //			if( g && i==j ) fprintf( stderr, "match
          //penal3=%f, %c-%c\n", g, seq1[0][i], seq2[0][j] );
          wm += g;
          ...
          //			g = fgcp2g[j] * ( 1.0-fgcp1g[i]-digf1[i] ) *
          //fpenalty * 0.5;
          //			if( g && i==j ) fprintf( stderr, "match
          //penal4=%f, %c-%c\n", g, seq1[0][i], seq2[0][j] );
          wm += g;
...
float Q__align_gapmap(char **seq1, char **seq2, double *eff1, double *eff2,
                      int icyc, int jcyc, int alloclen, LocalHom ***localhom,
                      float *impmatch, char *sgap1, char *sgap2, char *egap1,
                      char *egap2, int *gapmap1, int *gapmap2)
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
  //	int k;
  register int i, j;
...
//	float fpenalty = (float)penalty;
  ...
  if (localhom)
    {
      imp_match_out_vead_tateQ_gapmap(initverticalw, gapmap2[0], lgth1,
                                      gapmap1);  // 060306
    }
...
//%d\n", lgth1, i );
  ...
  */
  if (localhom)
    {
      Atracking_localhom_gapmap(impmatch, currentw, lastverticalw, seq1, seq2,
                                mseq1, mseq2, cpmx1, cpmx2, ijp, icyc, jcyc,
                                gapmap1, gapmap2);
    }
  else
    {
