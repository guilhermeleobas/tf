...
static int addlocalhom_r(char *al1, char *al2, LocalHom *localhompt, int off1,
                         int off2, int opt, int overlapaa, int skip)
{
  int pos1, pos2, start1, start2, end1, end2;
  char *pt1, *pt2;
  double score;
...
void putlocalhom3(char *al1, char *al2, LocalHom *localhompt, int off1,
                  int off2, int opt, int overlapaa)
{
  int pos1, pos2, start1, start2, end1, end2;
  char *pt1, *pt2;
  double score;
  ...
  while (*pt1 != 0)
    {
      ...
      if (st == 1 && (*pt1 == '-' || *pt2 == '-'))
      ...
      else if (*pt1 != '-' && *pt2 != '-')
        {
          ...
          score +=
              (double)n_dis[(int)amino_n[(int)*pt1]]
                           [(int)amino_n[(int)*pt2]];  // - offset はいらないかも
          //			fprintf( stderr, "%c-%c, score(0) = %f\n", *pt1,
          //*pt2, score );
        }
      if (*pt1++ != '-')
...
void putlocalhom_ext(char *al1, char *al2, LocalHom *localhompt, int off1,
                     int off2, int opt, int overlapaa)
{
  int pos1, pos2, start1, start2, end1, end2;
  char *pt1, *pt2;
  ...
  while (*pt1 != 0)
    {
      ...
      if (st == 1 && (*pt1 == '-' || *pt2 == '-'))
      ...
      else if (*pt1 != '-' && *pt2 != '-')
        {
          ...
          iscore += n_dis[(int)amino_n[(int)*pt1]]
                         [(int)amino_n[(int)*pt2]];  // - offset はいらないかも
  ...
  if (!divpairscore)
    {
      for (tmppt = localhompt; tmppt; tmppt = tmppt->next)
        {
          tmppt->overlapaa = sumoverlap;
          //			tmppt->opt = (double)isumscore * 5.8 / ( 600 *
          //sumoverlap );
...
void putlocalhom2(char *al1, char *al2, LocalHom *localhompt, int off1,
                  int off2, int opt, int overlapaa)
{
  int pos1, pos2, start1, start2, end1, end2;
  char *pt1, *pt2;
...
void putlocalhom(char *al1, char *al2, LocalHom *localhompt, int off1, int off2,
                 int opt, int overlapaa)
{
  int pos1, pos2, start1, start2, end1, end2;
  char *pt1, *pt2;
  double score;
...
void FRead(FILE *fp, char name[][B], int nlen[], char **seq)
{
  ...
  else if (strstr(b, "M-Y") || strstr(b, "iyata"))
    {
      scoremtx = 2;
    }
  else
...
#endif
      fgets(b, B - 1, fp);
      nlen[i] = atoi(b); /* seq i no nagasa */
      seq[i][0] = 0;
      if (nlen[i])
        {
          for (j = 0; j <= (nlen[i] - 1) / C; j++)
            {
              getaline_fp_eof_new(b, B - 1, fp);
              /*	b[C] = 0;  */
              strcat(seq[i], b);
...
void WriteGapFill(FILE *fp, int locnjob, char name[][B], int nlen[M],
                  char **aseq)
{
  static char b[N];
  int i, j;
...
void writeDataforgaln(FILE *fp, int locnjob, char **name, int *nlen,
                      char **aseq)
{
  int i, j;
  int nalen;
...
void writeData_pointer(FILE *fp, int locnjob, char **name, int *nlen,
                       char **aseq)
{
  int i, j;
  int nalen;
  ...
  while (1)
    {
      ...
      *pt = 0;
      ...
      //		fprintf( stderr, "t=%d, score = %f, qstart=%d, qend=%d, tstart=%d,
      //tend=%d, overlapaa=%d\n", junban[count], score, qstart, qend, tstart,
      //tend, overlapaa );
  ...
  while (1)
    {
      ...
      *pt = 0;
      ...
      //		nlocalhom += addlocalhom_r( qal, tal, localhomlist+junban[count],
      //qstart, tstart, score, overlapaa, nlocalhom );
...
int ReadBlastm7(FILE *fp, double *dis, int qmem, char name[M][B],
                LocalHom *localhomlist)
{
  int count = 0;
  char b[B];
  char *pt;
  ...
  while (1)
    {
      ...
      *pt = 0;
      ...
      nlocalhom += addlocalhom_r(qal, tal, localhomlist + junban[count], qstart,
                                 tstart, score, overlapaa, nlocalhom);
...
int ReadFasta34noalign(FILE *fp, double *dis, int qmem, char name[M][B],
                       LocalHom *localhomlist)
{
  int count = 0;
  char b[B];
...
int ReadFasta34m10_nuc(FILE *fp, double *dis, int qmem, char name[M][B],
                       LocalHom *localhomlist)
{
  int count = 0;
  char b[B];
  ...
  while (1)
    {
      ...
      while (fgets(b, B - 1, fp))
        {
          if (!strncmp("; fa_opt:", b, 9) || !strncmp("; sw_s-w opt:", b, 13))
            {
              break;
            }
        }
      ...
      while (fgets(b, B - 1, fp))
        {
          if (!strncmp("_display_start:", b + 4, 15))
            {
              break;
            }
        }
      ...
      *pt = 0;
      ...
      //junban[count-1], opt );
      putlocalhom(qal2, tal2, localhomlist + junban[count - 1], qstart, tstart,
                  opt, overlapaa);
    }
  //	fprintf( stderr, "count = %d\n", count );
  return count;
}
int ReadFasta34m10(FILE *fp, double *dis, int qmem, char name[M][B],
                   LocalHom *localhomlist)
{
  int count = 0;
  char b[B];
...
int ReadFasta34(FILE *fp, double *dis, int nseq, char name[M][B],
                LocalHom *localhomlist)
{
  int count = 0;
  char b[B];
...
void WriteForFasta(FILE *fp, int locnjob, char name[][B], int nlen[M],
                   char **aseq)
{
  static char b[N];
  int i, j;
...
void readlocalhomtable2(FILE *fp, int njob, LocalHom **localhomtable)
{
  ...
  while (NULL != fgets(buff, B - 1, fp))
    {
      //		fprintf( stderr, "\n" );
      sscanf(buff, "%d %d %d %lf %d %d %d %d", &i, &j, &overlapaa, &opt, &start1,
             &end1, &start2, &end2);
      ...
      //		if( i < j )
      {
        ...
        tmpptr1->end1 = end1;
        tmpptr1->end2 = end2;
        //			tmpptr1->opt = ( opt / overlapaa + 0.00 ) / 5.8
        //* 600;
        //			tmpptr1->opt = opt;
      ...
      //		else
      {
        ...
        tmpptr2->end2 = end1;
        tmpptr2->end1 = end2;
        //			tmpptr2->opt = ( opt / overlapaa + 0.00 ) / 5.8
        //* 600;
        //			tmpptr2->opt = opt;
        tmpptr2->opt = (opt + 0.00) / 5.8 * 600;
        tmpptr2->overlapaa = overlapaa;

        //			fprintf( stderr, "i=%d, j=%d, st1=%d, en1=%d,
          ...
          do
            {
              fprintf(stderr, "reg1=%d-%d, reg2=%d-%d, imp=%f, opt=%f\n",
                      tmpptr->start1, tmpptr->end1, tmpptr->start2, tmpptr->end2,
          ...
          do
            {
              fprintf(stderr, "reg1=%d-%d, reg2=%d-%d, imp=%f, opt=%f, wimp=%f\n",
                      tmpptr->start1, tmpptr->end1, tmpptr->start2, tmpptr->end2,
                      tmpptr->importance, tmpptr->opt, tmpptr->wimportance);
            }
          while ((tmpptr = tmpptr->next));
        }
    }
...
void clustalout_pointer(FILE *fp, int nseq, int maxlen, char **seq, char **name,
                        char *mark, char *comment, int *order)
{
  int pos, j;
  pos = 0;
  if (comment == NULL)
...
void writeData_reorder_pointer(FILE *fp, int locnjob, char **name, int *nlen,
                               char **aseq, int *order)
{
  int i, j, k;
  int nalen;
...
void writeData_reorder(FILE *fp, int locnjob, char name[][B], int nlen[],
                       char **aseq, int *order)
{
  int i, j, k;
  int nalen;
...
void miyataout_reorder_pointer(FILE *fp, int locnjob, int nlenmax, char **name,
                               int *nlen, char **aseq, int *order)
{
  int i, j, k;
  int nalen;
...
void readpairfoldalign(FILE *fp, char *s1, char *s2, char *aln1, char *aln2,
                       int q1, int q2, int *of1, int *of2, int sumlen)
{
  char gett[1000];
  int *maptoseq1;
  int *maptoseq2;
  ...
  while (!feof(fp))
    {
      ...
      //		fprintf( stderr, "gett = %s\n", gett );
      sscanf(gett, "%c %c %s %s %d %d", &dumc, &dumc, sinseq, sinaln, &dumi,
             &dumi);
