...
static void extendmseq(char **mseq1, char **mseq2, char **seq1, char **seq2,
                       int i, int j, int prevhiti, int prevhitj)
{
  char gap[] = "-";
  int l;
...
void backdp(float **WMMTX, float wmmax, float *maxinw, float *maxinh, int lgth1,
            int lgth2, int alloclen, float *w1, float *w2, float *initverticalw,
            float *m, int *mp, int iin, int jin, char **seq1, char **seq2,
            char **mseq1, char **mseq2)
{
  ...
  for (i = lgth1 - 2; i > -1; i--)
  {
    ...
    for (j = lgth2 - 2; j > -1; j--)
    {
      //			fprintf( stderr, "i,j=%d,%d %c-%c ", i, j,
      //seq1[0][i], seq2[0][j] );
      wm = *prept;
      g = mi + fpenalty;
#if 0
...
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
  ...
  *mseq2[0] = 0;
  ...
  backdp(WMMTX, wmmax, maxinw, maxinh, lgth1, lgth2, alloclen, w1, w2,
         initverticalw, m, mp, iin, jin, seq1, seq2, mseq1, mseq2);
