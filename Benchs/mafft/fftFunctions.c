...
Fukusosuu ***AllocateFukusosuuCub(int l1, int l2, int l3)
{
  ...
  if (!value)
    {
      ErrorExit("Cannot allocate Fukusosuu");
    }
  for (i = 0; i < l1; i++)
    {
      value[i] = AllocateFukusosuuMtx(l2, l3);
    }
  value[l1] = NULL;
  return (value);
}

void FreeFukusosuuVec(Fukusosuu *vec) { free((void *)vec); }
...
void zurasu2(int lag, int clus1, int clus2, char **seq1, char **seq2,
...
void zurasu(int lag, int clus1, int clus2, char **seq1, char **seq2,
            char **aseq1, char **aseq2)
{
  int i;
#if DEBUG
  ...
  if (lag > 0)
    {
      for (i = 0; i < clus1; i++)
        {
          strcpy(aseq1[i], seq1[i]);
        }
      for (i = 0; i < clus2; i++)
        {
          strcpy(aseq2[i], seq2[i] + lag);
        }
    }
  else
    {
      for (i = 0; i < clus1; i++)
        {
          strcpy(aseq1[i], seq1[i] - lag);
        }
      for (i = 0; i < clus2; i++)
        {
          strcpy(aseq2[i], seq2[i]);
        }
    }
}

int alignableReagion(int clus1, int clus2, char **seq1, char **seq2,
                     double *eff1, double *eff2, Segment *seg)
{
  int i, j, k;
  ...
  for (i = 0; i < len; i++)
    {
      ...
      for (j = 0; j < clus1; j++)
        {
          prf1[amino_n[(int)seq1[j][i]]] += eff1[j];
        }
#endif
      for (j = 0; j < clus2; j++)
        {
          prf2[amino_n[(int)seq2[j][i]]] += eff2[j];
        }
  ...
  for (j = i; j < MAXSEG; j++)
    {
      ...
      if (result1[j] == result1[j - 1] || result2[j] == result2[j - 1])
        {
          if (ocrossscore[result1[j]][result2[j]] >
              ocrossscore[result1[j - 1]][result2[j - 1]])
            {
              count--;
            }
        }
...
static int permit(Segment *seg1, Segment *seg2)
{
  ...
  if (seg1->pair->end >= seg2->pair->start)
    {
      return (0);
    }
  else
...
void blockAlign2(int *cut1, int *cut2, Segment **seg1, Segment **seg2,
                 double **ocrossscore, int *ncut)
{
  ...
  for (i = 1; i < *ncut; i++)
    {
      ...
      for (j = 1; j < *ncut; j++)
        ...
        */
              if (k && k < *ncut - 1 && j < *ncut - 1 &&
                  !permit(seg1[k - 1], seg1[j - 1]))
                {
                  continue;
                }
          ...
          for (k = 0; k < klim; k++)
            {
              if (k && k < *ncut - 1 && i < *ncut - 1 &&
                  !permit(seg2[k - 1], seg2[i - 1]))
                {
                  continue;
                }
...
void blockAlign3(int *cut1, int *cut2, Segment **seg1, Segment **seg2,
                 double **ocrossscore, int *ncut)
