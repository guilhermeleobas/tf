...
static void part_imp_match_out_vead_gapmapQ(float *imp, int i1, int lgth2,
                                            int start2, int *gapmap2)
{
#if FASTMACHCALC
  float *pt = imp;
  int *gapmappt = gapmap2;
  while (lgth2--) *pt++ += impmtx[i1][start2 + *gapmappt++];
#else
  int j;
  for (j = 0; j < lgth2; j++)
    {
...
static void part_imp_match_out_vead_tate_gapmapQ(float *imp, int j1, int lgth1,
                                                 int start1, int *gapmap1)
{
#if FASTMACHCALC
  float *pt = imp;
  int *gapmappt = gapmap1;
  while (lgth1--) *pt++ = impmtx[start1 + *gapmappt++][j1];
#else
  int i;
  for (i = 0; i < lgth1; i++)
    {
...
void part_imp_match_init_strictQ(float *imp, int clus1, int clus2, int lgth1,
                                 int lgth2, char **seq1, char **seq2,
                                 double *eff1, double *eff2,
                                 LocalHom ***localhom, int forscore)
{
...
void part_imp_rnaQ(int nseq1, int nseq2, char **seq1, char **seq2, double *eff1,
...
void part_imp_match_initQ(float *imp, int clus1, int clus2, int lgth1,
                          int lgth2, char **seq1, char **seq2, double *eff1,
                          double *eff2, LocalHom ***localhom)
{
      ...
      for (j = 0; j < clus2; j++)
        {
          ...
          while (*pt != 0)
            {
              //				fprintf( stderr, "tmpint = %d, end1 = %d pos =
              //%d\n", tmpint, localhom[i][j].end1, pt-seq1[i] );
              if (*pt++ != '-')
                {
                  tmpint++;
                }
              if (tmpint == localhom[i][j]->end1)
                {
                  break;
                }
            }
          ...
          while (*pt != 0)
            {
              ...
              if (tmpint == localhom[i][j]->end2)
                {
                  break;
                }
            }
          end2 = pt - seq2[j] - 1;
          //			fprintf( stderr, "start1 = %d, end1 = %d, start2
          //= %d, end2 = %d\n", start1, end1, start2, end2 );
          k1 = start1;
          k2 = start2;
          fprintf(stderr, "step 0\n");
  ...
  for (k = 0; k <= lgth1 + lgth2; k++)
    {
      ...
      if (iin != lgth1 && jin != lgth2)  // ??
        {
          *impwmpt +=
              part_imp_match_out_scQ(gapmap1[iin] + start1, gapmap2[jin] + start2);
...
float partQ__align(char **seq1, char **seq2, double *eff1, double *eff2,
                   int icyc, int jcyc, int alloclen, LocalHom ***localhom,
                   float *impmatch, int start1, int end1, int start2, int end2,
                   int *gapmap1, int *gapmap2, char *sgap1, char *sgap2,
                   char *egap1, char *egap2)
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
  ...
  if (localhom)
    {
      part_imp_match_out_vead_tate_gapmapQ(initverticalw, gapmap2[0] + start2,
                                           lgth1, start1, gapmap1);
    }

  match_calc(currentw, cpmx1, cpmx2, 0, lgth2, floatwork, intwork, 1);
  if (localhom)
    {
      part_imp_match_out_vead_gapmapQ(currentw, gapmap1[0] + start1, lgth2,
                                      start2, gapmap2);
    }
#if 0  // -> tbfast.c
	if( localhom )
  ...
  for (i = 1; i < lasti; i++)
    {
      ...
      if (localhom)
        {
          //			fprintf( stderr, "Calling imp_match_calc (o)
          //lgth = %d, i = %d\n", lgth1, i );
          //			imp_match_out_vead( currentw, i, lgth2 );
          part_imp_match_out_vead_gapmapQ(currentw, gapmap1[i] + start1, lgth2,
                                          start2, gapmap2);
        }
#if XXXXXXX
      fprintf(stderr, "\n");
  ...
  if (localhom)
    {
      Atracking_localhom(impmatch, currentw, lastverticalw, seq1, seq2, mseq1,
                         mseq2, cpmx1, cpmx2, ijp, icyc, jcyc, start1, end1,
                         start2, end2, gapmap1, gapmap2);
    }
  else
    {
