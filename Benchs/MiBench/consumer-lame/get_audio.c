...
int read_samples_pcm(lame_global_flags *gfp, short sample_buffer[2304],
                     int frame_size, int samples_to_read);
int read_samples_mp3(lame_global_flags *gfp, FILE *musicin,
...
void lame_close_infile(lame_global_flags *gfp) { CloseSndFile(gfp); }
/************************************************************************
*
* lame_readframe()
*
...
int get_audio(lame_global_flags *gfp, short buffer[2][1152], int stereo)
{
  ...
  if (count_samples_carefully)
     ...
     * are using LIBSNDFILE, this is not necessary */
      ...
      if (remaining < (unsigned long)framesize) samples_to_read = remaining;
    }
  ...
  if (gfp->input_format == sf_mp3)
    ...
    {
      samples_read = read_samples_pcm(gfp, insamp, num_channels * framesize,
...
int GetSndBitrate(void) { return input_bitrate; }
int read_samples_mp3(lame_global_flags *gfp, FILE *musicin,
...
unsigned long GetSndSamples(void) { return gs_wfInfo.samples; }
int GetSndSampleRate(void) { return gs_wfInfo.samplerate; }
...
FILE *OpenSndFile(lame_global_flags *gfp, const char *lpszFileName,
                  int default_samp, int default_channels)
{
  input_bitrate = 0;
  if (gfp->input_format == sf_mp3)
    {
#ifdef AMIGA_MPEGA
      if (-1 == lame_decode_initfile(lpszFileName, &num_channels, &samp_freq,
                                     &input_bitrate, &num_samples))
        {
      ...
      if (-1 == lame_decode_initfile(musicin, &num_channels, &samp_freq,
                                     &input_bitrate, &num_samples))
        {
          fprintf(stderr, "Error reading headers in mp3 input file %s.\n",
                  lpszFileName);
                     ...
                     int frame_size, int samples_to_read)
{
  ...
  if (samples_read < frame_size)
    {
      /*fprintf(stderr,"Insufficient PCM input for one frame - fillout with
     * zeros\n");
    */
  ...
  if (8 == gs_wfInfo.pcmbitwidth)
    for (; samples_read >= 0;
         sample_buffer[samples_read] = sample_buffer[samples_read--] * 256)
      ;
...
unsigned long GetSndSamples(void) { return num_samples; }
int GetSndSampleRate(void) { return samp_freq; }
int GetSndChannels(void) { return num_channels; }
...
/* Read from standard input. */
  ...
  if (gfp->input_format == sf_mp3)
    {
#ifdef AMIGA_MPEGA
      if (-1 == lame_decode_initfile(inPath, &num_channels, &samp_freq,
                                     &input_bitrate, &num_samples))
        {
          fprintf(stderr, "Error reading headers in mp3 input file %s.\n", inPath);
          exit(1);
                     ...
                     int frame_size, int samples_to_read)
{
  ...
  if (gfp->swapbytes == TRUE) SwapBytesInWords(sample_buffer, samples_read);
...
static int parse_wave_header(FILE *sf)
{
  ...
  if (Read32BitsHighLow(sf) != WAV_ID_WAVE) return 0;
  ...
  if (is_wav)
    {
      ...
      num_samples =
          data_length / (wave_info.channels * wave_info.bits_per_sample / 8);
    }
  return is_wav;
}
...
static int parse_aiff_header(FILE *sf)
{
  ...
  if (Read32BitsHighLow(sf) != IFF_ID_AIFF) return 0;
