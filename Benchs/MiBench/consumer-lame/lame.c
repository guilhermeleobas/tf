...
void lame_init_params(lame_global_flags *gfp)
{
  ...
  */
  if (gfp->out_samplerate == 0)
  {
    ...
    if (gfp->brate > 0)
    {
      ...
      compression_ratio =
          gfp->out_samplerate * 16 * gfp->stereo / (1000.0 * gfp->brate);
  ...
  if (gfp->brate == 0)
  { /* user didn't specify a bitrate, use default */
    gfp->brate = 128;
    if (gfp->mode_gr == 1)
    {
      gfp->brate = 64;
  ...
  gfp->totalframes = 0;
  gfp->totalframes =
      2 + gfp->num_samples / (gfp->resample_ratio * gfp->framesize);
  ...
  */
  if (gfp->brate >= 320)
  {
    gfp->VBR = 0; /* dont bother with VBR at 320kbs */
  }
  compression_ratio =
      gfp->out_samplerate * 16 * gfp->stereo / (1000.0 * gfp->brate);
  ...
  */
  ...
  {
    gfp->highpass1 =
        2.0 * gfp->highpassfreq / gfp->out_samplerate; /* will always be >=0 */
    if (gfp->highpasswidth >= 0)
    {
      gfp->highpass2 =
          2.0 * (gfp->highpassfreq + gfp->highpasswidth) / gfp->out_samplerate;
    }
    else
    {
      /* 15% above on default */
  ...
  {
    gfp->lowpass2 =
        2.0 * gfp->lowpassfreq / gfp->out_samplerate; /* will always be >=0 */
    if (gfp->lowpasswidth >= 0)
    {
      gfp->lowpass1 =
          2.0 * (gfp->lowpassfreq - gfp->lowpasswidth) / gfp->out_samplerate;
      if (gfp->lowpass1 < 0)
      { /* has to be >= 0 */
        gfp->lowpass1 = 0;
      }
  ...
  if (gfp->filter_type == 0)
  {
    ...
    if (gfp->lowpass1 > 0)
    {
      ...
      for (band = 0; band <= 31; ++band)
      {
        ...
        if (gfp->lowpass1 < freq && freq < gfp->lowpass2)
        {
          ...
          amp = cos((PI / 2) * (gfp->lowpass1 - freq) /
                    (gfp->lowpass2 - gfp->lowpass1));
        }
    ...
    if (gfp->highpass2 > 0)
    {
      if (gfp->highpass2 < .9 * (.75 / 31.0))
      {
        ...
        gfp->highpass2 = 0;
        fprintf(stderr,
                "Warning: highpass filter disabled.  highpass frequency to "
                "small\n");
      }
    }
    ...
    if (gfp->highpass2 > 0)
    {
      ...
      for (band = 0; band <= 31; ++band)
      {
        ...
        if (gfp->highpass1 < freq && freq < gfp->highpass2)
        {
          minband = Min(minband, band);
          maxband = Max(maxband, band);
          amp = cos((PI / 2) * (gfp->highpass2 - freq) /
                    (gfp->highpass2 - gfp->highpass1));
        }
  ...
  if ((gfp->bitrate_index =
           BitrateIndex(gfp->brate, gfp->version, gfp->out_samplerate)) < 0)
  {
    display_bitrates(stderr);
    exit(1);
  }
  ...
  if (gfp->VBR)
  {
    ...
    if (0 == gfp->VBR_max_bitrate_kbps)
    {
      ...
      if (gfp->VBR_q == 0)
      {
        gfp->VBR_max_bitrate = 14; /* allow 320kbs */
      }
      if (gfp->VBR_q >= 4)
      {
        gfp->VBR_max_bitrate = 12; /* max = 224kbs */
      }
      if (gfp->VBR_q >= 8)
      {
        gfp->VBR_max_bitrate = 9; /* low quality, max = 128kbs */
      }
    }
    else
    {
      if ((gfp->VBR_max_bitrate = BitrateIndex(
               gfp->VBR_max_bitrate_kbps, gfp->version, gfp->out_samplerate)) <
          0)
      {
        display_bitrates(stderr);
        exit(1);
    ...
    {
      if ((gfp->VBR_min_bitrate = BitrateIndex(
               gfp->VBR_min_bitrate_kbps, gfp->version, gfp->out_samplerate)) <
          0)
      {
        display_bitrates(stderr);
        exit(1);
  ...
  if (gfp->VBR)
  {
    gfp->quality = Min(gfp->quality, 2); /* always use quality <=2  with VBR */
  }
  ...
  if (gfp->VBR)
  {
    if (disp_brhist) brhist_init(gfp, 1, 14);
  }
  else
    disp_brhist = 0;
#endif
...
{
  static const char *mode_names[4] = {"stereo", "j-stereo", "dual-ch",
                                      "single-ch"};
  ...
  {
    fprintf(stderr,
            "Using polyphase highpass filter, transition band: %.0f Hz -  %.0f "
            "Hz\n",
  ...
  {
    fprintf(stderr,
            "Using polyphase lowpass filter,  transition band:  %.0f Hz - %.0f "
            "Hz\n",
  ...
  else
  {
    ...
    {
      fprintf(stderr,
              "Encoding as %.1fkHz VBR(q=%i) %s MPEG%i LayerIII  qval=%i\n",
              gfp->out_samplerate / 1000.0, gfp->VBR_q, mode_names[gfp->mode],
              2 - gfp->version, gfp->quality);
    }
    else
    {
      fprintf(
          stderr,
          "Encoding as %.1f kHz %d kbps %s MPEG%i LayerIII (%4.1fx)  qval=%i\n",
          gfp->out_samplerate / 1000.0, gfp->brate, mode_names[gfp->mode],
          2 - gfp->version, compression, gfp->quality);
    }
  }
  fflush(stderr);
}
...
*/
int lame_encode_frame(lame_global_flags *gfp, short int inbuf_l[],
                      short int inbuf_r[], int mf_size, char *mp3buf,
                      int mp3buf_size)
{
  ...
  if (gfp->frameNum == 0)
  {
    ...
    bitsPerSlot = 8;
    avg_slots_per_frame =
        (bit_rate * gfp->framesize) / (sampfreq * bitsPerSlot);
  ...
  if (!gfp->gtkflag && !gfp->silent)
  {
    ...
    {
      timestatus(gfp->out_samplerate, gfp->frameNum, gfp->totalframes,
                 gfp->framesize);
#ifdef BRHIST
      if (disp_brhist)
      {
  ...
  if (gfp->psymodel)
  {
    ...
    for (gr = 0; gr < gfp->mode_gr; gr++)
    {
      ...
      L3psycho_anal(gfp, bufp, gr, &ms_ratio[gr], &ms_ratio_next,
                    &ms_ener_ratio[gr], masking_ratio, masking_MS_ratio, pe[gr],
                    pe_MS[gr], blocktype);
  ...
  if (check_ms_stereo)
  {
    ...
    check_ms_stereo = (l3_side.gr[0].ch[0].tt.block_type ==
                       l3_side.gr[0].ch[1].tt.block_type) &&
                      (l3_side.gr[1].ch[0].tt.block_type ==
                       l3_side.gr[1].ch[1].tt.block_type);
  }
  if (check_ms_stereo)
  {
    ...
    ms_ratio_ave =
        .25 * (ms_ratio[0] + ms_ratio[1] + ms_ratio_prev + ms_ratio_next);
    ms_ener_ratio_ave = .5 * (ms_ener_ratio[0] + ms_ener_ratio[1]);
    if (ms_ratio_ave < .35 /*&& ms_ener_ratio_ave<.75*/)
    {
      gfp->mode_ext = MPG_MD_MS_LR;
    }
  }
  if (gfp->force_ms)
  {
  ...
  if (gfp->gtkflag)
  {
    ...
    for (gr = 0; gr < gfp->mode_gr; gr++)
    {
      for (ch = 0; ch < gfp->stereo; ch++)
      {
        ...
        pinfo->blocktype[gr][ch] = l3_side.gr[gr].ch[ch].tt.block_type;
  ...
  */
  ...
  { /* a program failure */
    fprintf(stderr, "Sent %ld bits = %ld slots plus %ld\n", frameBits,
            frameBits / bitsPerSlot, frameBits % bitsPerSlot);
  }
  sentBits += frameBits;
...
int fill_buffer_resample(lame_global_flags *gfp, short int *outbuf,
                         int desired_len, short int *inbuf, int len,
                         int *num_used, int ch)
{
...
#define OLDBUFSIZE 5
  ...
  if (gfp->frameNum != 0)
  {
    init[ch] = 0; /* reset, for next time framenum=0 */
  }

  /* if downsampling by an integer multiple, use linear resampling,
   * otherwise use quadratic */
  linear =
      (fabs(gfp->resample_ratio - floor(.5 + gfp->resample_ratio)) < .0001);
  ...
  for (k = 0; k < desired_len; k++)
  {
    ...
    if (j + 2 >= len)
    {
      break; /* not enough data in input buffer */
    }
    ...
    else
    {
      ...
      value = floor(.5 + -y0 * x1 * x2 * x3 / 6 + y1 * x0 * x2 * x3 / 2 -
                    y2 * x0 * x1 * x3 / 2 + y3 * x0 * x1 * x2 / 6);
      if (value > 32767)
      {
        outbuf[k] = 32767;
      }
...
int fill_buffer(lame_global_flags *gfp, short int *outbuf, int desired_len,
                short int *inbuf, int len)
{
  int j;
...
*/
int lame_encode_buffer(lame_global_flags *gfp, short int buffer_l[],
                       short int buffer_r[], int nsamples, char *mp3buf,
                       int mp3buf_size)
{
  ...
  while (nsamples > 0)
  {
    ...
    {
      /* encode the frame */
      ret = lame_encode_frame(gfp, mfbuf[0], mfbuf[1], mf_size, mp3buf,
                              mp3buf_size);
      if (ret == -1)
      {
...
int lame_encode_buffer_interleaved(lame_global_flags *gfp, short int buffer[],
 ...
 * lame_encode_finish()
 */
int lame_encode(lame_global_flags *gfp, short int in_buffer[2][1152],
                char *mp3buf, int size)
{
  int imp3, save;
