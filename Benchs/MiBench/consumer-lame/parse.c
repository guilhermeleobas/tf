...
void lame_help(lame_global_flags *gfp, char *name) /* print syntax & exit */
{
  ...
  fprintf(stdout,
          "    -s sfreq        sampling frequency of input file(kHz) - default "
          "44.1kHz\n");
  ...
  fprintf(stdout,
          ...
          "below freq\n");
  fprintf(stdout,
          "  --highpass-width freq  frequency(kHz) - default 15%% of highpass "
          "freq\n");
  fprintf(stdout,
          "  --resample sfreq  sampling frequency of output file(kHz)- "
          "default=input sfreq\n");
  fprintf(stdout,
          "  --cwlimit freq    compute tonality up to freq (in kHz) default "
          "8.8717\n");
  fprintf(stdout, "\n");
  fprintf(stdout, "  Operational options:\n");
  fprintf(stdout,
          "    -m mode         (s)tereo, (j)oint, (f)orce or (m)ono  (default "
          "j)\n");
  fprintf(
      stdout,
  ...
  fprintf(stdout,
          "    -p              error protection.  adds 16bit checksum to every "
          "frame\n");
  ...
  fprintf(stdout,
          "     --ty \"year\"      year in which the song/album was made (max "
          "4 chars)\n");
  fprintf(stdout, "     --tc \"comment\"   additional info (max 30 chars)\n");
  fprintf(stdout,
          "                      (or max 28 chars if using the \"track\" "
          "option)\n");
  fprintf(
      stdout,
      "     --tn \"track\"     track number of the song on the CD (1 to 99)\n");
  fprintf(
      stdout,
      "                      (using this option will add an ID3v1.1 tag)\n");
...
void lame_presets_info(lame_global_flags *gfp,
                       char *name) /* print syntax & exit */
{
  lame_print_version(stdout);
  fprintf(stdout, "\n");
...
void lame_parse_args(lame_global_flags *gfp, int argc, char **argv)
{
    ...
    if (*token++ == '-')
    {
      ...
      if (*token == '-')
      {
        ...
        else if (strcmp(token, "lowpass") == 0)
        {
          ...
          {
            fprintf(stderr,
                    "Must specify lowpass with --lowpass freq, freq >= 0.001 "
                    "kHz\n");
            exit(1);
          }
        }
        else if (strcmp(token, "lowpass-width") == 0)
        {
          ...
          {
            fprintf(stderr,
                    "Must specify lowpass width with --lowpass-width freq, "
                    "freq >= 0 kHz\n");
            exit(1);
          }
        }
        else if (strcmp(token, "highpass") == 0)
        {
          ...
          {
            fprintf(stderr,
                    "Must specify highpass with --highpass freq, freq >= 0.001 "
                    "kHz\n");
            exit(1);
          }
        }
        else if (strcmp(token, "highpass-width") == 0)
        {
          ...
          {
            fprintf(stderr,
                    "Must specify highpass width with --highpass-width freq, "
                    "freq >= 0 kHz\n");
            exit(1);
          }
        }
        ...
        else if (strcmp(token, "preset") == 0)
        {
          ...
          {
            fprintf(stderr,
                    "%s: --preset type, type must be phone, voice, fm, tape, "
                    "hifi, cd or studio, not %s\n",
                    programName, nextArg);
            exit(1);
          }
        } /* --preset */
        ...
        while ((c = *token++))
        {
          ...
          switch (c)
          {
...
#ifdef LIBSNDFILE
              fprintf(stderr,
                      "WARNING: libsndfile may ignore -r and perform fseek's "
                      "on the input.\n");
              fprintf(stderr,
            ...
            case 'p': /* (jo) error_protection: add crc16 information to stream
              ...
              break;
            case 'a': /* autoconvert input file from stereo to mono - for mono
                         mp3 encoding */
              autoconvert = 1;
              gfp->mode = MPG_MD_MONO;
              gfp->mode_fixed = 1;
...
#else
              fprintf(stderr,
                      "LAME not compiled with GTK support, -g not supported.\n",
                      programName, arg);
#endif
              break;
  ...
  {
    fprintf(stderr,
            "Error: libmp3lame not compiled with mp3 *decoding* support \n");
    exit(1);
  }
#endif
