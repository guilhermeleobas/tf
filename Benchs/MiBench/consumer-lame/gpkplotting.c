...
/* compute a gdkcolor */
void setcolor(GtkWidget *widget, GdkColor *color, gint red, gint green,
              gint blue)
{
  ...
  color->pixel =
      (gulong)(color->red * 65536 + color->green * 256 + color->blue);
...
void gpk_redraw(GdkPixmap *pixmap, GtkWidget *pixmapbox)
{
  ...
  gdk_draw_pixmap(pixmapbox->window,
                  pixmapbox->style->fg_gc[GTK_WIDGET_STATE(pixmapbox)], pixmap,
                  0, 0, 0, 0, pixmapbox->allocation.width,
