...
void BF_BitstreamFrame(BF_FrameData *frameInfo, BF_FrameResults *results)
{
  ...
  */
  ...
  results->nextBackPtr =
      (BitsRemaining / 8) + (forwardFrameLength / 8) - (forwardSILength / 8);
}
...
typedef int (*PartWriteFcnPtr)(BF_BitstreamPart *part,
...
static int store_side_info(BF_FrameData *info)
{
  ...
  l->side_info.headerPH =
      BF_LoadHolderFromBitstreamPart(l->side_info.headerPH, info->header);
  l->side_info.frameSIPH =
      BF_LoadHolderFromBitstreamPart(l->side_info.frameSIPH, info->frameSI);
...
BF_PartHolder *BF_LoadHolderFromBitstreamPart(BF_PartHolder *theHolder,
                                              BF_BitstreamPart *thePart)
{
...
*/
BF_PartHolder *BF_addElement(BF_PartHolder *thePH,
                             BF_BitstreamElement *theElement)
{
