...
void L3para_read(FLOAT8 sfreq, int numlines[CBANDS], int numlines_s[CBANDS],
                 int partition_l[HBLKSIZE], FLOAT8 minval[CBANDS],
                 FLOAT8 qthr_l[CBANDS], FLOAT8 s3_l[CBANDS + 1][CBANDS + 1],
                 FLOAT8 s3_s[CBANDS + 1][CBANDS + 1], FLOAT8 qthr_s[CBANDS],
                 FLOAT8 SNR_s[CBANDS], int bu_l[SBPSY_l], int bo_l[SBPSY_l],
                 FLOAT8 w1_l[SBPSY_l], FLOAT8 w2_l[SBPSY_l], int bu_s[SBPSY_s],
                 int bo_s[SBPSY_s], FLOAT8 w1_s[SBPSY_s], FLOAT8 w2_s[SBPSY_s]);

void L3psycho_anal(lame_global_flags *gfp, short int *buffer[2], int gr_out,
                   FLOAT8 *ms_ratio, FLOAT8 *ms_ratio_next,
                   FLOAT8 *ms_ener_ratio, III_psy_ratio masking_ratio[2][2],
 ...
 */
  ...
  if ((gfp->frameNum == 0) && (gr_out == 0))
  {
    ...
    L3para_read((FLOAT8)gfp->out_samplerate, numlines_l, numlines_s,
                partition_l, minval, qthr_l, s3_l, s3_s, qthr_s, SNR_s, bu_l,
                bo_l, w1_l, w2_l, bu_s, bo_s, w1_s, w2_s);
 ...
 **********************************************************************/
      ...
      if (tbb != 0)
      {
        ...
        else
        {
          ...
          tbb = exp(((TMN - NMT) * (LN_TO_LOG10 * 0.299)) +
                    ((TMN - NMT) * (LN_TO_LOG10 * 0.43)) *
                        tbb); /* conv1=-0.299, conv2=-0.43 */
        }
      }
    ...
    for (sb = 0; sb < SBPSY_s; sb++)
    {
      for (sblock = 0; sblock < 3; sblock++)
      {
            ...
            thm[1].s[sb][sblock] <= 1.58 * thm[0].s[sb][sblock])
        {
          ...
          rmid = Max(thm[chmid].s[sb][sblock],
                     Min(thm[chside].s[sb][sblock], mld));

          mld = mld_s[sb] * en[chmid].s[sb][sblock];
          rside = Max(thm[chside].s[sb][sblock],
                      Min(thm[chmid].s[sb][sblock], mld));
  ...
  for (chn = 0; chn < gfp->stereo; chn++)
  {
    ...
    blocktype_d[chn] =
        blocktype_old[chn];              /* value returned to calling program */
...
void L3para_read(FLOAT8 sfreq, int *numlines_l, int *numlines_s,
                 int *partition_l, FLOAT8 *minval, FLOAT8 *qthr_l,
                 FLOAT8 s3_l[64][64], FLOAT8 s3_s[CBANDS + 1][CBANDS + 1],
                 FLOAT8 *qthr_s, FLOAT8 *SNR, int *bu_l, int *bo_l,
  ...
  for (i = 0; i < part_max; i++)
  {
    ...
    for (j = 0; j < part_max; j++)
    {
...
#ifdef NEWS3
      if (j >= i)
        tempy = (bval_l[j] - bval_l[i]) * (-15);
      else
        tempy = (bval_l[j] - bval_l[i]) * 25;
      x = 0;
#endif
      /*
      ...
      */
      ...
      {
        s3_l[i][j] = exp((x + tempy) * LN_TO_LOG10);
      }
    }
  }
...
#ifdef NEWS3
      if (j >= i)
        tempy = (bval_s[j] - bval_s[i]) * (-15);
      else
        tempy = (bval_s[j] - bval_s[i]) * 25;
      x = 0;
#endif
      if (tempy <= -60.0)
      {
