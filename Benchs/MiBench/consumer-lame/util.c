...
void getframebits(lame_global_flags *gfp, int *bitsPerFrame, int *mean_bits)
{
  ...
  whole_SpF =
      floor((gfp->framesize / samp) * (bit_rate / (FLOAT8)bitsPerSlot) + 1e-9);
...
int SmpFrqIndex(/* convert samp frq in Hz to index */
                long sRate, /* legal rates 16000, 22050, 24000, 32000, 44100,
                               48000 */
                int *version)
{
...
void *mem_alloc(unsigned long block, char *item)
{
  ...
  ptr = (void *)malloc((size_t)block /* <<1 */); /* allocate twice as much
                                       memory as needed. fixes dodgy
...
void alloc_buffer(Bit_stream_struc *bs, /* bit stream structure */
                  int size)
{
  bs->buf = (unsigned char *)mem_alloc(
      (unsigned long)(size * sizeof(unsigned char)), "buffer");
  bs->buf_size = size;
}
...
void putbits(Bit_stream_struc *bs, /* bit stream structure */
