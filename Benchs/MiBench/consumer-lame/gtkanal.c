...
int gtkmakeframe(void)
{
  ...
  else
  {
    while (gfp->frameNum == pinfo->frameNum)
    {
      ...
      mp3count = lame_encode(gfp, Buffer, mp3buffer,
                             sizeof(mp3buffer)); /* encode frame */
    ...
    mp3out = lame_decode(mp3buffer, mp3count, mpg123pcm[0],
                         mpg123pcm[1]); /* re-synthesis to pcm */
...
void plot_frame(void)
{
  ...
  static GdkColor yellow, gray, cyan, magenta, orange, pink, red, green, blue,
      black, oncolor, offcolor;
  int blocktype[2][2];
  int headbits;
  int mode_gr = 2;
  ...
  sprintf(title2, "c1=%i,%i ", pplot1->big_values[0][ch],
          pplot1->big_values[1][ch]);
  ...
  if (gtkinfo.filetype)
    ...
    sprintf(title2, " mdb=%i   %i/%i", pplot1->maindata, pplot1->totbits,
            pplot->resvsize);
  gtk_text_insert(GTK_TEXT(headerbox), NULL, &oncolor, NULL, title2, -1);
  ...
  sprintf(title2, "%s  mask_ratio=%3.2f  %3.2f  ener_ratio=%3.2f  %3.2f", title,
          pplot->ms_ratio[0], pplot->ms_ratio[1], pplot->ms_ener_ratio[0],
  ...
  if (!gtkinfo.filetype)
  {
    ...
    /* data left of frame */
    gpk_graph_draw(pcmbox, n + 1, xcord, ycord, xmn, ymn, xmx, ymx, 0, title2,
                   &black);
    /* data right of frame */
    gpk_graph_draw(pcmbox, n + 1, &xcord[1152 + n - 1], &ycord[1152 + n - 1],
                   xmn, ymn, xmx, ymx, 0, title2, &black);
    /* the actual frame */
    gpk_graph_draw(pcmbox, 1152, &xcord[n], &ycord[n], xmn, ymn, xmx, ymx, 0,
                   title2, &black);
  }
  ...
  n = 1152;
  /*
  sprintf(title2,"Re-synthesis  mask_ratio=%3.2f  %3.2f  ener_ratio=%3.2f
  %3.2f",
  ...
  */
  ...
  gpk_graph_draw(winbox, n + 1, xcord, ycord, xmn, ymn, xmx, ymx, 0, title,
                 &black);
  ...
  for (gr = 0; gr < mode_gr; gr++)
  {
    ...
    sprintf(title2, "MDCT%1i(%s) bits=%i q=%i ", gr, label, bits,
            pplot1->qss[gr][ch]);
    gpk_bargraph_draw(mdctbox[gr], 0, xcord, ycord, xmn, ymn, xmx, ymx, 1,
                      title2, 0, barcolor);
    ...
    if (gtkinfo.sfblines)
    {
      ...
      for (i = nsfb - 7; i < nsfb; i++)
      {
        ...
        xcord[1] = xcord[0];
        gpk_rectangle_draw(mdctbox[gr], xcord, ycord, xmn, ymn, xmx, ymx,
                           &yellow);
      }
    }
    ...
    for (i = 0; i < n; i++)
    {
      ...
      if (blocktype[gr][ch] == SHORT_TYPE && !subblock_draw[i % 3]) coeff = 0;
    ...
    */
    ...
    ymn = 0;
    ymx = 11;
    gpk_bargraph_draw(mdctbox[gr], n, xcord, ycord, xmn, ymn, xmx, ymx, 0,
                      title2, 0, barcolor);
  }
  ...
  if (gtkinfo.kbflag)
  {
    for (gr = 0; gr < mode_gr; gr++)
    {
      ...
      for (en = 0, j = 0; j < BLKSIZE; j++) en += pplot->energy[gr][ch][j];
      ...
      xmn = 1;
      xmx = n;
      gpk_bargraph_draw(enerbox[gr], n, xcord, ycord, xmn, ymn, xmx, ymx, 1,
                        title2, 0, barcolor);
    }
  }
  else
  {
    ...
    for (gr = 0; gr < mode_gr; gr++)
    {
      ...
      sprintf(title2, "FFT%1i pe=%4.1fK/%3.1f n=%i/%3.1f/%3.1f/%3.1f", gr,
              ...
              pplot->max_noise[gr][ch], pplot->over_noise[gr][ch],
      ...
      gpk_bargraph_draw(enerbox[gr], n, xcord, ycord, xmn, ymn, xmx, ymx, 0,
                        title2, 3, &red);
  ...
  for (gr = 0; gr < mode_gr; gr++)
  {
    ...
    if (blocktype[gr][ch] == 2)
    {
      sprintf(label2, "SFB scale=%i %i%i%i", pplot1->scalefac_scale[gr][ch],
              pplot1->sub_gain[gr][ch][0], pplot1->sub_gain[gr][ch][1],
    ...
    if (gtkinfo.flag123)
      ggain = -(pplot1->qss[gr][ch] - 210) / 4.0;
    else
      ggain = -(pplot->LAMEqss[gr][ch] - 210) / 4.0;
    ...
    xmn = 1;
    xmx = n + 1;
    gpk_bargraph_draw(sfbbox[gr], n, xcord, ycord, xmn, ymn, xmx, ymx, 1,
                      label2, 0, grcolor[gr]);
...
static void update_progress(void)
{
  ...
  sprintf(label, "Frame:%4i/%4i  %6.2fs", pplot->frameNum, (int)tf - 1,
          pplot->frametime);
...
static void plotclick(GtkWidget *widget, gpointer data) { analyze(); }
static int frameadv1(GtkWidget *widget, gpointer data)
{
  ...
  if (idle_keepgoing)
  {
    ...
    else
    {
      ...
      if (mp3done)
      {
        /* dont try to read any more frames, and quit if "finish MP3" was
         * selected */
...
static void text_window(GtkWidget *widget, gpointer data)
{
  ...
  switch (option)
  {
    case 0:
      ...
      gtk_text_insert(
          GTK_TEXT(box), NULL, NULL, NULL,
          "Frame header information: "
          "First the bitrate, sampling frequency and mono, stereo or jstereo "
          "indicators are displayed .  If the bitstream is jstereo, then "
          "mid/side "
          "stereo or intensity stereo may be on (indicated in red).  If "
          "de-emphasis is used, this is also indicated in red.  The mdb value "
          "is "
          "main_data_begin.  The encoded data starts this many bytes *before* "
          "the "
          "frame header.  A large value of mdb means the bitstream has saved "
          "some "
          "bits into the reservoir, which it may allocate for some future "
          "frame. "
      ...
      gtk_text_insert(
          GTK_TEXT(box), NULL, NULL, NULL,
          "PCM data (top graph): "
          "The PCM data is plotted in black.  The layer3 frame is divided into "
          "2 "
          "granules of 576 samples (marked with yellow vertical lines).  In "
          "the "
          "case of normal, start and stop blocks, the MDCT coefficients for "
          "each "
          "granule are computed using a 1152 sample window centered over the "
          "granule.  In the case of short blocks, the granule is further "
          "divided "
          "into 3 blocks of 192 samples (also marked with yellow vertical "
          "lines)."
          "The MDCT coefficients for these blocks are computed using 384 "
          "sample "
          "windows centered over the 192 sample window.  (This info not "
          "available "
          "when analyzing .mp3 files.)  For the psycho-acoustic model, a "
          "windowed "
      ...
      gtk_text_insert(
          ...
          "number of extra bits allocated from the reservoir.  The MDCT pull "
          "down "
          "window will toggle between the original unquantized MDCT "
          "coefficients "
      ...
      gtk_text_insert(
          ...
          "psycho-acoustic model.  Granule 0 is in the left window, granule 1 "
          "in "
          "the right window.  The green and blue bars show how much distortion "
          "is "
          "allowable, as computed by the psycho-acoustic model. The red bars "
          "show "
          "the actual distortion after encoding.  There is one FFT for each "
          "granule, computed with a 1024 Hann window centered over the "
          "appropriate granule.  (the range of this 1024 sample window is "
          "shown "
          "by the blue and green bars in the PCM data window).  The Spectrum "
          "pull "
          "down window will toggle between showing the energy in equally "
          "spaced "
          "frequency domain and the scale factor bands used by layer3.  "
          "Finally, "
          "the perceptual entropy, total energy and number of scalefactor "
          "bands "
...
int gtkcontrol(lame_global_flags *gfp2)
{
  ...
  */
  ...
  adj = (GtkAdjustment *)gtk_adjustment_new(0, 0, (gint)gfp->totalframes - 1, 0,
                                            0, 0);
  ...
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(frameadv),
                     (gpointer) "-1");
  ...
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(plotclick),
                     NULL);
  gtk_box_pack_start(GTK_BOX(box1), button, TRUE, TRUE, 0);
  gtk_widget_show(button);
  ...
  gtk_table_attach(GTK_TABLE(table), pcmbox, 0, 2, 0, 1, tableops, tableops, 2,
                   2);
  gtk_widget_show(pcmbox);

  winbox = gpk_plot_new(graphy, graphy);
  gtk_table_attach(GTK_TABLE(table), winbox, 0, 2, 1, 2, tableops, tableops, 2,
                   2);
  gtk_widget_show(winbox);

  mdctbox[0] = gpk_plot_new(graphy, graphy);
  gtk_table_attach(GTK_TABLE(table), mdctbox[0], 0, 1, 2, 3, tableops, tableops,
                   2, 2);
  gtk_widget_show(mdctbox[0]);

  mdctbox[1] = gpk_plot_new(graphy, graphy);
  gtk_table_attach(GTK_TABLE(table), mdctbox[1], 1, 2, 2, 3, tableops, tableops,
                   2, 2);
  gtk_widget_show(mdctbox[1]);

  enerbox[0] = gpk_plot_new(graphy, graphy);
  gtk_table_attach(GTK_TABLE(table), enerbox[0], 0, 1, 3, 4, tableops, tableops,
                   2, 2);
  gtk_widget_show(enerbox[0]);

  enerbox[1] = gpk_plot_new(graphy, graphy);
  gtk_table_attach(GTK_TABLE(table), enerbox[1], 1, 2, 3, 4, tableops, tableops,
                   2, 2);
  gtk_widget_show(enerbox[1]);

  sfbbox[0] = gpk_plot_new(graphy, graphy);
  gtk_table_attach(GTK_TABLE(table), sfbbox[0], 0, 1, 4, 5, tableops, tableops,
                   2, 2);
  gtk_widget_show(sfbbox[0]);

  sfbbox[1] = gpk_plot_new(graphy, graphy);
  gtk_table_attach(GTK_TABLE(table), sfbbox[1], 1, 2, 4, 5, tableops, tableops,
                   2, 2);
  ...
  idle_count_max =
      READ_AHEAD + 1; /* number of frames to process before plotting */
