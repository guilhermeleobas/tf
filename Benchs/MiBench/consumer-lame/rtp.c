...
int sendrtp(int fd, struct sockaddr_in *sSockAddr, struct rtpheader *foo,
            void *data, int len)
{
  ...
  return sendto(fd, buf, len + sizeof(*foo), 0, (struct sockaddr *)sSockAddr,
                sizeof(*sSockAddr));
  /*  return write(fd,buf,len+sizeof(*foo))==len+sizeof(*foo); */
}

/* create a sender socket. */
int makesocket(char *szAddr, unsigned short port, int TTL,
  ...
  if ((ntohl(tempaddr) >> 28) == 0xe)
    {
      ...
      cLoop = 1; /* !? */
      iRet = setsockopt(iSocket, IPPROTO_IP, IP_MULTICAST_LOOP, &cLoop,
                        sizeof(char));
      if (iRet < 0)
        {
          fprintf(stderr,
