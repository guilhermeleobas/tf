...
/************************************************************************/
void set_pinfo(gr_info *cod_info, III_psy_ratio *ratio,
               III_scalefac_t *scalefac, FLOAT8 xr[576],
               FLOAT8 xfsf[4][SBPSY_l], FLOAT8 noise[4], int gr, int ch)
{
  int sfb;
  FLOAT ifqstep;
  ...
  else
  {
    for (sfb = 0; sfb < SBPSY_l; sfb++)
    {
      ...
      bw = end - start;
      for (en0 = 0.0, l = start; l < end; l++) en0 += xr[l] * xr[l];
      en0 = Max(en0 / bw, 1e-20);
      /*
...
/*  iteration_loop()                                                    */
/************************************************************************/
void iteration_loop(lame_global_flags *gfp, FLOAT8 pe[2][2],
                    FLOAT8 ms_ener_ratio[2], FLOAT8 xr[2][2][576],
                    III_psy_ratio ratio[2][2], III_side_info_t *l3_side,
...
#ifdef HAVEGTK
      if (gfp->gtkflag)
        set_pinfo(cod_info, &ratio[gr][ch], &scalefac[gr][ch], xr[gr][ch], xfsf,
                  noise, gr, ch);
#endif
                        ...
                        int l3_enc[2][2][576], III_scalefac_t scalefac[2][2])
{
   ...
   *******************************************************************/
  for (gfp->bitrate_index = 1; gfp->bitrate_index <= gfp->VBR_max_bitrate;
       gfp->bitrate_index++)
  {
    getframebits(gfp, &bitsPerFrame, &mean_bits);
...
#ifdef HAVEGTK
        if (gfp->gtkflag)
          set_pinfo(cod_info, &ratio[gr][ch], &scalefac[gr][ch], xr[gr][ch],
                    xfsf, noise, gr, ch);
#endif
        analog_silence = 1;
 ...
 */
      ...
      max_bits =
          1200 + frameBits[gfp->VBR_max_bitrate] / (gfp->stereo * gfp->mode_gr);
...
#endif
        outer_loop(gfp, xr[gr][ch], this_bits, noise, &l3_xmin, l3_enc[gr][ch],
...
#ifdef HAVEGTK
          if (gfp->gtkflag) memcpy(&bst_pinfo, pinfo, sizeof(plotting_data));
#endif
          /*
           * try with fewer bits
           */
...
#ifdef HAVEGTK
        if (gfp->gtkflag) memcpy(pinfo, &bst_pinfo, sizeof(plotting_data));
#endif
      }
 ...
 */
  ...
  for (gfp->bitrate_index = (analog_silence ? 1 : gfp->VBR_min_bitrate);
       gfp->bitrate_index < gfp->VBR_max_bitrate; gfp->bitrate_index++)
  {
  ...
  if (used_bits > bits)
  {
    ...
    for (gr = 0; gr < gfp->mode_gr; gr++)
    {
      ...
      {
        save_bits[gr][ch] =
            (save_bits[gr][ch] * frameBits[gfp->bitrate_index]) / used_bits;
      }
    }
    used_bits = 0;
    for (gr = 0; gr < gfp->mode_gr; gr++)
 ...
 */
          ...
          outer_loop(gfp, xr[gr][ch], save_bits[gr][ch], noise, &l3_xmin,
                     l3_enc[gr][ch], &scalefac[gr][ch], cod_info, xfsf, ch);
        }
#ifdef HAVEGTK
        if (gfp->gtkflag)
...
#ifdef HAVEGTK
      if (gfp->gtkflag) pinfo->LAMEmainbits[gr][ch] = cod_info->part2_3_length;
#endif
      ResvAdjust(gfp, cod_info, l3_side, mean_bits);
    }
  }
    ...
    gr_info *cod_info, FLOAT8 xfsf[4][SBPSY_l], int ch)
{
  ...
  while (notdone)
  {
    ...
    if (compute_stepsize)
    {
      ...
      bits_found = bin_search_StepSize2(gfp, targ_bits, OldValue[ch], l3_enc_w,
                                        xrpow, cod_info);
    ...
    else
    {
      ...
      {
        better = quant_compare(gfp->experimentalX, best_over, best_tot_noise,
                               best_over_noise, best_max_noise, over, tot_noise,
                               over_noise, max_noise);
      }
...
int calc_noise1(FLOAT8 xr[576], int ix[576], gr_info *cod_info,
                ...
                FLOAT8 *over_noise, FLOAT8 *tot_noise, FLOAT8 *max_noise)
{
...
/* max -30db noise below threshold */
      ...
      noise =
          10 *
          log10(Max(.001, Min(ath_max, xfsf[i + 1][sfb] / l3_xmin->s[sfb][i])));
#else
...
*/
void amp_scalefac_bands(FLOAT8 xrpow[576], gr_info *cod_info,
                        III_scalefac_t *scalefac, FLOAT8 distort[4][SBPSY_l])
{
  int start, end, l, i;
  u_int sfb;
  FLOAT8 ifqstep34;
...
int quant_compare(int experimentalX, int best_over, FLOAT8 best_tot_noise,
                  FLOAT8 best_over_noise, FLOAT8 best_max_noise, int over,
                  FLOAT8 tot_noise, FLOAT8 over_noise, FLOAT8 max_noise)
{
  /*
  ...
  {
    better =
        (((0 >= max_noise) && (best_max_noise > 2)) ||
         ((0 >= max_noise) && (best_max_noise < 0) &&
          ((best_max_noise + 2) > max_noise) && (tot_noise < best_tot_noise)) ||
         ((0 >= max_noise) && (best_max_noise > 0) &&
          ((best_max_noise + 2) > max_noise) &&
          (tot_noise < (best_tot_noise + best_over_noise))) ||
         ((0 < max_noise) && (best_max_noise > -0.5) &&
          ((best_max_noise + 1) > max_noise) &&
          ((tot_noise + over_noise) < (best_tot_noise + best_over_noise))) ||
         ((0 < max_noise) && (best_max_noise > -1) &&
          ((best_max_noise + 1.5) > max_noise) &&
          ((tot_noise + over_noise + over_noise) <
           (best_tot_noise + best_over_noise + best_over_noise))));
  }
  if (experimentalX == 5)
  {
    ...
    better =
        ...
        ((over_noise == best_over_noise) &&
         ((max_noise < best_max_noise) ||
          ((max_noise == best_max_noise) && (tot_noise <= best_tot_noise))));
  }

  return better;
}

int VBR_compare(int best_over, FLOAT8 best_tot_noise, FLOAT8 best_over_noise,
                FLOAT8 best_max_noise, int over, FLOAT8 tot_noise,
                FLOAT8 over_noise, FLOAT8 max_noise)
{
  /*
    noise is given in decibals (db) relative to masking thesholds.
  ...
  better = ((over <= best_over) && (over_noise <= best_over_noise) &&
            (tot_noise <= best_tot_noise) && (max_noise <= best_max_noise));
  return better;
}
