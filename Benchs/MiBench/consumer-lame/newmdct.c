...
static void mdct_short(FLOAT8 *out, FLOAT8 *in)
{
  ...
  for (m = NS / 2 - 1; m >= 0; --m)
  {
    ...
    for (l = 2; l >= 0; l--)
    {
      out[3 * m + l] = a0 * in[6 * l] + a1 * in[6 * l + 1] +
                       a2 * in[6 * l + 2] + a3 * in[6 * l + 3] +
                       a4 * in[6 * l + 4] + a5 * in[6 * l + 5];
    }
  }
}
...
static void mdct_long(FLOAT8 *out, FLOAT8 *in)
{
  ...
  {
    out[all[j]] =
        in[0] * cos_l0[0] + in[1] * cos_l0[1] + in[2] * cos_l0[2] +
        in[3] * cos_l0[3] + in[4] * cos_l0[4] + in[5] * cos_l0[5] +
        in[6] * cos_l0[6] + in[7] * cos_l0[7] + in[8] * cos_l0[8] +
        in[9] * cos_l0[9] + in[10] * cos_l0[10] + in[11] * cos_l0[11] +
        in[12] * cos_l0[12] + in[13] * cos_l0[13] + in[14] * cos_l0[14] +
        in[15] * cos_l0[15] + in[16] * cos_l0[16] + in[17] * cos_l0[17];
    cos_l0 += 18;
  } while (--j >= 0);
  ...
  /* 16 */
  out[16] = s0 * cos_l0[0] + s1 * cos_l0[1] + s2 * cos_l0[2] + s3 * cos_l0[3] +
            s4 * cos_l0[4] + s5 * cos_l0[5];
  cos_l0 += 6;

  /* 10 */
  out[10] = s0 * cos_l0[0] + s1 * cos_l0[1] + s2 * cos_l0[2] + s3 * cos_l0[3] +
            s4 * cos_l0[4] + s5 * cos_l0[5];
  cos_l0 += 6;

  /* 7 */
  out[7] = s0 * cos_l0[0] + s1 * cos_l0[1] + s2 * cos_l0[2] + s3 * cos_l0[3] +
           s4 * cos_l0[4] + s5 * cos_l0[5];
  cos_l0 += 6;

  /* 1 */
  out[1] = s0 * cos_l0[0] + s1 * cos_l0[1] + s2 * cos_l0[2] + s3 * cos_l0[3] +
           s4 * cos_l0[4] + s5 * cos_l0[5];
  cos_l0 += 6;
...
void mdct_sub48(lame_global_flags *gfp, short *w0, short *w1,
                FLOAT8 mdct_freq[2][2][576], III_side_info_t *l3_side)
{
  int gr, k, ch;
  short *wk;
  static int init = 0;
...
#ifdef ALLOW_MIXED
        if (gi->mixed_block_flag && band < 2) type = 0;
#endif
        ...
        else
        {
          if (type == SHORT_TYPE)
          {
            for (k = 2; k >= 0; --k)
            {
              ...
              work[k] = sb_sample[ch][gr][k + 6][band] * w1 -
                        sb_sample[ch][gr][11 - k][band];
              work[k + 3] = sb_sample[ch][gr][k + 12][band] +
                            sb_sample[ch][gr][17 - k][band] * w1;

              work[k + 6] = sb_sample[ch][gr][k + 12][band] * w1 -
                            sb_sample[ch][gr][17 - k][band];
              work[k + 9] = sb_sample[ch][1 - gr][k][band] +
                            sb_sample[ch][1 - gr][5 - k][band] * w1;

              work[k + 12] = sb_sample[ch][1 - gr][k][band] * w1 -
                             sb_sample[ch][1 - gr][5 - k][band];
              work[k + 15] = sb_sample[ch][1 - gr][k + 6][band] +
          ...
          else
          {
            for (k = 8; k >= 0; --k)
            {
              work[k] = win[type][k] * sb_sample[ch][gr][k][band] -
...
void mdct_init48(void)
{
  ...
  for (k = 0; k < 8; k++)
  {
    /*
      ...
      */
    static const FLOAT8 c[8] = {-0.6,   -0.535, -0.33,   -0.185,
  ...
  sq = 4.0 / NL;
  {
    ...
    do
    {
      ...
      for (k = 0; k < NL / 4; k++)
      {
        *cos_l0++ = sq * cos((PI / (4 * NL)) * (2 * m + 1) * (4 * k + 2 + NL));
      }
      for (k = 0; k < NL / 4; k++)
      {
        *cos_l0++ =
            sq * cos((PI / (4 * NL)) * (2 * m + 1) * (4 * k + 2 + NL * 3));
      }
    } while (--j >= 0);
    ...
    {
      m = d9[j];
      *cos_l0++ = sq * cos((PI / (4 * NL)) * (2 * m + 1) * (2 + NL));
      *cos_l0++ = sq * cos((PI / (4 * NL)) * (2 * m + 1) * (4 * 2 + 2 + NL));
    } while (--j >= 0);
  }
  ...
  for (i = 0; i < NS / 4; i++)
  {
    ...
    for (m = 0; m < NS / 2; m++)
    {
      cos_s[m][i] = w2 * cos((PI / (4 * NS)) * (2 * m + 1) * (4 * i + 2 + NS));
      cos_s[m][i + NS / 4] =
          w2 * cos((PI / (4 * NS)) * (2 * m + 1) * (4 * i + 2 + NS * 3));
    }
  }
}
