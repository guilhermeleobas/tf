...
3:  convert_mdct = 0, convert_psy=0,  reduce_sidechannel=1   (this mode no
longer used)
...
int inner_loop(lame_global_flags *gfp, FLOAT8 xrpow[576], int l3_enc[576],
               int max_bits, gr_info *cod_info)
{
  int bits;
  assert(max_bits >= 0);
...
int scale_bitcount(III_scalefac_t *scalefac, gr_info *cod_info)
{
  ...
  static int slen1_tab[16] = {0,  18, 36, 54, 54, 36,  54,  72,
                              54, 72, 90, 72, 90, 108, 108, 126};
  static int slen2_tab[16] = {0,  10, 20, 30, 33, 21, 31, 41,
...
*/
static unsigned max_range_sfac_tab[6][4] = {{15, 15, 7, 7}, {15, 15, 7, 0},
                                            {7, 3, 0, 0},   {15, 31, 31, 0},
                                            {7, 7, 7, 0},   {3, 3, 0, 0}};
...
#ifdef DEBUG
  if (over) printf("---WARNING !! Amplification of some bands over limits\n");
#endif
  if (!over)
  {
...
/*-------------------------------------------------------------------------*/
int bin_search_StepSize2(lame_global_flags *gfp, int desired_rate, int start,
                         int *ix, FLOAT8 xrspow[576], gr_info *cod_info)
/*-------------------------------------------------------------------------*/
{
...
#define XRPOW_FTOI(src, dest) asm("fistpl %0 " : "=m"(dest) : "t"(src) : "st")
...
#define XRPOW_FTOI(src, dest)                         \
  ...
  {                                                   \
    FLOAT8 src_ = (src);                              \
    int dest_;                                        \
    {__asm fld src_ __asm fistp dest_}(dest) = dest_; \
  } while (0)
#else
...
void quantize_xrpow_ISO(FLOAT8 xr[576], int ix[576], gr_info *cod_info)
{
...
#if defined(USE_GNUC_ASM)
  {
    __asm__ __volatile__(
        ...
        : /* no outputs */
        : "r"(576 / 4), "u"((FLOAT8)(0.4054 - 0.5)), "t"(istep), "r"(xr),
          "r"(ix)
        : "memory", "cc");
  }
#elif defined(USE_MSC_ASM)
...
#endif
  {
    ...
    /* depending on architecture, it may be worth calculating a few more
       ...
       On a pentium166, it's only worth doing the one compare (as done here), as
       the second
