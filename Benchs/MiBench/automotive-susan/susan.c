...
<mpettove@umdsun2.umd.umich.edu>:
                      Use the FOPENB option below
...
#ifndef PPC
typedef int TOTAL_TYPE; /* this is faster for "int" but should be "float" for
                           large d masks */
#else
...
usage()
{
  ...
  printf(
      "-d <thresh> : Distance threshold, smoothing mode, (default=4) (use next "
      "option instead for flat 3x3 mask)\n");
  printf("-3 : Use flat 3x3 mask, edges or smoothing mode\n");
  printf(
      "-n : No post-processing on the binary edge map (runs much faster); "
      "edges mode\n");
  printf(
      "-q : Use faster (and usually stabler) corner mode; edge-like corner "
      "suppression not carried out; corners mode\n");
  printf(
      "-b : Mark corners/edges with single black points instead of black with "
      "white border; corners or edges mode\n");
  printf(
      "-p : Output initial enhancement image only; corners or edges mode "
      "(default is edges mode)\n");

  printf(
      "\nSUSAN Version 2l (C) 1995-1997 Stephen Smith, DRA UK. "
...
put_image(filename, in, x_size, y_size) char filename[100], *in;
int x_size, y_size;
{
  FILE *fd;
...
int_to_uchar(r, in, size) uchar *in;
int *r, size;
{
...
void setup_brightness_lut(bp, thresh, form) uchar **bp;
int thresh, form;
{
  int k;
  float temp;
...
susan_principle(in, r, bp, max_no, x_size, y_size) uchar *in, *bp;
...
susan_principle_small(in, r, bp, max_no, x_size, y_size) uchar *in, *bp;
int *r, max_no, x_size, y_size;
{
  int i, j, n;
  uchar *p, *cp;
...
uchar median(in, i, j, x_size) uchar *in;
int i, j, x_size;
{
  int p[8], k, l, tmp;
...
enlarge(in, tmp_image, x_size, y_size, border) uchar **in;
uchar *tmp_image;
  ...
  for (i = 0; i < border;
       i++) /* copy top and bottom rows; invert as many as necessary */
    {
      memcpy(tmp_image + (border - 1 - i) * (*x_size + 2 * border) + border,
             *in + i * *x_size, *x_size);
      memcpy(tmp_image + (*y_size + border + i) * (*x_size + 2 * border) + border,
...
void susan_smoothing(three_by_three, in, dt, x_size, y_size,
                     bp) int three_by_three,
    x_size, y_size;
uchar *in, *bp;
float dt;
{
  ...
  tmp_image =
      (uchar *)malloc((x_size + mask_size * 2) * (y_size + mask_size * 2));
...
edge_draw(in, mid, x_size, y_size, drawing_mode) uchar *in, *mid;
...
susan_thin(r, mid, x_size, y_size) uchar *mid;
int *r, x_size, y_size;
{
          ...
          if (mid[i * x_size + j] < 8)
            {
              ...
              /* extension is only allowed a few times - the value of mid is used to
         * control this */
...
susan_edges(in, r, mid, bp, max_no, x_size, y_size) uchar *in, *bp, *mid;
int *r, max_no, x_size, y_size;
{
  float z;
...
susan_edges_small(in, r, mid, bp, max_no, x_size, y_size) uchar *in, *bp, *mid;
int *r, max_no, x_size, y_size;
{
  float z;
  int do_symmetry, i, j, m, n, a, b, x, y, w;
...
corner_draw(in, corner_list, x_size, drawing_mode) uchar *in;
...
susan_corners(in, r, bp, max_no, corner_list, x_size, y_size) uchar *in, *bp;
int *r, max_no, x_size, y_size;
CORNER_LIST corner_list;
{
...
susan_corners_quick(in, r, bp, max_no, corner_list, x_size, y_size) uchar *in,
    *bp;
int *r, max_no, x_size, y_size;
CORNER_LIST corner_list;
{
              ...
              if ((x > r[(i - 1) * x_size + j + 2]) && (x > r[(i)*x_size + j + 1]) &&
                ...
                if ((x > r[(i - 3) * x_size + j - 3]) &&
...
#endif
                  {
                    ...
                    corner_list[n].x = j;
                    corner_list[n].y = i;
                    x = in[(i - 2) * x_size + j - 2] + in[(i - 2) * x_size + j - 1] +
                        in[(i - 2) * x_size + j] + in[(i - 2) * x_size + j + 1] +
                        in[(i - 2) * x_size + j + 2] + in[(i - 1) * x_size + j - 2] +
                        in[(i - 1) * x_size + j - 1] + in[(i - 1) * x_size + j] +
                        in[(i - 1) * x_size + j + 1] + in[(i - 1) * x_size + j + 2] +
                        in[(i)*x_size + j - 2] + in[(i)*x_size + j - 1] +
                        in[(i)*x_size + j] + in[(i)*x_size + j + 1] +
                        in[(i)*x_size + j + 2] + in[(i + 1) * x_size + j - 2] +
                        in[(i + 1) * x_size + j - 1] + in[(i + 1) * x_size + j] +
                        in[(i + 1) * x_size + j + 1] + in[(i + 1) * x_size + j + 2] +
                    ...
                    /*corner_list[n].I=in[i*x_size+j];*/
                    x = in[(i - 2) * x_size + j + 2] + in[(i - 1) * x_size + j + 2] +
                        in[(i)*x_size + j + 2] + in[(i + 1) * x_size + j + 2] +
                        in[(i + 2) * x_size + j + 2] -
                        (in[(i - 2) * x_size + j - 2] + in[(i - 1) * x_size + j - 2] +
                         in[(i)*x_size + j - 2] + in[(i + 1) * x_size + j - 2] +
                    ...
                    x += x + in[(i - 2) * x_size + j + 1] +
                         in[(i - 1) * x_size + j + 1] + in[(i)*x_size + j + 1] +
                         in[(i + 1) * x_size + j + 1] + in[(i + 2) * x_size + j + 1] -
                         (in[(i - 2) * x_size + j - 1] + in[(i - 1) * x_size + j - 1] +
                          in[(i)*x_size + j - 1] + in[(i + 1) * x_size + j - 1] +
                          in[(i + 2) * x_size + j - 1]);

                    y = in[(i + 2) * x_size + j - 2] + in[(i + 2) * x_size + j - 1] +
                        in[(i + 2) * x_size + j] + in[(i + 2) * x_size + j + 1] +
                        in[(i + 2) * x_size + j + 2] -
                        (in[(i - 2) * x_size + j - 2] + in[(i - 2) * x_size + j - 1] +
                         in[(i - 2) * x_size + j] + in[(i - 2) * x_size + j + 1] +
...
char *argv[];
{
  ...
  uchar *in, *bp, *mid;
  float dt = 4.0;
  int *r, argindex = 3, bt = 20, principle = 0, thin_post_proc = 1,
          three_by_three = 0, drawing_mode = 0, susan_quick = 0,
          max_no_corners = 1850, max_no_edges = 2650, mode = 0, i, x_size,
          y_size;
  CORNER_LIST corner_list;
      ...
      case 2:
        ...
        if (principle)
        ...
        else
          {
            if (susan_quick)
              {
                susan_corners_quick(in, r, bp, max_no_corners, corner_list, x_size,
                                    y_size);
              }
            else
              {
