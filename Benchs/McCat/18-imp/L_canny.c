...
int L_canny(sigma, image, nc, nr, edge, err) float sigma;
float *image;
int nc;
int nr;
float **edge;
...
int GaussianMask(sigma, tg, mask, err) float sigma;
int *tg;
float **mask;
char *err;
{
...
int DGaussianMask(sigma, tg, mask, err) float sigma;
int *tg;
float **mask;
char *err;
{
...
int dfilter(image, g, f, tm_g, tm_f, nc, nr, filt, err) float *image, *g, *f;
int nc, nr, tm_g, tm_f;
float **filt;
char *err;
{
...
float *dnon_max(gr, Ix, Iy, nc, nr) float *gr;
float *Ix;
float *Iy;
int nc;
int nr;
