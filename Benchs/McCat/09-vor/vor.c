...
point compute_v(CHpoints *P)
{
  ...
  maxY = 10000.0;
  An.x = 1;
  An.y = 0; /* Setup normalvectores for the border lines */
  Bn.x = 0;
  Bn.y = 1;
  Cn.x = 1;
  Cn.y = 0;
...
CHpoints *maximize_radius_and_angle(CHpoints *S)
{
  ...
  p1 = before(p2);
  p3 = next(p2);
  key1.radius = radius2(p1->node, centre(before(p1)->node, p1->node, p2->node));
  key1.angle = angle(before(p1), p1, p2);
  key1.number = p1->number;
  CHdelete(&CHSplaytree, key1); /* delete before(p) */
  key2.radius = radius2(p3->node, centre(p2->node, p3->node, next(p3)->node));
