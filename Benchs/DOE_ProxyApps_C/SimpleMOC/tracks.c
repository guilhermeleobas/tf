...
void generate_2D_segments(Input I, Track2D *tracks, size_t *nbytes)
{
  ...
  Segment *contiguous_segments =
      (Segment *)malloc(total_segments * sizeof(Segment));
  ...
  for (long i = 0; i < I.ntracks_2D; i++)
    {
      for (long j = 0; j < tracks[i].n_segments; j++)
        {
          tracks[i].segments[j].length =
              urand() * I.assembly_width / tracks[i].n_segments;
...
Track ***generate_tracks(Input I, Track2D *tracks_2D, size_t *nbytes)
{
  ...
  *nbytes += I.ntracks_2D * sizeof(Track **);
  ...
  Track **tracks_in_track2D =
      (Track **)malloc(I.ntracks_2D * I.n_polar_angles * sizeof(Track *));
  ...
  for (long i = 0; i < I.ntracks_2D; i++)
    {
      for (int j = 0; j < I.n_polar_angles; j++)
        {
          tracks[i][j] = &track_data[(i * I.n_polar_angles + j) * I.z_stacked];
        }
    }
...
void free_tracks(Track ***tracks) { free(tracks); }
// assign polar angles
...
Track2D *load_OpenMOC_tracks(char *fname, bool CMFD_obj, Input *I,
                             size_t *nbytes)
{
  int ret;
  FILE *in;
  ...
  *nbytes += I->ntracks_2D * sizeof(Track2D);
  ...
  // Allocate contiguous space for segments
  Segment *contiguous_segments =
      (Segment *)malloc(tot_num_segments * sizeof(Segment));
