...
#pragma simd_level(10)
  ...
  for (int g = 0; g < I.n_egroups; g++)
  {
    ...
    flux_integral[g] =
        (q0[g] * tau[g] + (sigT[g] * psi[g] - q0[g]) * expVal[g]) / sigT2[g] +
        q1[g] * mu * reuse[g] +
        q2[g] * mu2 *
...
#pragma omp parallel default(none) shared( \
    I, params, node_delta_z, fine_delta_z) reduction(+ : segments_processed)
  {
#ifdef OPENMP
...
// pick a random FSR (cache miss expected)
              ...
              if (I->axial_exp == 2)
              {
                attenuate_fluxes(track, true, &params->sources[QSR_id], I,
              ...
              {
                attenuate_FSR_fluxes(track, true, &params->sources[QSR_id], I,
                                     params, ds, mu,
...
#pragma omp parallel default(none)                \
    shared(I, params, node_delta_z, fine_delta_z, \
           num_intervals) reduction(+ : segments_processed)
  {
#ifdef OPENMP
    int thread = omp_get_thread_num();
    int nthreads = omp_get_num_threads();
...
// pick a random FSR (cache miss expected)
#ifdef OPENMP
              QSR_id = rand_r(&seed) % I->n_source_regions_per_node;
#else
        ...
        for (int k = 0; k < I->z_stacked; k++)
        {
          for (int n = seg_idx[k] - 1; n >= 0; n--)
          {
            ...
            if (I->axial_exp == 2)
            {
              attenuate_fluxes(track, false, seg_src[k][n], I, params, ds, -mu,
                               params->tracks_2D[i].az_weight, &A);
              segments_processed++;
            }
            ...
            {
              attenuate_FSR_fluxes(track, false, seg_src[k][n], I, params, ds,
                                   -mu, params->tracks_2D[i].az_weight, &A);
            }
...
int get_neg_interval(float z, float dz)
{
  ...
  int interval = INT_MAX - (int)((double)INT_MAX - (double)(z / dz));
  return interval;
}
                          ...
                          Params *params, float ds, float mu, float az_weight)
{
  ...
  for (int g = 0; g < I->n_egroups; g++)
  {
    ...
    // add contribution to new source flux
    float flux_integral =
        (q0 * tau + (sigT * psi[g] - q0) * expVal) / sigT2 +
        q1 * mu * (tau * (tau - 2) + 2 * expVal) / (sigT * sigT2) +
...
#pragma omp atomic
    ...
    psi[g] = psi[g] * (1.0 - expVal) + q0 * expVal / sigT +
             q1 * mu * (tau - expVal) / sigT2 +
             q2 * mu2 * (tau * (tau - 2) + 2 * expVal) / (sigT2 * sigT);
  }
}
...
void attenuate_FSR_fluxes(Track *track, bool forward, Source *FSR, Input *I,
                          Params *params_in, float ds, float mu,
                          float az_weight, AttenuateVars *A)
{
...
#pragma omp for schedule(dynamic)
    for (int i = 0; i < I.n_source_regions_per_node; i++)
    {
      ...
      for (int j = 0; j < I.fai; j++)
      {
        for (int g = 0; g < I.n_egroups; g++)
        {
          g_fission_rates[g] = src.fine_flux[j][g] * src.vol * src.XS[g][0];
        }
    ...
    node_fission_rate =
        pairwise_sum(fission_rates, I.n_source_regions_per_node);
...
#pragma omp parallel for default(none) shared(I, params) private(norm_factor) \
                                                  schedule(dynamic)
  for (int i = 0; i < I.n_source_regions_per_node; i++)
  {
    Source *src = &params.sources[i];
...
#pragma omp parallel for default(none) shared(I, params) private(norm_factor) \
  ...
  if (I.mype == 0)
  {
    printf("Renormalizing Flux Complete.\n");
  }
  return;
}
...
float update_sources(Params params, Input I, float keff)
{
  ...
  float *residuals =
      (float *)malloc(I.n_source_regions_per_node * sizeof(float));
    ...
    for (int j = 0; j < I.fai; j++)
    {
      ...
      for (int g = 0; g < I.n_egroups; g++)
      {
        for (int g2 = 0; g2 < I.n_egroups; g2++)
        {
          ...
          scatter_rates[g2] =
              src.scattering_matrix[g][g2] * src.fine_flux[j][g2];
        }
        ...
        group_res[g] =
            (newSrc - oldSrc) * (newSrc - oldSrc) / (oldSrc * oldSrc);
