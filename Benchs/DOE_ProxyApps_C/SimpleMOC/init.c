...
void calculate_derived_inputs(Input* I)
{
  ...
  I->ntracks_2D =
      I->n_azimuthal * (I->assembly_width * sqrt(2) / I->radial_ray_sep);
  ...
  I->n_source_regions_per_node =
      I->n_2D_source_regions_per_assembly * I->cai / I->decomp_assemblies_ax;
}
...
omp_lock_t* init_locks(Input I)
{
  ...
  for (long i = 0; i < n_locks; i++) omp_init_lock(&locks[i]);
