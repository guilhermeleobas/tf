...
Source *initialize_sources(Input I, size_t *nbytes)
{
  // Allocate space
  Source *sources =
      (Source *)malloc(I.n_source_regions_per_node * sizeof(Source));
  ...
  *nbytes += n_xs_regions * sizeof(float **);
  ...
  float **s_matrix_ptrs =
      (float **)malloc(n_xs_regions * I.n_egroups * sizeof(float *));
  *nbytes += n_xs_regions * sizeof(float **);
...
// Allocate space for ALL scattering data
  ...
  if (I.mype == 0)
    printf(
        "Scattering data requires %zu MB of data...\n",
        n_xs_regions * I.n_egroups * I.n_egroups * sizeof(float) / 1024 / 1024);
#endif
  float *s_matrix_data =
      (float *)malloc(n_xs_regions * I.n_egroups * I.n_egroups * sizeof(float));
  ...
  for (long i = 0; i < n_xs_regions; i++)
    {
      for (long j = 0; j < I.n_egroups; j++)
        {
          s_matrices[i][j] =
              &s_matrix_data[i * I.n_egroups * I.n_egroups + j * I.n_egroups];
        }
    }
  ...
  *nbytes += n_xs_regions * I.n_egroups * sizeof(float *);
  ...
  float *XS_data =
      (float *)malloc(n_xs_regions * I.n_egroups * 3 * sizeof(float));
  *nbytes += n_xs_regions * I.n_egroups * 3 * sizeof(float);
  ...
  if (I.mype == 0)
    {
      printf("Beginning Source and Flux Parameter Allocation...\n");
    }
  ...
  *nbytes += I.n_source_regions_per_node * sizeof(float **);
  ...
  float **fineSourcePtrs =
      (float **)malloc(I.n_source_regions_per_node * I.fai * sizeof(float *));
  ...
  *nbytes += I.n_source_regions_per_node * I.fai * sizeof(float *);
  ...
  float *data = (float *)malloc((2 * I.fai + 1) * I.n_source_regions_per_node *
  ...
  for (long i = 0; i < I.n_source_regions_per_node; i++)
    {
      for (long j = 0; j < I.fai; j++)
        {
          fineSource[i][j] = &data[i * I.fai * I.n_egroups + j * I.n_egroups];
        }
    }
  ...
  for (long i = 0; i < I.n_source_regions_per_node; i++)
    {
      for (int j = 0; j < I.fai; j++)
        {
          fineFlux[i][j] =
              &data[I.n_egroups * I.fai * (I.n_source_regions_per_node + i) +
                    j * I.n_egroups];
        }
    }
