...
float urand(void) { return (float)rand() / (float)RAND_MAX; }
...
size_t est_mem_usage(Input I)
{
  ...
  nbytes += I.ntracks_2D * I.n_polar_angles * z_stacked * I.n_egroups *
            sizeof(float) * 2;
  ...
  nbytes += I.nthreads * I.z_stacked * sizeof(int);
  nbytes +=
      I.nthreads * I.z_stacked * 2 * I.segments_per_track * sizeof(double);
  nbytes +=
      I.nthreads * I.z_stacked * 2 * I.segments_per_track * sizeof(Source *);
...
double time_per_intersection(Input I, double time)
{
  ...
  double tpi = time / (double) I.ntracks /
          ...
          */
  double tpi =
      time / (double)I.segments_processed * 1.0e9 / (double)I.n_egroups;
  return tpi;
}
