...
void fast_transfer_boundary_fluxes(Params params, Input I, CommGrid grid)
{
  ...
  if (I.mype == 0)
    printf("MPI Message Size: %.2lf (MB)\n", bytes / 1024. / 1024.);
#endif
  ...
  int send_dest[6] = {grid.x_pos_dest, grid.x_neg_dest, grid.y_pos_dest,
                      grid.y_neg_dest, grid.z_pos_dest, grid.z_neg_dest};
  // make an array of radial receiving sources
  int rec_sources[6] = {grid.x_pos_src, grid.x_neg_src, grid.y_pos_src,
                        grid.y_neg_src, grid.z_pos_src, grid.z_neg_src};

  // make an array of number of messages
  // NOTE: There is some rounding here, should be corrected in real app
  long num_messages[6] = {ntracks_per_radial_direction / tracks_per_msg,
  ...
  for (int i = 0; i < 6; i++)
    if (num_messages[i] > max_msgs_per_dir) max_msgs_per_dir = num_messages[i];

  // Flux Memory is assumed to be laid out as follows:
  // (Border Flux) --- (Send_MSG_Dir_0) (Recv_MSG_Dir_0) (Send_MSG_Dir_1)
  // (Recv_MSG_Dir_1)....

  // New Comms
  float **buffer = (float **)malloc(6 * sizeof(float *));
  float *_buffer =
      (float *)malloc(6 * tracks_per_msg * I.n_egroups * sizeof(float));
  for (int i = 0; i < 6; i++)
