...
void papi_serial_init(void)
{
  ...
  if ((PAPI_thread_init((long unsigned int (*)(void))pthread_self)) != PAPI_OK)
  {
 ...
 *num_papi_events = sizeof(events) / sizeof(int);
 ...
 if ((stat = PAPI_thread_init((long unsigned int (*)(void)) omp_get_thread_num))
!= PAPI_OK){
 PAPI_perror("PAPI_thread_init");
 exit(1);
 }
...
#pragma omp master
  {
    ...
    for (int i = 0; i < num_papi_events; i++) I->vals_accum[i] = 0;
  }
#pragma omp barrier
...
#pragma omp critical(papi)
  {
    ...
    for (int i = 0; i < num_papi_events; i++)
    {
      ...
      if (strcmp(info.symbol, "PAPI_L3_TCM") == 0) LLC_cache_miss += values[i];
      if (strcmp(info.symbol, "PAPI_TOT_CYC") == 0) total_cycles += values[i];
      if (strcmp(info.symbol, "PAPI_SP_OPS") == 0) FLOPS += values[i];
      if (strcmp(info.symbol, "RESOURCE_STALLS:ANY") == 0)
        stall_any += values[i];
      if (strcmp(info.symbol, "RESOURCE_STALLS:SB") == 0) stall_SB += values[i];
      if (strcmp(info.symbol, "RESOURCE_STALLS:RS") == 0) stall_RS += values[i];
      if (strcmp(info.symbol, "RESOURCE_STALLS2:OOO_RSRC") == 0)
        stall_OO += values[i];
      if (strcmp(info.symbol, "perf::DTLB-LOADS") == 0) tlb_load += values[i];
      if (strcmp(info.symbol, "perf::DTLB-LOAD-MISSES") == 0)
        tlb_load_m += values[i];
      if (strcmp(info.symbol, "perf::DTLB-STORES") == 0) tlb_store += values[i];
...
#pragma omp master
  {
    ...
    if (I->papi_event_set == 1) printf("Bandwidth: %.3lf (GB/s)\n", bw);
    if (I->papi_event_set == 2)
    {
      printf("%-30s %.2lf%%\n", "Store Buffer Full:",
