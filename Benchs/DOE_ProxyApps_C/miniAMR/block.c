...
{
  static int side[6][2][2] = {{{0, 2}, {4, 6}}, {{1, 3}, {5, 7}}, {{0, 1}, {4, 5}}, {{2, 3}, {6, 7}}, {{0, 1}, {2, 3}}, {{4, 5}, {6, 7}}};
  static int off[6] = {1, -1, 2, -2, 4, -4};
  int i, j, k, m, n, o, v, nl, xp, yp, zp, c, c1, other, i1, i2, j1, j2, k1, k2,
                  ...
                  for (o = 0; o < 8; o++)
                    {
                      ...
                      k1 = (o / 4);
                      bp1->number =
                          ((2 * zp + k1) * (p2[level + 1] * npy * init_block_y) +
                           (2 * yp + j1)) *
                              (p2[level + 1] * npx * init_block_x) +
                          2 * xp + i1 + block_start[level + 1];
                      add_sorted_list(m, bp1->number, (level + 1));
                      bp1->cen[0] =
                          bp->cen[0] + (2 * i1 - 1) * p2[num_refine - level - 1];
                      bp1->cen[1] =
                          bp->cen[1] + (2 * j1 - 1) * p2[num_refine - level - 1];
                      bp1->cen[2] =
                          bp->cen[2] + (2 * k1 - 1) * p2[num_refine - level - 1];
                              ...
                              for (j2 = j = 1; j <= y_block_half; j++, j2 += 2)
                                {
                                  for (k2 = k = 1; k <= z_block_half; k++, k2 += 2)
                                    {
                                      bp1->array[v][i2][j2][k2] = bp1->array[v][i2 + 1][j2][k2] =
                                          bp1->array[v][i2][j2 + 1][k2] = bp1->array[v][i2 + 1]
                                                                                    [j2 + 1][k2] =
                                              bp1->array[v][i2][j2][k2 + 1] =
                                                  bp1->array[v][i2 + 1][j2][k2 + 1] =
                                                      bp1->array[v][i2][j2 + 1][k2 + 1] =
                                                          bp1->array[v][i2 + 1][j2 + 1][k2 + 1] =
                                                              bp->array[v][i + i1][j + j1]
                                                                       [k + k1] /
                                                              8.0;
                                    }
                                }
                      ...
                      for (i = 0; i < 2; i++)
                        {
                          for (j = 0; j < 2; j++)
                            {
                              blocks[sib[side[c][i][j] + off[c]]].nei_level[c] = level + 1;
                      ...
                      if (bp->nei_level[c] == -2)
                        ...
                        {  // level less parent
                          if (bp->nei[c][0][0] >= 0)
                            {  // error
                              printf(
                                  "%d ERROR: internal misconnect block %d num %d c %d %d\n",
                      ...
                      else
                        {
                          printf("%d ERROR: misconnected b %d %d l %d nei[%d] %d\n", my_pe,
                                 n, bp->number, level, c, bp->nei_level[c]);
                          exit(-1);
                        }
                    }
...
{
  static int side[6][2][2] = {{{0, 2}, {4, 6}}, {{1, 3}, {5, 7}}, {{0, 1}, {4, 5}}, {{2, 3}, {6, 7}}, {{0, 1}, {2, 3}}, {{4, 5}, {6, 7}}};
  int n, p, i, j, k, i1, j1, k1, i2, j2, k2, level, o, v, f, c, offset, other,
      c1, dir, fcase, pe, nl, pos[3], d, in;
  block *bp, *bp1;
  parent *pp;

  if (stencil == 7)
    {  // add to face case when diags are needed
      f = 0;
    }
  else
    {
