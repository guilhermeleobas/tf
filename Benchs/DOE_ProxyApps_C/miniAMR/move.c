...
void check_objects(void)
{
   ...
   *    * types of objects: spheroid, solid spheroid, rectangle, solid
   * rectangle,
  ...
  for (in = 0; in < sorted_index[num_refine + 1]; in++)
  {
    ...
    if ((bp = &blocks[n])->number >= 0)
    {
      ...
      else if (refine_ghost && bp->level)
      {
        ...
        cor[0][0] -=
            (((double)sz) / ((double)x_block_size)) / ((double)mesh_size[0]);
        cor[0][1] +=
            (((double)sz) / ((double)x_block_size)) / ((double)mesh_size[0]);
        cor[1][0] -=
            (((double)sz) / ((double)y_block_size)) / ((double)mesh_size[1]);
        cor[1][1] +=
            (((double)sz) / ((double)y_block_size)) / ((double)mesh_size[1]);
        cor[2][0] -=
            (((double)sz) / ((double)z_block_size)) / ((double)mesh_size[2]);
        cor[2][1] +=
            (((double)sz) / ((double)z_block_size)) / ((double)mesh_size[2]);
        if (check_block(cor))
        {
...
{
  int o, tmp, ca, c1, c2, intersect, xc, xv, yc, yv, zc,
      zv; /* where is center of object to block */
  object *op;
  ...
  {
    op = &objects[o];
    if (intersect > 0 || op->size[0] < 0.0 || op->size[1] < 0.0 ||
        op->size[2] < 0)
    {
            ...
            if (zc)
            {
              if ((((cor[c1][1 - yv] - op->cen[c1]) / op->size[c1]) *
                   ...
                   ((cor[c2][1 - zv] - op->cen[c2]) / op->size[c2]) *
                       ((cor[c2][1 - zv] - op->cen[c2]) / op->size[c2])) > 1.0)
              {
                intersect = 1;
              }
            }
