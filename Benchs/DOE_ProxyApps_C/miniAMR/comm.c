...
void comm(int start, int num_comm, int stage)
{
  ...
  int permutations[6][3] = {{0, 1, 2}, {1, 2, 0}, {2, 0, 1}, {0, 2, 1}, {1, 0, 2}, {2, 1, 0}};
  double t1, t2, t3, t4;
  block *bp;
...
void on_proc_comm_diff(int n, int n1, int l, int iq, int jq, int start,
                       int num_comm)
{
  int i, j, k, m;
          ...
          for (m = start; m < start + num_comm; m++)
            {
              ...
              for (k = 1; k <= z_block_half; k++)
                {
                  bp1->array[m][i2][0][2 * k - 1] = bp1->array[m][i2][0][2 * k] =
                      bp->array[m][i1][j1][k + k1] / 4.0;
                }
              ...
              if (jq == 0)
                {
                  ...
                  for (k = 1; k <= z_block_half; k++)
                    {
                      bp->array[m][i0][0][k + k1] =
                          (bp1->array[m][i3][0][2 * k - 1] + bp1->array[m][i3][0][2 * k]);
                    }
                }
              for (j = 1; j <= y_block_half; j++)
                {
                  bp1->array[m][i2][2 * j - 1][0] = bp1->array[m][i2][2 * j][0] =
                      bp->array[m][i1][j + j1][k1] / 4.0;
                  if (iq == 0)
                    {
                      bp->array[m][i0][j + j1][0] =
                          (bp1->array[m][i3][2 * j - 1][0] + bp1->array[m][i3][2 * j][0]);
                    }
                  else
                    {
                  ...
                  bp1->array[m][i2][2 * j - 1][k2] = bp1->array[m][i2][2 * j][k2] =
                      bp->array[m][i1][j + j1][k3 + k1] / 4.0;
                }
              bp1->array[m][i2][j2][0] = bp->array[m][i1][j3 + j1][k1] / 4.0;
              for (k = 1; k <= z_block_half; k++)
                {
                  bp1->array[m][i2][j2][2 * k - 1] = bp1->array[m][i2][j2][2 * k] =
                      bp->array[m][i1][j3 + j1][k + k1] / 4.0;
                }
          ...
          for (m = start; m < start + num_comm; m++)
            {
              ...
              for (k = 1; k <= z_block_half; k++)
                {
                  bp1->array[m][0][j2][2 * k - 1] = bp1->array[m][0][j2][2 * k] =
                      bp->array[m][i1][j1][k + k1] / 4.0;
                }
              ...
              if (jq == 0)
                {
                  ...
                  for (k = 1; k <= z_block_half; k++)
                    {
                      bp->array[m][0][j0][k + k1] =
                          (bp1->array[m][0][j3][2 * k - 1] + bp1->array[m][0][j3][2 * k]);
                    }
                }
              for (i = 1; i <= x_block_half; i++)
                {
                  bp1->array[m][2 * i - 1][j2][0] = bp1->array[m][2 * i][j2][0] =
                      bp->array[m][i + i1][j1][k1] / 4.0;
                  if (iq == 0)
                    {
                      bp->array[m][i + i1][j0][0] =
                          (bp1->array[m][2 * i - 1][j3][0] + bp1->array[m][2 * i][j3][0]);
                    }
                  else
                    {
                  ...
                  bp1->array[m][2 * i - 1][j2][k2] = bp1->array[m][2 * i][j2][k2] =
                      bp->array[m][i + i1][j1][k3 + k1] / 4.0;
                }
              bp1->array[m][i2][j2][0] = bp->array[m][i3 + i1][j1][k1] / 4.0;
              for (k = 1; k <= z_block_half; k++)
                {
                  bp1->array[m][i2][j2][2 * k - 1] = bp1->array[m][i2][j2][2 * k] =
                      bp->array[m][i3 + i1][j1][k + k1] / 4.0;
                }
          ...
          for (m = start; m < start + num_comm; m++)
            {
              ...
              for (j = 1; j <= y_block_half; j++)
                {
                  bp1->array[m][0][2 * j - 1][k2] = bp1->array[m][0][2 * j][k2] =
                      bp->array[m][i1][j + j1][k1] / 4.0;
                }
              ...
              if (jq == 0)
                {
                  ...
                  for (j = 1; j <= y_block_half; j++)
                    {
                      bp->array[m][0][j + j1][k0] =
                          (bp1->array[m][0][2 * j - 1][k3] + bp1->array[m][0][2 * j][k3]);
                    }
                }
              for (i = 1; i <= x_block_half; i++)
                {
                  bp1->array[m][2 * i - 1][0][k2] = bp1->array[m][2 * i][0][k2] =
                      bp->array[m][i + i1][j1][k1] / 4.0;
                  if (iq == 0)
                    {
                      bp->array[m][i + i1][0][k0] =
                          (bp1->array[m][2 * i - 1][0][k3] + bp1->array[m][2 * i][0][k3]);
                    }
                  else
                    {
                  ...
                  bp1->array[m][2 * i - 1][j2][k2] = bp1->array[m][2 * i][j2][k2] =
                      bp->array[m][i + i1][j3 + j1][k1] / 4.0;
                }
              bp1->array[m][i2][0][k2] = bp->array[m][i3 + i1][j1][k1] / 4.0;
              for (j = 1; j <= y_block_half; j++)
                {
                  bp1->array[m][i2][2 * j - 1][k2] = bp1->array[m][i2][2 * j][k2] =
                      bp->array[m][i3 + i1][j + j1][k1] / 4.0;
                }
      ...
      switch (l)
        {
          case 1:
            ...
            f = x_block_size;
          case 0:
            for (var = start; var < start + num_comm; var++)
              {
                for (j = 1; j <= y_block_size; j++)
                  {
                    for (k = 1; k <= z_block_size; k++)
