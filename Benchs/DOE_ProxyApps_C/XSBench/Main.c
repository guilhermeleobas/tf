...
// Prepare Unionized Energy Grid Framework
  ...
  GridPoint *energy_grid =
      generate_energy_grid(in.n_isotopes, in.n_gridpoints, nuclide_grids);
#else
  GridPoint *energy_grid =
      (GridPoint *)malloc(in.n_isotopes * in.n_gridpoints * sizeof(GridPoint));
...
#pragma omp parallel default(none) private(                            \
    i, thread, p_energy, mat, seed) shared(max_procs, in, energy_grid, \
                                           nuclide_grids, mats, concs, \
                                           num_nucs, mype, vhash)
#endif
    {
...
#ifdef VERIFICATION
        char line[256];
        sprintf(line, "%.5lf %d %.5lf %.5lf %.5lf %.5lf %.5lf", p_energy, mat,
                macro_xs_vector[0], macro_xs_vector[1], macro_xs_vector[2],
                macro_xs_vector[3], macro_xs_vector[4]);
