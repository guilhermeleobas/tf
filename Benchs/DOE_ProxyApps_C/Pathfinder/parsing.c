...
void buildEntranceList(Node *outerNode, IntVector *nodeData)
{
  ...
  if (!outerNode || !nodeData)
  {
    return;
  }

  entranceCount = nodeData->vector[1]; /* count is the first entry, (second of
                                          original data); */
  if (entranceCount == 0)
  {
  ...
  {
    fprintf(stderr,
            "Specified entrance edge length does not match data size for node: "
            "%d\n",
            outerNode->id);
  }
...
Node *buildNodeFromData(IntVector *nodeData)
{
  ...
  Node *node =
      Node_new(nodeData->vector[0], -1); /* Calling method must set index. */
  if (!node)
  {
    return (NULL);
  }
  ...
  {
    fprintf(stderr,
            "Specified edge length does not match data size for node: %d\n",
            nodeData->vector[0]);
  }
...
void resolveNodeEdges(Graph *graph, Node *sourceNode, bool outerNode)
{
  ...
  for (edges = sourceNode->edges; edges != NULL; edges = edges->nextEdge)
  {
    if (outerNode)
    {
      ...
      node = Graph_findNode(graph, edges->targetNodeId,
                            true); /* true-> check outer and interior nodes */
      if (node)
      {
        okSoFar = true;
        /* Since entrance edges are now part of our edge list, we have to make
         * sure
        ...
        if (node->type == entranceNode || node->type == interiorNode)
        {
          ...
          {
            fprintf(stderr,
                    "resolveNodeEdges: Outer node %d edge to non-contained "
                    "node: %d\n",
                    sourceNode->id, edges->targetNodeId);
            fprintf(stderr, "\t%d is type: %s (%d)\n", node->id,
                    node->type == interiorNode ? "interior" : "entrance",
                    (int)node->type);
            okSoFar = false;
          }
        }
      ...
      {
        fprintf(stderr,
                "resolveNodeEdges: Outer node %d has edge reference to "
                "undefined node: %d\n",
                sourceNode->id, edges->targetNodeId);
      }
    }
    else
    {
      ...
      node = Graph_findNode(graph, edges->targetNodeId,
                            false); /* false-> only check outer nodes */
      if (node)
      {
        edges->targetNode = node;
      }
      else
      {
        ...
        node =
            Graph_findContainedNode(sourceNode->container, edges->targetNodeId);
        if (node)
        {
          edges->targetNode = node;
        }
        else
        {
          ...
          fprintf(stderr,
                  "resolveNodeEdges: Interior node %d has edge reference to "
                  "undefined node: %d\n",
                  sourceNode->id, edges->targetNodeId);
        }
      }
...
Graph *parseFile(char *fileName)
{
  ...
  Graph *newGraph = NULL;
  int funcCount =
      50; /* used as a default initial size for the system call table */
  int blockCount = 0;
  ...
  fileString =
      CharVector_new(1024); /* allocate our data parsing storage - 1024 is a
                               semi-arbitrary power of 2 */
  ...
  {
    fprintf(stderr,
            "Malformed file, Function count not specified in first line\n");
    return (NULL);
  }
  ...
  fileData = IntVector_new(8); /* allocate our data parsing storage - 8 is a
                                  semi-arbitrary power of 2 */
  ...
  while (!feof(inFile)) /* clumsy use of feof */
  {
    ...
    if (feof(inFile))
    { /* that read line was the last line */
      break;
    }

    if (strncmp("----------", fileString->string, 10) == 0)
  ...
  while (!feof(inFile))
  {
    /* The first line will identify the outer node for which the inner nodes are
     * defined. */
    CharVector_getLineFromFile(fileString, inFile);

    if (feof(inFile))
    { /* that fgets was the last line */
      break;
    }
    ...
    outerNode = Graph_findNode(newGraph, temp,
                               false); /* false -> only check outer nodes */

    if (outerNode == NULL)
    {
      fprintf(stderr,
              "Error in parseFile: Node %d undefined as an outer node\n", temp);
    ...
    while (!feof(inFile))
    {
      ...
      {
        /*
         * special case: If the node specified is the outerNode, the edge nodes
         * specified
  ...
  /* All the node mappings have been done, now we build a map of system calls to
   * the
   * node keys.
   */
  ...
  if (systemCallSection)
  {
    while (!feof(inFile))
    {
      ...
      if (temp >= 0) /* valid key */
      {
        ...
        for (i = 0;
             fileString->string[i] != '\0' && fileString->string[i] != ' '; ++i)
        {
        }
        if (fileString->string[i] == ' ')
        {
          ...
          SystemCallMap_insert(newGraph->systemCallMap, fileString->string + i,
                               findNode);
        }
      }
    }
  ...
  for (outerNodeList = newGraph->outerNodes; outerNodeList != NULL;
       outerNodeList = outerNodeList->nextNode)
  {
    resolveNodeEdges(newGraph, outerNodeList->node,
                     true); /* true -> outer node */
    for (innerNodeList = outerNodeList->node->interiorNodes;
         innerNodeList != NULL; innerNodeList = innerNodeList->nextNode)
    {
      resolveNodeEdges(newGraph, innerNodeList->node,
                       false); /* false -> interior node */
    }
  }
  ...
  newGraph->searchDiagram =
      SearchDiagram_build(newGraph->outerNodes, newGraph->totalNodes);
...
{
  int i = 0;
  int labelCount = 0;  // Used to be 1. The algorithm depends on a terminating
                       // space, so it counts based on the END of the label.
  int copiedLabels = 0;
  char **labels = NULL;
  char *debug = NULL;
...
bool parseConfigFile(char *configFileName, Configuration *config)
{
  ...
  {
    printf(
        "Error in config file: doesn't start with \"Pathfinder "
        "Configuration\"\n");
    return false;
  }
  ...
  for (i = 0; i < sigNum; ++i)
  {
    fgets(data, 1000, configFile);
    if (feof(configFile) ||
        (strncmp("signature ", data, 10) != 0)) /* note the ' ' */
    {
