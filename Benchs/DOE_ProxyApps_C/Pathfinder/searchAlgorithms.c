...
/* findNextLabel is a recursive method that determines if a path exists between
 * a
 * specific node and the label that is at the start of the label array argument.
 * The
 * label array is an array of char pointers that is terminated with a NULL
 * pointer. The
 * first pointer is the "current" label.
 *
 * The node's edge nodes are checked against the current label. If none of them
 * match, then
 * this method recurses with each edge node and the current label.
 *
 * If an edge matches, then we are beginning a search for the next "leg" of the
 * labels.
 * We recurse with that specific edge and the "next" label. That is the [1]
 * element of
 * the label array. This starts the search along the next leg - at this point we
 * have
 * to start a new "result" vector, because we check that vector for loops. It
 * MAY be the
 * case that the next path segment passes through nodes already traversed in a
 * previous
 * leg. A new "result" vector will solve that.
 *
 * If the next element in the labels list is NULL, the matching edge signifies
 * the end of
 * our search! VICTORY!
 *
 * If we reach the last edge with out a match (directly or recursively) we
 * return a false
 * list.
 */

bool findNextLabel(Node *node, Signature labels, NodePtrVec *result,
                   Bitfield *visited) /* NodePtrVec *visited ) */
{
   ...
   *       rather than doing a bunch of inefficient strcmp calls. Another
   * approach
...
/* findAndRecordAllPaths is almost exactly like findNextLabel. It differs only
 * in that it
 * 1) doesn't stop with the first discovery of the specified path and 2) records
 * the discovered
...
void findAndRecordAllPaths(Node *node, Signature labels, int *labelIdxs,
                           ...
                           NodeVecVec *storage, SearchOptions *options)
{
  EdgeList *edge;
  /* NodePtrVec *nextLegVisited = NULL; */
  Bitfield *nextLegVisited = NULL;
...
/* findLabelPath takes an array of labels, determines if there are nodes
 * associated
 * with the labels (each label search will return a NodePtrVec*), and determine
 * if
 ...
 *
 * The labels passed in is an array of char pointers. The array is null
 * terminated -
 * i.e. the last valid entry in the array is followed by a pointer with NULL in
 * it.
 */
bool findLabelPath(Graph *graph, Signature labels, NodePtrVec *result,
                   SearchType searchType)
{
  NodePtrVec *startNodes =
      NULL; /* set of nodes with the first label - potential path start nodes */
  int i;
  bool found =
      false; /* stopping as soon as we can find a path to a final node */
   ...
   * to
   * iterate through the start nodes and see if they can get to the next
   * label...
   */
...
int findAllPossibleLegs(Graph *graph, SearchType searchType)
{
  ...
  //#pragma omp parallel private(i,j) shared(graph) reduction(+:found)
  //reduction(+:searches)
  //#pragma omp single
  {
 ...
 * that all the legs be logged requires a more sophisticated means of
 * parallelization in order to do a
...
#pragma omp single
#endif
    {
      ...
      lastingResults[maxThreads] =
          0; /* Null terminated to avoid having to keep track of bitsNeeded */
    }
...
#endif
    {
      lastingResults[myThread] = NodeVecVec_new(
          64); /* Arbitrary - multiple of 8, eventually cache line aligned... */
...
#pragma omp collapse(2)
  ...
  /* At some point, we will want to use the FullPath argument and pass it along
   * to
   * buildGraphFromPaths. Until then, however, we're only going to build the
   * most
   * minimal graph possible.
   */
