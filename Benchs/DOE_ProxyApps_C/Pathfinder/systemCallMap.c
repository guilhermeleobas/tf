...
bool SystemCallMap_insert(SystemCallMap *map, char *label, Node *node)
{
  ...
  if (map->contentSize == map->allocatedSize)
  {
    ...
    map->vector = realloc(
        map->vector, (map->allocatedSize * 2 * sizeof(SystemCallMapElement *)));
    if (map->vector)
    {
      map->allocatedSize *= 2;
    }
  ...
  node->labelIdx = newElement->index =
      map->contentSize; /* Does newElement really need the index? */
...
NodePtrVec *SystemCallMap_findLabeledNodes(SystemCallMap *systemCalls,
                                           char *label)
{
  int i;
