...
/*
 * A utility method that takes an input string to the next number. If it
 * currently
 * points to a number, we iterate past that number, past any whitespace or
 * non-number
 * and point to the first digit character we find. If input currently points a
 * non-
 * digit character, we find the next digit character and return a pointer to it.
 * If
...
/* Screams through the vector from bottom to top to find the specified element.
 * If found,
 * returns true.
 */
...
/* Screams through the vector from top to bottom to find the specified element.
 * If found,
 * returns true.  */
 ...
 * wants duplication?)
 */
void NodePtrVec_appendVectors(NodePtrVec *first, NodePtrVec *second,
                              bool keepFirst)
{
  int i;
