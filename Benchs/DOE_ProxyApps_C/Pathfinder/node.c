...
Node *Node_new(int id, int nodeCount)
{
  ...
  node->id = id;
  node->nodeCount =
      nodeCount; /* index into bitfield - caller must increment! */
  node->label = NULL;
  node->labelIdx = -1;
...
bool Node_addEdgeToNode(Node *this, Node *node)
{
  ...
  newEdge->nextEdge =
      this->edges; /* First time, this is NULL, terminates our list. */
  this->edges = newEdge;
  ++this->edgeCount;
  return (true);
}
...
bool NodeList_insertFront(NodeList *this, Node *newFront)
{
  ...
  oldFront->nextNode =
      this->nextNode; /* Now points to the remainder of the list (or NULL) */
...
bool EdgeList_insertNodeId(EdgeList *this, int targetNodeId)
{
  ...
  newElement->targetNodeId = targetNodeId;
      ...
      newElement; /* new edge list element has a null node and next */
