...
Pole **generate_poles(Input input, int *n_poles)
{
  ...
  Pole *contiguous =
      (Pole *)malloc(input.n_nuclides * input.avg_n_poles * sizeof(Pole));
  ...
  for (int i = 0; i < input.n_nuclides; i++)
  {
    ...
    {
      R[i][j].MP_EA =
          f * ((double)rand() / RAND_MAX + (double)rand() / RAND_MAX * I);
      R[i][j].MP_RT =
          f * (double)rand() / RAND_MAX + (double)rand() / RAND_MAX * I;
      R[i][j].MP_RA =
          f * (double)rand() / RAND_MAX + (double)rand() / RAND_MAX * I;
  ...
  Tf = %lf\n", i, j, R[i][j].Eo, R[i][j].lambda_o, R[i][j].Tn, R[i][j].Tg,
  R[i][j].Tf);
  */
...
Window **generate_window_params(Input input, int *n_windows, int *n_poles)
{
  ...
  Window *contiguous =
      (Window *)malloc(input.n_nuclides * input.avg_n_windows * sizeof(Window));
...
double **generate_pseudo_K0RS(Input input)
{
  ...
  double *contiguous =
      (double *)malloc(input.n_nuclides * input.numL * sizeof(double));
