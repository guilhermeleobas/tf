...
int RandomPadding = -1;
//------------------------------------------------------------------------------------------------------------------------------
int create_box(box_type *box, int numGrids, int low_i, int low_j, int low_k,
               int dim_i, int dim_j, int dim_k, int ghosts)
{
  ...
  // nominally the stencil assumes VL is a perfect multiple of $U and unrolls
  // without cleanup.  However, this means you can walk $U-1 beyond the last
  // point you should update and start updating the ghost zone of the next
  // plane.
  ...
  // box->plane  =(
  // ((dim_j+2*ghosts)*box->pencil)+paddingToAvoidStencilCleanup+0xF) & ~0xF; //
  ...
  // box->plane  =(
  // ((dim_j+2*ghosts)*box->pencil)+paddingToAvoidStencilCleanup+0x1) & ~0x1; //
  // multiple of  16 bytes (required for SSE)
  // printf("%2d^2 = %5d ->
  ...
  // 16KB / 8way / 8bytes / 7 arrays
  // if(dim_i>=32){while( ((box->volume %  256) !=  32) )box->volume+=8;} //
  // 16KB / 8way / 8bytes / 8 arrays
...
#else
  double *tmpbuf;
  posix_memalign((void **)&tmpbuf, 64,
                 box->volume * box->numGrids * sizeof(double));
