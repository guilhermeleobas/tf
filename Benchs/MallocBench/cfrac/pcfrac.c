...
void setBit(bv, bno, value) register BitVector bv;
...
unsigned getBit(bv, bno) register BitVector bv;
...
BitVector newBitVector(value, size) register solnvec value;
unsigned size;
{
...
void printSoln(stream, prefix, suffix, pm, m, p, t, e) FILE *stream;
char *prefix, *suffix;
 ...
 */
void combineSoln(x, t, e, pm, m, n, bp) precision *x, *t, n;
uvec pm;
register solnvec e;
unsigned m;
SolnPtr bp;
{
  ...
  for (j = 1; j <= m; j++)
  {
    ...
    {
      pset(t, pmod(pmul(*t,
                        ppowmod(utop(pm[j - 1]), utop((unsigned)e[j] >> 1), n)),
                   n));
      e[j] &= 1;
    }
    else if (e[j] == 2)
 ...
 */
SolnPtr newSoln(n, pm, m, next, x, t, e) precision n;
unsigned m;
uvec pm;
SolnPtr next;
precision x, t;
...
void freeSoln(p) register SolnPtr p;
{
  if (p != (SolnPtr)0)
  {
    pdestroy(p->x);
...
void freeSolns(p) register SolnPtr p;
{
  register SolnPtr l;
...
SolnPtr findSoln(sp, t) register SolnPtr sp;
precision t;
{
  (void)pparm(t);
  while (sp != (SolnPtr)0)
 ...
 */
double pomeranceLpow(n, y) double n;
double y;
{
  double lnN = log(n);
 ...
 * Theory", part 1, 1982.
 */
double cfracA(n, aborts) double n;
unsigned aborts;
{
 ...
 */
uvec pfactorbase(n, k, m, aborts) precision n;
unsigned k;
unsigned *m, aborts;
{
  double dn, a;
  ...
  if (*m == 0)
  { /* compute a suitable m */
    dn = ptod(nk);
    a = cfracA(dn, aborts);
    maxpm = (unsigned)(pomeranceLpow(dn, a) + 0.5);
    do
    {
      if ((unsigned)*primePtr++ >= maxpm)
      {
        break;
      }
 ...
 */
  ...
  *pm++ = (unsigned)*primePtr++; /* two is first element */
  ...
  if (count != *m)
  {
    do
    {
      if (picmp(plegendre(nk, utop((unsigned)*primePtr)), 1) <= 0)
      { /* 0,1 */
        *pm++ = *primePtr;
        count++;
        if (count == *m)
 ...
 */
EasPtr getEas(n, k, pm, m, aborts) precision n;
 ...
 */
int pfactorQ(f, t, pm, e, m, eas) precision *f;
precision t;
 ...
 */
precision pcfrac(n, maxCount) precision n;
unsigned *maxCount;
{
  unsigned k = pcfrac_k;
  unsigned m = pcfrac_m;
  ...
  precision t = pUndef, r = pUndef, twog = pUndef, u = pUndef, lastU = pUndef,
  ...
  F2:
    ...
    { /* if partial */
      ...
      if (verbose > 2)
      {
        printSoln(stdout, "PartSum: ", "", pm, m, x, t, e);
      }
    }
    else
    {
      ...
      if (verbose > 2)
      {
        printSoln(stdout, "Full:    ", "", pm, m, x, t, e);
      }
    }
 ...
 */
int pcfracInit(m, k, aborts) unsigned m;
unsigned k;
unsigned aborts;
{
  pcfrac_m = m;
