...
/* currentlinewidth */
float gs_currentlinewidth(gs_state *pgs)
{
...
/* currentlinecap */
gs_line_cap gs_currentlinecap(gs_state *pgs) { return pgs->line_params->cap; }
/* setlinejoin */
...
/* currentlinejoin */
gs_line_join gs_currentlinejoin(gs_state *pgs)
{
...
/* currentmiterlimit */
float gs_currentmiterlimit(gs_state *pgs)
{
  return pgs->line_params->miter_limit;
}

/* setdash */
int gs_setdash(gs_state *pgs, float *pattern, uint length, floatp offset)
{
  uint n = length;
  float *dfrom = pattern;
  char ink = 1;
...
/* currentdash */
uint gs_currentdash_length(gs_state *pgs)
{
  return pgs->line_params->dash.pattern_size;
}
int gs_currentdash_pattern(gs_state *pgs, float *pattern)
{
  memcpy(pattern, pgs->line_params->dash.pattern,
         pgs->line_params->dash.pattern_size * sizeof(float));
  return 0;
}
float gs_currentdash_offset(gs_state *pgs)
{
