...
/* Forward references */
private
int interp(P1(ref *pref));
private
op_proc_(interp_exit);
...
private
ref ostack[os_guard_under + max_ostack + os_guard_over];
...
private
op_proc_p special_ops[] = {zadd, zdup, zexch, zifelse, zle, zpop, zsub};
enum
{
  tx_op_add = tx_op,
  tx_op_dup,
...
void interp_fix_op(ref *opref)
{
  ...
  if (i >= 0)
  {
    make_tav(opref, tx_op + i, a_executable, opproc, proc);
  }
}
...
int interpret(ref *pref, int user_errors)
{
  ref *epref = pref;
  ref erref;
...
private
int interp_exit(ref *op) { return e_InterpreterExit; }
/* Main interpreter. */
...
private
int interp(ref *pref /* object to interpret */)
{
...
sw: /* We have to populate enough cases of the switch statement */
  ...
  switch (r_type_xe(iref))
  {
    ...
    case nox(t_packedarray):
    case nox_exec(t_packedarray):
    case nox(t_string):
    ...
    case lit(t_string):
    case plain(t_color):
    case plain_exec(t_color):
    case plain(t_device):
    case plain_exec(t_device):
      break;
    /* Special operators. */
    case plain_exec(tx_op_add):
      if ((code = zadd(iosp)) < 0) return_with_error(code, iref);
      iosp--;
      next();
    case plain_exec(tx_op_dup):
      if (iosp < osp_nargs[1]) return_with_error(e_stackunderflow, iref);
      iosp++;
      s_store_r(iosp, 0, -1);
      next();
    case plain_exec(tx_op_exch):
      if (iosp < osp_nargs[2]) return_with_error(e_stackunderflow, iref);
      s_store_i(ptoken, iosp);
      s_store_r(iosp, 0, -1);
      s_store_b(iosp, -1, ptoken, 0);
      next();
    case plain_exec(tx_op_ifelse):
      if (r_type(iosp - 2) != t_boolean) return_with_error(e_typecheck, iref);
      if (iesp >= estop) return_with_error(e_execstackoverflow, iref);
      store_state(iesp);
      iosp -= 3;
      /* Open code "up" for the array case(s) */
      {
        ref *whichp = (iosp[1].value.index ? iosp + 2 : iosp + 3);
    ...
    case plain_exec(tx_op_sub):
      if ((code = zsub(iosp)) < 0) return_with_error(code, iref);
      iosp--;
      next();
    /* Executable types. */
    ...
    case plain_exec(t_name):
    {
      ...
      switch (r_type_xe(pvalue))
      {
        ...
        case exec(t_packedarray):
          ...
          if (iesp >= estop) return_with_error(e_execstackoverflow, pvalue);
          ++iesp;
        ...
        case lit(t_string):
          /* Just push the value */
          if (iosp >= ostop) return_with_error(e_stackoverflow, pvalue);
          ++iosp;
