...
int gs_closepath(gs_state *pgs) { return gx_path_close_subpath(pgs->path); }
/* ------ Points and lines ------ */
...
private
int arc_either(P7(gs_state *, floatp, floatp, floatp, floatp, floatp, int));
private
int arc_add(P8(gs_state *, floatp, floatp, floatp, floatp, floatp, floatp,
               int));

int gs_arc(gs_state *pgs, floatp xc, floatp yc, floatp r, floatp ang1,
           floatp ang2)
{
  return arc_either(pgs, xc, yc, r, ang1, ang2, 0);
}

int gs_arcn(gs_state *pgs, floatp xc, floatp yc, floatp r, floatp ang1,
            floatp ang2)
{
...
private
int arc_either(gs_state *pgs, floatp axc, floatp ayc, floatp ar, floatp aang1,
               floatp aang2, int clockwise)
{
...
#define fixed360 int2fixed(360)
  ...
  /* Compute the intersection of the tangents. */
  {
    ...
    code =
        arc_add(pgs, x0, y0, (axc + ar * cos(ang2r)), (ayc + ar * sin(ang2r)),
                (x0 - trad * sin0), (y0 + trad * cos0), first);
  }
  return code;
}

int gs_arcto(gs_state *pgs, floatp ax1, floatp ay1, floatp ax2, floatp ay2,
             floatp arad, float *retxy) /* float retxy[4] */
{
...
private
int arc_add(gs_state *pgs, floatp x0, floatp y0, floatp x3, floatp y3,
            floatp xt, floatp yt, int first)
{
...
int gs_rcurveto(gs_state *pgs, floatp dx1, floatp dy1, floatp dx2, floatp dy2,
