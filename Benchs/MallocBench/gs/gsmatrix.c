...
/*static*/ gs_matrix gs_identity_matrix = {identity_matrix_body};

/* ------ Matrix creation ------ */

/* Create an identity matrix */
void gs_make_identity(gs_matrix *pmat) { *pmat = gs_identity_matrix; }
/* Create a translation matrix */
int gs_make_translation(floatp dx, floatp dy, register gs_matrix *pmat)
{
...
int gs_make_scaling(floatp sx, floatp sy, register gs_matrix *pmat)
{
  *pmat = gs_identity_matrix;
  pmat->xx = sx;
  pmat->yy = sy;
...
int gs_make_rotation(floatp ang, register gs_matrix *pmat)
{
...
int gs_matrix_multiply(gs_matrix *pm1, gs_matrix *pm2, register gs_matrix *pmr)
{
...
int gs_matrix_invert(register gs_matrix *pm, register gs_matrix *pmr)
...
int gs_matrix_rotate(register gs_matrix *pm, floatp ang,
                     register gs_matrix *pmr)
{
  float mxx, mxy;
  int quads;
  float tsin, tcos;
  ...
  if (ang >= -360 && ang <= 360 && ang == (quads = (int)ang / 90) * 90)
  {
    int isin = 0, icos = 1, t;
    quads &= 3;
    while (quads--)
    {
      t = isin, isin = icos, icos = -t;
    }
    tsin = isin, tcos = icos;
  }
  else
...
/* Transform a point. */
int gs_point_transform(floatp x, floatp y, register gs_matrix *pmat,
...
int gs_point_transform_inverse(floatp x, floatp y, register gs_matrix *pmat,
                               register gs_point *ppt)
{
  if (!is_skewed(pmat))
  {
...
int gs_distance_transform(floatp dx, floatp dy, register gs_matrix *pmat,
...
/* Return gs_error_undefinedresult if the matrix is not invertible. */
int gs_distance_transform_inverse(floatp dx, floatp dy,
...
/* Return gs_error_undefinedresult if the matrix is not invertible. */
int gs_bbox_transform_inverse(gs_rect *pbox_in, gs_matrix *pmat,
                              gs_rect *pbox_out)
{
  int code;
  gs_point p, w, h;
  ...
  if ((code = gs_point_transform_inverse(pbox_in->p.x, pbox_in->p.y, pmat,
                                         &p)) < 0 ||
      (code = gs_distance_transform_inverse(
           (float)(pbox_in->q.x - pbox_in->p.x), 0.0, pmat, &w)) < 0 ||
      (code = gs_distance_transform_inverse(
           0.0, (float)(pbox_in->q.y - pbox_in->p.y), pmat, &h)) < 0)
  {
    return code;
  }
  xmin = xmax = p.x;
...
int gs_point_transform2fixed(register gs_matrix_fixed *pmat, floatp x, floatp y,
                             gs_fixed_point *ppt)
{
...
int gs_distance_transform2fixed(register gs_matrix_fixed *pmat, floatp dx,
                                floatp dy, gs_fixed_point *ppt)
{
