...
/* Forward references */
private
int name_alloc_sub(P1(name_table *));
...
int name_ref(byte *ptr, uint isize, ref *pref, int enterflag)
{
...
void name_string_ref(ref *pnref /* t_name */, ref *psref /* result, t_string */)
{
  name *pname = pnref->value.pname;
  make_tasv(psref, t_string, a_read + a_execute, pname->string_size, bytes,
            pname->string_bytes);
}
...
private
int name_alloc_sub(name_table *nt)
{
