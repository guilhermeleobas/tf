...
void refcpy(register ref *to, register ref *from, register uint size)
{
  while (size--)
    {
      store_i(to++, from++);
...
int obj_eq(register ref *pref1, register ref *pref2)
{
  ref nref;
  ...
  switch (r_btype(pref1))
    {
      ...
      case t_operator:
        return ((long)pref1->value.opproc == (long)pref2->value.opproc);
      case t_real:
      ...
      case t_color:
        {
          struct gs_color_s *pc1 = pref1->value.pcolor, *pc2 = pref2->value.pcolor;
...
int bytes_compare(byte *s1, uint len1, byte *s2, uint len2)
{
...
uint string_hash(byte *ptr, uint len)
{
  register byte *p = ptr;
...
int string_to_ref(char *cstr, ref *pref, char *cname)
{
...
char *ref_to_string(ref *pref, char *client_name)
{
  uint size = pref->size;
...
int num_params(ref *op, int count, float *pval)
{
  int mask = 0;
  while (--count >= 0)
    {
      ...
      switch (r_type(op))
        {
          case t_real:
            if (pval)
              {
                pval[count] = op->value.realval;
              }
            break;
          case t_integer:
            if (pval)
              {
                pval[count] = op->value.intval;
              }
            mask++;
            break;
          default:
...
int real_param(ref *op, float *pparam, int unit)
{
  float fval;
  switch (r_type(op))
    {
      case t_integer:
        fval = op->value.intval;
        break;
      case t_real:
...
int read_matrix(ref *op, gs_matrix *pmat)
{
  check_array(*op);
  if (op->size != 6)
    {
...
int write_matrix(register ref *op)
{
  ref *aptr;
  int i;
