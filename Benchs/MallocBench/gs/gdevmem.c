...
#define mem_procs(map_rgb_color, map_color_rgb, copy_mono, copy_color, \
                  fill_rectangle)                                      \
  {                                                                    \
...
#define check_rect()              \
  if (w <= 0 || h <= 0) return 0; \
  if (x < 0 || x > mdev->width - w || y < 0 || y > mdev->height - h) return -1
...
ulong gx_device_memory_bitmap_size(gx_device_memory *dev)
{
  unsigned raster = ((mdev->width * mdev->bits_per_color_pixel + 31) >> 5) << 2;
  mdev->raster = raster;
...
int gs_device_is_memory(gx_device *dev)
...
int mem_bytes_per_scan_line(gx_device_memory *dev)
{
...
int mem_copy_scan_lines(gx_device_memory *dev, int start_y, byte *str,
                        uint size)
{
...
private
int mem_no_fault_proc(gx_device_memory *dev, int x, int y, int w, int h,
                      int writing)
{
  return 0;
}
...
/* Procedures */
declare_mem_procs(mem_mono_copy_mono, mem_mono_copy_color,
...
gx_device_memory mem_mono_device = mem_device("image(mono)", 1, mem_mono_procs);
...
private
int mem_mono_fill_rectangle(gx_device *dev, int x, int y, int w, int h,
...
#define scan_chunk chunk
private
int mem_mono_copy_mono(gx_device *dev, byte *base, int sourcex, int raster,
                       int x, int y, int w, int h, gx_color_index zero,
                       gx_color_index one)
{
  chunk *line;
...
private
int mem_mono_copy_color(gx_device *dev, byte *base, int sourcex, int raster,
...
#define scan_chunk byte
private
int copy_byte_rect(gx_device *dev, byte *source, int sraster, int offset, int y,
                   int w, int h)
{
...
declare_mem_procs(mem_mapped_copy_mono, mem_mapped_copy_color,
                  mem_mapped_fill_rectangle);

/* The device descriptor. */
private
gx_device_procs mem_mapped_procs = mem_procs(
    mem_mapped_map_rgb_color, mem_mapped_map_color_rgb, mem_mapped_copy_mono,
    mem_mapped_copy_color, mem_mapped_fill_rectangle);
...
private
gx_color_index mem_mapped_map_rgb_color(gx_device *dev, ushort r, ushort g,
                                        ushort b)
{
...
private
int mem_mapped_map_color_rgb(gx_device *dev, gx_color_index color, ushort *prgb)
{
...
/* Fill a rectangle with a color. */
private
int mem_mapped_fill_rectangle(gx_device *dev, int x, int y, int w, int h,
                              gx_color_index color)
{
  declare_scan_ptr(dest_line, dest, offset);
...
/* Copy a monochrome bitmap. */
private
int mem_mapped_copy_mono(gx_device *dev, byte *base, int sourcex, int raster,
                         int x, int y, int w, int h, gx_color_index zero,
                         gx_color_index one)
{
  byte *line;
...
/* Copy a color bitmap. */
private
int mem_mapped_copy_color(gx_device *dev, byte *base, int sourcex, int raster,
                          int x, int y, int w, int h)
{
  check_fault_copy_color(x, w);
  check_rect();
  return copy_byte_rect(dev, base + x_to_byte(sourcex), raster, x_to_byte(x), y,
                        x_to_byte(w), h);
}
...
#define mem_true_procs(copy_mono, copy_color, fill_rectangle)          \
  mem_procs(mem_true_map_rgb_color, mem_true_map_color_rgb, copy_mono, \
...
/* Map a r-g-b color to a color index. */
private
gx_color_index mem_true_map_rgb_color(gx_device *dev, ushort r, ushort g,
                                      ushort b)
{
...
/* Map a color index to a r-g-b color. */
private
int mem_true_map_color_rgb(gx_device *dev, gx_color_index color, ushort *prgb)
{
...
/* The device descriptor. */
private
gx_device_procs mem_true24_procs = mem_true_procs(
...
#define put3(ptr, r, g, b) *ptr++ = r, *ptr++ = g, *ptr++ = b

/* Fill a rectangle with a color. */
private
int mem_true24_fill_rectangle(gx_device *dev, int x, int y, int w, int h,
                              gx_color_index color)
{
...
/* Copy a monochrome bitmap. */
private
int mem_true24_copy_mono(gx_device *dev, byte *base, int sourcex, int raster,
                         int x, int y, int w, int h, gx_color_index zero,
                         gx_color_index one)
{
  byte *line;
...
/* Copy a color bitmap. */
private
int mem_true24_copy_color(gx_device *dev, byte *base, int sourcex, int raster,
                          int x, int y, int w, int h)
{
...
/* The device descriptor. */
private
gx_device_procs mem_true32_procs = mem_true_procs(
...
/* Fill a rectangle with a color. */
private
int mem_true32_fill_rectangle(gx_device *dev, int x, int y, int w, int h,
                              gx_color_index color)
{
  declare_scan_ptr(dest_line, dest, offset);
...
/* Copy a monochrome bitmap. */
private
int mem_true32_copy_mono(gx_device *dev, byte *base, int sourcex, int raster,
                         int x, int y, int w, int h, gx_color_index zero,
                         gx_color_index one)
{
  byte *line;
...
/* Copy a color bitmap. */
private
int mem_true32_copy_color(gx_device *dev, byte *base, int sourcex, int raster,
                          int x, int y, int w, int h)
{
