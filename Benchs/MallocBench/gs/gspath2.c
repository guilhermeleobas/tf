...
private
int common_clip(P2(gs_state *, int));
private
int set_clip_path(P3(gs_state *, gx_path *, int));
...
int gs_pathbbox(gs_state *pgs, gs_rect *pbox)
{
...
void gs_path_enum_init(gs_path_enum *penum, gs_state *pgs)
{
...
int gs_initclip(gs_state *pgs)
{
  ...
  return gx_clip_to_rectangle(pgs, (fixed)0, (fixed)0, int2fixed(dev->width),
                              int2fixed(dev->height));
}

int gs_clip(gs_state *pgs) { return common_clip(pgs, gx_rule_winding_number); }
int gs_eoclip(gs_state *pgs) { return common_clip(pgs, gx_rule_even_odd); }
...
int gx_clip_to_rectangle(gs_state *pgs, fixed x0, fixed y0, fixed x1, fixed y1)
{
  gx_path cpath;
  gx_path *ppath = &cpath;
  int code;
...
private
int common_clip(gs_state *pgs, int rule)
{
  gx_path cpath;
