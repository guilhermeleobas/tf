...
/* Forward references */
private
int access_check(P3(ref *, int, int));
...
/* .typenumber */
int ztypenumber(register ref *op)
{
  check_op(1);
...
/* executeonly */
int zexecuteonly(register ref *op)
{
  check_op(1);
...
/* noaccess */
int znoaccess(register ref *op) { return access_check(op, 0, 1); }
/* readonly */
int zreadonly(register ref *op)
{
  return access_check(op, a_read + a_execute, 1);
}

/* rcheck */
int zrcheck(register ref *op)
{
...
void ztype_op_init()
{
  static op_def my_defs[] = {{"1cvi", zcvi},
                             {"1cvlit", zcvlit},
                             {"1cvn", zcvn},
                             {"1cvr", zcvr},
                             {"3cvrs", zcvrs},
