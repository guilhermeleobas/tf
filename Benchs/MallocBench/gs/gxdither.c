...
void gx_color_render(gs_color *pcolor, gx_device_color *pdevc, gs_state *pgs)
{
...
#define divval (max_color_param_long + 1)
  ...
  if (max_value >= 255)
  {
    ...
    pdevc->color2 = pdevc->color1 = (*dev->procs->map_rgb_color)(dev, r, g, b);
    pdevc->halftone_level = 0; /* pure color */
    return;
  }
  ...
  if (!dev->has_color || pcolor->is_gray)
  {
    ...
    {
      ++l;
      pdevc->color2 = (*dev->procs->map_rgb_color)(dev, l, l, l);
...
#define half ((color_param)(max_color_param_long >> 1))
    if (rem_r > half)
    {
      rem_r = max_color_param - rem_r, adjust_r = -1, r++,
      lum_invert += lum_red_weight;
    }
    else
    {
      adjust_r = 1;
    ...
    if (rem_g > half)
    {
      rem_g = max_color_param - rem_g, adjust_g = -1, g++,
      lum_invert += lum_green_weight;
    }
    else
    {
      adjust_g = 1;
    ...
    if (rem_b > half)
    {
      rem_b = max_color_param - rem_b, adjust_b = -1, b++,
      lum_invert += lum_blue_weight;
    }
    else
    {
      adjust_b = 1;
    ...
    if (dot1 > dot2)
    {
      ...
      {
        diagc = DIAG_RGB, level = (hsize * dot3) / (3 * max_color_param_long);
      }
      else
      {
        diagc = axisc, level = (hsize * amax) / max_color_param_long;
      }
    }
    else
    {
    ...
    {
      printf("[c]rgb=%x,%x,%x -->\n", (unsigned)pcolor->red,
             (unsigned)pcolor->green, (unsigned)pcolor->blue);
      printf("   %x+%x,%x+%x,%x+%x; adjust=%d,%d,%d;\n", (unsigned)r,
             (unsigned)rem_r, (unsigned)g, (unsigned)rem_g, (unsigned)b,
             (unsigned)rem_b, adjust_r, adjust_g, adjust_b);
    }
#endif
    ...
    if (gs_debug['c'])
    {
      printf("[c]diagc=%d; color1=%lx, color2=%lx, level=%d/%d\n", diagc,
             (ulong)pdevc->color1, (ulong)pdevc->color2, level,
             (unsigned)hsize);
    }
#endif
  }
