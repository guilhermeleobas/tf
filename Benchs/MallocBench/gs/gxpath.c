...
/* Forward references */
private
subpath *path_alloc_copy(P1(gx_path *));
#ifdef DEBUG
...
/* Initialize a path */
void gx_path_init(register gx_path *ppath, gs_memory_procs *pprocs)
{
...
#define path_alloc_segment(pseg, ctype, stype, cname)                        \
  path_unshare(psub =);                                                      \
  if (!(pseg =                                                               \
            (ctype *)(*ppath->memory_procs.alloc)(1, sizeof(ctype), cname))) \
...
/* Open a new subpath */
int gx_path_new_subpath(gx_path *ppath)
{
  ...
  ppath->subpath_count++;
#ifdef DEBUG
  if (gs_debug['p']) printf("[p]"), gx_print_segment(stdout, (segment *)spp);
#endif
  return 0;
}

/* Add a point to the current path (moveto). */
int gx_path_add_point(register gx_path *ppath, fixed x, fixed y)
{
  ppath->subpath_open = 0;
...
int gx_path_add_line(gx_path *ppath, fixed x, fixed y)
{
  subpath *psub = ppath->current_subpath;
  ...
  ppath->segment_count++;
#ifdef DEBUG
  if (gs_debug['p']) printf("[p]"), gx_print_segment(stdout, (segment *)lp);
#endif
  return 0;
}
...
int gx_path_add_rectangle(gx_path *ppath, fixed x0, fixed y0, fixed x1,
                          fixed y1)
{
...
/* and a close_subpath. */
int gx_path_add_pgram(gx_path *ppath, fixed x0, fixed y0, fixed x1, fixed y1,
                      fixed x2, fixed y2)
{
  int code;
...
int gx_path_add_curve(gx_path *ppath, fixed x1, fixed y1, fixed x2, fixed y2,
                      fixed x3, fixed y3)
{
  subpath *psub = ppath->current_subpath;
...
int gx_path_add_arc(gx_path *ppath, fixed x0, fixed y0, fixed x3, fixed y3,
                    fixed xt, fixed yt)
{
  fixed xm = (fixed)(xt * arc_magic), ym = (fixed)(yt * arc_magic);
  return gx_path_add_curve(ppath, (fixed)(x0 * (1 - arc_magic)) + xm,
                           (fixed)(y0 * (1 - arc_magic)) + ym,
                           (fixed)(x3 * (1 - arc_magic)) + xm,
                           (fixed)(y3 * (1 - arc_magic)) + ym, x3, y3);
}

/* Close the current subpath. */
int gx_path_close_subpath(gx_path *ppath)
{
  subpath *psub = ppath->current_subpath;
  register line_segment *lp;
  if (!ppath->subpath_open)
...
#ifdef DEBUG
  if (gs_debug['p'])
    if (lp != 0) printf("[p]"), gx_print_segment(stdout, (segment *)lp);
#endif
  return 0;
}
...
private
subpath *path_alloc_copy(gx_path *ppath)
{
  gx_path path_new;
  int code;
...
void gx_dump_path(gx_path *ppath, char *tag)
{
  printf("[p]Path %lx %s:\n", (ulong)ppath, tag);
...
/* Print a path */
void gx_path_print(FILE *file, gx_path *ppath)
{
  ...
  fprintf(file, "   subpaths=%d, segments=%d, curves=%d, point=(%f,%f)\n",
          ...
          fixed2float(ppath->position.x), fixed2float(ppath->position.y));
...
{
  char out[80];
  sprintf(out, "   %lx<%lx,%lx>: %%s (%6g,%6g) ", (ulong)pseg,
