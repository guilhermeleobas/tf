...
private
dev_proc_tile_rectangle(null_tile_rectangle);
private
dev_proc_copy_mono(null_copy_mono);
private
dev_proc_copy_color(null_copy_color);
private
dev_proc_draw_line(null_draw_line);
private
dev_proc_fill_trapezoid(null_fill_trapezoid);
private
dev_proc_tile_trapezoid(null_tile_trapezoid);
...
int gs_copyscanlines(gx_device *dev, int start_y, byte *data, uint size,
...
gx_device *gs_currentdevice(gs_state *pgs) { return pgs->device->info; }
/* Get the name of a device */
char *gs_devicename(gx_device *dev) { return dev->name; }
/* Read out the current device parameters */
void gs_deviceparams(gx_device *dev, gs_matrix *pmat, int *pwidth, int *pheight)
{
...
gx_device *gs_getdevice(int index)
{
  int i;
...
int gs_makedevice(gx_device **pnew_dev, gx_device *dev, gs_matrix *pmat,
...
int gs_makeimagedevice(gx_device **pnew_dev, gs_matrix *pmat, uint width,
                       uint height, float *colors, int num_colors,
                       proc_alloc_t palloc)
{
...
void gs_nulldevice(gs_state *pgs) { gs_setdevice(pgs, gx_device_null_p); }
/* Install enough of a null device to suppress graphics output */
/* during the execution of stringwidth. */
void gx_device_no_output(gs_state *pgs) { pgs->device->info = &null_device; }
/* Dummy device procedures */
private
int null_fill_rectangle(gx_device *dev, int x, int y, int w, int h,
                        gx_color_index color)
{
  return 0;
}
private
int null_tile_rectangle(gx_device *dev, gx_bitmap *tile, int x, int y, int w,
                        int h, gx_color_index zero, gx_color_index one)
{
  return 0;
}
private
int null_copy_mono(gx_device *dev, byte *data, int dx, int raster, int x, int y,
                   int w, int h, gx_color_index zero, gx_color_index one)
{
  return 0;
}
private
int null_copy_color(gx_device *dev, byte *data, int dx, int raster, int x,
                    int y, int w, int h)
{
  return 0;
}
private
int null_draw_line(gx_device *dev, int x0, int y0, int x1, int y1,
                   gx_color_index color)
{
  return 0;
}
private
int null_fill_trapezoid(gx_device *dev, int x0, int y0, int w0, int x1, int y1,
                        int w1, gx_color_index color)
{
  return 0;
}
private
int null_tile_trapezoid(gx_device *dev, gx_bitmap *tile, int x0, int y0, int w0,
                        int x1, int y1, int w1, gx_color_index color0,
...
void gx_default_get_initial_matrix(register gx_device *dev,
                                   register gs_matrix *pmat)
{
...
int gx_default_sync_output(gx_device *dev) { return 0; }
int gx_default_output_page(gx_device *dev)
{
  return (*dev->procs->sync_output)(dev);
}
int gx_default_close_device(gx_device *dev) { return 0; }
gx_color_index gx_default_map_rgb_color(gx_device *dev, ushort r, ushort g,
                                        ushort b)
{
  return (gx_color_index)max(max(r, g), b);
}
int gx_default_map_color_rgb(gx_device *dev, gx_color_index color, ushort *prgb)
{
...
private
gx_device *trace_cache_device = NULL;
private
gx_device_procs *trace_cache_procs;
private
trace_record *trace_list = NULL;

#define rprocs \
  (dev == trace_cache_device ? trace_cache_procs : trace_find_procs(dev))

/* Procedure structure */
private
dev_proc_open_device(trace_open_device);
private
dev_proc_get_initial_matrix(trace_get_initial_matrix);
private
dev_proc_sync_output(trace_sync_output);
private
dev_proc_output_page(trace_output_page);
private
dev_proc_close_device(trace_close_device);
private
dev_proc_map_rgb_color(trace_map_rgb_color);
private
dev_proc_map_color_rgb(trace_map_color_rgb);
private
dev_proc_fill_rectangle(trace_fill_rectangle);
private
dev_proc_tile_rectangle(trace_tile_rectangle);
private
dev_proc_copy_mono(trace_copy_mono);
private
dev_proc_copy_color(trace_copy_color);
private
dev_proc_draw_line(trace_draw_line);
private
dev_proc_fill_trapezoid(trace_fill_trapezoid);
private
dev_proc_tile_trapezoid(trace_tile_trapezoid);
...
private
gx_device_procs *trace_find_procs(gx_device *tdev)
{
...
/* Trace a device. */
gx_device *gs_trace_device(gx_device *rdev)
{
  trace_record *tp;
  if (rdev->procs->open_device == trace_procs.open_device)
    return rdev; /* already traced */
  tp = (trace_record *)gs_malloc(1, sizeof(trace_record), "gs_trace_device");
  if (tp == 0) return 0;
  tp->next = trace_list;
...
/* Utilities */
private
int trace_print_code(int result)
{
  if (result == 0)
    printf(";\n");
  else
...
private
void trace_print_tile(gx_bitmap *tile)
{
  int i;
...
/* Procedures */
private
int trace_open_device(gx_device *dev)
{
  int result = (*rprocs->open_device)(dev);
  if (gs_debug['v']) printf("[v]\topen_device(dev)"), trace_print_code(result);
  return result;
}
private
void trace_get_initial_matrix(gx_device *dev, gs_matrix *pmat)
{
...
private
int trace_sync_output(gx_device *dev)
{
  int result = (*rprocs->sync_output)(dev);
  if (gs_debug['v']) printf("[v]\tsync_output(dev)"), trace_print_code(result);
  return result;
}
private
int trace_output_page(gx_device *dev)
{
  int result = (*rprocs->output_page)(dev);
  if (gs_debug['v']) printf("[v]\toutput_page(dev)"), trace_print_code(result);
  return result;
}
private
int trace_close_device(gx_device *dev)
{
  int result = (*rprocs->close_device)(dev);
  if (gs_debug['v']) printf("[v]\tclose_device(dev)"), trace_print_code(result);
  return result;
}
private
gx_color_index trace_map_rgb_color(gx_device *dev, ushort r, ushort g, ushort b)
{
...
private
int trace_map_color_rgb(gx_device *dev, gx_color_index color, ushort *prgb)
{
  int result = (*rprocs->map_color_rgb)(dev, color, prgb);
  if (gs_debug['v'])
    printf(
        "\t{ ushort rgb[3]; T(map_color_rgb)(dev, %ld, rgb /* %u, %u, %u */); "
        "}",
...
private
int trace_fill_rectangle(gx_device *dev, int x, int y, int w, int h,
                         gx_color_index color)
{
  int result = (*rprocs->fill_rectangle)(dev, x, y, w, h, color);
  if (gs_debug['v'])
    printf("[v]\tfill_rectangle(dev, %d, %d, %d, %d, %ld)", x, y, w, h,
           (long)color),
        trace_print_code(result);
  return result;
}
private
int trace_tile_rectangle(gx_device *dev, gx_bitmap *tile, int x, int y, int w,
                         int h, gx_color_index zero, gx_color_index one)
{
...
private
int trace_copy_mono(gx_device *dev, byte *data, int dx, int raster, int x,
                    int y, int w, int h, gx_color_index zero,
                    gx_color_index one)
{
  int result =
      (*rprocs->copy_mono)(dev, data, dx, raster, x, y, w, h, zero, one);
  if (gs_debug['v'])
    printf("[v]\tcopy_mono(dev, data, %d, %d, %d, %d, %d, %d, %ld, %ld)", dx,
           raster, x, y, w, h, (long)zero, (long)one),
        trace_print_code(result);
  return result;
}
private
int trace_copy_color(gx_device *dev, byte *data, int dx, int raster, int x,
                     int y, int w, int h)
{
  int result = (*rprocs->copy_color)(dev, data, dx, raster, x, y, w, h);
  if (gs_debug['v'])
    printf("[v]\tcopy_color(dev, data, %d, %d, %d, %d, %d, %d)", dx, raster, x,
           y, w, h),
        trace_print_code(result);
  return result;
}
private
int trace_draw_line(gx_device *dev, int x0, int y0, int x1, int y1,
                    gx_color_index color)
{
  int result = (*rprocs->draw_line)(dev, x0, y0, x1, y1, color);
  if (gs_debug['v'])
    printf("[v]\tdraw_line(dev, %d, %d, %d, %d, %ld)", x0, y0, x1, y1,
           (long)color),
        trace_print_code(result);
  return result;
}
private
int trace_fill_trapezoid(gx_device *dev, int x0, int y0, int w0, int x1, int y1,
                         int w1, gx_color_index color)
{
  int result = (*rprocs->fill_trapezoid)(dev, x0, y0, w0, x1, y1, w1, color);
  if (gs_debug['v'])
    printf("[v]\tfill_trapezoid(dev, %d, %d, %d, %d, %d, %d, %ld)", x0, y0, w0,
           x1, y1, w1, (long)color),
        trace_print_code(result);
  return result;
}
private
int trace_tile_trapezoid(gx_device *dev, gx_bitmap *tile, int x0, int y0,
                         int w0, int x1, int y1, int w1, gx_color_index color0,
                         gx_color_index color1)
{
