...
private
int vmem_get_page(P3(gx_vmem *, char **, int));
private
int vmem_choose_page(P2(gx_vmem *, int));
private
void vmem_read_page(P2(gx_vmem *, int));
private
void vmem_write_page(P2(gx_vmem *, int));
private
void vmem_annul_page(P2(gx_vmem *, int));
private
void vmem_assign_page(P3(gx_vmem *, int, char *));
private
FILE *vmem_file(P1(gx_vmem *));
private
void vmem_seek(P2(gx_vmem *, int));

/* ------ Public interface ------ */

/* Initialize an instance of virtual memory */
int vmem_init(register gx_vmem *vmem, char **lines, int line_size,
              int num_lines, char init_value, int page_size, long max_memory,
...
int vmem_close(register gx_vmem *vmem)
{
...
int vmem_bring_in_page(register gx_vmem *vmem, char **pline, int force)
{
  int fault;
  if (*pline != NO_LINE)
  {
  ...
  if (fault < 0)
  {
    dprintf1("vmem_get_page failed in vmem_bring_in_page, lnum = %ld!\n",
             pline - vmem->lines);
    exit(1);
  }
  return 0;
...
int vmem_bring_in_rect(gx_vmem *vmem, int ignore_x, int lnum, int ignore_w,
                       int lcount, int writing)
{
...
private
int vmem_get_page(gx_vmem *vmem, char **pline, int force)
{
  ...
  while (bottom <= top)
  {
    if (pnum - bottom > top - pnum)
    {
      if (page_ptr(vmem, bottom) != NO_LINE &&
          (bottom < vmem->lock_first || bottom > vmem->lock_last))
      {
        return bottom;
      }
      bottom++;
    ...
    else
    {
      if (page_ptr(vmem, top) != NO_LINE &&
          (top < vmem->lock_first || top > vmem->lock_last))
      {
        return top;
      }
      top--;
...
private
void vmem_read_page(register gx_vmem *vmem, int pnum)
{
  char *page = page_ptr(vmem, pnum);
...
private
void vmem_write_page(register gx_vmem *vmem, int pnum)
{
  char *page = page_ptr(vmem, pnum);
  if (page != NO_LINE)
  {
...
private
void vmem_annul_page(register gx_vmem *vmem, int pnum)
{
...
private
void vmem_assign_page(register gx_vmem *vmem, int pnum, char *page)
{
  int lnum = pnum * vmem->lines_per_page;
  char **ppage = &vmem->lines[lnum];
  char *line = page;
  ...
  while (lnum++ < limit)
  {
    *ppage++ = line, line += vmem->line_size;
  }
}

/* Open the virtual memory file if needed */
private
FILE *vmem_file(gx_vmem *vmem)
{
  FILE *file = vmem->file;
  if (file == NULL) /* first time, open the file */
  {
    char *fname =
        (*vmem->alloc)(strlen(SCRATCH_TEMP) + 1, 1, "vmem_file(filename)");
    if (fname == NULL)
    {
...
private
void vmem_seek(gx_vmem *vmem, int pnum)
{
...
char *talloc(unsigned size, char *str) { return (char *)malloc(size); }
void tfree(char *blk, unsigned size, char *str) { free(blk); }
...
{ /* Following may be enabled if necessary
   ...
   */
  vmem_init(&my_vmem, my_lines, MY_LINE_SIZE, MY_NUM_LINES, 0xbd, MY_PAGE_SIZE,
            (long)MY_MAX_MEMORY, talloc, tfree);
