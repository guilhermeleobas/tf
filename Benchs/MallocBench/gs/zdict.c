...
/* maxlength */
int zmaxlength(register ref *op)
{
...
/* setmaxlength */
int zsetmaxlength(register ref *op)
{
  uint new_size;
  int code;
  ref *op1 = op - 1;
...
int zknown(register ref *op)
{
  ...
  make_bool(
      op1,
      (r_type(op) == t_null ? 0 : dict_find(op1, op, &pvalue) > 0 ? 1 : 0));
  pop(1);
  return 0;
}

/* where */
int zwhere(register ref *op)
{
  ref *pdref = dsp;
  ref *pvalue;
  check_op(1);
...
int zcopy_dict(register ref *op)
{
  ref *op1 = op - 1;
  check_type(*op1, t_dictionary);
  check_dict_read(*op1);
...
/* currentdict */
int zcurrentdict(register ref *op)
{
  push(1);
  *op = *dsp;
  return 0;
...
/* countdictstack */
int zcountdictstack(register ref *op)
{
  push(1);
...
/* dictstack */
int zdictstack(register ref *op)
{
...
void zdict_op_init()
{
  static op_def my_defs[] = {{"1begin", zbegin},
