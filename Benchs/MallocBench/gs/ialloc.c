...
private
int alloc_add_chunk(P1(alloc_state_ptr));
private
char *alloc_large(P3(alloc_state_ptr, uint, char *));
private
void alloc_free_large(P3(char *, uint, char *));
...
#define ptr_le(p1, p2) (ptr_ord_t)(p1) <= (ptr_ord_t)(p2)
#define ptr_lt(p1, p2) (ptr_ord_t)(p1) < (ptr_ord_t)(p2)
#define ptr_between(ptr, lo, hi) (ptr_le(lo, ptr) && ptr_lt(ptr, hi))
...
void alloc_init(proc_alloc_t palloc, proc_free_t pfree, uint chunk_size)
{
...
void alloc_status(long *pused, long *ptotal)
{
  register alloc_state_ptr ap = &as_current;
...
/* must not be freed. */
void alloc_free(char *cobj, uint num_elts, uint elt_size, char *client_name)
{
  register alloc_state_ptr ap = &as_current;
  uint size = num_elts * elt_size;
  uint block_size;
...
byte *alloc_grow(byte *obj, uint old_num, uint new_num, uint elt_size,
                 char *client_name)
{
  register alloc_state_ptr ap = &as_current;
...
/* Shrink an object. */
byte *alloc_shrink(byte *obj, uint old_num, uint new_num, uint elt_size,
                   char *client_name)
{
  register alloc_state_ptr ap = &as_current;
  uint old_size = old_num * elt_size;
...
int alloc_save_change(char *where, uint size)
{
  register alloc_state_ptr ap = &as_current;
...
int alloc_save_level() { return as_current.save_level; }
/* Test whether a reference would be invalidated by a restore. */
int alloc_is_since_save(char *ptr, alloc_save *save)
{
...
int alloc_restore_state_check(alloc_save *save)
{
...
void alloc_restore_state(alloc_save *save)
{
  register alloc_state_ptr ap = save->cap;
  alloc_save *sprev;
...
private
char *alloc_large(alloc_state_ptr ap, uint size, char *client_name)
{
  alloc_block *mblock =
...
private
int alloc_add_chunk(register alloc_state_ptr ap)
{
...
private
void alloc_free_large(char *cobj, uint size, char *client_name)
{
  alloc_block **prev;
