...
/* Forward references */
private
int num_param(P2(ref *, int (*)(P2(gs_state *, floatp))));
private
int line_param(P2(ref *, int *));
...
/* grestoreall */
int zgrestoreall(register ref *op)
{
  gs_grestoreall(igs);
...
/* initgraphics */
int zinitgraphics(register ref *op) { return gs_initgraphics(igs); }
/* setlinewidth */
int zsetlinewidth(register ref *op) { return num_param(op, gs_setlinewidth); }
/* currentlinewidth */
int zcurrentlinewidth(register ref *op)
{
  push(1);
...
/* setlinecap */
int zsetlinecap(register ref *op)
{
  int param;
...
/* currentlinecap */
int zcurrentlinecap(register ref *op)
{
  push(1);
...
/* setlinejoin */
int zsetlinejoin(register ref *op)
{
  int param;
  int code = line_param(op, &param);
  if (!code)
...
/* currentlinejoin */
int zcurrentlinejoin(register ref *op)
{
  push(1);
...
/* setmiterlimit */
int zsetmiterlimit(register ref *op) { return num_param(op, gs_setmiterlimit); }
/* currentmiterlimit */
int zcurrentmiterlimit(register ref *op)
{
  push(1);
...
/* currentdash */
int zcurrentdash(register ref *op)
{
...
/* setflat */
int zsetflat(register ref *op) { return num_param(op, gs_setflat); }
/* currentflat */
int zcurrentflat(register ref *op)
{
  push(1);
...
/* .setgray */
int zsetgray(register ref *op) { return num_param(op, gs_setgray); }
/* currentgray */
int zcurrentgray(register ref *op)
{
  push(1);
...
/* sethsbcolor */
int zsethsbcolor(register ref *op)
{
  float par[3];
  int code;
...
/* currenthsbcolor */
int zcurrenthsbcolor(register ref *op)
{
  float par[3];
...
/* setrgbcolor */
int zsetrgbcolor(register ref *op)
{
  float par[3];
  int code;
  if ((code = num_params(op, 3, par)) >= 0 &&
...
/* currentrgbcolor */
int zcurrentrgbcolor(register ref *op)
{
  float par[3];
...
/* settransfer */
int zsettransfer(register ref *op)
{
  check_proc(*op);
  istate.transfer = *op;
  pop(1);
...
/* currenttransfer */
int zcurrenttransfer(register ref *op)
{
  push(1);
  *op = istate.transfer;
  return 0;
...
/* currentscreen */
int zcurrentscreen(register ref *op)
{
  float freq, angle;
...
void zgstate_op_init()
{
  static op_def my_defs[] = {{"0currentdash", zcurrentdash},
...
private
int num_param(ref *op, int (*pproc)(P2(gs_state *, floatp)))
{
  float param;
...
private
int line_param(register ref *op, int *pparam)
{
...
void tri_put(register ref *op, float pf3[3])
{
