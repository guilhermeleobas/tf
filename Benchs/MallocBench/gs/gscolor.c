...
private
int check_unit(P2(floatp, float *));
private
int tri_param(P4(floatp, floatp, floatp, color_param[3]));
private
void tri_return(P4(color_param, color_param, color_param, float[3]));
...
/* currentgray */
float gs_currentgray(gs_state *pgs)
{
...
/* currentgscolor */
int gs_currentgscolor(gs_state *pgs, gs_color *pcolor)
{
  *pcolor = *pgs->color;
  return 0;
}
...
/* currenthsbcolor */
int gs_currenthsbcolor(gs_state *pgs, float pr3[3])
{
  color_param hsb[3];
...
/* currentrgbcolor */
int gs_currentrgbcolor(gs_state *pgs, float pr3[3])
{
  gs_color *pcolor = pgs->color;
...
int gs_colorrgb(gs_color *pcolor, float pr3[3])
{
  tri_return(pcolor->red, pcolor->green, pcolor->blue, pr3);
  return 0;
}
...
/* setscreen */
int gs_setscreen(gs_state *pgs, floatp freq, floatp angle,
...
/* currentscreen */
int gs_currentscreen(gs_state *pgs, float *pfreq, float *pangle,
                     float (**pproc)(P2(floatp, floatp)))
{
...
int gs_screen_init(gs_screen_enum *penum, gs_state *pgs, floatp freq,
                   floatp angle)
{
  int cwidth, cheight;
  int code;
  ...
  /* interval (-1..1), and rotation by the negative of the angle. */
  {
    ...
    if (gs_debug['h'])
      printf("[h]Screen: w=%d h=%d [%f %f %f %f %f %f]\n", cwidth, cheight,
             penum->mat.xx, penum->mat.xy, penum->mat.yx, penum->mat.yy,
             penum->mat.tx, penum->mat.ty);
#endif
  }
  return 0;
}
...
private
int gx_screen_finish(P1(gs_screen_enum *));
...
private
int gx_screen_finish(gs_screen_enum *penum)
{
  ht_bit *order = penum->order;
...
private
int tri_param(floatp p1, floatp p2, floatp p3, color_param pq3[3])
{
  int code;
  float f1, f2, f3;
...
private
void tri_return(color_param p1, color_param p2, color_param p3, float pr3[3])
{
