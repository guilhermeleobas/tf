...
/* Forward references */
private
int image_setup(P3(ref *, int, int));
private
int image_continue(P1(ref *));

/* erasepage */
int zerasepage(register ref *op) { return gs_erasepage(igs); }
/* fill */
int zfill(register ref *op) { return gs_fill(igs); }
/* eofill */
int zeofill(register ref *op) { return gs_eofill(igs); }
/* stroke */
int zstroke(register ref *op) { return gs_stroke(igs); }
/* colorimage */
int zcolorimage(register ref *op)
{
  ...
  if (op[-1].value.index)
    { /* planar format */
      npop += spp - 1, procp -= spp - 1, spp = -spp;
    }
  check_type(procp[-2], t_integer); /* bits/sample */
  if ((ulong)(procp[-2].value.intval) > 8)
    {
      return e_rangecheck;
    }
...
/* imagemask */
int zimagemask(register ref *op)
{
  int code;
...
private
int image_setup(ref *op, int param3 /* bits/sample or invert */, int spp)
{
  int code;
  gs_image_enum *penum;
  gs_matrix mat;
...
#define inumpush 7
  ...
  if ((penum =
           (gs_image_enum *)alloc(1, gs_image_enum_sizeof, "image_setup")) == 0)
    {
      return e_VMerror;
    }
  code =
      (spp == 0 ? gs_imagemask_init(penum, igs, (int)op[-4].value.intval,
...
private
int image_continue(register ref *op)
{
  ...
  if (r_type(op) != t_string)
    { /* Procedure didn't return a string.  Quit. */
      esp -= inumpush;
      alloc_free((char *)penum, 1, gs_image_enum_sizeof, "image_continue(quit)");
      return e_typecheck;
    }
...
void zpaint_op_init()
{
  static op_def my_defs[] = {{"0eofill", zeofill}, {"0erasepage", zerasepage}, {"0fill", zfill}, {"7colorimage", zcolorimage}, {"5image", zimage}, {"5imagemask", zimagemask}, {"0stroke", zstroke}, op_def_end};
  z_op_init(my_defs);
}
