...
private
int copy_path(P3(gx_path *, gx_path *,
                 int (*)(P7(gx_path *, fixed, fixed, fixed, fixed, fixed,
                            fixed))));
private
int flatten_curve(P7(gx_path *, fixed, fixed, fixed, fixed, fixed, fixed));
...
int gx_path_has_curves(gx_path *ppath) { return ppath->curve_count != 0; }
/* Test if a path has any segments. */
int gx_path_is_void(gx_path *ppath) { return ppath->segment_count == 0; }
/* Test if a path is a rectangle. */
/* If so, return its bounding box. */
int gx_path_is_rectangle(gx_path *ppath, gs_fixed_rect *pbox)
{
  subpath *pseg0;
...
int gx_cpath_box_for_check(register gx_path *ppath, gs_fixed_rect *pbox)
{
  *pbox = ppath->cbox;
  return 0;
}
...
/* This only works for paths that have gone through set_clip_path. */
/* which is why the name is different. */
int gx_cpath_includes_rectangle(register gx_path *ppath, fixed x0, fixed y0,
                                fixed x1, fixed y1)
{
  return (x0 <= x1 ? (ppath->cbox.p.x <= x0 && x1 <= ppath->cbox.q.x)
                   : (ppath->cbox.p.x <= x1 && x0 <= ppath->cbox.q.x)) &&
         (y0 <= y1 ? (ppath->cbox.p.y <= y0 && y1 <= ppath->cbox.q.y)
                   : (ppath->cbox.p.y <= y1 && y0 <= ppath->cbox.q.y));
}

/* Copy a path */
int gx_path_copy(gx_path *ppath_old, gx_path *ppath)
{
...
int gx_path_merge(gx_path *ppfrom, gx_path *ppto)
...
{
  segment *pseg;
#define translate_xy(pt) pt.x += dx, pt.y += dy
...
fixed scaled_flat;
private
float scaled_flat_sq;

/* Flatten a path */
int gx_path_flatten(gx_path *ppath_old, gx_path *ppath, floatp flatness)
...
private
int copy_path(gx_path *ppath_old, gx_path *ppath,
              int (*curve_proc)(P7(gx_path *, fixed, fixed, fixed, fixed, fixed,
                                   fixed)))
{
  gx_path old;
  segment *pseg;
  int code;
#ifdef DEBUG
  if (gs_debug['p']) gx_dump_path(ppath_old, "before copy_path");
#endif
  old = *ppath_old;
...
#ifdef DEBUG
private
void print_curve_point(fixed x, fixed y)
{
...
private
int flatten_curve(gx_path *ppath, fixed x1, fixed y1, fixed x2, fixed y2,
                  fixed x3, fixed y3)
{
...
top:
  ...
  if (gs_debug['u'])
    printf("[u]x0=%f y0=%f x1=%f y1=%f\n   x2=%f y2=%f x3=%f y3=%f\n",
           fixed2float(x0), fixed2float(y0), fixed2float(x1), fixed2float(y1),
           fixed2float(x2), fixed2float(y2), fixed2float(x3), fixed2float(y3));
#endif
