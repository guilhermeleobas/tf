...
#ifdef DEBUG
private
void trace_ctm(P1(gs_state *));
private
void trace_matrix(P1(gs_matrix *));
#endif
...
#define update_ctm(pgs)                         \
  ...
  pgs->ctm.ty_fixed = float2fixed(pgs->ctm.ty), pgs->inverse_valid = 0
...
int gs_initmatrix(gs_state *pgs)
{
  ...
  pgs->char_tm_valid = 0;
#ifdef DEBUG
  if (gs_debug['x']) printf("[x]initmatrix:\n"), trace_ctm(pgs);
#endif
  return 0;
}

int gs_defaultmatrix(gs_state *pgs, gs_matrix *pmat)
{
  gx_device *dev = pgs->device->info;
...
int gs_currentmatrix(gs_state *pgs, gs_matrix *pmat)
{
  *pmat = ctm_only(pgs);
  return 0;
}

int gs_setmatrix(gs_state *pgs, gs_matrix *pmat)
{
  ctm_only(pgs) = *pmat;
  update_ctm(pgs);
  pgs->char_tm_valid = 0;
#ifdef DEBUG
  if (gs_debug['x']) printf("[x]setmatrix:\n"), trace_ctm(pgs);
#endif
  return 0;
}
...
#ifdef DEBUG
  if (gs_debug['x']) printf("[x]scale: %f %f\n", sx, sy), trace_ctm(pgs);
#endif
  return 0;
}
...
int gs_rotate(gs_state *pgs, floatp ang)
{
  ...
  pgs->inverse_valid = 0, pgs->char_tm_valid = 0;
#ifdef DEBUG
  if (gs_debug['x']) printf("[x]rotate: %f\n", ang), trace_ctm(pgs);
#endif
  return code;
}

int gs_concat(gs_state *pgs, gs_matrix *pmat)
{
  int code = gs_matrix_multiply(pmat, &ctm_only(pgs), &ctm_only(pgs));
  update_ctm(pgs);
  pgs->char_tm_valid = 0;
#ifdef DEBUG
  if (gs_debug['x']) printf("[x]concat:\n"), trace_matrix(pmat), trace_ctm(pgs);
#endif
  return code;
}
...
private
void trace_matrix(register gs_matrix *pmat)
{
