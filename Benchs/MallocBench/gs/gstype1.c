...
/* Encrypt a string. */
int gs_type1_encrypt(byte *dest, byte *src, uint len, crypt_state *pstate)
{
...
/* Decrypt a string. */
int gs_type1_decrypt(byte *dest, byte *src, uint len, crypt_state *pstate)
{
  register crypt_state state = *pstate;
  register byte *from = src;
  register byte *to = dest;
...
int gs_type1_init_matrix(register gs_type1_state *pis)
{
  gs_matrix ctm;
  int scale = -10000;
  int expt, shift;
  ...
  if (gs_debug['1'])
  {
    printf("[1]ctm: [%6g %6g %6g %6g ; %6g %6g] scale=%d\n", ctm.xx, ctm.xy,
           ctm.yx, ctm.yy, ctm.tx, ctm.ty, scale);
    printf("   fc: [%lx %lx %lx %lx] shift=%d\n", pis->fc.xx, pis->fc.xy,
           pis->fc.yx, pis->fc.yy, pis->fc.shift);
  }
#endif
  return 0;
}
...
int gs_type1_init(register gs_type1_state *pis, gs_show_enum *penum,
                  int charpath_flag, int paint_type, byte *str,
                  gs_type1_data *pdata)
{
...
private
void type1_trace(byte *cip, byte c, char *str)
{
  if (gs_debug['1']) printf("[1]%lx: %02x %s\n", (ulong)(cip - 1), c, str);
}
#else
#define dc(str)
#endif
...
int gs_type1_interpret(register gs_type1_state *pis, byte *str)
{
...
top: /* Skip initial random bytes */
  ...
  while (1)
  {
    ...
    {
      case c_hstem:
        dc("hstem") /* hint, ignore */
            clear;
        break;
      case c_vstem:
        dc("vstem") /* hint, ignore */
            clear;
        break;
      case c_vmoveto:
        dc("vmoveto") accum_y(cs0);
        goto mto;
      case c_rlineto:
        dc("rlineto") accum_xy(cs0, cs1);
      lto:
        code = gx_path_add_line(ppath, ptx, pty);
      moved:
        if (code < 0)
        {
          return code;
      ...
      pp:
        ...
        break;
      case c_hlineto:
        dc("hlineto") accum_x(cs0);
        goto lto;
      case c_vlineto:
        dc("vlineto") accum_y(cs0);
        goto lto;
      case c_rrcurveto:
        dc("rrcurveto") accum_xy(cs0, cs1);
        x1 = ptx, y1 = pty;
        accum_xy(cs2, cs3);
        x2 = ptx, y2 = pty;
        accum_xy(cs4, cs5);
      curve:
        code = gx_path_add_curve(ppath, x1, y1, x2, y2, ptx, pty);
        goto moved;
      case c_closepath:
        dc("closepath")
      ...
      case c_return:
        ...
        goto itop;
      case c_escape:
        dc("escape:") decrypt_next(*cip, state, c);
        ++cip;
        switch ((char_extended_command)c)
        {
          case ce_dotsection:
            dc("  dotsection") /* hint, ignore */
                clear;
            break;
          case ce_vstem3:
            dc("  vstem3") /* hint, ignore */
                clear;
            break;
          case ce_hstem3:
            dc("  hstem3") /* hint, ignore */
                clear;
            break;
          case ce_seac:
            dc("  seac")
          ...
          case ce_sbw:
            dc("  sbw") pis->lsb.x = cs0, pis->lsb.y = cs1;
            pis->width.x = cs2, pis->width.y = cs3;
            goto setp;
          case ce_div:
            dc("  div") csp[-1] = float2fixed((float)csp[-1] / (float)*csp);
            --csp;
            break;
          case ce_testadd:
            dc("  testadd")
          ...
          case ce_setcurrentpoint:
            dc("  setcurrentpoint") ptx = ftx, pty = fty;
            goto setp;
          default:
        ...
        break;
      case c_hsbw:
        dc("hsbw") pis->lsb.x = cs0, pis->lsb.y = 0;
      ...
      case c_endchar:
        ...
        gx_path_add_point(ppath, pgs->ctm.tx_fixed, pgs->ctm.ty_fixed);
        if (pis->charpath_flag)
        {
          code = gs_setcharwidth(pis->penum, fixed2float(pis->width.x),
        ...
        else
        {
          ...
          code = gs_setcachedevice(pis->penum, fixed2float(pis->width.x),
                                   fixed2float(pis->width.y), bbox.p.x,
        ...
        return code;
      case c_rmoveto:
        dc("rmoveto") accum_xy(cs0, cs1);
      mto:
        code = gx_path_add_point(ppath, ptx, pty);
        goto moved;
      case c_hmoveto:
        dc("hmoveto") accum_x(cs0);
        goto mto;
      case c_vhcurveto:
        dc("vhcurveto") accum_y(cs0);
        x1 = ptx, y1 = pty;
        accum_xy(cs1, cs2);
        x2 = ptx, y2 = pty;
        accum_x(cs3);
      ...
      default: /* a number */
        ...
        else
        {
          ...
          else /* c = 255 */
          {
            byte c1, c2;
            long lw;
            decrypt_next(*cip, state, c1);
            ++cip;
            decrypt_next(*cip, state, c2);
            ++cip;
            decrypt_next(*cip, state, lw);
            ++cip;
            lw += (long)c0 << 24;
            lw += (uint)c1 << 16;
            lw += (uint)c2 << 8;
...
int gs_type1_pop(gs_type1_state *pis, fixed *pf)
{
