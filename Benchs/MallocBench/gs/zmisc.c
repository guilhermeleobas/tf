...
/* currenttime */
int zcurrenttime(register ref *op)
{
  long date_time[2];
...
int zsetdebug(register ref *op)
{
...
#ifdef DEBUG
  {
    ...
    for (i = 0; i < op[-1].size; i++)
      gs_debug[op[-1].value.bytes[i] & 127] = op->value.index;
  }
#endif
  pop(2);
  return 0;
...
private
int type1crypt(P2(ref *, int (*)(P4(byte *, byte *, uint, ushort *))));
int ztype1encrypt(ref *op) { return type1crypt(op, gs_type1_encrypt); }
int ztype1decrypt(ref *op) { return type1crypt(op, gs_type1_decrypt); }
private
int type1crypt(register ref *op,
               int (*proc)(P4(byte *, byte *, uint, ushort *)))
{
  crypt_state state;
...
void zmisc_op_init()
{
  static op_def my_defs[] = {{"1bind", zbind},
