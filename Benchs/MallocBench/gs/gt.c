...
main(int argc, char *argv[]) { return real_main(argc, argv); }
...
float htproc(floatp x, floatp y) { return (float)(1.0 - (x * x + y * y)); }
/* Process switches */
...
void ellipse(register gs_state *pgs, int x, int y, int xrad, int yrad,
...
void arrow(register gs_state *pgs, int tailx, int taily, int tipx, int tipy,
           int thickness, int headthickness, floatp hl)
{
...
int i6proc(byte **pdata, int *psize)
{
  *pdata = &i6data[ii++ * 23];
  *psize = 23;
  return 0;
