...
int gs_fill(gs_state *pgs) { return gs_fill_trim(pgs, (fixed)0); }
/* This is a hack, see gx_fill_path and gs_type1_interpret. */
int gs_fill_trim(gs_state *pgs, fixed trim)
{
  int code;
...
int gs_colorimage(gs_state *pgs, int width, int height, int bps, int spp,
...
/* Render a mask */
int gs_imagemask(gs_state *pgs, int width, int height, int invert,
                 gs_matrix *pmat, byte *data)
{
  gs_image_enum ienum;
  int code;
