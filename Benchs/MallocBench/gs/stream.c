...
/* Generic */
private
int snull(P1(stream *)), snoavailable(P2(stream *, long *));
/* Strings */
private
int ssread(P1(stream *)), sswrite(P2(stream *, byte)),
    ssavailable(P2(stream *, long *)), ssseek(P2(stream *, long));
private
void ssinit(P4(stream *, byte *, uint, stream_procs *));
/* Files */
private
int sfread(P1(stream *)), sfavailable(P2(stream *, long *)),
    srseek(P2(stream *, long)), srclose(P1(stream *));
private
int sfwrite(P2(stream *, byte)), swseek(P2(stream *, long)),
    swflush(P1(stream *)), swclose(P1(stream *));
private
void sfinit(P5(stream *, FILE *, byte *, uint, stream_procs *));
/* decrypting */
private
int sxread(P1(stream *)), sxavailable(P2(stream *, long *));

/* ------ String streams ------ */

/* Initialize a stream for reading a string. */
void sread_string(register stream *s, byte *ptr, uint len)
{
  static stream_procs p = {ssread, NULL, ssavailable, ssseek, snull, snull};
  ssinit(s, ptr, len, &p);
  s->writing = 0;
}
...
private
int ssavailable(stream *s, long *pl)
{
  *pl = sbufavailable(s);
  if (*pl == 0)
  {
...
{
  static stream_procs p = {NULL, sswrite, snoavailable, ssseek, snull, snull};
  ssinit(s, ptr, len, &p);
  s->writing = 1;
}
/* Handle end-of-buffer when writing a string. */
private
int sswrite(stream *s, byte c)
{
  s->cptr = s->endptr;
  return EOFC;
}
...
private
void ssinit(register stream *s, byte *ptr, uint len, stream_procs *p)
{
  s->cbuf = ptr;
  s->cptr = ptr - 1;
  s->endptr = s->cptr + len;
...
{
  static stream_procs p = {sfread, NULL, sfavailable, srseek, snull, srclose};
...
private
int srseek(register stream *s, long pos)
{
...
private
int srclose(stream *s) { return fclose(s->file); }
...
#define fwrite_buf(s) fwrite(s->cbuf, 1, (uint)(s->cptr + 1 - s->cbuf), s->file)
private
int sfwrite(register stream *s, byte c)
{
  fwrite_buf(s);
  if (s->position >= 0)
  { /* file is positionable */
...
/* Private initialization */
private
void sfinit(register stream *s, FILE *file, byte *buf, uint len,
            stream_procs *p)
{
  s->cbuf = buf;
  s->cptr = buf - 1;
...
void sread_decrypt(register stream *s, stream *xs, byte *buf, uint len,
                   ushort /*crype_state*/ state)
{
  static stream_procs p = {sxread, NULL, sxavailable, NULL, snull, snull};
...
private
int snull(stream *s) { return 0; }
/* Indicate an error when asked for available input bytes. */
private
int snoavailable(stream *s, long *pl) { return -1; }
...
uint sgets(register stream *s, byte *str, uint rlen)
{
  uint len = rlen;
  while (len > 0)
  {
...
uint sputs(register stream *s, byte *str, uint wlen)
{
  uint len = wlen;
  while (len > 0)
  {
...
int sreadhex(register stream *s, byte *str, uint rlen, uint *nread,
             int *odd_digit)
{
  byte *ptr = str;
  byte *limit = ptr + rlen;
