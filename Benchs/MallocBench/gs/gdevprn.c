...
private
int prn_bring_in_rect(gx_device_memory *mdev, int x, int y, int w, int h,
                      int writing)
...
gx_color_index gdev_prn_map_rgb_color(gx_device *dev, ushort r, ushort g,
                                      ushort b)
{
  return ((r | g | b) ? 0 : 1);
}
...
int gdev_prn_map_color_rgb(gx_device *dev, gx_color_index color, ushort *prgb)
{
...
int gdev_prn_fill_rectangle(gx_device *dev, int x, int y, int w, int h,
                            gx_color_index color)
{
  return (*prn_dev->mem.procs->fill_rectangle)((gx_device *)&prn_dev->mem, x, y,
                                               w, h, color);
}
int gdev_prn_copy_mono(gx_device *dev, byte *data, int dx, int raster, int x,
                       int y, int w, int h, gx_color_index zero,
                       gx_color_index one)
{
  return (*prn_dev->mem.procs->copy_mono)((gx_device *)&prn_dev->mem, data, dx,
                                          raster, x, y, w, h, zero, one);
}
int gdev_prn_copy_color(gx_device *dev, byte *data, int dx, int raster, int x,
                        int y, int w, int h)
{
  return (*prn_dev->mem.procs->copy_color)((gx_device *)&prn_dev->mem, data, dx,
                                           raster, x, y, w, h);
}
...
void gdev_prn_transpose_8x8(byte *inp, register int line_size, byte *outp,
                            register int dist)
{
  byte *inp4 = inp + (line_size << 2);
  register ushort ae = ((ushort)*inp << 8) + *inp4;
  register ushort bf =
      ((ushort) * (inp += line_size) << 8) + *(inp4 += line_size);
  register ushort cg =
      ((ushort) * (inp += line_size) << 8) + *(inp4 += line_size);
  register ushort dh = ((ushort)inp[line_size] << 8) + inp4[line_size];
