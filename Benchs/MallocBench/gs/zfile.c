...
private
int open_std_file(P3(ref *, char *, ref *));
private
ref *get_current_file();
private
int write_string(P2(ref *, stream *));
...
#define make_file(pref, a, pfe) make_tav(pref, t_file, a, pfile, pfe)
...
private
file_entry invalid_file_entry = {(stream *)0};
private
byte
...
private
stream std_file_streams[num_std_files];
private
file_entry std_files[num_std_files] = {{&std_file_streams[0], 0},
                                       {&std_file_streams[1], 0},
                                       {&std_file_streams[2], 0},
                                       {&std_file_streams[3], -1},
                                       {&std_file_streams[4], -1}};
private
char *std_file_names[num_std_files] = {"%stdin", "%stdout", "%stderr",
                                       "%lineedit", "%statementedit"};
private
int std_file_attrs[num_std_files] = {a_read + a_execute, a_write + a_execute,
                                     a_write + a_execute, a_read + a_execute,
                                     a_read + a_execute};
...
#define check_file_access(svar, op, acc)              \
  {                                                   \
    ...
    if (svar == 0 || !(acc)) return e_invalidaccess;  \
  }
...
/* closefile */
int zclosefile(register ref *op)
{
  stream *s;
  int code;
  check_file(s, op);
...
/* readhexstring */
int zreadhexstring(register ref *op)
{
  stream st;
  stream *s;
  ref *op1 = op - 1;
...
/* writehexstring */
int zwritehexstring(register ref *op)
{
  stream *s;
  byte *p;
  uint len;
...
/* readstring */
int zreadstring(register ref *op)
{
  stream *s;
  uint len, rlen;
  check_read_file(s, op - 1);
...
/* writestring */
int zwritestring(register ref *op)
{
  stream *s;
  int code;
  check_write_file(s, op - 1);
...
int zreadline_stdin(byte *ptr, uint size, uint *pcount)
{
...
int zreadline_from(byte *ptr, uint size, uint *pcount, stream *s)
{
  uint count = 0;
  int ch;
  while (count < size)
...
int ztoken_file(register ref *op)
{
  stream *s;
  ref token;
  int code;
...
/* bytesavailable */
int zbytesavailable(register ref *op)
{
  stream *s;
  long avail;
  check_read_file(s, op);
...
/* flushfile */
int zflushfile(register ref *op)
{
  stream *s;
  check_file(s, op);
  sflush(s);
...
/* resetfile */
int zresetfile(register ref *op)
{
  NYI("resetfile");
  pop(1);
  return 0;
...
/* currentfile */
int zcurrentfile(register ref *op)
{
  ref *fp;
  push(1);
...
/* setfileposition */
int zsetfileposition(register ref *op)
{
  stream *s;
  check_file(s, op - 1);
  check_type(*op, t_integer);
...
/* fileposition */
int zfileposition(register ref *op)
{
  stream *s;
  check_file(s, op);
  if (!sseekable(s))
...
/* deletefile */
int zdeletefile(register ref *op)
{
  char *str;
  int stat;
  check_read_type(*op, t_string);
...
/* renamefile */
int zrenamefile(register ref *op)
{
  char *str1 = 0, *str2 = 0;
  check_read_type(*op, t_string);
...
/* findlibfile */
int zfindlibfile(register ref *op)
{
  int code;
  check_type(*op, t_string);
...
/* writeppmfile */
int zwriteppmfile(register ref *op)
{
  stream *s;
  int code;
  check_write_file(s, op - 1);
...
/* type1decryptfile */
int ztype1decryptfile(register ref *op)
{
  stream *s;
  ushort state;
  ref dec_file;
...
void zfile_op_init()
{
  static op_def my_defs[] = {{"1bytesavailable", zbytesavailable},
...
int lib_file_open(byte *fname, uint len, ref *pfile)
{
  int code;
  char **ppath;
  char cname[PATH_MAX];
...
int file_open(byte *fname, uint len, char *file_access, ref *pfile)
{
  byte *buffer;
  stream *s;
  file_entry *fe;
...
int file_check_read(ref *op, stream **ps)
{
  stream *s;
...
int file_close(ref *fp /* t_file */, stream *s)
{
...
/* return e_undefinedfilename. */
private
int open_std_file(ref *pfname, char *file_access, ref *pfile)
{
  int i;
  for (i = 0; i < num_std_files; i++)
  {
...
/* but not the string. */
private
int write_string(ref *op, stream *s)
{
  uint len;
  check_read_type(*op, t_string);
  len = op->size;
