...
/* an appropriate cap. */
private
int stroke_add(P4(int, partial_line *, partial_line *, gs_state *));
private
int stroke_fill(P4(int, partial_line *, partial_line *, gs_state *));

/* Other forward declarations */
private
int stroke(P3(gx_path *,
              int (*)(P4(int, partial_line *, partial_line *, gs_state *)),
              gs_state *));
private
void compute_caps(P2(partial_line *, gs_state *));
private
int add_capped(P4(gx_path *, gs_line_cap, int (*)(P3(gx_path *, fixed, fixed)),
                  endpoint *));
...
int gx_stroke_add(gx_path *ppath, gx_path *topath, gs_state *pgs)
{
  stroke_path = topath;
...
private
int stroke(gx_path *ppath, int (*line_proc)(P4(int, partial_line *, partial_line *, gs_state *)),
           gs_state *pgs)
{
  subpath *psub;
  int code;
  ...
  if (gs_debug['o'])
    {
      ...
      for (i = 0; i < count; i++) printf(",%f", lp->dash.pattern[i]);
  ...
  if (!ppath->curve_count) /* don't need to flatten */
    ...
    {
      if ((code = gx_path_flatten(ppath, &fpath, pgs->flatness)) < 0)
        {
          return code;
        }
...
#define next_line(ex, ey)                                                     \
  (pl.o.p.x = x, pl.o.p.y = y, pl.e.p.x = (ex), pl.e.p.y = (ey),              \
   ...
   (index++ ? (*line_proc)(index - 2, &pl_prev, &pl, pgs) : 0), pl_prev = pl, \
   0)
      ...
      while (segcount--)
        {
          ...
          if (!always_thin || dash_count)
            {
              ...
              if (skewed)
                {
                  gs_idtransform(pgs, (float)udx, (float)udy, &dpt);
                }
              else /* shortcut */
                {
                  dpt.x = udx / xx;
              ...
              unscaled_dist = sqrt(dpt.x * dpt.x + dpt.y * dpt.y);
              if (!always_thin)
                {
                  float wl = line_width_and_scale / unscaled_dist;
                  ...
                  if (skewed)
                    {
                      pl.width.x += (fixed)(dpt.x * pgs->ctm.yx),
                          pl.width.y -= (fixed)(dpt.y * pgs->ctm.xy);
                    }
                  pl.thin =
                      ((pl.width.x < 0 ? -pl.width.x : pl.width.x) +
                       (pl.width.y < 0 ? -pl.width.y : pl.width.y)) < float2fixed(0.75);
                  if (pl.thin)
                    {
                    ...
                    {
                      pl.cap_length.x = (fixed)(dpt.x * xx);
                      pl.cap_length.y = (fixed)(dpt.y * yy);
                      if (skewed)
                        {
                          pl.cap_length.x += (fixed)(dpt.y * pgs->ctm.yx),
                              pl.cap_length.y += (fixed)(dpt.x * pgs->ctm.xy);
                        }
                    }
                }
              else
                {
                  pl.width.x = pl.width.y = 0, pl.cap_length.x = pl.cap_length.y = 0,
                  pl.thin = 1;
                }
            }
          else
          ...
          if (dash_count)
            { /* Account for dash pattern */
              float dist_total = unscaled_dist * (1 / (float)int2fixed(1));
      ...
      if (index)
        { /* If closed, join back to start, else cap */
          (*line_proc)(
              index - 1, &pl_prev,
              (psub->closed && ink_on && dash->init_ink_on ? &pl_first
                                                           : (partial_line *)0),
              pgs);
        }
      psub = (subpath *)pline;
      if (stroke_path == &stroke_path_body)
        { /* Fill and release the accumulated path */
          gx_fill_path(stroke_path, pgs->dev_color, pgs, gx_rule_winding_number,
                       (fixed)0);
          gx_path_release(stroke_path);
          stroke_path = 0;
        }
...
private
void line_intersect(gs_fixed_point *pp1, /* point on 1st line */
  ...
  if (gs_debug['o'])
    {
      printf("[o]Intersect %f,%f(%f/%f) & %f,%f(%f/%f) ->\n", fixed2float(pp1->x),
             fixed2float(pp1->y), fixed2float(pd1->x), fixed2float(pd1->y),
             fixed2float(pp2->x), fixed2float(pp2->y), fixed2float(pd2->x),
             fixed2float(pd2->y));
...
private
int stroke_fill(int first, register partial_line *plp, partial_line *nplp,
                gs_state *pgs)
{
  if (plp->thin)
    { /* Minimum-width line, don't have to be careful. */
      ...
      if (gx_cpath_includes_rectangle(pgs->clip_path, lix, liy, litox, litoy))
        {
          return gz_draw_line_fixed(lix, liy, litox, litoy, pgs->dev_color, pgs);
        }
...
int add_capped(P4(gx_path *, gs_line_cap, int (*)(P3(gx_path *, fixed, fixed)),
                  endpoint *));
private
int stroke_add(int first, register partial_line *plp, partial_line *nplp,
               gs_state *pgs)
{
  ...
  if ((code =
           add_capped(ppath, (first == 0 ? pgs->line_params->cap : gs_cap_butt),
                      gx_path_add_point, &plp->o)) < 0)
    {
      return code;
    }
  if (nplp == 0)
...
#ifdef DEBUG
      if (gs_debug['o']) printf("[o]e.cd-side=%d nw/side=%d\n", cside, nside);
#endif
...
#endif
          if (num >= 0 ? (check > 0 && num >= denom * check)
            ...
            { /* OK to use a miter join. */
              ...
              float fndx = nplp->o.cdelta.x, fndy = nplp->o.cdelta.y;
...
private
void compute_caps(register partial_line *plp, gs_state *pgs)
{
...
#ifdef DEBUG
  if (gs_debug['o'])
    printf("[o]Stroke o=(%f,%f) e=(%f,%f)\n", fixed2float(plp->o.p.x),
           fixed2float(plp->o.p.y), fixed2float(plp->e.p.x),
           fixed2float(plp->e.p.y)),
        printf("	wxy=(%f,%f) lxy=(%f,%f)\n", fixed2float(wx2),
               fixed2float(wy2), fixed2float(lx2), fixed2float(ly2));
#endif
#undef lx2
#undef ly2
}
...
private
int add_capped(gx_path *ppath, gs_line_cap type,
               int (*add_proc)(P3(gx_path *, fixed,
                                  fixed)), /* gx_path_add_point/line */
...
#define cdy endp->cdelta.y
  ...
  if (gs_debug['o'])
    printf("[o]cap: p=(%g,%g), co=(%g,%g), ce=(%g,%g), cd=(%g,%g)\n",
           fixed2float(px), fixed2float(py), fixed2float(xo), fixed2float(yo),
           fixed2float(xe), fixed2float(ye), fixed2float(cdx),
           fixed2float(cdy));
#endif
  switch (type)
    {
      case gs_cap_round:
        {
          ...
          if ((code = (*add_proc)(ppath, xo, yo)) < 0 ||
              (code = gx_path_add_arc(ppath, xo, yo, xm, ym, xo + cdx, yo + cdy)) <
                  0 ||
              (code = gx_path_add_arc(ppath, xm, ym, xe, ye, xe + cdx, ye + cdy)) <
                  0)
            {
              return code;
            }
