...
private
gx_device_procs sun_procs = {sun_open,
...
#define check_rect()              \
  if (w <= 0 || h <= 0) return 0; \
  if (x < 0 || x > xdev->width - w || y < 0 || y > xdev->height - h) return -1

void sun_window_create(int *argc_ptr, char **argv)
{
  x_device.frame = window_create(NULL, FRAME, FRAME_LABEL, "ghostscript",
                                 FRAME_ARGC_PTR_ARGV, argc_ptr, argv, 0);
}

int sun_open(register gx_device *dev)
{
#ifdef gs_DEBUG
  if (gs_debug['X'])
    {
  ...
  xdev->canvas = window_create(
      xdev->frame, CANVAS, CANVAS_AUTO_EXPAND, FALSE, CANVAS_AUTO_SHRINK, FALSE,
      CANVAS_WIDTH, xdev->width, CANVAS_HEIGHT, xdev->height,
      WIN_VERTICAL_SCROLLBAR, scrollbar_create(0), WIN_HORIZONTAL_SCROLLBAR,
...
int sun_sync(register gx_device *dev)
{
  (void)notify_dispatch();
  return 0;
}
...
int sun_fill_rectangle(register gx_device *dev, int x, int y, int w, int h,
                       gx_color_index color)
{
  check_rect();
  if (color != gx_no_color_index)
    pw_write(xdev->pw, x, y, w, h, (color ? PIX_CLR : PIX_SET), (Pixrect *)0, 0,
             0);
  (void)notify_dispatch();
  return 0;
}
...
int sun_copy_mono(register gx_device *dev, byte *base, int sourcex, int raster,
                  ...
                  gx_color_index one)
{
  ...
  check_rect();
  if ((op = optab[zero + 1][one + 1]) == PIX_DST) return 0;
...
int sun_copy_color(register gx_device *dev, byte *base, int sourcex, int raster,
...
/* Draw a line */
int sun_draw_line(register gx_device *dev, int x0, int y0, int x1, int y1,
                  gx_color_index color)
{
  if (color != gx_no_color_index)
