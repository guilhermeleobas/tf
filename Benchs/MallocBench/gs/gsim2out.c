...
private
int round_coord(P1(floatp));
private
int put_int(P2(status *, int));
private
void fill_cells(P4(byte *, byte *, int, int));
private
int trace_cells(P4(byte *, int, int, status *));
 ...
 */
int gs_type1imagepath(gs_state *pgs, byte *data, int width, int height,
...
#ifdef DEBUG
    if (gs_debug['0'])
      printf("[0]ctm=[%g %g %g %g %g %g]\n", stat.ifm.xx, stat.ifm.xy,
             stat.ifm.yx, stat.ifm.yy, stat.ifm.tx, stat.ifm.ty);
#endif
...
xit: /* Encode the origin, width, and side bearing. */
  {
    ...
    if ((code = gs_distance_transform(origin_x * outline_scale,
                                      origin_y * outline_scale, &stat.ifm,
                                      &opt)) < 0 ||
        (code = gs_distance_transform(wx * outline_scale, wy * outline_scale,
                                      &stat.ifm, &wpt)) < 0 ||
        (code = gs_distance_transform((lsbx - origin_x) * outline_scale,
                                      (floatp)0, &stat.ifm, &lsbpt)) < 0)
      {
        return code;
      }
    ...
    if (gs_debug['0'])
      {
        ...
        for (cy = 0; cy < height; cy++)
          {
            printf("[0]%3d ", cy);
            for (cx = 0; cx < width; cx += 8) printf("%02x", (int)*cp++);
            putchar('\n');
          }
      }
#endif
    if ((code = put_int(out, ilsbx)) < 0)
      {
        return code;
      }
    if (iwy != 0 || ilsby != 0)
      {
        if ((code = put_int(out, ilsby)) < 0 || (code = put_int(out, iwx)) < 0 ||
...
/* bottom row first. */
private
void fill_cells(byte *cells, byte *data, int width, int height)
{
  int y;
  byte *dptr = data - 1;
...
private
int trace_from(P3(status *, byte *, int));
private
int add_dxdy(P4(status *, int, int, int));
#define add_deltas(s, dx, dy, n) \
  if ((code = add_dxdy(s, dx, dy, n)) < 0) return code
private
int put_dxdy(P4(status *, int, int, int));
#define put_deltas(s, dx, dy, moving) \
  if ((code = put_dxdy(s, dx, dy, moving)) < 0) return code
private
int trace_cells(byte *cells, int width, int height, register status *out)
{
  byte *cptr;
  int code;
  for (cptr = cells + (width + 2) * (height + 1) - 2; cptr >= cells; cptr--)
    {
      if (*cptr == 1 && cptr[-(width + 2)] == 0)
        { /* Found a starting point */
          int x = (cptr - cells) % (width + 2) - 1;
          int y = (cptr - cells) / (width + 2) - 1;
          put_deltas(out, x * outline_scale + 1 - out->px,
                     y * outline_scale - out->py, 1);
          out->count = 0;
...
/* Trace a path */
private
int trace_from(register status *out, byte *cptr, int width)
{
  typedef enum
  ...
  static dir_descr nesw[4 + 1] = {/* Going north (along a western edge) */
                                  {-1, 1, 0, 1},
...
private
int add_dxdy(register status *out, int dx, int dy, int count)
{
  int code;
  if (count != 0)
    {
      if (dx == out->dx && dy == out->dy)
      ...
      else
        {
          if (out->count != 0)
            {
              put_deltas(out, out->dx * out->count, out->dy * out->count, 0);
            }
          out->dx = dx, out->dy = dy;
          out->count = count;
        }
...
private
int put_dxdy(register status *out, int dx, int dy, int moving)
{
  int code;
  ...
  if (gs_debug['0'])
    printf("[0]  pxy=(%d,%d)+(%d,%d)  cpxy=(%d,%d)+(%d,%d)\n", out->px, out->py,
           dx, dy, out->cpx, out->cpy, cdx, cdy);
#endif
  if (cdx != 0 || cdy == 0)
    { /* encode dx if needed */
      if ((code = put_int(out, cdx)) < 0)
        {
          return code;
        }
    }
  ...
  *out->next++ = (byte)(cdy == 0 ? /* use hmove/lineto */
...
private
int round_coord(floatp v)
{
  long c = (long)(v + 0.5);
  return (c > 0x7fff ? 0x7fff : c < -0x7fff ? -0x7fff : (int)c);
}
/* Encode a single number in Type 1 representation. */
private
int put_int(status *out, register int v)
{
