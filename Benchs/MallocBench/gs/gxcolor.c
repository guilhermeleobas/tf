...
void gx_color_from_rgb(register gs_color *pcolor)
{
...
color_param gx_color_luminance(register gs_color *pcolor)
{
  if (!pcolor->luminance_set)
  {
    pcolor->luminance = (pcolor->red * (unsigned long)lum_red_weight +
...
/* Convert RGB to HSB */
void gx_color_to_hsb(register gs_color *pcolor, color_param hsb[3])
{
#define rhue hsb[0]
#define rsat hsb[1]
#define rbri hsb[2]
  ...
  { /* Convert rgb to hsb */
    ...
    { /* V == c.blue */
      H = (c.red - c.green) * (long)max_color_param / diff +
          4 * max_color_param;
    }
    if (H < 0)
    {
...
void gx_color_from_hsb(register gs_color *pcolor, color_param hue,
                       color_param saturation, color_param brightness)
{
  if (saturation == 0)
  {
...
void gx_sort_ht_order(ht_bit *recs, uint N)
{
  uint l = N >> 1;
  uint r = N - 1;
  uint j;
