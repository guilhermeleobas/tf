...
/* Forward references */
private
int setup_show(P2(ref *, op_proc_p));
private
int show_continue(P1(ref *));
private
int finish_show(P1(ref *));
private
int finish_stringwidth(P1(ref *));
private
gs_show_enum *find_show();
private
void free_show();

/* show */
int zshow(register ref *op)
{
...
/* widthshow */
int zwidthshow(register ref *op)
{
  int code;
  float cxy[2];
...
/* awidthshow */
int zawidthshow(register ref *op)
{
  int code;
  float cxy[2], axy[2];
...
private
int finish_show(ref *op) { return 0; }
/* stringwidth */
int zstringwidth(register ref *op)
{
...
private
int finish_stringwidth(register ref *op)
{
  gs_point width;
...
/* setcachedevice */
int zsetcachedevice(register ref *op)
{
  float wbox[6];
  int npop = 6;
...
/* setcharwidth */
int zsetcharwidth(register ref *op)
{
  float width[2];
  gs_show_enum *penum = find_show();
...
int ztype1addpath(register ref *op)
{
  int code;
  gs_show_enum *penum = find_show();
...
/* type1imagepath */
int ztype1imagepath(register ref *op)
{
  float woxy[4];
  int code;
...
int z1_subr_proc(gs_type1_data *pdata, int index, byte **pstr)
{
...
int z1_pop_proc(gs_type1_data *pdata, fixed *pf)
{
...
void zchar_op_init()
{
  static op_def my_defs[] = {{"3ashow", zashow},
...
private
int setup_show(ref *op, op_proc_p endproc /* end procedure */)
{
  gs_show_enum *penum;
...
private
int show_continue(register ref *op)
{
