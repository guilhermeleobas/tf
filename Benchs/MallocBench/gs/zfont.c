...
/* Forward references */
private
int font_param(P2(ref *, gs_font **));
private
int add_FID(P2(ref *, gs_font *));
private
int make_font(P2(ref *, gs_matrix *));
private
void make_uint_array(P3(ref *, uint *, int));
...
ref name_FontBBox;
private
ref name_Encoding; /* only needed for seac */
private
ref name_PaintType;
private
ref name_UniqueID;
...
ref name_BuildChar;
private
ref name_Type1BuildChar;
private
ref name_Private;
private
ref name_CharStrings; /* only needed for seac */
private
ref name_FID;
...
/* definefont */
int zdefinefont(register ref *op)
{
  ref *pFontDirectory;
  int code;
  ref *pfid;
  ...
  make_tasv(&no_subrs, t_string, 0, 0, bytes, (byte *)0), psubrs = &no_subrs;
...
/* scalefont */
int zscalefont(register ref *op)
{
  int code;
  float scale;
  gs_matrix mat;
...
/* currentfont */
int zcurrentfont(register ref *op)
{
  push(1);
  *op = istate.font;
  return 0;
...
/* cachestatus */
int zcachestatus(register ref *op)
{
  uint status[7];
...
/* setcachelimit */
int zsetcachelimit(register ref *op)
{
...
/* setcacheparams */
int zsetcacheparams(register ref *op)
{
  uint params[2];
  int i, code;
...
/* currentcacheparams */
int zcurrentcacheparams(register ref *op)
{
  uint params[2];
...
void zfont_op_init()
{
  static op_def my_defs[] = {{"0currentfont", zcurrentfont},
...
private
int font_param(ref *fp, gs_font **pfont)
...
private
int add_FID(ref *fp /* t_dictionary */, gs_font *pfont)
{
  ref fid;
...
private
int make_font(ref *op, gs_matrix *pmat)
{
  ref *fp = op - 1;
  ...
  if ((code = font_param(fp, &oldfont)) < 0 ||
      (code = gs_makefont(ifont_dir, oldfont, pmat, &newfont, &ffont)) < 0 ||
...
private
void make_uint_array(register ref *op, uint *intp, int count)
{
  int i;
