...
private
int continue_show(P1(gs_show_enum *));
private
int continue_show_update(P1(gs_show_enum *));
private
int continue_stringwidth(P1(gs_show_enum *));
private
int continue_stringwidth_update(P1(gs_show_enum *));
private
int show_setup(P3(gs_show_enum *, gs_state *, char *));
private
int stringwidth_setup(P3(gs_show_enum *, gs_state *, char *));

/* Print the ctm if debugging */
#define print_ctm(s, pgs)                                               \
  printf("[p]%sctm=[%g %g %g %g %g %g]\n", s, pgs->ctm.xx, pgs->ctm.xy, \
         pgs->ctm.yx, pgs->ctm.yy, pgs->ctm.tx, pgs->ctm.ty)

/* ------ String writing operators ------ */

/* Setup macros for show operators */
#define setup_show() penum->size = strlen(str)
#define setup_show_n() penum->size = size
#define setup_a() \
  penum->add = 1, penum->ax = ax, penum->ay = ay, penum->slow_show = 1
#define setup_width() \
  penum->chr = chr, penum->cx = cx, penum->cy = cy, penum->slow_show = 1
#define no_chr 0x100

/* show[_n] */
int gs_show_init(register gs_show_enum *penum, gs_state *pgs, char *str)
{
  setup_show();
  penum->slow_show = 0;
  return show_setup(penum, pgs, str);
}
int gs_show_n_init(register gs_show_enum *penum, gs_state *pgs, char *str,
                   uint size)
{
  setup_show_n();
  penum->slow_show = 0;
...
/* ashow[_n] */
int gs_ashow_init(register gs_show_enum *penum, gs_state *pgs, floatp ax,
                  floatp ay, char *str)
{
  int code;
  setup_show();
...
int gs_ashow_n_init(register gs_show_enum *penum, gs_state *pgs, floatp ax,
                    floatp ay, char *str, uint size)
{
  int code;
  setup_show_n();
  code = show_setup(penum, pgs, str);
...
/* widthshow[_n] */
int gs_widthshow_init(register gs_show_enum *penum, gs_state *pgs, floatp cx,
                      floatp cy, char chr, char *str)
{
  int code;
  setup_show();
  code = show_setup(penum, pgs, str);
...
int gs_widthshow_n_init(register gs_show_enum *penum, gs_state *pgs, floatp cx,
                        floatp cy, char chr, char *str, uint size)
{
  int code;
  setup_show_n();
  code = show_setup(penum, pgs, str);
...
/* awidthshow[_n] */
int gs_awidthshow_init(register gs_show_enum *penum, gs_state *pgs, floatp cx,
                       floatp cy, char chr, floatp ax, floatp ay, char *str)
{
  int code;
  setup_show();
  code = show_setup(penum, pgs, str);
...
int gs_awidthshow_n_init(register gs_show_enum *penum, gs_state *pgs, floatp cx,
                         floatp cy, char chr, floatp ax, floatp ay, char *str,
                         uint size)
{
  int code;
...
/* kshow[_n] */
int gs_kshow_init(register gs_show_enum *penum, gs_state *pgs, char *str)
{
  int code;
  setup_show();
  code = show_setup(penum, pgs, str);
...
int gs_kshow_n_init(register gs_show_enum *penum, gs_state *pgs, char *str,
                    uint size)
{
  int code;
  setup_show_n();
...
/* stringwidth[_n] */
int gs_stringwidth_init(gs_show_enum *penum, gs_state *pgs, char *str)
{
  setup_show();
  return stringwidth_setup(penum, pgs, str);
}
int gs_stringwidth_n_init(gs_show_enum *penum, gs_state *pgs, char *str,
                          uint size)
{
  setup_show_n();
  return stringwidth_setup(penum, pgs, str);
...
private
int stringwidth_setup(gs_show_enum *penum, gs_state *pgs, char *str)
{
...
/* charpath[_n] */
int gs_charpath_init(gs_show_enum *penum, gs_state *pgs, char *str, int bool)
{
  int code;
  setup_show();
  code = show_setup(penum, pgs, str);
...
int gs_charpath_n_init(gs_show_enum *penum, gs_state *pgs, char *str, uint size,
                       int bool)
{
  int code;
  setup_show_n();
...
/* setcachedevice */
int gs_setcachedevice(register gs_show_enum *penum, floatp wx, floatp wy,
                      floatp llx, floatp lly, floatp urx, floatp ury)
{
  ...
  if (!penum->can_cache || !pgs->char_tm_valid || llx > urx || lly > ury)
  {
    return 0;
  }
  {
    ...
    if (gs_debug['k'])
    {
      printf("[k]cbox=[%g %g %g %g]\n", fixed2float(cbox_ll.x),
             fixed2float(cbox_ll.y), fixed2float(cbox_ur.x),
             fixed2float(cbox_ur.y));
      print_ctm("  ", pgs);
    }
#endif
    iwidth = fixed2long(cdim.x) + 2;
    iheight = fixed2long(cdim.y) + 2;
    if (iwidth != (ushort)iwidth || iheight != (ushort)iheight)
    {
    ...
    if ((cc =
             gx_alloc_char_bits(dir, (gx_device_memory *)&penum->dev_cache_info,
                                (ushort)iwidth, (ushort)iheight)) == 0)
    {
    ...
    if ((code = gx_clip_to_rectangle(pgs, (fixed)0, (fixed)0, int2fixed(iwidth),
                                     int2fixed(iheight))) < 0)
    {
      return code;
    }
...
/* setcharwidth */
int gs_setcharwidth(register gs_show_enum *penum, floatp wx, floatp wy)
{
...
int gs_show_next(gs_show_enum *penum) { return (*penum->continue_proc)(penum); }
...
private
int continue_show_update(register gs_show_enum *penum)
{
...
private
int continue_show(register gs_show_enum *penum) { return show_proceed(penum); }
/* Update position for show */
private
int show_update(register gs_show_enum *penum)
{
...
private
int show_move(register gs_show_enum *penum)
{
  register gs_state *pgs = penum->pgs;
  if (penum->add)
  {
...
private
int show_proceed(register gs_show_enum *penum)
{
  register gs_state *pgs = penum->pgs;
  byte *str = penum->str;
  uint index;
...
no_cache:
  ...
  /* < 0 means error, 0 means success, 1 means failure. */
  {
    ...
    code =
        (*font->build_char_proc)(penum, pgs, font, chr, font->build_char_data);
    if (code < 0)
    {
      return code;
    }
...
private
int continue_stringwidth_update(register gs_show_enum *penum)
{
...
private
int continue_stringwidth(register gs_show_enum *penum)
{
  return stringwidth_proceed(penum);
}
/* Update position for stringwidth */
private
int stringwidth_update(register gs_show_enum *penum)
{
  register gs_state *pgs = penum->pgs;
  /* Update position for last character */
  switch (penum->width_set)
...
private
int stringwidth_move(register gs_show_enum *penum)
{ /* wxy is in device coordinates */
  return show_fast_move(penum->wxy, penum->pgs);
}
/* Process next character for stringwidth */
private
int stringwidth_proceed(register gs_show_enum *penum)
{
  register gs_state *pgs = penum->pgs;
  byte *str = penum->str;
  uint index;
...
more: /* Proceed to next character */
  ...
  {
    gs_fixed_point cpt;
    if ((code = gx_path_current_point_inline(pgs->path, &cpt)) < 0)
    {
      return code;
    }
    gs_setmatrix(pgs, &pgs->char_tm);
...
byte gs_show_current_char(gs_show_enum *penum)
{
  return penum->str[penum->index - 1];
}

/* Return the just-displayed character for kerning. */
byte gs_kshow_previous_char(gs_show_enum *penum)
{
  return penum->str[penum->index - 1];
}

/* Return the about-to-be-displayed character for kerning. */
byte gs_kshow_next_char(gs_show_enum *penum)
{
  return penum->str[penum->index];
}

/* Return the accumulated width for stringwidth. */
void gs_show_width(gs_show_enum *penum, gs_point *ppt) { *ppt = penum->width; }
/* Return the charpath flag. */
int gs_show_in_charpath(gs_show_enum *penum) { return penum->charpath_flag; }
/* ------ Internal routines ------ */

/* Initialize a show enumerator */
private
int show_setup(register gs_show_enum *penum, gs_state *pgs, char *str)
{
  int code;
  penum->pgs = pgs;
  ...
  {                                  /* Compute combined transformation */
    ...
    code =
        gs_matrix_multiply(&pgs->font->matrix, &ctm_only(pgs), &pgs->char_tm);
    if (code < 0)
    {
      return code;
    }
