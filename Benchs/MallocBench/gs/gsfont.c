...
gs_font_dir *gs_font_dir_alloc(proc_alloc_t palloc, proc_free_t pfree)
{
                                      ...
                                      uint cmax, uint upper)
{
  register gs_font_dir *pdir =
      (gs_font_dir *)(*palloc)(1, sizeof(gs_font_dir), "font_dir_alloc(dir)");
  byte *bdata;
...
/* scalefont */
int gs_scalefont(gs_font_dir *pdir, gs_font *pfont, floatp scale,
...
/* makefont */
int gs_makefont(gs_font_dir *pdir, gs_font *pfont, gs_matrix *pmat,
                gs_font **ppfont, gs_font **pdfont)
{
  int code;
...
/* a real unique_id (i.e., not -1). */
  ...
  *ppfont = pf_out;
#ifdef DEBUG
  if (gs_debug['m']) printf("[m]new font=%lx\n", (ulong)pf_out);
#endif
  return 1;
}
...
/* currentfont */
gs_font *gs_currentfont(gs_state *pgs) { return pgs->font; }
/* cachestatus */
void gs_cachestatus(register gs_font_dir *pdir, register uint pstat[7])
{
  pstat[0] = pdir->bsize;
  pstat[1] = pdir->bmax;
  pstat[2] = pdir->msize;
...
/* setcachelimit */
int gs_setcachelimit(gs_font_dir *pdir, uint size)
{
  pdir->upper = size;
  return 0;
}
...
/* currentcacheparams */
uint gs_currentcachelower(gs_font_dir *pdir) { return pdir->lower; }
uint gs_currentcacheupper(gs_font_dir *pdir) { return pdir->upper; }
/* Dummy (ineffective) BuildChar procedure */
int gs_no_build_char_proc(struct gs_show_enum_s *penum, gs_state *pgs,
