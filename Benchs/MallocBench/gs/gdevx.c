...
private
int max_temp_pixmap = 20000;

/* Forward references */
private
int set_tile(P2(gx_device *, gx_bitmap *));
private
void free_cp(P1(gx_device *));
/* Screen updating machinery */
#define update_init(dev) \
  ((gx_device_X *)(dev))->up_area = 0, ((gx_device_X *)(dev))->up_count = 0
#define update_flush(dev) \
  if (((gx_device_X *)(dev))->up_area != 0) update_do_flush(dev)
private
void update_do_flush(P1(gx_device *));
private
void update_add(P5(gx_device *, int, int, int, int));
...
int x_tile_rectangle(P8(gx_device *, gx_bitmap *, int, int, int, int,
                        gx_color_index, gx_color_index));

int x_copy_mono(P10(gx_device *, _byte *, int, int, int, int, int, int,
                    gx_color_index, gx_color_index));
...
int x_fill_trapezoid(P8(gx_device *, int, int, int, int, int, int,
                        gx_color_index));

int x_tile_trapezoid(P10(gx_device *, gx_bitmap *, int, int, int, int, int, int,
                         gx_color_index, gx_color_index));

/* The device descriptor */
private
gx_device_procs x_procs = {x_open,           gx_default_get_initial_matrix,
                           x_sync,           gx_default_output_page,
                           x_close,          x_map_rgb_color,
                           x_map_color_rgb,  x_fill_rectangle,
                           x_tile_rectangle, x_copy_mono,
                           x_copy_color,     x_draw_line,
                           x_fill_trapezoid, x_tile_trapezoid};
...
gx_device_X gs_x11_device = {
    ...
    DEFAULT_HEIGHT_INCHES, /* x and y extent */
    DEFAULT_X_RESOLUTION,
    DEFAULT_Y_RESOLUTION, /* x and y density */
    ...
    NULL,
    0,
    NULL,           /* dpy, scr, depth_all_ones, vis */
...
/* Open the X device */
int x_open(register gx_device *dev)
{
  Screen *scr;
  ...
  x_resolution_str = XGetDefault(xdev->dpy, PROGRAM_NAME, ARG_X_RESOLUTION);
  y_resolution_str = XGetDefault(xdev->dpy, PROGRAM_NAME, ARG_Y_RESOLUTION);
  ...
  border_width_str = XGetDefault(xdev->dpy, PROGRAM_NAME, ARG_BORDER_WIDTH);

  border_width = (border_width_str == NULL ? DEFAULT_BORDER_WIDTH
                                           : atoi(border_width_str));

  border_color_str = XGetDefault(xdev->dpy, PROGRAM_NAME, ARG_BORDER_COLOR);

  border_color =
      (border_color_str == NULL ||
               !XAllocNamedColor(xdev->dpy, xdev->cmap, border_color_str,
  ...
  { /*
    ...
    int bitmask = XGeometry(
        xdev->dpy, DefaultScreen(xdev->dpy), geometry, DEFAULT_GEOMETRY,
        border_width, 1, 1, /* ``Font'' width and height. */
    ...
    if (bitmask & (WidthValue | HeightValue)) sizehints.flags |= USSize;
  }
  ...
  xdev->win = XCreateWindow(xdev->dpy, RootWindowOfScreen(scr), sizehints.x,
                            sizehints.y, /* upper left */
                            sizehints.width, sizehints.height, border_width,
                            xdev->depth, InputOutput, /* class */
                            xdev->vis,                /* visual */
                            CWEventMask | CWBackPixel | CWBorderPixel, &xswa);
  if (use_backing)
    xdev->bpixmap = XCreatePixmap(xdev->dpy, xdev->win, xdev->width,
                                  xdev->height, xdev->depth);
  else
  ...
  if (xdev->bpixmap != (Pixmap)0)
  {
    ...
    XFillRectangle(xdev->dpy, xdev->bpixmap, xdev->gc, 0, 0, xdev->width,
                   xdev->height);
  }
  ...
  xdev->ht.no_pixmap = XCreatePixmap(xdev->dpy, xdev->win, 1, 1, xdev->depth);
...
int x_close(gx_device *dev) { return 0; }
/* Map a color.  The "device colors" are just r,g,b packed together. */
gx_color_index x_map_rgb_color(register gx_device *dev, _ushort r, _ushort g,
                               _ushort b)
{
  return (r << 2) + (g << 1) + b;
}
...
int x_map_color_rgb(register gx_device *dev, gx_color_index color,
                    _ushort *prgb)
{
  prgb[0] = (color >> 2) & 1;
  prgb[1] = (color >> 1) & 1;
...
int x_sync(register gx_device *dev)
{
  update_flush(dev);
  XSync(xdev->dpy, 0);
  return 0;
...
int x_fill_rectangle(register gx_device *dev, int x, int y, int w, int h,
...
/* Tile a rectangle. */
int x_tile_rectangle(register gx_device *dev, gx_bitmap *tile, int x, int y,
                     int w, int h, gx_color_index zero, gx_color_index one)
{
  ...
  if (one == gx_no_color_index && zero == gx_no_color_index) return -1;
...
private
int set_tile(register gx_device *dev, register gx_bitmap *tile)
{
#ifdef DEBUG
  if (gs_debug['T']) return 0;
#endif
  /* Set up the tile Pixmap */
  if (tile->width != xdev->ht.width || tile->height != xdev->ht.height ||
      xdev->ht.pixmap == (Pixmap)0)
  {
    if (xdev->ht.pixmap != (Pixmap)0) XFreePixmap(xdev->dpy, xdev->ht.pixmap);
    if (xdev->ht.bits)
    {
      free(xdev->ht.bits);
      xdev->ht.bits = 0;
    }
    xdev->ht.pixmap = XCreatePixmap(xdev->dpy, xdev->win, tile->width,
                                    tile->height, xdev->depth);
    if (xdev->ht.pixmap == (Pixmap)0) return 0;
  ...
  if (memcmp(xdev->ht.bits, tile->data, xdev->ht.bits_size))
  {
    ...
    XPutImage(xdev->dpy, xdev->ht.pixmap, xdev->gc, &xdev->image, 0, 0, 0, 0,
...
int x_copy_mono(register gx_device *dev, _byte *base, int sourcex, int raster,
...
{
  int function = GXcopy;
  unsigned long bc = xdev->colors[zero], fc = xdev->colors[one];
  ...
  if (zero != gx_no_color_index)
  {
    ...
    { /* Simply replace existing bits with what's in the image */
      ...
      function = GXand, fc = xdev->accum_color;
    else if (!(~bc & xdev->accum_color))
      function = GXorInverted, fc = bc, bc = 0;
    else
      goto hard;
  }
  else
  {
    if (one == gx_no_color_index) /* no-op */
      ...
      function = GXandInverted, bc = 0, fc = xdev->accum_color;
  ...
  XPutImage(xdev->dpy, xdev->dest, xdev->gc, &xdev->image, sourcex, 0, x, y, w,
            h);
...
hard: /* Handle the hard 1-color case. */
  ...
  { /* Must allocate a new pixmap and GC. */
    ...
    xdev->cp.pixmap = XCreatePixmap(xdev->dpy, xdev->win, raster << 3, h, 1);
  ...
  if (one == gx_no_color_index)
  {
    ...
    XPutImage(xdev->dpy, xdev->cp.pixmap, xdev->cp.gc, &xdev->image, sourcex, 0,
              0, 0, w, h);
  ...
  if (raster * h > max_temp_pixmap) free_cp(dev);
...
private
void free_cp(register gx_device *dev)
{
  if (xdev->cp.gc != NULL)
  {
...
int x_copy_color(register gx_device *dev, _byte *base, int sourcex, int raster,
                 int x, int y, int w, int h)
{
  return x_copy_mono(dev, base, sourcex, raster, x, y, w, h, (gx_color_index)0,
                     (gx_color_index)7);
}

/* Draw a line */
int x_draw_line(register gx_device *dev, int x0, int y0, int x1, int y1,
                gx_color_index color)
{
  set_fore_color(color);
  set_fill_style(FillSolid);
...
/* Fill a trapezoid */
int x_fill_trapezoid(register gx_device *dev, int x0, int y0, int w0, int x1,
                     int y1, int w1, gx_color_index color)
{
  setup_trapezoid();
  set_fore_color(color);
  set_fill_style(FillSolid);
...
/* Tile a trapezoid */
int x_tile_trapezoid(register gx_device *dev, gx_bitmap *tile, int x0, int y0,
                     int w0, int x1, int y1, int w1, gx_color_index zero,
                     gx_color_index one)
{
  /* Check for a colored tile.  We should implement this */
  /* properly someday, since X can handle it. */
...

  {
    ...
    {
      set_fill_style(FillSolid);
      return gx_default_tile_trapezoid(dev, tile, x0, y0, w0, x1, y1, w1, zero,
                                       one);
    }
...
private
void update_do_flush(register gx_device *dev)
{
  int xo = xdev->update.xo, yo = xdev->update.yo;
  set_function(GXcopy);
  XCopyArea(xdev->dpy, xdev->bpixmap, xdev->win, xdev->gc, xo, yo,
            xdev->update.xe - xo, xdev->update.ye - yo, xo, yo);
  update_init(dev);
}
...
private
void update_add(register gx_device *dev, int xo, int yo, int w, int h)
{
  int xe = xo + w, ye = yo + h;
  ...
  { /* See whether adding this rectangle */
    ...
    if (new_up_area > 100 && old_area + new_area < new_up_area * 2 / 3 ||
        xdev->up_count >= 200)
      update_do_flush(dev);
    else
    {
...
private
void alt_put_image(gx_device *dev, Display *dpy, Drawable win, GC gc,
                   XImage *pi, int sx, int sy, int dx, int dy, unsigned w,
                   unsigned h)
{
