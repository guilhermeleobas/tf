...
#define _xaty(alp, yv) ((yv) - (alp)->start.y) * al_dx(alp) / al_dy(alp)
...
#define dir_down (-1)
  ...
  struct active_line_s *prev, *next;
} active_line;

#ifdef DEBUG
/* Internal procedure for printing an active line */
private
void print_active_line(char *label, active_line *alp)
{
  printf("[f]%s %lx(%d): x_current=%f x_next=%f\n", label, (ulong)alp, alp->tag,
         fixed2float(alp->x_current), fixed2float(alp->x_next));
  printf("    start=(%f,%f) pt_end=%lx(%f,%f)\n", fixed2float(alp->start.x),
         fixed2float(alp->start.y), (ulong)alp->pseg, fixed2float(alp->end.x),
         fixed2float(alp->end.y));
  printf("    mark=%d dir=%d prev=%lx next=%lx\n", alp->end_mark,
         alp->direction, (ulong)alp->prev, (ulong)alp->next);
}
...
private
int alloc_line_list(P2(line_list *, int));
private
void add_y_list(P4(gx_path *, short, line_list *, gs_fixed_rect *));
private
void add_y_line(P4(segment *, segment *, int, line_list *));
private
void fill_loop(P6(gx_device_color *, int, line_list *, gs_fixed_rect *,
                  gs_state *, fixed));
private
void insert_x_new(P2(active_line *, line_list *));
private
void swap_group(P1(active_line *));
private
void ended_line(P1(active_line *));
...
private
int alloc_line_list(line_list *ll, int count)
{
  ll->area_count = count;
  ll->area =
...
private
void add_y_list(gx_path *ppath, short tag, line_list *ll, gs_fixed_rect *pbox)
{
...
private
void add_y_line(segment *prev_lp, segment *lp, int dir, line_list *ll)
{
...
private
fixed find_cross_y(register active_line *endp, register active_line *alp)
{
...
/* x_list as needed. */
private
void fill_loop(gx_device_color *pdevc, int rule, line_list *ll,
...
private
void insert_x_new(active_line *alp, line_list *ll)
{
...
private
void swap_group(active_line *alp)
...
private
void ended_line(register active_line *alp)
{
