...
/* restore */
private
int restore_check_stack(P3(ref *, ref *, alloc_save *));
int zrestore(register ref *op)
{
  vm_save *vmsave;
  alloc_save *asave;
  int code;
...
private
int restore_check_stack(ref *bot, ref *top, alloc_save *asave)
{
  ref *stkp;
...
void zvmem_op_init()
{
  static op_def my_defs[] = {{"1restore", zrestore},
                             {"0save", zsave},
                             {"0vmstatus", zvmstatus},
                             op_def_end};
  z_op_init(my_defs);
