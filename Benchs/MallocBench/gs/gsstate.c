...
/* Forward references */
private
int alloc_state_contents(P2(gs_state *, proc_alloc_t));
private
void free_state_contents(P1(gs_state *));

/* Allocate and initialize a graphics state */
gs_state *gs_state_alloc(proc_alloc_t palloc, proc_free_t pfree)
{
  ...
  pgs->halftone->width = pgs->halftone->height = pgs->halftone->order_size = 0;
  gs_nulldevice(pgs);
  gs_setflat(pgs, 1.0);
...
int gs_initgraphics(register gs_state *pgs)
{
  int code;
  gs_initmatrix(pgs);
...
/* currentflat */
float gs_currentflat(gs_state *pgs) { return pgs->flatness; }
/* ------ Internal routines ------ */

/* Allocate the contents of a graphics state object. */
/* Return -1 if the allocation fails. */
private
int alloc_state_contents(gs_state *pgs, proc_alloc_t palloc)
{
  char *elt;
...
private
void free_state_contents(gs_state *pgs)
{
