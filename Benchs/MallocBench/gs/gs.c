...
private
char *gs_lib_default_path = GS_LIB_DEFAULT; /* defined in the makefile */
...
void set_lib_paths();
private
void run_file(P2(char *file_name, int user_errors));
private
void debug_dump_stack(P1(int code));
...
int interpret_string(char *str)
{
  ref stref;
  make_tasv(&stref, t_string, a_executable + a_read + a_execute, strlen(str),
            bytes, (byte *)str);
  return interpret(&stref, user_errors);
}

private
int init1_done = 0, init2_done = 0;
private
void init1()
{
  if (!init1_done)
...
private
void run_file(char *file_name, int user_errors)
{
  ref initial_file;
  int code;
...
private
void debug_dump_stack(int code)
{
