...
private
int image_init(P10(gs_image_enum *, int, int, int, int, int, gs_matrix *,
                   gs_state *, gx_color_index, gx_color_index));
/* Procedures for unpacking the input data into 8 bits/sample. */
private
void image_unpack_0(iunpack_proc_args);
private
void image_unpack_0_spread(iunpack_proc_args);
private
void image_unpack_1(iunpack_proc_args);
private
void image_unpack_1_spread(iunpack_proc_args);
private
void image_unpack_2(iunpack_proc_args);
private
void image_unpack_3(iunpack_proc_args);
private
void image_unpack_3_spread(iunpack_proc_args);
...
private
int image_render_skip(irender_proc_args);
private
int image_render_direct(irender_proc_args);
private
int image_render_mono(irender_proc_args);
private
int image_render_color(irender_proc_args);

/* Start processing an image */
int gs_image_init(gs_image_enum *penum, gs_state *pgs, int width, int height,
                  int bps, int spp, gs_matrix *pmat)
{
  int log2_bps, spread;
  ...
  return image_init(penum, width, height, log2_bps, spp, spread, pmat, pgs,
                    pgs->device->black, pgs->device->white);
}

/* Start processing a masked image */
int gs_imagemask_init(gs_image_enum *penum, gs_state *pgs, int width,
                      int height, int invert, gs_matrix *pmat)
{
  ...
  if (invert)
  {
    color0 = gx_no_color_index, color1 = pgs->dev_color->color1;
  }
  else
  {
    color0 = pgs->dev_color->color1, color1 = gx_no_color_index;
  }
  return image_init(penum, width, height, 0, 1, 1, pmat, pgs, color0, color1);
}
...
#define map4tox(a, b, c, d)                                               \
  0, a, b, a + b, c, a + c, b + c, a + b + c, d, a + d, b + d, a + b + d, \
      c + d, a + c + d, b + c + d, a + b + c + d
#if big_endian
private
uint32_t map_4_to_32[16] = {map4tox(0xffL, 0xff00L, 0xff0000L, 0xff000000L)};
private
uint16_t map_4_to_16[16] = {map4tox(0x55, 0xaa, 0x5500, 0xaa00)};
#else /* !big_endian */
private
uint32_t map_4_to_32[16] = {map4tox(0xff000000L, 0xff0000L, 0xff00L, 0xffL)};
private
uint16_t map_4_to_16[16] = {map4tox(0x5500, 0xaa00, 0x55, 0xaa)};
#endif
private
int image_init(register gs_image_enum *penum, int width, int height,
               int log2_bps, int spp, int spread, gs_matrix *pmat,
               gs_state *pgs, gx_color_index color0, gx_color_index color1)
{
  int code;
  gs_matrix mat;
...
#define chtl(i) penum->dev_colors[i].halftone_level
    switch (log2_bps)
    {
      case 3:
      { /* Yes, clearing the entire table is slow, */
        ...
        do
        {
          pcht[0].halftone_level = pcht[1].halftone_level =
              pcht[2].halftone_level = pcht[3].halftone_level = -1;
          pcht += 4;
        } while (--n > 0);
        break;
      }
      case 2:
        chtl(17) = chtl(2 * 17) = chtl(3 * 17) = chtl(4 * 17) = chtl(6 * 17) =
            chtl(7 * 17) = chtl(8 * 17) = chtl(9 * 17) = chtl(11 * 17) =
...
#undef chtl
  ...
  /* rectangle, clipping is never required. */
  {
    ...
    penum->never_clip = (mdx < 0 ? mtx + mdx >= xmin && mtx <= xmax
                                 : mtx >= xmin && mtx + mdx <= xmax) &&
                        (mdy < 0 ? mty + mdy >= ymin && mty <= ymax
                                 : mty >= ymin && mty + mdy <= ymax);
#ifdef DEBUG
    if (gs_debug['b'])
      printf("[b]Image: xmin=%g ymin=%g xmax=%g ymax=%g\n", fixed2float(xmin),
             fixed2float(ymin), fixed2float(xmax), fixed2float(ymax)),
          printf("     mtx=%g mty=%g mdx=%g mdy=%g never_clip=%d\n",
                 fixed2float(mtx), fixed2float(mty), fixed2float(mdx),
                 fixed2float(mdy), penum->never_clip);
#endif
  }
  {
    static void (*procs[4])(iunpack_proc_args) = {
        image_unpack_0, image_unpack_1, image_unpack_2, image_unpack_3};
    static void (*spread_procs[4])(iunpack_proc_args) = {
        image_unpack_0_spread, image_unpack_1_spread, image_unpack_2,
        image_unpack_3_spread};
    penum->slow_loop =
        !penum->never_clip || penum->skewed ||
  ...
  if (gs_debug['b'])
    printf("[b]Image: w=%d h=%d %s\n   [%f %f %f %f %f %f]\n", width, height,
           (penum->never_clip ? "no clip" : "must clip"), mat.xx, mat.xy,
...
int gs_image_next(register gs_image_enum *penum, byte *dbytes, uint dsize)
{
...
private
void image_unpack_0(gs_image_enum *penum, byte *bptr, register byte *data,
                    uint dsize)
{
...
private
void image_unpack_0_spread(gs_image_enum *penum, register byte *bufp,
...
private
void image_unpack_1(gs_image_enum *penum, byte *bptr, register byte *data,
                    uint dsize)
{
...
private
void image_unpack_1_spread(gs_image_enum *penum, register byte *bufp,
                           register byte *data, uint dsize)
{
  register int spread = penum->spread;
  int left = dsize;
...
private
void image_unpack_2(gs_image_enum *penum, register byte *bufp,
                    register byte *data, uint dsize)
{
  register int spread = penum->spread;
  int left = dsize;
...
private
void image_unpack_3(gs_image_enum *penum, byte *bufp, byte *data, uint dsize)
{
  if (data != bufp)
  {
...
private
void image_unpack_3_spread(gs_image_enum *penum, register byte *bufp,
                           register byte *data, uint dsize)
{
  register int spread = penum->spread;
...
private
int image_render_skip(gs_image_enum *penum, byte *data, uint w, int h)
{
  return h;
}
...
private
int image_render_direct(gs_image_enum *penum, byte *data, uint w, int h)
{
  fixed xt = penum->xcur;
  ...
  int (*proc)(P10(gx_device *, byte *, int, int, int, int, int, int,
                  gx_color_index, gx_color_index)) = dev->procs->copy_mono;
  gx_color_index zero = penum->icolor0, one = penum->icolor1;
  if (ht == 1)
  { /* We can do the whole thing at once. */
    (*proc)(dev, data, 0, (w + 7) >> 3, ix, iy, w, h, zero, one);
    return h;
  }
  else
  { /* Do just one row. */
    ...
    for (dy = 0; dy < ht; dy++)
    {
      (*proc)(dev, data, 0, (w + 7) >> 3, ix, iy + dy, w, 1, zero, one);
    }
    return 1;
  }
}
...
private
int image_render_mono(gs_image_enum *penum, byte *buffer, uint w, int h)
{
  gx_color_index zero = penum->icolor0, one = penum->icolor1;
  gs_state *pgs = penum->pgs;
  fixed dxx = penum->fxx, dxy = penum->fxy, dyx = penum->fyx, dyy = penum->fyy;
  gs_fixed_rect cbox;
...
#define image_set_gray(sample_value)          \
  ...
  int (*fill_proc)(P6(gx_device *, int, int, int, int, gx_color_index)) =
...
#ifdef DEBUG
  if (gs_debug['b'])
    printf("[b]y=%d w=%d xt=%f yt=%f yb=%f\n", penum->y, w, fixed2float(xt),
           fixed2float(ytf), fixed2float(ytf + dyy));
#endif
    ...
    { /* Fill the region between */
      ...
      { /* Must use general fill */
        ...
        { /* Do the empty/clipping check */
          ...
          code = gx_path_add_pgram(ppath, xrr, ytf, xlr, ytf, xlr, yn);
        }
        else
        {
          gx_path_init(ppath, &pgs->memory_procs);
          code = gx_path_add_pgram(ppath, xrun, yrun, xl, ytf, xl + dyx,
                                   ytf + dyy);
        }
        if (code < 0)
        {
...
#undef check_clipped_or_empty
        ...
        gx_fill_path(ppath, pdevc, pgs, gx_rule_winding_number, (fixed)0);
      nofill:
        gx_path_release(ppath);
      trans:;
      }
...
private
int image_render_color(gs_image_enum *penum, byte *buffer, uint w, int h)
{
  gs_state *pgs = penum->pgs;
  fixed dxx = penum->fxx, dxy = penum->fxy, dyx = penum->fyx, dyy = penum->fyy;
  int skew = penum->skewed;
  ...
  /* to handle final run */
  {
    ...
    if (next.all != run.all)
    {
      ...
      if (devc1.color1 != devc2.color1 ||
          ...
          (devc1.halftone_level && devc1.color2 != devc2.color2) ||
