...
/* Forward references */
private
int scan_hex_string(P2(stream *, ref *)),
...
#define dynamic_begin(pda, dnum, desize)                                  \
  ((pda)->base = alloc_dynamic((pda)->num_elts = (dnum),                  \
   ...
   (pda)->limit = (pda)->base + (dnum) * (desize), (pda)->is_dynamic = 1, \
   (pda)->next = (pda)->base)

/* Grow a dynamic object */
private
int dynamic_grow(register dynamic_area *pda)
{
  ...
  else
  {
    byte *base = pda->base;
    uint old_size = size;
    if (!dynamic_begin(pda, new_num, pda->elt_size))
    {
      return 0;
    }
...
#define dynamic_free(pda)                                                      \
  if ((pda)->is_dynamic)                                                       \
  alloc_free((char *)((pda)->base), (pda)->num_elts, (pda)->elt_size, "scanne" \
                                                                      "r")
...
void scan_init()
{
  ...
  /* Initialize decoder array */
  {
    ...
    decoder[char_NULL] = decoder[char_VT] = decoder[char_DOS_EOF] = ctype_space;
...
int scan_token(register stream *s, int from_string, ref *pref)
{
  ref *myref = pref;
...
top:
  c = sgetc(s);
#ifdef DEBUG
  if (gs_debug['s']) printf((c >= 32 && c <= 126 ? "`%c'" : "`%03o'"), c);
#endif
  switch (c)
  {
    case ' ':
    ...
    case '}':
      ...
      }
      {
        ...
        { /* Top-level procedure, shrink in place. */
          ...
          aref = (ref *)alloc_shrink((byte *)myref, proc_da.num_elts, size,
                                     sizeof(ref), "scanner(top proc)");
          if (aref == 0)
          {
            return e_VMerror;
          }
        ...
        if (array_packing)
        {
          make_tasv(myref, t_packedarray, a_executable + a_read + a_execute,
                    size, refs, aref);
        }
        else
        {
    ...
    /* Name overran buffer. */
    dyn_name:
      while (decoder[c = sgetc(s)] <= ctype_name)
      {
        if (ptr == da.limit)
        {
          da.next = ptr;
...
int scan_number(register stream *s, ref *pref)
{ /* Powers of 10 up to 6 can be represented accurately as */
  /* a single-precision float. */
#define num_powers_10 6
  static float powers_10[num_powers_10 + 1] = {1e0, 1e1, 1e2, 1e3,
                                               1e4, 1e5, 1e6};
  int sign = 0;
  long ival;
  double dval;
...
fe: /* dval contains the value, negated if necessary */
  ...
  if (exp10 > 0)
  {
    ...
    {
      dval *= powers_10[num_powers_10], exp10 -= num_powers_10;
    }
    if (exp10 > 0)
    {
...
private
int scan_int(register stream *s, int radix, long *pval, double *pdval)
{
...
#define set_max(vmax, vrem, big)          \
  if (radix == 10)                        \
    vmax = (big) / 10, vrem = (big) % 10; \
  else                                    \
  vmax = (big) / radix, vrem = (big) % radix
  set_max(imax, irem, max_uint >> 1);
#define convert_digit_fails(c, d) (d = decoder[c]) >= radix
  while (1)
  {
    c = sgetc(s);
  ...
  while (1)
  {
    ...
    if (lval >= lmax && (lval > lmax || d > lrem)) break; /* overflow */
    lval = lval * radix + d;
  }
#endif
...
/* Make a string */
private
int mk_string(ref *pref, dynamic_area *pda)
{
  uint size = pda->next - pda->base;
  byte *body =
      alloc_shrink(pda->base, pda->num_elts, size, 1, "scanner(string)");
  if (body == 0)
  {
    return e_VMerror;
  }
...
private
int scan_string(register stream *s, int from_string, ref *pref)
{
  dynamic_area da;
  register int c;
...
private
int scan_hex_string(stream *s, ref *pref)
{
  dynamic_area da;
  int c1, c2, val1, val2;
