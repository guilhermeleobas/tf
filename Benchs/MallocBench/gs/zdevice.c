...
/* copypage */
int zcopypage(register ref *op) { return gs_copypage(igs); }
/* copyscanlines */
int zcopyscanlines(register ref *op)
{
  ref *op1 = op - 1;
  ref *op2 = op - 2;
  gx_device *dev;
...
/* currentdevice */
int zcurrentdevice(register ref *op)
{
...
/* devicename */
int zdevicename(register ref *op)
{
  char *dname;
  int code;
...
/* deviceparams */
int zdeviceparams(register ref *op)
{
  ...
  pmat = op->value.refs;
  make_real(pmat + 0, imat.xx);
  make_real(pmat + 1, imat.xy);
  make_real(pmat + 2, imat.yx);
  make_real(pmat + 3, imat.yy);
  make_real(pmat + 4, imat.tx);
  make_real(pmat + 5, imat.ty);
  push(2);
  make_mark(op - 3);
  make_int(op - 1, width);
  make_int(op, height);
...
/* flushpage */
int zflushpage(register ref *op) { return gs_flushpage(igs); }
/* getdevice */
int zgetdevice(register ref *op)
{
  gx_device *dev;
...
/* makedevice */
int zmakedevice(register ref *op)
{
  gs_matrix imat;
  gx_device *new_dev;
  int code;
...
/* makeimagedevice */
int zmakeimagedevice(register ref *op)
{
  gs_matrix imat;
  gx_device *new_dev;
  float colors[256 * 3];
...
/* .nulldevice */
int znulldevice(register ref *op)
{
  gs_nulldevice(igs);
  return 0;
}
...
/* .setdevice */
int zsetdevice(register ref *op)
{
  int code;
  check_type(*op, t_device);
...
void zdevice_op_init()
{
  static op_def my_defs[] = {{"0copypage", zcopypage},
