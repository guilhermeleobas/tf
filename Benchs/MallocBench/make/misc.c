...
void collapse_continuations(line) char *line;
{
...
void remove_comments(line) char *line;
{
  register char *p, *p2;
  register int backslash;
  register unsigned int bs_write;
...
char *concat(s1, s2, s3) register char *s1, *s2, *s3;
{
...
void makefile_error(file, lineno, s1, s2, s3, s4, s5, s6) char *file;
unsigned int lineno;
...
void makefile_fatal(file, lineno, s1, s2, s3, s4, s5, s6) char *file;
unsigned int lineno;
char *s1, *s2, *s3, *s4, *s5, *s6;
{
  fprintf(stderr, "%s:%u: ", file, lineno);
...
char *savestring(str, length) char *str;
unsigned int length;
{
...
char *sindex(big, blen, small, slen) char *big;
unsigned int blen;
char *small;
unsigned int slen;
{
...
char *lindex(s, limit, c) register char *s, *limit;
int c;
{
  while (s < limit)
    {
...
char *find_next_token(ptr, lengthptr) char **ptr;
...
struct dep *copy_dep_chain(d) register struct dep *d;
{
...
char *dep_name(dep) struct dep *dep;
{
...
void user_access()
{
  ...
  if (setuid(user_uid) < 0) pfatal_with_name("setuid");
  if (setgid(user_gid) < 0) pfatal_with_name("setgid");
#else
