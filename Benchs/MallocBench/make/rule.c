...
static int new_pattern_rule(rule, override) register struct rule *rule;
int override;
{
    ...
    for (i = 0; rule->targets[i] != 0; ++i)
    {
      for (j = 0; r->targets[j] != 0; ++j)
      {
        if (streq(rule->targets[i], r->targets[j]))
        {
          ...
          for (d = rule->deps, d2 = r->deps; d != 0 && d2 != 0;
               d = d->next, d2 = d2->next)
          {
...
void install_pattern_rule(p, terminal) struct pspec *p;
int terminal;
{
  ...
  ptr = p->dep;
  r->deps = (struct dep *)multi_glob(
      parse_file_seq(&ptr, '\0', sizeof(struct dep)), sizeof(struct dep));
...
static void freerule(rule, lastrule) register struct rule *rule, *lastrule;
{
...
void create_pattern_rule(targets, target_percents, terminal, deps, commands,
                         override) char **targets,
    **target_percents;
int terminal;
struct dep *deps;
...
int override;
{
  ...
  for (i = 0; targets[i] != 0; ++i)
  {
    if (i == max_targets - 1)
    {
      ...
      r->suffixes =
          (char **)xrealloc((char *)r->suffixes, max_targets * sizeof(char *));
    }
    r->lens[i] = strlen(targets[i]);
    r->suffixes[i] =
        (target_percents == 0 ? find_percent(targets[i]) : target_percents[i]) +
        1;
    if (r->suffixes[i] == 0)
    {
      abort();
  ...
  if (i < max_targets - 1)
  {
    ...
    r->suffixes =
        (char **)xrealloc((char *)r->suffixes, (i + 1) * sizeof(char *));
  }
