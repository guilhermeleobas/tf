...
struct file *lookup_file(name) char *name;
{
...
struct file *enter_file(name) char *name;
{
...
void rename_file(file, name) register struct file *file;
char *name;
{
...
void remove_intermediates(sig) int sig;
{
  register int i;
  register struct file *f;
  char doneany;
  ...
  for (i = 0; i < FILE_BUCKETS; ++i)
  {
    ...
    for (file = files[i]; file != 0; file = file->next)
    {
      ...
      for (f = file; f != 0; f = f->prev)
      {
        ...
        {
          printf("#  Last modified %.24d (%ld)\n", ctime(&f->last_mtime),
                 (long int)f->last_mtime);
        }
