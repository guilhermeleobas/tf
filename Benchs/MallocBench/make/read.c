...
static char *default_include_directories[] = {
    "/usr/gnu/include", "/usr/local/include", "/usr/include", 0};
...
struct dep *read_all_makefiles(makefiles) char **makefiles;
{
  ...
  if (num_makefiles == 0)
    {
      static char *default_makefiles[] = {"GNUmakefile", "makefile", "Makefile",
                                          0};
...
static void read_makefile(filename, type) char *filename;
int type;
{
  ...
  do                                                                            \
    {                                                                           \
      record_files(filenames, pattern, pattern_percent, deps, commands_started, \
                   commands, commands_idx, two_colon, filename, lineno,         \
                   type != 1);                                                  \
      filenames = 0;                                                            \
      commands_idx = 0;                                                         \
      pattern = 0;                                                              \
...
#define word1eq(s, l) ((p[l] == '\0' || isblank(p[l])) && !strncmp(s, p, l))
      if (word1eq("ifdef", 5) || word1eq("ifndef", 6) || word1eq("ifeq", 4) ||
          word1eq("ifneq", 5) || word1eq("else", 4) || word1eq("endif", 5))
        {
          ...
          if (!strncmp(p2, "define", 6))
            {
              ...
              lineno = do_define(p, len, o_override, lineno, infile, filename);
            }
          ...
          if (*p2 != '\0')
            {
              *p2++ = '\0';
              if (*next_token(p2) != '\0')
                {
                  makefile_error(filename, lineno, "extraneous text after `include'");
                }
            }
...
#undef word1eq
      ...
      else
        {
          ...
          filenames = multi_glob(parse_file_seq(&p2, ':', sizeof(struct nameseq)),
          ...
          deps = (struct dep *)multi_glob(
              parse_file_seq(&p2, '\0', sizeof(struct dep)), sizeof(struct dep));
...
static unsigned int do_define(name, namelen, origin, lineno, infile,
                              filename) char *name;
...
static int conditional_line(line, filename, lineno) char *line;
char *filename;
unsigned int lineno;
{
  int notdef;
  ...
  if (*cmdname == 'e')
    {
      if (*line != '\0')
        {
          makefile_error(filename, lineno, "Extraneous text after `%s' directive",
                         cmdname);
        }
      /* "Else" or "endif".  */
  ...
  if (conditionals->if_cmds > conditionals->max_ignoring)
    {
      ...
      conditionals->ignoring =
          (char *)xrealloc(conditionals->ignoring, conditionals->max_ignoring);
    }
  ...
  if (conditionals->if_cmds > 1 &&
  ...
  else
    {
      ...
      *line = '\0';
      ...
      conditionals->ignoring[conditionals->if_cmds - 1] = streq(s1, s2) == notdef;
    }
...
void uniquize_deps(chain) struct dep *chain;
{
  register struct dep *d;
...
static void record_files(filenames, pattern, pattern_percent, deps,
                         commands_started, commands, commands_idx, two_colon,
                         filename, lineno,
                         set_default) struct nameseq *filenames;
      ...
      if (implicit)
        {
          if (targets == 0)
          ...
          else if (target_idx == max_targets - 1)
            {
              max_targets += 5;
              targets =
                  (char **)xrealloc((char *)targets, max_targets * sizeof(char *));
...
static char *find_semicolon(string) char *string;
{
  char *found, *p;
...
char *find_percent(pattern) char *pattern;
{
...
struct nameseq *parse_file_seq(stringp, stopchar, size) char **stringp;
char stopchar;
unsigned int size;
{
      ...
      while (1)
        {
          ...
          else if (c == '\\' && (*p == '\\' || isblank(*p) || *p == stopchar))
            {
              ++p;
            }
...
static unsigned int readline(linebuffer, stream,
                             filename) struct linebuffer *linebuffer;
FILE *stream;
char *filename;
{
...
void construct_include_path(arg_dirs) char **arg_dirs;
{
  register unsigned int i;
  struct stat stbuf;
              ...
              if (idx == max - 1)
                {
                  max += 5;
                  dirs =
                      (char **)xrealloc((char *)dirs, (max + defsize) * sizeof(char *));
                }
              dirs[idx++] = dir;
            }
        }
...
struct nameseq *multi_glob(chain, size) struct nameseq *chain;
unsigned int size;
{
  register struct nameseq *new = 0;
              ...
              if (pwent != 0)
                {
                  if (userend == 0)
                    {
                      free(old->name);
                      old->name = savestring(pwent->pw_dir, strlen(pwent->pw_dir));
                    }
                  else
                    {
              ...
              for (i = 0; i < gl.gl_pathc; ++i)
                {
                  ...
                  elt->name = savestring(gl.gl_pathv[i], strlen(gl.gl_pathv[i]));
                  elt->next = new;
                  new = elt;
                }
              globfree(&gl);
