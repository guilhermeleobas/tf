...
char *subst_expand(o, text, subst, replace, slen, rlen, by_word,
                   suffix_only) char *o;
char *text;
char *subst, *replace;
  ...
  while ((p = sindex(t, 0, subst, slen)) != 0)
    {
      ...
      if ((by_word && ((p > t && !isblank(p[-1])) ||
                       (p[slen] != '\0' && !isblank(p[slen])))) ||
          (suffix_only && (p[slen] != '\0' && !isblank(p[slen]))))
        {
...
char *patsubst_expand(o, text, pattern, replace, pattern_percent,
                      replace_percent) char *o;
char *text;
  ...
  while ((t == find_next_token(&text, &len)))
    {
      ...
      if (!fail && pattern_postpercent_len > 0 &&
          (t[len - 1] != pattern_percent[pattern_postpercent_len] ||
           ...
           strncmp(&t[len - pattern_postpercent_len], &pattern_percent[1],
          ...
          if (replace_percent != 0)
           ...
           matched the % in the pattern.  */
              o = variable_buffer_output(
                  ...
                  len - (pattern_prepercent_len + pattern_postpercent_len));
...
} function_table[] = {{"subst", 5, function_subst},
...
int pattern_matches(pattern, percent, word) register char *pattern, *percent,
    *word;
{
  unsigned int len;
...
#define BADARGS(func)                                                \
  if (reading_filename != 0)                                         \
    makefile_fatal(reading_filename, *reading_lineno_ptr,            \
                   "insufficient arguments to function `%s'", func); \
  else                                                               \
    fatal("insufficient arguments to function `%s'", func)

static char *expand_function(o, function, text, end) char *o;
enum function function;
char *text;
char *end;
{
          ...
          if (reading_filename != 0)
            {
              ...
              sprintf(error_prefix, "%s:%u: ", reading_filename, *reading_lineno_ptr);
            }
          else
            {
              error_prefix = "";
...
int handle_function(op, stringp) char **op;
char **stringp;
  ...
  for (code = 0; function_table[code].name != 0; ++code)
    {
      ...
      if (isblank(*endref) &&
          !strncmp(function_table[code].name, beg, function_table[code].len))
        {
          break;
        }
    }
...
static char *string_glob(line) char *line;
{
