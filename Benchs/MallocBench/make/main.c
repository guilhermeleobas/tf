...
int main(argc, argv, envp) int argc;
char **argv;
char **envp;
{
...
#define FATAL_SIG(sig)                                                      \
  if (SIGNAL((sig), (SIGHANDLER)fatal_error_signal) == (SIGHANDLER)SIG_IGN) \
...
static void decode_switches(argc, argv) int argc;
char **argv;
{
  char bad = 0;
  register unsigned int i;
...
static void define_makeflags(pf, makefile) int pf, makefile;
{
  register struct command_switch *cs;
  char flags[200];
  register unsigned int i;
...
static void log_working_directory(entering) int entering;
{
