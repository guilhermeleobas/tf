...
static void child_error(target_name, exit_code, exit_sig, coredump,
                        ignored) char *target_name;
  ...
  if (exit_sig == 0)
  ...
  else
    {
      ...
      if (exit_sig > 0 && exit_sig < NSIG)
        {
          error("*** [%s] %s%s", target_name, sys_siglist[exit_sig],
                coredump_string);
        }
      else
        {
...
void push_signals_blocked_p(flag) int flag;
{
  int blocked;
      ...
      if (signals_blocked_p_depth == signals_blocked_p_max)
        {
          ...
          signals_blocked_p_stack =
              (char *)xrealloc(signals_blocked_p_stack, signals_blocked_p_max);
        }
          ...
          if (!unknown_children_possible)
            {
              ...
              if (child_failed)
                {
                  child_error(buf, exit_code, exit_sig, coredump, ignore_errors_flag);
                }
              else
                {
...
#ifdef USG
  if (sig > 0) (void)SIGNAL(sig, child_handler);
#endif
...
void wait_for_children(n, err) unsigned int n;
int err;
{
...
static void free_child(child) register struct child *child;
{
...
static void start_job(child) register struct child *child;
{
  ...
  if (start_remote_job_p())
    {
      ...
      if (start_remote_job(argv, child->good_stdin ? 0 : bad_stdin, &is_remote,
                           &id, &used_stdin))
        {
          goto error;
        }
      ...
      if (child->pid == 0)
        {
          ...
          child_execute_job(child->good_stdin ? 0 : bad_stdin, 1, argv,
                            child->environment);
        }
      else if (child->pid < 0)
        {
...
void new_job(file) register struct file *file;
{
  ...
  switch (file->command_state)
    {
      ...
      default:
        error("internal error: `%s' command_state == %d in new_job", file->name,
              (int)file->command_state);
        abort();
        break;
    }
...
void child_execute_job(stdin_fd, stdout_fd, argv, envp) int stdin_fd, stdout_fd;
char **argv, **envp;
{
  if (stdin_fd != 0)
    {
...
static int search_path(file, path, program) char *file, *path, *program;
{
  if (path == 0 || path[0] == '\0')
    {
      path = default_path;
...
void exec_command(argv, envp) char **argv, **envp;
{
  char *shell, *path;
  PATH_VAR(program);
  register char **ep;
...
static char **construct_command_argv_internal(line, restp, shell,
                                              ifs) char *line,
    **restp;
char *shell, *ifs;
{
  ...
  static char *sh_cmds[] = {
      ...
      "umask", "wait", "while", "for", "case", "if", ":",
      ".", "break", "continue", "export", "read", "readonly", "shift",
      "times", "trap", "switch", 0};
  register int i;
  register char *p;
  register char *ap;
...
slow:;
  ...
  if (shell == 0 || !strcmp(shell, default_shell))
  ...
  else
    {
      ...
      char *new_line =
          (char *)alloca(shell_len + (sizeof(minus_c) - 1) + (line_len * 2) + 1);
      ...
      for (p = line; *p != '\0'; ++p)
        {
          if (*p == '\\' || *p == '\'' || isspace(*p) || index(sh_chars, *p) != 0)
            {
              *ap++ = '\\';
            }
          *ap++ = *p;
      ...
      *ap = '\0';
      ...
      new_argv =
          construct_command_argv_internal(new_line, restp, (char *)0, (char *)0);
    }
...
char **construct_command_argv(line, restp, file) char *line, **restp;
struct file *file;
{
