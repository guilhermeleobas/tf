...
static struct directory *find_directory(name) register char *name;
{
  ...
  if (dir == 0)
    {
      ...
      if (dir->dirstream == 0)
      ...
      else
        {
          ...
          dir->files =
              (struct dirfile **)xmalloc(sizeof(struct dirfile) * DIRFILE_BUCKETS);
...
int dir_file_exists_p(dirname, filename) register char *dirname;
  ...
  while ((d = readdir(dir->dirstream)) != 0)
    {
      ...
      if (filename != 0 && newhash == hash && streq(d->d_name, filename))
        {
          return 1;
        }
    }
...
int file_exists_p(name) register char *name;
{
  char *dirend;
  char *dirname;
...
void file_impossible(filename) register char *filename;
{
  char *dirend;
...
int file_impossible_p(filename) char *filename;
{
  char *dirend;
  register char *p = filename;
  register unsigned int hash;
