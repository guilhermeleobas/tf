...
void ar_parse_name(name, arname_p, memname_p) char *name, **arname_p,
    **memname_p;
{
  char *p = index(name, '('), *end = name + strlen(name) - 1;
...
time_t ar_member_date(name) char *name;
{
  char *arname;
  int arname_used = 0;
  char *memname;
...
/* ARGSUSED */
static long int ar_member_date_1(desc, mem, hdrpos, datapos, size, date, uid,
                                 gid, mode, name) int desc;
char *mem;
