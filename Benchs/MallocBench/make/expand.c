...
static char *recursively_expand(v) register struct variable *v;
{
  char *value;
...
char *variable_expand(line) register char *line;
{
          ...
          if (p != 0 && lindex(beg, p, closeparen) == 0)
          {
            ...
            if (v != 0 && *v->value != '\0')
            {
              ...
              if (lindex(subst_beg, subst_beg + subst_len, '%') != 0)
              {
                ...
                o = patsubst_expand(o, value, p, p1, index(p, '%'),
                                    index(p1, '%'));
                free(p);
                free(p1);
              }
              ...
              {
                o = subst_expand(o, value, subst_beg, replace_beg, subst_len,
                                 replace_len, 0, 1);
              }
              if (v->recursive)
              {
...
char *expand_argument(str, end) char *str, *end;
{
...
char *variable_expand_for_file(line, file) char *line;
...
char *allocated_variable_expand(line) char *line;
{
...
char *allocated_variable_expand_for_file(line, file) char *line;
struct file *file;
{
  char *save;
  char *value;
