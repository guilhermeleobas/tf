...
int update_goal_chain(goals, makefiles) register struct dep *goals;
int makefiles;
{
...
#define MTIME(file) (makefiles ? file_mtime_no_search(file) : file_mtime(file))
...
static int update_file(file, depth) struct file *file;
unsigned int depth;
{
...
static int update_file_1(file, depth) struct file *file;
unsigned int depth;
{
...
void notice_finished_file(file) register struct file *file;
{
  struct dep *d;
...
static int check_dep(file, depth, this_mtime, must_make_ptr) struct file *file;
unsigned int depth;
time_t this_mtime;
int *must_make_ptr;
{
...
time_t f_mtime(file, search) register struct file *file;
int search;
{
...
static time_t name_mtime(name) register char *name;
{
  struct stat st;
...
static time_t library_file_mtime(lib) char *lib;
{
  time_t mtime;
  char *name;
