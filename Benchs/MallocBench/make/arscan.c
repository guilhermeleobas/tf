...
#if (defined(sun386) || defined(USGr3) || \
     defined(HPUX) && !defined(PORTAR) && !defined(PORT5AR))
#define PORTAR 1
#endif
...
long int ar_scan(archive, function, arg) char *archive;
long int (*function)();
long int arg;
{
#ifdef AIAMAG
...
int ar_name_equal(name, mem) char *name, *mem;
{
  char *p;
...
/* ARGSUSED */
static long int ar_member_pos(desc, mem, hdrpos, datapos, size, date, uid, gid,
                              mode, name) int desc;
char *mem;
...
int ar_member_touch(arname, memname) char *arname, *memname;
{
...
long int describe_member(desc, name, hdrpos, datapos, size, date, uid, gid,
                         mode) int desc;
char *name;
long int hdrpos, datapos, size, date;
int uid, gid, mode;
