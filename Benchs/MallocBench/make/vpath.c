...
void construct_vpath_list(pattern, dirpath) char *pattern, *dirpath;
{
...
int vpath_search(file) char **file;
{
  ...
  if (general_vpath != 0 && selective_vpath_search(general_vpath, file))
    {
      return 1;
    }
...
static int selective_vpath_search(path, file) struct vpath *path;
char **file;
{
  int not_target;
  char *name, *n;
