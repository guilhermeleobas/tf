...
static struct pspec default_terminal_rules[] = {
    /* RCS.  */
    "%", "%,v", "test -f $@ || $(CO) $(COFLAGS) $< $@", "%", "RCS/%,v",
    "test -f $@ || $(CO) $(COFLAGS) $< $@",

    /* SCCS.  */
    "%", "s.%", "$(GET) $(GFLAGS) $<", "%", "SCCS/s.%", "$(GET) $(GFLAGS) $<",
...
$(LINT.c) -i $*.c -o $@ \n $(RM) $*.c",
    ...
    ".F.f", "$(PREPROCESS.F) $< $(OUTPUT_OPTION)", ".r.f",
...
static char *default_variables[] = {
    "AR", "ar", "ARFLAGS", "rv", "AS", "as", "CC", "cc", "C++", "g++", "CO",
    "co", "CPP", "$(CC) -E", "FC", "f77",
    /* System V uses these, so explicit rules using them should work.
       However, there is no way to make implicit rules use them and FC.  */
    "F77", "$(FC)", "F77FLAGS", "$(FFLAGS)",
#ifdef USG
    "GET", "get",
#else
    "GET", "/usr/sccs/get",
#endif
    "LD", "ld", "LEX", "lex", "LINT", "lint", "M2C", "m2c",
#ifdef pyr
    "PC", "pascal",
#else
    "PC", "pc",
    ...
    "MAKEINFO", "makeinfo", "TEX", "tex", "TEXI2DVI", "texi2dvi", "WEAVE",
    "weave", "CWEAVE", "cweave", "TANGLE", "tangle", "CTANGLE", "ctangle",

    "RM", "rm -f",

    "LINK.o", "$(CC) $(LDFLAGS) $(TARGET_ARCH)", "COMPILE.c",
    "$(CC) $(CFLAGS) $(CPPFLAGS) $(TARGET_ARCH) -c", "LINK.c",
    "$(CC) $(CFLAGS) $(CPPFLAGS) $(LDFLAGS) $(TARGET_ARCH)", "COMPILE.cc",
    "$(C++) $(C++FLAGS) $(CPPFLAGS) $(TARGET_ARCH) -c", "COMPILE.C",
    "$(COMPILE.cc)", "LINK.cc",
    "$(C++) $(C++FLAGS) $(CPPFLAGS) $(LDFLAGS) $(TARGET_ARCH)", "LINK.C",
    "$(LINK.cc)", "YACC.y", "$(YACC) $(YFLAGS)", "LEX.l", "$(LEX) $(LFLAGS) -t",
    "COMPILE.f", "$(FC) $(FFLAGS) $(TARGET_ARCH) -c", "LINK.f",
    ...
    "PREPROCESS.r", "$(FC) $(FFLAGS) $(RFLAGS) $(TARGET_ARCH) -F", "LINT.c",
...
void install_default_implicit_rules()
{
