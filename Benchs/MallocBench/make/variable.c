...
static struct variable_set global_variable_set = {variable_table,
                                                  VARIABLE_BUCKETS};
...
static struct variable *define_variable_in_set(name, length, value, origin,
                                               recursive, set) char *name;
unsigned int length;
char *value;
...
struct variable *define_variable(name, length, value, origin,
                                 recursive) char *name;
unsigned int length;
char *value;
enum variable_origin origin;
...
struct variable *define_variable_for_file(name, length, value, origin,
                                          recursive, file) char *name;
unsigned int length;
char *value;
enum variable_origin origin;
int recursive;
...
struct variable *lookup_variable(name, length) char *name;
unsigned int length;
{
...
void initialize_file_variables(file) struct file *file;
{
...
void push_new_variable_scope()
{
  ...
  set->table =
      (struct variable **)xmalloc(set->buckets * sizeof(struct variable *));
  bzero((char *)set->table, set->buckets * sizeof(struct variable *));

  setlist =
      (struct variable_set_list *)xmalloc(sizeof(struct variable_set_list));
  setlist->set = set;
...
static void merge_variable_sets(set0, set1) struct variable_set *set0, *set1;
{
...
void merge_variable_set_lists(setlist0,
                              setlist1) struct variable_set_list **setlist0,
    *setlist1;
{
...
void define_automatic_variables()
{
...
char *variable_buffer_output(ptr, string, length) char *ptr, *string;
unsigned int length;
{
...
char **target_environment(file) struct file *file;
{
...
int try_variable_definition(line, origin) char *line;
enum variable_origin origin;
{
  register int c;
...
static void print_variable(v, prefix) register struct variable *v;
char *prefix;
{
  char *origin;
...
static void print_variable_set(set, prefix) register struct variable_set *set;
char *prefix;
{
...
void restore_variable_output(save) char *save;
{
