...
function compile() {
  ...
  if [[ -n $CPU2006 && $CPU2006 -eq 1 ]]; then
    ...
    $LLVM_PATH/opt -mem2reg -load $PHOENIX_PATH/build/Pass/DCCPhoenix.$suffix -Instrument -S $lnk_name -o $rbc_name ;
  ...
  parallel --tty --jobs=${JOBS} $LLVM_PATH/opt -S -mem2reg -load $PHOENIX_PATH/build/Pass/DCCPhoenix.$suffix -Instrument {.}.bc -o {.}.rbc ::: "${source_files[@]}" ;
